...
per pair: the first-order gap is an element of ℚ(ζ_2n), decided exactly and found nonzero. The other
223 stand at first order (the same element is exactly zero) and every one of them leaves at second
order, read at 40 digits with a decade law; at u = 1/2 no former collision is closed, to forty digits.
...
n = 9 pair separates on the road although it is protected against ZZ. Each collision is a met
coincidence; the rate at which the road dissolves it is forced.
...
- **[`docs/CAUGHT_ERRORS.md`](../docs/CAUGHT_ERRORS.md).** Of the eight entries of 2026-08-31 and
  ...
  rounds, the F160 mint); nothing on the comb laws. The round on this page is the entry of 2026-09-02.
...
neighbourhood of u = 0); **D = 0 leaves the pair to second order**, where this page reads rather than
derives. Gates R1a (sympy, n = 5, 6, 7, 9) and R1b (finite differences at every firing n, worst
...
separates at the full 12/n): the collision is a met coincidence, and the rate at which the road
...
**At even n the pairs that stand are read, not derived (gate R3c).** k ↦ k(n+2) is not an
...
DOUBLED sums agree: for the 12 non-mirror pairs a second cyclotomic coincidence on F65's own doubled
angle, at twice the modulus, not a symmetry; for the 11 Θ-mirror pairs one line, since w_{n−k} = w_k
...
inequality the other way round, while the code had it right). Four pairs at n = 30 show why the class
has to be read at the finer pair: at u = 1e-2 their
small c₂ and large c₃ still compete and the coarse ratio sits between 56 and 140. Three earlier
...
to a nonzero c₂ (the smallest |c₂| is 3.27e-2, far above the 40-digit floor), with a residual falling
two decades per decade in every one of the 446 ratios: no u³ term in any of them.

**Why there is no u³ term, for eleven of them (R4c).** K = diag((−1)^site), the chiral K of
...
(decade ratio within 15% of 1e5 for every one). So c₃ = 0 for every pair that stands at first order,
and for 212 of them this is reported, not explained.
...
by a theorem, and 11 pairs are even in u by a theorem. What it holds at second order is a decade law
on all 223. What it holds at finite u is forty digits.
...
the parity count of the labels (forced, by Galois), and the speed is an integer multiple of 4/n; at
even n, first order is decided by whether the collision persists on the doubled angle (met again),
and the Θ-mirror pairs among those are then even in u by the chiral K (forced). Two senses of *forced*
...
- **What is exact and what is read.** D and its vanishing: exact (cyclotomic vectors, the F129
  ...
  and with no theorem for the other 12. Second order and u = 1/2: 40-digit readings with decade laws, not certificates. The
  labelling at finite u: a convention resting on a proven simplicity (0 ≤ u < 1).
- **First order at u = 0 only.** Theorem G is a statement at the chain end, and "separates" means as
...
- Registry: [F160](../docs/ANALYTICAL_FORMULAS.md) (the road), [F129](../docs/ANALYTICAL_FORMULAS.md)
...
- Record: [`docs/CAUGHT_ERRORS.md`](../docs/CAUGHT_ERRORS.md), the entry of 2026-09-02.
