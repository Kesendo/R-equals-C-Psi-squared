...
| [Proton Water Chain](PROTON_WATER_CHAIN.md) | Grotthuss chain N=1-5. Heisenberg formulas match exactly (V(N), Q_max). TFI model: 222 frequencies at N=5. Sub-2γ modes at N≥4 (sector mixing). Sacrifice zone 5.1x at N=5. Water = DNA at same N (universal palindrome). The runs sit at Q < 1 by an input whose stated source does not carry it; where water actually sits is a band, 0.04 ≲ Q ≲ 4.6 |
...
| [Neural Gamma Cavity](NEURAL_GAMMA_CAVITY.md) | WITHDRAWN 2026-08-25: 97.3% pairing was the matching tolerance, Dale's law made no difference, the 18 unpaired modes were an ordering artifact. Standing: a limit cycle inside the gamma band at the same parameters, and a zero-multiplicity excess in the wiring |
