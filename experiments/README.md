...
| [Envelope Rise Boundary](ENVELOPE_RISE_BOUNDARY.md) | A finite atlas of full-state CΨ peak rises on the XY chain, read at matched (N, Q=J/γ, K): named N=4/N=5 rows rise above a reporting bar, the named N=3 rows resolve none (on the Heisenberg chain N=3 rises), and a finite scan puts the change across the bar near Q ≈ 27 at N=4 and Q ≈ 45 at N=5. The samples draw no N floor or critical contour; at N=2 successive maxima can rise too |
...
| [XOR Space](XOR_SPACE.md) | The N+1 fastest modes X^⊗N·P_k are left and right eigenvectors, so a state's XOR share is invariant: GHZ's coherence 100%, W's 0% (N ≥ 3); the mixed-XY predictor is retired |
...
| [Random Matrix Theory](RANDOM_MATRIX_THEORY.md) | Direct raw-multiset consecutive-gap ratios (N=2-7, 21,840 eigenvalues): zero gaps retained, 0/0 counted separately. N=7: 16,366 defined ratios, 16 undefined, mean 0.2021456120489688. No standard-ensemble calibration for this degenerate unresolved population. The Π P-type anticommutation is exact; the full irreducible-sector SRP class is OPEN. Band-frequency multisets likewise do not calibrate universality |
| [PT-Symmetry Analysis](PT_SYMMETRY_ANALYSIS.md) | Π is linear and order 4 globally, not PT; after Π²-parity resolution it supplies an involutive sectorwise P generator, while the full SRP class remains OPEN. The Fragile Bridge's spectral abscissa leaves zero, at generic couplings and two qubits per chain, at a second-order exceptional point on the real γ axis, with inversion and conjugate pairing retained; K≈403 is one finite-offset reading of its K ∝ 1/δ. In the composite depolarizing sweep the pairing error and γ_crit co-vary (r = +0.987); their causal contributions are not isolated |
...
| [Spectral Form Factor](SPECTRAL_FORM_FACTOR.md) | Raw non-unfolded frequency SFF of 21,840 eigenvalues (N=2-7). FFT candidates match ω_min within 1% at N=2-4,6 (N=4 third-largest, N=6 second-largest); no match at N=7 among reported candidates. The reached window shows modulation, compatible with integrability or fragmentation; not a universality-class proof. Bins use a multiplicity-dependent raw multiset density scale, not physical time regimes. |
...
| [Entropy-Rate and Spectral-Pairing Audit](ENTROPY_PRODUCTION.md) | For positive-rate mirror pairs, d_fast+d_slow=2Σγ and ln(d_fast/d_slow)=2 artanh(Δd/(2Σγ)) are algebraic identities. The script's separate ⟨exp(−Δd)⟩≈0.93 average retains all palindrome pairs, including stationary/max-rate endpoints; it is an ad hoc rate transform, not a Jarzynski observable. One Bell+ trajectory supplies a dS/dt curve; no temperature, heat/work, efficiency, or unique global stationary state is inferred. CΨ=1/4 and Var=1/4 are independent in that run |
| [Information Geometry](INFORMATION_GEOMETRY.md) | Bures metric g(CΨ) = 3.36 at the fold (FINITE, no singularity). θ shrinks the metric, does not regularize. The Bell+ trajectory runs exactly along a Bures geodesic (the Hamiltonian leaves it alone; states it moves leave the geodesic from the start). The path coefficient's coordinate-shape second derivative is −25, not a curvature. θ is a compass, not a coordinate |
| [Concentrator Qubit Mapping](CONCENTRATOR_MAPPING.md) | Optimal chain selection on IBM Torino's own coupling map. 359 chains, sacrifice ranking vs mean-T2 ranking: zero overlap in top-10. Sacrifice achieves 3.12x vs 1.19x protection. Worse qubits, better modes. Time-stable over 5 months |
...
| [Thermal Breaking](THERMAL_BREAKING.md) | The Q gain V(N) = 1 + cos(π/N), exact on the (0,1) block with γ cancelling (uniform γ only); the dephasing census 43 → 112 frequencies; a finite-occupation bath trades Q for frequencies; the sacrifice profile's advantage fades with n̄; the palindrome centre moves with the total rate and breaks only where amplitude damping shares Z-dephasing's axis; no self-consistent occupation exists (an identity) |
...
| [Absorption Theorem Discovery](ABSORPTION_THEOREM_DISCOVERY.md) | α=2γ⟨n_XY⟩ exact: absorption rate = 2×dephasing×light content. Linear in γ. Proven from L_H anti-Hermitian. Gives the boundary formula and sum rule a common reading; relocates the spectral gap. 1,342 modes, ratio 1.000000 and CV 0.0000 at the printed digits |
...
| [F1 Pair-Rate Balance](PI_PAIR_FLUX_BALANCE.md) | At N=5, ten unambiguous bond-perturbation matches preserve `Re(λ_s)+Re(λ_f)=−2Σγ` to machine precision; the aggregate greedy continuation is unresolved in degenerate spaces. The former binary-inheritance and `N≡4 mod 10` claims conflated linear F1 with the conjugate-composite map and are withdrawn |
...
| [Degeneracy Hunt](DEGENERACY_HUNT.md) | The apparent 14-fold N=5 cluster is basis-sensitive: Z-dephasing breaks Hamiltonian SU(2), while integer n_diff labels only computational-basis dissipator cells. Full eigenmodes obey Re(λ)=−2γ⟨n_XY⟩_v, possibly with fractional expectation; equality of that expectation fixes only the real part and does not explain a full complex degeneracy. Its complete multiplicity mechanism remains open |
...
| [Majorana Axis Modes](MAJORANA_AXIS_MODES.md) | Historical N=4 center-line scan. Its 18-mode self-Π classification and golden-ratio mechanism are withdrawn: it classified the conjugate-composite fixed line as the linear F1 fixed set. The corrected chain census has 14 F1-fixed eigenvalues at `λ=−Σγ` and 121 two-member F1 orbits |
...
| [Filling Threshold Chaos](FILLING_THRESHOLD_CHAOS.md) | Finite-size filling-associated crossover evidence at N=6..8 (`inspect --root fillcsr`): movement toward GinUE is stronger in dense than dilute blocks at canonical Delta=1 plus disorder; not a causal or thermodynamic threshold theorem |
...
| [The Shared Skeleton](THE_SHARED_SKELETON.md) | Six small systems (chain, water, benzene, butadiene, cyclobutadiene, hexatriene) at Q = 1.5 with their Liouvillian spectra overlaid: what is shared is law (F1 palindrome, dissipator cell rungs and full-mode absorption expectation, the real-axis diagonal); the fine structure is each Hamiltonian's own |
...
| [Fixed Point Shadow](FIXED_POINT_SHADOW.md) | The Q52 late-time residual and the pre-registered March question: not a boundary effect. Q52's own record answers the proposal (the coherence kept turning through the crossing, and the late direction is not the fixed point's but a static offset with the pattern of a measurement (SPAM) offset, mechanism open); the March test measured its qubits' detuning and could not see a residual of that size |
| [IBM Absorption Theorem](IBM_ABSORPTION_THEOREM.md) | Absorption Theorem ratio 1.03 on IBM Q52, the consistency of two fits to one decay; the rate ladder needs N ≥ 2. A qubit-frame detuning (−5.7 kHz as the alias nearest zero); a 2.8% static tail at the resolution limit, most likely a measurement (SPAM) offset |
...
| [γ₀ Is Always There](GAMMA0_IS_ALWAYS_THERE.md) | Historical locator for a four-point ibm_kingston J scan: the finite-time transfer overshoot changes between J=0.05 and 0.1; it does not measure γ₀, certify critical damping, or anchor F86 |
...
| [Path-K Diabolic](F89_PATH_K_DIABOLIC.md) | Complete N=5 and N=6 A2 inventories with local character; Tier 2 for the residual integrability mechanism, a conditional twin-scalar restriction consistent with finite-N Δ controls: under XXZ Δ the sampled crossings split into Jordan EP2s (N=4..7). The N=7 A2 inventory and an all-N existence mechanism remain open; the reflection-parity witness constrains real-axis placement |
...
| [Seed Existence Reduction](F89_SEED_EXISTENCE_REDUCTION.md) | Proves the all-odd endpoint-nullity surplus N−1. A finite-q defective seed additionally needs a literal count drop and local character: the drop is probed exactly at N=5,7,9 and measured at N=11, the character certified at N=5,7,9 and classified at N=11, and the all-odd implication remains open |
...
| [The Motion and the Missing Phase](THE_MOTION_AND_THE_MISSING_PHASE.md) | Exploration note: light on the centre seat of an odd chain leaves the reflection-odd motion (F157) and its X^N copy untouched while the phase between the copies fades at e^(−2γt), unseen by every page below N−2 sites (F70) yet a real entanglement loss; a CNOT decoder brings it onto the centre, and the cross-copy block is the Lattice's one-sided reading X^N ρ as a block of a state. With an end defect (XY, N = 7) one parity-forced blind vector survives and the decoded distance plateaus at 0.763 / 0.734. Short-time onsets live-witnessed in exact ℚ(i) (`inspect --root missingphase`). Two proofs settle its local N = 7 spectral and linear-readout questions: [the relaxation scale](../docs/proofs/PROOF_MISSING_PHASE_RELAXATION_SCALE.md) derives the slowest A rate (γ/2)ε² + O(ε³), and [the slow readout](../docs/proofs/PROOF_MISSING_PHASE_SLOW_READOUT.md) couples it to the end preparation: ZZ amplitude → 1, three odd pairs never see the slow cluster, and a Z-tagged three-site current reads what every pair loses (`inspect --root missingphasereadout`); the nonlinear d_out/d₂ lifetimes stay open |
...
| [The Endpoints are a Density Law](THE_ENDPOINTS_ARE_A_DENSITY_LAW.md) | Why the resonances saturate on the size-class centres: on scalar-parity eigenspaces the compressed-density identity follows from reflection, the parity census shows that covers everything up to N = 7, and on the R₉₀ locus it gives containment; endpoint attainment at positive γ̄ additionally needs pure vectors of the extreme size classes. The N = 8, Δ = 0 mixed-space law is derived in the successor proof, and Δ = 1 adds a uniform density s/N per size class. At N = 11 the XY `(1,1)` block breaks the identity and keeps its interval by a separate positivity proof; the interval's ends sit in the zero-frequency room, where the identity holds and the compression is −4γ̄(I − G) with F143's Gram G, and the lower end is F140's frozen root at every J |
