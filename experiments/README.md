...
| [Random Matrix Theory](RANDOM_MATRIX_THEORY.md) | RMT analysis (N=2-7, 21,840 eigenvalues): Poisson level statistics (⟨r⟩=0.383), compatible with integrability or block fragmentation; this does not prove integrability. Chiral symmetry (class AIII) exact to machine precision. An early within-band GOE hint at N=5 was driven to a verdict and is small-sample noise |
| [PT-Symmetry Analysis](PT_SYMMETRY_ANALYSIS.md) | Π is a linear order-4 chiral operator (class AIII, NOT PT). The Fragile Bridge transition is a genuine EP on the real γ axis at γ_crit; K≈403 is one finite-grid approach reading, not another complex-γ EP. Depolarizing noise breaks the palindrome but stabilizes the sampled system (r = +0.988) |
