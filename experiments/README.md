...
| [IBM Cavity Spectral](IBM_CAVITY_SPECTRAL_ANALYSIS.md) | Cavity modes meet IBM hardware: sacrifice zone protects the slowest oscillating modes at 2.81x (vs 1.97x measured). Same 43 frequencies, different damping. Palindromic to the eigensolver's floor under 26x asymmetric noise |
...
| [Spectral Form Factor](SPECTRAL_FORM_FACTOR.md) | SFF of 21,832 eigenvalues (N=2-7). Palindromic modulation at ω_min confirmed (<1% match, N=2-4,6). Visibility fades as ~1/4^N. t_Π/t_H → 0 (modulation is short-time). The light-content bands at w and N-w have identical SFF. Neither Poisson nor GUE: unique palindromic signature |
