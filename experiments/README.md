...
| [PT-Symmetry Analysis](PT_SYMMETRY_ANALYSIS.md) | Π is linear and order 4 globally, not PT; after Π²-parity resolution it supplies an involutive sectorwise P generator, while the full SRP class remains OPEN. The Fragile Bridge has a real-γ spectral-abscissa axis departure with inversion and conjugate pairing retained; K≈403 is one finite-offset simple-mode non-normality reading, while EP character remains OPEN. In the composite depolarizing sweep the pairing error and γ_crit co-vary (r = +0.987); their causal contributions are not isolated |
