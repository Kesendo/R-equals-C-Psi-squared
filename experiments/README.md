# Experiments: Testing R = C*Psi*^2 Against Known Physics
...
The value 1/4 is the bifurcation point where real fixed points of the quadratic iteration z -> z^2 + c merge and become complex. This is well-known mathematics. What is new: the self-referential iteration R_{n+1} = C(Psi + R_n)^2 from the R = C*Psi*^2 consciousness framework produces the same boundary at C*Psi = 1/4. Same equation, different origin. Two independent paths to the same phase transition, forty years apart.
...
The measurement problem asks why quantum systems "choose" definite outcomes. In this framework, they don't choose. The iteration R_{n+1} = C(Psi + R_n)^2 runs until decoherence pushes C*Psi below 1/4. At that moment, complex fixed points become real. The oscillation freezes into a value. No choice, no collapse -- a phase transition. The apparent "randomness" of the outcome is the phase theta at the moment of crossing, deterministic in the complex regime but unreadable by classical observers.
...
- The equivalence between R = C(Psi + R)^2 and z -> z^2 + c bifurcation structure
- Complex fixed points above 1/4 with frequency theta
