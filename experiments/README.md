...
| [The Endpoints are a Density Law](THE_ENDPOINTS_ARE_A_DENSITY_LAW.md) | On scalar-parity eigenspaces the compressed-density identity follows from reflection and gives containment on the R₉₀ locus; endpoint attainment at positive γ̄ additionally needs pure vectors of the extreme size classes. The N = 8, Δ = 0 mixed-space law is derived in the successor proof, and the N = 11 XY `(1,1)` block has a separate positivity interval with block-wide endpoint witnesses even where the conditional identity fails |
