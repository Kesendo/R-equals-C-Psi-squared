...
The leakage's slow complex residue is weaker, `Tr(QM₋) = [(−1+i√2γ)/16]ε² + O_γ(ε³)`, and the physical pairs (1,3), (1,5), (3,5), using zero-based sites, miss this cluster exactly by chirality. At the uniform point the sine quadrature is invisible to every two-site map while the decoder retains half trace norm 1/2 for the isolated contribution. On a complex residue, the linear version of §8.2 uses `v = (M_ab+M_ba)/2`; `Re M_ab` applies only to a Hermitian contribution. The live exact [`MissingPhaseSlowReadoutWitness`](../compute/RCPsiSquared.Diagnostics/Foundation/MissingPhaseSlowReadoutWitness.cs) reconstructs the uniform residue, the physical maps and the leakage derivative:

```powershell
dotnet run --project compute/RCPsiSquared.Cli -c Release -- inspect --root missingphasereadout
```

The historical `d_out` still subtracts a moving unitary reference before taking its norm, and `d_2` then maximizes such norms over pages. Their nonlinear lifetime questions do not follow from the isolated envelope or decoder norm. Those questions, the all-odd-N extension and a neighbourhood uniform in γ remain separate from the solved local spectral and linear-readout results.
