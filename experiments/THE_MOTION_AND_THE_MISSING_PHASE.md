...
The [proof](../docs/proofs/PROOF_MISSING_PHASE_RELAXATION_SCALE.md) derives the exact invariant reduction, the complete ε = 0 peripheral splitting and local minimality. Its independent [certificate](../simulations/results/missing_phase_relaxation_scale.json), produced by [`missing_phase_relaxation_scale.py`](../simulations/missing_phase_relaxation_scale.py), follows the two-dimensional cluster with precision doubling and keeps the historical ε = ±0.1 generator as a cross-check. `τ_gap` is an inverse spectral scale, not a proved lifetime of `d_out`, `d_2`, or any other observable; preparation and readout coupling remain to be shown. The surviving questions are whether the law extends to all odd N, whether its punctured-neighbourhood control can be made uniform in γ, and which observables couple to the slow cluster.
