...
**Status:** Exploration note. The encoding identities, the F70 invisibility, the negativity and the veil identity B_γ(t) = e^{−2γt}·A_{−γ}(t) of §8.1 are exact algebra assembled from F157, F70, F158 and the gamma fold, at every Δ. The short-time defect coefficients (Δ = 0) are derived in §8 and their exact-arithmetic checks at N = 5, 7, 9, 11, 13 run live, in ℚ(i), in the object §9 names. The N = 7 long-time run (Δ = 0, t ≤ 6000, two grids) is floating point with its diagnostics beside the data. No hardware run and no F number.
**Date:** 2026-09-05
...
`compute/*/Numerics` was swept for the arithmetic §9 needs, and it held ℚ(i) once: a private struct inside `FoldResultantCertificate`, unreachable from outside, beside a public ℤ[i] that is division-free on purpose. A Taylor recurrence divides by k+1 at every order, so the field is what it needs and the ring is not; the field now lives at [`GaussianRational`](../compute/RCPsiSquared.Core/Numerics/GaussianRational.cs) and the certificate uses it, one copy rather than two. No Taylor-coefficient engine and no trace norm existed anywhere, and none was built: the leading coefficients of §8 are reached through the closed reductions of §8.2 and §8.3, which is why they stay rational.
...
The exact-arithmetic checks live in [`MissingPhaseOnsetWitness`](../compute/RCPsiSquared.Diagnostics/Foundation/MissingPhaseOnsetWitness.cs), which iterates the recurrence above in exact ℚ(i) at inspect time and compares every onset and every leading coefficient with the closed forms of §8 by exact equality:

```powershell
dotnet run --project compute/RCPsiSquared.Cli -- inspect --root missingphase
```

The tree carries N = 5, 7, 9, 11, 13 at ε = 1/3, γ = 2/5, and the reduction of §8.1 against the full 2^N Lindbladian at N = 5. Its [tests](../compute/RCPsiSquared.Diagnostics.Tests/Foundation/MissingPhaseOnsetWitnessTests.cs) add N = 7 at ε = −1/3, −1, −3 (a severed and a sign-reversed end bond among them) and the ε = 0 control, where δA vanishes identically while δB does not, B wearing the veil whatever the chain does, and only its centre row, the part the decoder reads, is zero.

How far the two routes are independent is worth stating exactly, because it differs by reading. The propagation is independent: §8 counts hops, the recurrence iterates the generator. Of the three readout maps only the leakage is, computing Tr(QδA) off the coefficient matrix where §8.4 runs a Dyson integral; the other two evaluate the reductions §8.2 and §8.3 derive, so on those the agreement confirms the coefficients rather than the reduction, and the reduction is gated separately. These are finite checks of the derivation, not its all-N proof.
...
The object keeps what the reduction needs and no more: the B block and the decoded output are retained, because the ordinary single-excitation propagator alone loses the intercopy coherence and with it d_out. Its arithmetic is ℚ(i) rather than ℤ[i] because the recurrence divides by k+1 at every order. No OpenArcs item or hardware confirmation is closed by this note.
...
The defaults are γ = 0.3 and t_max = 6000. Each run writes a full CSV and its JSON summary beside it. This is a one-shot Python experiment producer, a different route to different quantities than the exact short-time object of §9; neither reads the other's output. The framework supplies the Hamiltonian commutator; the two reduced dissipators are the diagonal rules of §8.1, applied directly in γ without a square-root round trip: passing √γ·z as a jump operator and squaring would put A and B at rates differing by rounding, and the fifth test asserts the adjoint identity L_B + L_A† = −2γ·Id exactly.
...
The generator question closes locally at N = 7. For fixed γ > 0 and one end hopping `2(1+ε)`, the slowest nonzero rate of the 49-dimensional A block in `(1,1)` is

    Δ_A(ε) = (γ/2)ε² + (γ/2)ε³ + O(ε⁴),
    τ_gap := 1/Δ_A ~ 2/(γε²).

The mechanism is the quadratic centre-light acquisition of the broken blind mode: the continued rank-two oscillatory cluster has centre-charged operator weight `w_c = ε²/4 + ε³/4 + O(ε⁴)`, and the Absorption Theorem gives `Δ_A = 2γw_c`. This makes the order of limits above singular without turning it into a finite-time jump. At ε = 0 the relevant mode is exactly blind; for every sufficiently small nonzero ε it fades, but on a spectral scale diverging as ε⁻².

The [relaxation proof](../docs/proofs/PROOF_MISSING_PHASE_RELAXATION_SCALE.md) derives the exact invariant reduction, the complete ε = 0 peripheral splitting and local minimality. Its independent [certificate](../simulations/results/missing_phase_relaxation_scale.json), produced by [`missing_phase_relaxation_scale.py`](../simulations/missing_phase_relaxation_scale.py), follows the two-dimensional cluster with precision doubling and keeps the historical ε = ±0.1 generator as a cross-check.

The [physical slow-readout proof](../docs/proofs/PROOF_MISSING_PHASE_SLOW_READOUT.md) supplies the preparation/readout coupling for this same end state. The full rank-two Riesz residue M₋ of A_init = d₀d₀† has endpoint ZZ residue −1/2 at ε = 0. Hence the slow contribution to ⟨Z₀Z₆⟩ is `2 Re[f_ZZ(M₋) exp(λ₋t)]`, with an envelope amplitude tending to 1 and decay scale `τ_gap ∼ 2/(γε²)`. The corresponding XX amplitude tends to 1/2. These are local statements at fixed γ > 0 about a linear expectation value; the complete signal also contains the stationary part and other modes.

The leakage's slow complex residue is weaker, `Tr(QM₋) = [(−1+i√2γ)/16]ε² + O_γ(ε³)`. The physical pairs (1,3), (1,5), (3,5), using zero-based sites, miss this cluster exactly at every small ε, because the surviving blind vector of §10.1 has only even-site entries; at ε = 0 the even pairs (0,2), (0,4), (2,6), (4,6) miss it as well, and once the defect is on they see it at order ε. At the uniform point the sine quadrature is invisible to every two-site map while the decoder retains half trace norm 1/2 for the isolated contribution, and the three-site product Y₀X₁Z_k reads it for every k from 2 to 6. On a complex residue, the linear version of §8.2 uses `v = (M_ab+M_ba)/2`; `Re M_ab` applies only to a Hermitian contribution. The live exact [`MissingPhaseSlowReadoutWitness`](../compute/RCPsiSquared.Diagnostics/Foundation/MissingPhaseSlowReadoutWitness.cs) reconstructs the uniform residue and its completeness, the physical maps with their null tiers, the three-site reading and the leakage derivative:

```powershell
dotnet run --project compute/RCPsiSquared.Cli -c Release -- inspect --root missingphasereadout
```

The `d_out` of §7 subtracts its matched γ = 0 trajectory, a moving unitary reference, before taking its norm, and `d_2` maximizes such norms over pages. Their nonlinear lifetime questions do not follow from the isolated envelope or decoder norm. Those questions, the all-odd-N extension and a neighbourhood uniform in γ remain separate from the solved local spectral and linear-readout results.
