...
That 0.974 comes with a caveat the recipe above does not carry, and it is larger than the sign question. **The selected slow mode sits in a doubly degenerate eigenvalue**, so "the first SE-accessible mode" names a two-dimensional subspace rather than a vector, and which basis vector emerges is whatever the eigensolver returned first. On this profile numpy and scipy disagree outright: SE ratio 1.0000 against 0.0351, and fidelity 0.974 against 0.281. Step 5 above therefore under-determines its own output, and so does `LensAnalysis.cs`, which runs the same first-match rule at the same 0.01 threshold. The F9 rows are unaffected in their conclusion (both solvers give 0.0022 there), but the gradient row's number is solver-dependent. What would make the selection well posed is an open question, not a repair, and it is the arc `concentrator_amplitude_signs`.
