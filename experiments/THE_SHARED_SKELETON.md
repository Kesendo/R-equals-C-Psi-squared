...
What all systems share is **law-determined**, identical because the dephasing is identical: the [F1 palindrome](../docs/proofs/MIRROR_SYMMETRY_PROOF.md) symmetry (every spectrum is mirror-symmetric about its own centre −σ) and the [absorption law](../docs/proofs/PROOF_ABSORPTION_THEOREM.md). A Pauli-basis cell has the integer dissipator cost −2γn_XY; a full eigenmode has Re λ=−2γ⟨n_XY⟩ and may lie between those rungs after Hamiltonian mixing. Where the systems part is **Hamiltonian-determined**: water's imaginary parts are tighter (the ZZ term compresses the turn frequencies), benzene and hexatriene are wider (more sectors), the rings differ from the chains. Verified from below: the only real-axis eigenvalues shared exactly by all three of chain/water/benzene are {0, −0.2, −0.3, −0.4} (the kernel plus a few integer rungs); the rest of each spectrum sits at its own places. The molecules are mostly distinct fingerprints that all obey one symmetry and one rate-law.
...
That clean gradient settles what the self-similar structure (the bands read by eye) is: it is NOT the sectors (those ARE the horizontal colour gradient: one sector is a vertical line of constant Re); the bands are orthogonal to that, horizontal strips stacked along Im, the Hamiltonian's frequency comb. The two axes separate: colour = what the light counts (the diagonal), structure = what the molecule does (the Hamiltonian).

And that is the name for all of it. **These are light spectra.** γ is the light falling on the system (the chain standing in the light); the sector is the light content, popcount(i ⊕ j), the lit-site count. Literally, in the language of spectroscopy: **Im λ is where the molecule's spectral lines sit (the transition frequencies), and Re λ is how broad each line is (the linewidth, the decoherence broadening, = −2γ·light).** The (Re, Im) plane is the molecule's spectrum with its lineshapes: the vertical lines are its colours, the horizontal spread is how the arriving light blurs them. That is why a spectroscopist would recognise these at a glance.
