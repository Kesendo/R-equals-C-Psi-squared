...
*Later (2026-05-16):* the deliberate Ω injection became operational in the Kingston Confirmation `f95_angle_steering_kingston_may2026` (Heron r2; complex-CΨ angle actively steerable via per-chunk RZ injection at rate Ω; 3 of 4 conditions, residuals 6.81° to 15.69°). The arc-length √(1 + (Ω/(4γ·|CΨ_com|_cross))²) is structurally the same square-root form that [F95](../docs/ANALYTICAL_FORMULAS.md#f95) gives for the angle θ(c; b) = arctan(√(c/b² − 1)). The 2D c-plane reading: the radial dwell is F57 K_dwell, the steered angle is arg(CΨ_com), the phase of the complex coherence, which shares F95 θ's square-root form in its arc length but is not θ itself, and the 1/4 cusp is where both readings meet. See [`PROOF_F95_ANGLE_AT_QUADRATIC_ZERO.md`](../docs/proofs/PROOF_F95_ANGLE_AT_QUADRATIC_ZERO.md) and [`reflections/ON_HOW_GAMMA_BECAME_THE_TICK.md`](../reflections/ON_HOW_GAMMA_BECAME_THE_TICK.md).
