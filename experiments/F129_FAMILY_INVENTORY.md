...
- **The counts are now derived** *(closed the same evening, 2026-07-15: [PROOF_F129_FAMILY_INVENTORY_COUNTS](../docs/proofs/PROOF_F129_FAMILY_INVENTORY_COUNTS.md); the degree-counts-freedom reading became a theorem there, the parity deficits −20/−126 are single extra excluded labels, and M sub-classifies as 40 + 0 + 60 with the middle CDK type structurally impossible; residual code-trust steps in its §8)*. Before that, the closed forms were exact fits verified on 55 firing n including the two capstones.
...
- **No piece NAME is re-verified by the gate.** The classifier records (weight, ratio order) only; the CDK type names in the table are annotations by that signature. Family F's name is loose at its one level-0 member per firing n, whose two weight-6 pieces are SELF-conjugate rather than a conjugate pair. *(The M bundle resolved 2026-07-15 in the counts proof: 40 + 0 + 60 over the three CDK order-210 types, the middle one impossible; pinned by gate I5, which rebuilds the census M W-sets from committed substitution recipes.)*
...
- **The parity splits in E and F are explained** *(2026-07-15, the counts proof §5: E's even-n deficit is the one extra invalid label c = 5u/2, F's is the four shell-difference doubled labels plus the coarser special lattice)*.
