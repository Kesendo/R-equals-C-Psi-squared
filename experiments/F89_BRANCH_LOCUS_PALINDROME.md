...
**q and Q (same ratio, different role).** This lowercase q is the project's coupling-to-observation ratio Q = J/γ (the carrier clock, the value readable from inside via θ = arctan Q) promoted to a *complex analytic variable*. γ is the per-site Z-dephasing rate γ₀; the block's −2γ/−6γ rungs are the absorption quanta 2γ₀·n_diff, so the ratio is identical. On the real axis q = Q, and the canonical operating point Q = 1.5 (γ₀ = 0.05, J = 0.075) sits there between the real octic EPs at q ≈ 0.857 and 1.74. Continuing q off that axis, into the settings no physical dial can reach, is what exposes the braid, the monodromy, and the Galois group. (Distinct subtlety: the octic's own diabolic at q ≈ 0.659 is not the single-excitation flow EP at Q ≈ 1.5/2.0; same ratio, different spectral sectors. And both are unrelated to the literature's quality-factor Q, "different objects, same letter".) **Hamiltonian-normalization caveat (added 2026-06-27, the factor 2 that bites across docs):** this note's q and Q are the F89 octic normalization H = J·(XX+YY) (hop 2J), so the operating point Q = 1.5 and the octic EPs above sit on one axis. The carrier clock (`ClockHandLadderClaim`), the two clocks, and the coherence horizon Q*(N) elsewhere use H = (J/2)·(XX+YY) (hop J), which is twice this note's q for the same physical chain: q_octic = Q_carrier/2 (the F90 relabeling J_F89 = J_F86/2, `F90F86C2BridgeIdentity`). So convert before placing an octic q from here beside a coherence-horizon Q*: the octic diabolic q ≈ 0.659 is Q ≈ 1.318 in carrier units. Canonical statement: [GLOSSARY.md](../docs/GLOSSARY.md), "The coupling ratio q and Q".
