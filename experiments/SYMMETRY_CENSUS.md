...
Four symmetries were checked. Three commute with the Liouvillian as a superoperator. One (Π) relates eigenvalues but does not block-diagonalize.

### 1.1 U(1) excitation-number conservation

**Generator:** N_bra = Σ |1⟩⟨1|_k acts on the bra side; N_ket on the ket side.
...
This decomposes the operator space into (N+1)² sectors labeled by (w_bra, w_ket) with w ∈ {0, ..., N}.

### 1.2 n_XY parity: redundant with U(1)

**Finding:** The n_XY parity of the operator basis element |i⟩⟨j| is popcount(i ⊕ j) mod 2, which equals |w_bra − w_ket| mod 2. This is entirely determined by the excitation-number sector (w_bra, w_ket).

**Proof sketch.** The Hamming distance d(i, j) = popcount(i ⊕ j) has the same parity as |popcount(i) − popcount(j)|, because each bit flip changes the weight by exactly ±1, so reaching weight w' from weight w requires at least |w − w'| flips, and any additional flips come in cancel-pairs (flip and unflip), preserving parity.
...
### 1.5 Π conjugation (spectral mirror, not block-diagonalizing)

The [Π operator](../docs/proofs/MIRROR_SYMMETRY_PROOF.md) satisfies LΠ = −Π(L + 2Σγ·I), which implies that for every eigenvalue λ, the value −λ − 2Σγ is also an eigenvalue. This produces the palindromic spectrum but does NOT block-diagonalize L (Π does not commute with L; it anti-commutes up to a shift).
...
| N | D | d² | Basic sectors (w_bra, w_ket) | With spin-flip |
|---|---|-----|------------------------------|----------------|
| 3 | 8 | 64 | 16 | 16 |
| 4 | 16 | 256 | 25 | 26 |
| 5 | 32 | 1024 | 36 | 36 |
...
The basic sector count is always (N+1)². For even N, one additional spin-flip sector appears from the self-conjugate sector (N/2, N/2).
...
| (0, N) and (N, 0) | 0 | No | 1 only | 2Σγ = 1.000 |
...
|----------|-------|------------|---------------------|----------------|----------------|
| Chain | 3 | 5 | 127 | 14 | Reflection ℤ₂ |
| Ring | 4 | 5 | 73 | 24 | Cyclic C₄ |
| Star | 3 | 5 | 75 | 16 | Leaf permutation S₃ |
| Complete | 6 | 5 | 43 | 36 | Full permutation S₄ |
...
2. **High degeneracies explained.** For N=5 uniform chain, max multiplicity is 14. The known block-diagonalizing symmetries predict at most 4× degeneracy, but the [absorption theorem](../docs/proofs/PROOF_ABSORPTION_THEOREM.md) rate formula Re(λ) = −2γ·n_XY places modes from different sectors at the same grid values (multiples of 2γ). SU(2) is broken by dephasing. See [Degeneracy Hunt](DEGENERACY_HUNT.md) for the full analysis.
...
4. **Sector-resolved spectral form factor.** The SFF analysis in [the D09 sector-SFF-pairing derivation](../docs/proofs/derivations/D09_SECTOR_SFF_PAIRING.md) works by XY-weight sectors. The Census shows these are (N+1)² sectors labeled by (w_bra, w_ket). A sector-resolved SFF analysis could reveal whether the transient dynamics within each sector shows universal (RMT) statistics or sector-specific structure.
