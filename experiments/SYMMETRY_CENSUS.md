...
2. **High degeneracies explained.** For N=5 uniform chain, max multiplicity is 14. The known block-diagonalizing symmetries predict at most 4× degeneracy, but the [absorption theorem](../docs/proofs/PROOF_ABSORPTION_THEOREM.md) rate formula Re(λ) = −2γ·n_XY places modes from different sectors at the same grid values (multiples of 2γ). SU(2) is broken by dephasing. See [DEGENERACY_HUNT](DEGENERACY_HUNT.md) for the full analysis.
