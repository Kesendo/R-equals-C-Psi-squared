...
4. **Block-resolved spectral form factor.** The existing SFF producer groups full-spectrum eigenvalues into approximate decay-rate/average-light bands; [D09](../docs/proofs/derivations/D09_SECTOR_SFF_PAIRING.md) proves equality for reflected bands. Those bands are not invariant fixed-XY-weight eigenspaces. The Census instead supplies exact joint-popcount blocks labeled by (p,q). An unfolded SFF within irreducible pieces of those blocks could test for block-specific spectral statistics without conflating the two decompositions.
