...
- **The dissipation.** Uniform Z-dephasing at rate γ on every site: `D[ρ] = γ Σ_l (Z_l ρ Z_l − ρ)`. `σ := N·γ`, so the palindrome reflects as `λ ↦ −2σ − λ` about the centre `−σ = −Nγ`, and `ΔE_max(H)` denotes the spectral spread `E_max − E_min` of H.
- **The ratio.** `Q := J/γ`, the dimensionless coupling-to-dephasing ratio. Note the canonical anchor table [`docs/Q_REGIME_ANCHORS.md`](../docs/Q_REGIME_ANCHORS.md) defines `Q = J/γ₀` against a **fixed** code-convention substrate `γ₀ = 0.05`; the two agree exactly when γ = γ₀, which is the setting of the Q-sweep below. The other setting used here is written out wherever it appears, `J = 1, γ = 0.5` (so `Q = 2`), and it deliberately has no name; see the note below.

  *Naming note (2026-08-06).* This setting used to be called the "Marrakesh convention". It is written out in full now, and the point of the note is that it should stay that way. The name was coined on 2026-05-19 (`fff9a5c`), in the sprint that produced these anchors; the gloss "named for the hardware run the repo calibrated against" was added later, on 2026-07-31 (`f49d09e`). What is checkable is not the chronology but the hardware, and the hardware says no. The Marrakesh calibration ([`gamma_0_marrakesh_calibration`](../compute/RCPsiSquared.Core/Confirmations/ConfirmationsRegistry.cs), 2026-04-29, job `d7mjnjjaq2pc73a1pk4g`) reads the same data through two models and gets `γ_Z = 0.05` continuous or `γ_Z = 0.1` with Trotter n=3; neither is 0.5, and no entry in the registry calibrates a Z-dephasing rate at 0.5. So the machine name lent hardware provenance to the one value that has none, and took it from γ₀ = 0.05, which has it. The distinction the old name drew is real and is kept: two settings do run side by side here, and before it the collision was invisible. What is dropped is the proper noun. A short name is what let a coincidence be read back as a reason, so the numbers are written out instead, and a replacement name would only invite the next misrouting. A short label is tempting and Q = 2 is the obvious candidate, but it does not pin this point either: Q = 2 is also reached at γ₀ = 0.05 with J = 0.1, a different absolute scale, so the numbers stay written out. The old name survives in commit messages and in `ClaudeTasks/`, both of which are history and are left alone.
...
The J = 2γ setting is the column Q = 2, where Im/σ = 1. That reading is the Q=2 specialization, not the law.
...
- Data: `simulations/results/f1_n8_n9_metrics/star_N{3..6}_python.json` and `star_N8.json`; `simulations/results/q_sweep_anchor/star_N{3..6}_Q*.json`. In the `f1_n8_n9_metrics` star JSONs, which are the Q = 2 runs at J = 1, γ = 0.5, the field `MaxImag` equals `N · GammaValue`; the `q_sweep_anchor` files run Q from 0.5 to 2.5 at γ₀ = 0.05, where that equality holds only in the Q = 2 column. (There is no `σ` field; `SigmaShift` is the reflection constant −2Nγ, which is the spectrum's floor and twice the distance from 0 to the centre, not the centre itself.)
