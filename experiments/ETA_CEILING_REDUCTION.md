...
2. **The exceptional couplings.** The step from the certificate's large-J regime to generic coupling is closed and needs no semicontinuity, but the finite exceptional set it leaves is REAL: at N = 5, 6, 7 there are 3, 6 and 11 couplings at which a further mode freezes, and those three counts are exact. At N = 5 and N = 6 the whole block has been enumerated and the extra mode is exactly one, an ℓ = 2 η multiplet that is a spin singlet and so never leaves the diagonal; at N = 7 neither the multiplicity nor the singlet character is read. No law is known for the count; that every exceptional mode is a singlet is proved at N = 5,
where the whole band was enumerated at every coupling at once, and at N = 6 on the block
(2,2); that (2,2) is the only N = 6 carrier is read numerically, and N = 7 stays measured. See [THE_EXCEPTIONAL_COUPLINGS](THE_EXCEPTIONAL_COUPLINGS.md).
