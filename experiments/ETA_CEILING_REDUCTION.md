...
2. **The exceptional couplings.** The step from the certificate's large-J regime to generic coupling is closed and needs no semicontinuity, but the finite exceptional set it leaves is REAL: on the block (2,2) there are 3, 6 and 11 couplings at N = 5, 6, 7 at which a further mode freezes, and those three counts are exact. There the extra mode is an ℓ = 2 η multiplet that is a spin singlet and so never leaves the diagonal: exactly, over all couplings, at N = 5, at one coupling at N = 6, and numerically over the whole band at N = 7. That block sees only the rungs ℓ ≤ 2, and higher rungs carry couplings of their own: at N = 6 the block (3,3) carries four further ones, exactly, at rung 3. So the exact counts are rung-2 counts, the complete count per N lives in the middle block, and no law is known for either. See [THE_EXCEPTIONAL_COUPLINGS](THE_EXCEPTIONAL_COUPLINGS.md).
