# The ceiling, reduced: an exact certificate on the multiplet count, and the law it is waiting for

*2026-07-27. [XY_FROZEN_BAND](XY_FROZEN_BAND.md) left the ceiling settled one N at a time, by an exact rank read on the block itself, and said what was missing was the argument that does all N at once. This note does not supply that argument. It does three other things. It moves the per-N certificate off the block and onto an object that is smaller by two orders of magnitude, so a rung that took hours on the block takes seconds, and rungs that were out of reach entirely become routine. What that buys is stated carefully below, because the two certificates do not certify the same thing: the block-level one settled a whole band at N = 9, this one settles one rung at a time. It turns the certificate into a NON-existence question, which is the direction a rank over a finite field can actually settle, and the earlier direction could not. And it states, with the evidence, the one closed form that would replace every per-N run with a single argument, together with the vectors that attain it and the reason exactly one N escapes. The structural half, the decomposition that makes all of this a question about where multiplets start, is proved in [PROOF_FROZEN_BAND_SO4](../docs/proofs/PROOF_FROZEN_BAND_SO4.md); this note is the measuring.*
...
What it reads, over two large primes (no square root of −1 is needed, the matrices being integer rather than Gaussian-integer):
...
- **It certifies the reduced first-order operator**, which by [PROOF_FROZEN_BAND_SO4](../docs/proofs/PROOF_FROZEN_BAND_SO4.md) Section 5 bounds the true multiplicity from above at generic coupling. With the proved floor holding at every coupling, that makes the multiplicity exactly ⌊N/2⌋ for generic J, on the rungs that are certified.
...
## The law that would replace it

Measured, not derived, and this is the piece the whole reduction is waiting for:
...
**What the law would buy.** The ceiling needs min 𝒦 > 1, that is ℓ(N − ℓ) > N + 1. On 2 ≤ ℓ ≤ ⌊N/2⌋ the product ℓ(N − ℓ) increases with ℓ, so the binding case is ℓ = 2 and the condition is 2(N − 2) > N + 1, that is **N ≥ 6**. One argument, every N, every rung.
...
The count of these vectors is C(⌊N/2⌋, 2), and it matches the exact rational nullity of [Z − 2ℓ(ℓ+1)I ; Ψ] at every N tested except two: **N = 11 gives 14 against 10, and N = 14 gives 29 against 21.** That surplus is a claim that vectors EXIST, so it needs the rank over ℚ and not over a prime, where a rank can only drop; gate check V9 computes it fraction-free (N = 11 by default, N = 14 under `--deep`).
...
1. **The upper bound.** min 𝒦 ≥ ℓ(N − ℓ)/(N + 1) on LW_ℓ ∩ V₀, or even just the weaker min 𝒦 > 1 there for ℓ ≥ 2 and N ≥ 6, which is all the ceiling needs. The extremal vectors above are the handle: they say what saturates, not yet why nothing beats it.
