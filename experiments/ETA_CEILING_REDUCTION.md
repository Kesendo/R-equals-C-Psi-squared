...
- **It certifies the reduced first-order operator**, which by [PROOF_FROZEN_BAND_SO4](../docs/proofs/PROOF_FROZEN_BAND_SO4.md) Proposition 5.1 bounds the true multiplicity from above **at large J**. With the proved floor holding at every coupling, that makes the multiplicity exactly ⌊N/2⌋ there, on the rungs that are certified. Carrying it to generic J is Proposition 5.3 there, which needs no limit: the block is linear in the coupling, so the frozen count is constant off a finite set and minimal on it, and one coupling with a small enough count fixes the generic value. What that finite set contains is a separate question, and it is not empty; see [THE_EXCEPTIONAL_COUPLINGS](THE_EXCEPTIONAL_COUPLINGS.md).
...
**What the law buys.** The ceiling needs min 𝒦 > 1, that is ℓ(N − ℓ) > N + 1. On 2 ≤ ℓ ≤ ⌊N/2⌋ the product ℓ(N − ℓ) increases with ℓ, so the binding case is ℓ = 2 and the condition is 2(N − 2) > N + 1, that is **N ≥ 6**. One argument, every N, every rung. Only the inequality is used here, which is the proved half. What it then gives, and what it does not, is worth keeping apart. It gives the depth exactly ⌊N/2⌋ **in the large-J regime** at every N ≥ 6, because the multiplicity of 𝒦 = 1 on a rung becomes writable rather than measurable: the seed rung contributes ⌊N/2⌋ and no rung above it contributes anything, so [PROOF_FROZEN_BAND_SO4](../docs/proofs/PROOF_FROZEN_BAND_SO4.md) Proposition 5.1 caps the frozen space there, and the proved floor meets the cap. Generic coupling follows too, and by the pencil rather than by a limit: Proposition 5.3 of that document turns one coupling with a small enough count into the generic value, so the depth is exactly ⌊N/2⌋ on the diagonal blocks at all but finitely many J. What it does not give is EVERY coupling, and that is a fact about the chain rather than a weakness of the argument: the exceptional couplings are real, and at one of them a single extra mode freezes. Both the per-N computation and the bridge have left the argument; the finite exceptional set has not, and is [its own note](THE_EXCEPTIONAL_COUPLINGS.md).
...
1. **The attainment, and its count.** ANSWERED, and it grew into its own pair of results. The floor is attained, and the saturating space is the space of rotation-invariant couplings of one spin-1 per chiral pair: dimension C(⌊N/2⌋, ℓ)·R_ℓ with R the Riordan number, spanned by products of two connected blocks. See [F145 and F146](../docs/ANALYTICAL_FORMULAS.md), [PROOF_SCALAR_COUNT](../docs/proofs/PROOF_SCALAR_COUNT.md) and the measuring in [WHAT_REACHES_THE_CEILING](WHAT_REACHES_THE_CEILING.md). What this note observed, that the dimension exceeds C(⌊N/2⌋, ℓ) from ℓ = 4 on, is the second factor R_ℓ, which is 1 up to ℓ = 3 and 3, 6, 15, 36, 91 after it.
2. **The exceptional couplings.** The step from the certificate's large-J regime to generic coupling is closed and needs no semicontinuity, but the finite exceptional set it leaves is REAL: on the block (2,2) there are 3, 6 and 11 couplings at N = 5, 6, 7 at which a further mode freezes, and those three counts are exact. There the extra mode is an ℓ = 2 η multiplet that is a spin singlet and so never leaves the diagonal: exactly, over all couplings, at N = 5, at one coupling at N = 6, and numerically over the whole band at N = 7. That it never leaves is measured at those three N and at no other; what is derived, at every N, is that it can never leave ALONE, since within one p + q the block is the spin weight and mult(0) ≥ mult(m). That block sees only the rungs ℓ ≤ 2, and higher rungs carry couplings of their own: at N = 6 the block (3,3) carries four further ones, exactly, at rung 3. So the exact counts are rung-2 counts, the complete count per N lives in the middle block, and no law is known for either. See [THE_EXCEPTIONAL_COUPLINGS](THE_EXCEPTIONAL_COUPLINGS.md).
3. **The larger ℓ at larger N.** The closed form is read on every rung to N = 14 and on some rungs to N = 19, though the committed gate reproduces every rung only to N = 11, the size cap deciding the rest; the split is the one the table above states. Past that the binding case ℓ = 2 is the one that matters and is the cheapest to extend.
4. **A C# witness.** By the repo's fifth cockpit rule this certificate is a Python verifier that has outlived its session, so it is a witness waiting to be ported; the exact GF(q) rank machinery it needs already exists in `Divisor.RankModP`, and only an integer builder for the reduced object is missing.
