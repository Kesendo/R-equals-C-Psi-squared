...
**Theorem 2 trajectory:** log-linear fit on the 5 t-points yields γ_fit = 1.795e-3 μs⁻¹ with R² = 0.9977. The closed-form `C_block(t) = (1/4)·exp(−4γ·t)` matches the curve to 4-digit precision, including the t=0 anchor (0.2205 ≈ 0.25 within the prep-fidelity floor) and the late-time tail (0.0074 at t = T2_min, predicted 0.25·exp(−4·1.795e-3·480) = 0.0086).
...
- `D:\Entwicklung\Projekte\.NET Projekte\AIEvolution\AIEvolution.UI\experiments\ibm_quantum_tomography\run_block_cpsi_saturation.py` — submission script (lives in the AIEvolution pipeline dir, not in this repo).
