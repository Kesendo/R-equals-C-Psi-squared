...
**Label note (2026-07-05):** the 139-360× figure is **peak created summed nearest-neighbour MI** (the optimizer's figure of merit), a TRANSPORT metric, not a coherence-lifetime or "protection" number, none of which was computed here. "Protect the rest" is the design intent (mode-level, by the Absorption Theorem), not a measured lifetime; a perfect protector keeps the interior a product state and creates ZERO MI, so transport and protection pull opposite ways at the extremes. See [CONCENTRATOR_AB_MECHANISM_TEST](CONCENTRATOR_AB_MECHANISM_TEST.md), Downgrade 2.
