...
**(b) Amplitude-damping-induced breakdown of the |Δn|=2 blind channel.** Under site-uniform amplitude damping D_A[ρ] = γ_1 Σ_i (σ^−_i ρ σ^+_i − ½{σ^+_i σ^−_i, ρ}), excitation number is **not** conserved. F70 itself (kinematic) still holds; what breaks is its operational consequence. Under U(1)-preserving dynamics an initial |Δn|=2 coherence stays in the |Δn|=2 block for all t and remains site-local invisible. Under AD, sector-mixing leaks |Δn|≤1 content from the |Δn|=2 seed (e.g. σ^−_i on the bra side of |vac⟩⟨S_2| generates a |vac⟩⟨S_1| component), which is site-local visible. **Prediction:** testing c_1_pr (purity-response) for probes with |Δn|=2 coherence content (e.g. (|vac⟩ + |S_2⟩)/√2) should transition from c_1_pr = 0 at γ_1 = 0 to non-zero at first order in γ_1. Kernel-level characterization (which `K_DD[n, m]` entries become non-zero, and their scaling) is deferred to empirical characterization.
