...
**(b) Amplitude-damping-induced breakdown of the |Δn|=2 blind channel.** Under site-uniform amplitude damping D_A[ρ] = γ_1 Σ_i (σ^−_i ρ σ^+_i − ½{σ^+_i σ^−_i, ρ}), excitation number is **not** conserved. F70 itself (kinematic) still holds; what breaks is its operational consequence. Under U(1)-preserving dynamics an initial |Δn|=2 coherence stays in the |Δn|=2 block for all t and remains site-local invisible. Under AD, sector-mixing leaks |Δn|≤1 content from the |Δn|=2 seed (e.g. σ^−_i on the bra side of |vac⟩⟨S_2| generates a |vac⟩⟨S_1| component), which is site-local visible. **Prediction:** a probe carrying pure |Δn|=2 coherence content, e.g. ρ_coh = (|vac⟩⟨S_2| + |S_2⟩⟨vac|)/2 (traceless Hermitian; usable as a linear perturbation because c_1_pr is linear in ρ_0), is expected to give c_1_pr = 0 exactly under U(1) and non-zero c_1_pr under γ_1 > 0. Scaling in γ_1 and kernel-level characterization (which K-entries become non-zero) remain open for empirical work. **Pitfall:** the physical superposition probe ρ_0 = |ψ⟩⟨ψ| with |ψ⟩ = (|vac⟩ + |S_2⟩)/√2 is NOT a clean probe for this test. Its diagonal content (|vac⟩⟨vac| + |S_2⟩⟨S_2|)/2 hits `K_DD[2, 2]_pr` which is not F70-protected (F70 annihilates only coherence blocks, not populations), giving a non-zero c_1_pr baseline even at γ_1 = 0. Verified numerically at N = 5: c_1_pr(|ψ⟩⟨ψ|, γ_1 = 0) = 1.352·10⁻³, matching `K_DD[2, 2]_pr / 4` to 5 figures (F70_AMPLITUDE_DAMPING_BREAK baseline diagnostic, 2026-04-21).
