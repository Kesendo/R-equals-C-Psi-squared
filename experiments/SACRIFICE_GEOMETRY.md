...
> *Partially addressed (2026-04-12): RETRACTED same day.* Initial attempt had a projection bug in the overlap measurement. Conclusion withdrawn. See CUSP_LENS_CONNECTION.md "Boundary test" retraction banner and review/MERGE_LOG_methodology.md. V2 task pending. Non-symmetric two-excitation states (Bell pair + displaced excitation) can reach the even-parity slow mode via their w=1 sector content, with overlap up to 12.6% at N=5. However, this overlap is with the even-parity slow mode, not the odd-n_XY modes. The odd modes remain inaccessible: the two-excitation states' overlap comes entirely from even-parity SE content. Whether ansatze beyond two-excitation states can reach the odd-n_XY modes remains open.
