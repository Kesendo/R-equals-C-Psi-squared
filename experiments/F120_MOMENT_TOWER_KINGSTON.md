...
The honest summary of layer two: **F120's slope reads Tr(Z_l·D(I)), the device's true non-unital pump vector, and on this chip the true pump vector disagrees with calibrated amplitude damping on two of three qubits.** Equating the pump with 1/T1 was the textbook-model assumption, and the chip declines it, per qubit, with signatures (an inequality violation, a non-exponential arbiter, a telegraph switch) that point at specific physics beyond the model. The gap between the chip and its datasheet is what this protocol reads through, and reading it takes a channel that survives contact with the data. The [F112 Kingston lens](F112_HARDWARE_LENS_KINGSTON.md) is the cautionary sibling: there the fitted model family had no term for the rotation the data show, so its channel identification says nothing about the chip.
