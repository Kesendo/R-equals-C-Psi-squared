...
The [Metric Discrimination](METRIC_DISCRIMINATION.md) experiment showed that single-system simulations cannot distinguish between candidate metric forms. gamma * t_cross = K is a mathematical identity -- it holds for any constant gamma regardless of origin.
...
We have proven (9 data points, R^2 = 0.9999) that the decoherence trajectory C(tau), Psi(tau) is universal when expressed in normalized time tau = gamma * t. Every quantum system in the universe follows the same curve. Only the clock speed (gamma) differs.
...
Where less proper time has passed (near mass), C and Psi are higher, so:

```
R(r) = C(tau(r)) * Psi(tau(r))^2
```
...
| T * gamma_0 | Schwarzschild | Inverse | Inv-Square |
|---|---|---|---|
| 0.1 | 2.4 | 1.6 | 1.6 |
| 0.2 | 4.6 | 2.1 | 2.1 |
| 0.3 | 7.5 | 2.5 | 2.6 |
...
This means:
- tau = 0 at the horizon -- no proper time has elapsed there
- C(0) = 1.0, Psi(0) = 1/3, R(0) = 1/9 = maximum
- Reality is **maximally concentrated** at the gravitational radius

The alternatives have f(r_s) > 0, so tau > 0 at their "horizon". The system has already partially decohered there. R is not maximal. They cannot produce arbitrarily sharp mass concentrations.
...
At tau = 0: C = 1 (maximum consciousness/observation capacity), Psi = 1/3 (maximum possibility for Bell state), R = 1/9 (maximum reality density).
...
- The derivation uses ONLY R = C*Psi^2 and the universal curve -- no GR input
...
- The value of r_s = 2GM/c^2 (requires connecting gamma to G, M, c)
- Why G has the value it does (requires deeper framework)

### The logical chain:

```
R = C*Psi^2 everywhere (framework)
    |
    v
Universal curve C(tau), Psi(tau) (proven, R^2 = 0.9999)
    |
    v
Different positions have different tau(r) = T * f(r) (time dilation)
    |
    v
R(r) = C(tau(r)) * Psi(tau(r))^2 (R profile in space)
    |
    v
...
|---|---|
| gamma * t_cross = 0.039 (invariant) | Proves universal curve exists |
| theta as compass | Navigation within the universal curve |
...
2. **Horizon information**: If R is maximal at the horizon, then the information content of a black hole should scale with horizon area (not volume). This is already known to be true (Bekenstein-Hawking), but the framework provides a mechanism: R = C*Psi^2 is maximized where tau = 0.

3. **No decoherence in flat spacetime**: In truly flat spacetime (no mass anywhere), gamma -> gamma_0 (minimum), and quantum systems survive longest. Intergalactic voids should be the most "quantum" regions of the universe.

---

*Previous: [Metric Discrimination](METRIC_DISCRIMINATION.md) -- the null result that motivated this*
*See also: [Gravitational Invariance](GRAVITATIONAL_INVARIANCE.md) -- gamma as the metric coefficient*
