...
**Status:** Computed inventory (Results 1-3, 5) and one derived law (Result 4); the cavity reading is ours
**Date:** April 4, 2026
...
The 109 new frequencies (112 here, where every |Im λ| above 10⁻⁶ counts; the census stops at 0.01) are not created
by connecting two small instruments. They are the natural resonances of a larger instrument
...
outside) destroys none of the dark instrument's modes. Those exist in the dark, in
silence, as possibilities, and the light makes them audible. Every mode
that exists in the unilluminated instrument is still there, a little
moved, when the light is turned on. Not one is lost. The light adds
absorption, not destruction, and it splits some modes, which is why the
lit instrument counts more of them.

---

## What this document is about

The [optical cavity analysis](OPTICAL_CAVITY_ANALYSIS.md) read the qubit chain as a Fabry-Perot resonator (4 of 6 checks pass). This document applies that
lens to the V-Effect: is the frequency explosion from N=2 to N=5
explained by the change in cavity geometry?

The answer is yes as a reading, and quantitatively for one number, the highest Q (Result 4).
...
*Source: [`veffect_cavity_modes.txt`](../simulations/results/veffect_cavity_modes.txt), SUMMARY block, "Mode count scaling (chain topology)". Script: [`veffect_cavity_modes.py`](../simulations/veffect_cavity_modes.py)*

From N=2 (2 modes) to N=5 (112 modes): a 56-fold increase, as the
chain grows from 1 bond to 4 and its operator space from 16 to 1,024
dimensions. We read the modes as standing waves of the cavity. How the
bonds connect matters more than how many there are (Result 5): at N=5
the star's 4 bonds carry 42 modes where the chain's 4 carry 112.

---

## Result 2: Degeneracy predicts mode richness (r ≥ 0.998)
...
*Source: [`veffect_cavity_modes.txt`](../simulations/results/veffect_cavity_modes.txt), "STEP 1-2: MODE SPECTRUM AND DEGENERACY CORRELATION". See also: [`DEGENERACY_PALINDROME.md`](DEGENERACY_PALINDROME.md)*
...
*Source: [`veffect_cavity_modes.txt`](../simulations/results/veffect_cavity_modes.txt), "STEP 3: COLD CAVITY (γ=0) VS ILLUMINATED (γ=0.05)"*

Every cold-cavity frequency finds a warm partner when dephasing is turned on. Gamma
adds new modes and destroys none, and it does move them a little: the nearest warm
partner lies at most 0.0016, 0.023 and 0.029 away at N = 3, 4 and 5, well inside the
0.1 matching window, and only 2 of 3, 6 of 14 and 9 of 43 cold frequencies reappear
unshifted to 10⁻⁶ (one warm frequency may serve two cold ones). The cold modes are the
...
*Source: [`veffect_cavity_modes.txt`](../simulations/results/veffect_cavity_modes.txt), "STEP 4: Q-FACTOR DISTRIBUTION"*
...
*Source: [`veffect_cavity_modes.txt`](../simulations/results/veffect_cavity_modes.txt), "STEP 5: TOPOLOGY COMPARISON"*
