...
**Three blocks, one free-fermion kernel** (the unity these relations point at, so a returning reader sees one object, not three): this (1,2)=(SE,DE) octic, F73's (vac,SE)=(0,1) block, and the coherence horizon's (1,1)=SE-SE block are the three coherence sectors of ONE chain's Liouvillian, all built from the SAME single-particle dispersion `ε_k = 2J·cos(πk/(N+1))` (entering each via −i[H,·] as a signed sum: a single mode for (0,1), a difference `ε_a−ε_b` for (1,1), a sum `ε_a+ε_b−ε_c` for (1,2)) on the SAME absorption diagonal `−2γ·n_diff` (the [Absorption Theorem](../docs/proofs/PROOF_ABSORPTION_THEOREM.md), the one diagonal the light writes). So F89 reads how this one object's rates braid in complex q; PTF reads how its eigenvectors shift in real t; the horizon reads where two of its rates meet defectively; all threaded by the one dose K=γ₀t. The bridge is the shared kernel, not a shared EP, and the defective-vs-diabolic EP-character seam is itself one phenomenon of this kernel (single-particle coalescence vs frequency-difference coincidence; the compass lives in `CoherenceHorizonClaim` / `F89Path3OcticEpClaim` / `EpCharacterWitness`).
