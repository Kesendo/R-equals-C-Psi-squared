...
- **The survivor is not the EP.** The co-located band-edge mode 2cos(π/(N+1)) is the γ-protected survivor (r ≈ 1), sharing the gap Re = −2γ at N = 2, 3 only, where the Absorption Theorem pins both (both ⟨n_diff⟩ = 1); from N = 4 the coalescer's coherence share exceeds ½ and it sits below the floor by 2γ(2w2 − 1), the gap being the survivor's alone (its excess light 2w2 − 1, `inspect --root horizon`). The divergence is the coalescer's, not the survivor's.
