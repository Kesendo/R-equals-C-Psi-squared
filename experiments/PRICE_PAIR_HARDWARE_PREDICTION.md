...
**Status:** PREDICTION, written and committed BEFORE any hardware shot (the pre-registration discipline; the fresh-context review round is recorded below before submission). No hardware data exists for this protocol yet (pipeline inventory 2026-07-03: the pattern-pair coherence-decay measurement is absent from all 96 result files). See [`FOLD_SHADOW_IN_EXISTING_HARDWARE.md`](FOLD_SHADOW_IN_EXISTING_HARDWARE.md) for the detailed existing-data check that confirms this: the fold peeks through only as a degraded shadow (the orphan `block_cpsi_ladder` run, fold ratio 1.36 vs the ideal 3.0; and `zn_mirror`'s mirror-odd ⟨I,X⟩), which is why this clean run is genuinely needed. *(As-registered status, preserved verbatim. The campaign has since RUN: four pre-registered runs on ibm_marrakesh, 2026-07-04; see the HARDWARE RECORD sections and the CAMPAIGN VERDICT at the end.)*

**Date:** 2026-07-03 (pre-registration); 2026-07-04 (runs 1-4 + campaign verdict)
...
a pattern-independent device invariant: every complementary pair of coherence patterns pays the same total. This is the F1 palindrome centered at Σγ, the fold-lattice lemma (PROOF_CODIM1_BY_ADDITIVITY §7) read at the ρ level, and the same physical price that MirrorWorld's `mirror`/`anti` run modes pay as 2Nγ (a convention bridge, not an identity of symbols: MirrorWorld's γ is the Lindblad rate with per-coherence decay 2γk, so its 2Nγ equals this doc's P = Σγ_j only once γ_here = 2γ_MirrorWorld, γ_j here being the measured single-coherence rate).
...
~33 circuits × 4096 shots ≈ 2 QPU minutes (Batch, ibm_kingston), inside the free monthly tier. Gate order: this document committed → fresh-context adversarial review (two independent lenses, findings folded back into the runner and this spec, recorded below) → fresh calibration pull → Tom's explicit go → the one Batch job. (As-planned figures. As-run the scope grew: the rebuild raised shots to 8192 on ~80 circuits, and the campaign became four pre-registered runs on ibm_marrakesh, ~23 QPU min total, with runs 3-4 drawing on Tom's released paid reserve rather than the free tier; see the run records and the CAMPAIGN VERDICT below.)
...
**Answer 2: the c-tomograph caught a real bond, and BOTH rulers agree.** Branch splitting reads **c₀₂ = +0.00253 ± 0.00049 (5.2σ)** on the OUTER pair (93, 95), with the two nearest-neighbour bonds local. Independently, the complement tomograph shows exactly the pairing with singleton q1 (94, the middle) elevated (0.0371 vs ~0.026), which by the corrected decoding row names the OPPOSITE bond (93, 95): the same bond, from a disjoint analysis path. P2 fires VIOLATED (spread 40.4 %) and the decoding attributes it: a fast correlated-dephasing channel between the two outer qubits (shared TLS or control line being the physical suspects), on top of the device-wide anti-correlated slow component (which pulls the GHZ low, consistent with P3's sign). Run 1's marginal c₁₂ (2.2σ, line [2,3,4]) neither repeats nor contradicts: different line, different bonds. [Superseded by run 3 (SAME line): this c₀₂ did NOT recur and is downgraded to a non-recurring, unexplained outlier. Note the tension the campaign verdict must own: run 4's ζ₀₂ ≈ 0 shows the (93,95) bond carries no coherent ZZ either, so the final "no correlated bath" model has no mechanism for a 5.2σ covariance here; "drifting TLS" would itself be a transient correlated channel, and look-elsewhere over three bonds leaves 5.2σ at ~5.0σ. The "anti-correlated slow component" invoked in this paragraph is re-attributed to coherent ZZ below.]
