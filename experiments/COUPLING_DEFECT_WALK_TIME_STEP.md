...
**The third reading: the peak-tracking exponent tends to zero at fixed γ > 0 in the far-site limit.** The windowed-max counterpart moves to the late diffusive scale. The small-momentum pole gives the asymptotic profile P_n(t) ~ (4πDt)^{−1/2}·exp(−n²/(4Dt)), D = 2J²/Γ: its peak lies near t = n²/(2D), with n·P_n → e^{−1/2}/√(2π) = 0.242. This is an asymptotic reading, not an exact finite-n formula for the global-in-time maximum; a near-clean first peak can be higher at small n, and γ = 0 has no diffusive plateau. Thus the survival exponent has three scoped readings: the fixed-time (caustic) reading pays 4 − φ/γ; the peak-tracking reading tends to 0 as n grows at fixed γ > 0; a finite arrival window interpolates. The excluded EDGE points (γ = 0.08 at n ≥ 30 in the survival fit, and the γ ≥ 0.05 rows of the artifact split) are consistent with late diffusive weight entering the arrival window and pulling that reading off the ceiling branch. The committed A ≈ 2.8 sits in a narrow front window at the ballistic time, on the ceiling branch, pre-asymptotic.
...
3. **The n-free content the fence does have is already owned and measured.** The coherent-transport window is L* = Q/2 (the ENAQT crossover MirrorWorld's `spread` mode validates). The exact small-N spectral ladder Q*(2)=1 and Q*(3)=√2 is a separate computed object from the ballistic front. Q*(3)=√2 is the Kingston three-site walk's own EP, at Q_label ≈ 0.71, below the finite-time population-revival handover Kingston measured across runner Q_label=1.5→2.5, which maps to canonical Q_Lindblad=3→5; populations supply no Q*, critical-damping, or EP certificate. The dose line at this experiment's distances and the band-edge slope Q*(N)=2N/π are likewise distinct observables, not one hardware-confirmed family.
...
- The play sequel (2026-07-14): `experiments/FRONT_PEDIGREE.md` resolves the F126 ladder by catch count, the observable the density-matrix engine hides: at K = 1.25 only 29% of the surviving front was never caught, survivors carry a quarter of the free walker's catches, and the last catch is spread over the whole trip (the bulk-dominance of I₁ made visible).
- The outward reading of the arc: `reflections/ON_THE_REFUND.md` (the watching taxes the wave and pays it back; the observable is an appointment; distance is time, sight is dose).
- Front speed and dispersion: F2b in `docs/ANALYTICAL_FORMULAS.md` (the single-excitation band propagated here); `docs/proofs/derivations/D10_W1_DISPERSION.md` (the (0,1) coherence block's Liouvillian dispersion, the same physics one popcount step up, background).
