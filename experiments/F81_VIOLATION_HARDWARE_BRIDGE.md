...
**Status:** The f81_violation discriminator, until now a post-fit operator diagnostic computed from a Liouvillian we build in simulation, is grounded to a measurable object and read out on existing hardware data for the first time. Three legs, all zero QPU: (1) the method demo on the F113-fitted Kingston Lindbladians, with the tautology of a parameterized fit made explicit; (2) the from-below grounding: within the local σ± + Pauli-channel family the violation IS 2^(N−1) times the root-sum-square of the velocities with which the maximally mixed state polarizes, per site the net cooling flux γ↓−γ↑ (class scope in leg 2's third corollary); (3) the first non-tautological readout on the Marrakesh price_pair T1-leg data via the asymptote recipe a = z∞/T1, no Lindblad fit anywhere. The missing half (a |0⟩ heating leg that separates γ↓ from γ↑ and settles the z∞ attribution) was simulator-validated, pre-registered, and FLOWN TWICE the same day: run 1 on ibm_kingston [82, 83, 13] (job d951mhkql68s73ca3u0g) closed the attribution with the systematic branch (bath cold, γ↑ ≈ 0, the sub-unity down-leg asymptotes are extrapolation artifacts of fluctuating-T1 decay, leg 3's one-leg recipe corrected by the discriminator designed to check it); run 2 on ibm_marrakesh [93, 94, 95] (job d953ti5gc6cc73ffomig) returned no-SPLIT/no-flag, resolved P3 to the mobile-defect branch, and delivered the protocol's first valid scalar, V_σ± = 0.02491 ± 0.00015 /µs (statistical; +6.9% early-rate estimator spread). Typed Confirmation: `f84_heating_leg_attribution_kingston_july2026` (both runs).
**Date:** 2026-07-05
...
**Data:** [`data/ibm_f95_angle_steering_may2026/`](../data/ibm_f95_angle_steering_may2026/) (Kingston, 2026-05-16), [`data/ibm_price_pair_july2026/`](../data/ibm_price_pair_july2026/) (Marrakesh, 2026-07-04, runs 1+2), [`data/ibm_heating_leg_july2026/`](../data/ibm_heating_leg_july2026/) (the heating-leg simulator validation + the Kingston and Marrakesh hardware runs)
...
A technical note that matters for reuse: the f95 Hamiltonian is the 1-body drive H = (ω/2)·Z on the steering qubit, which the script builds directly rather than through `pi_decompose_M`'s terms interface. At the N=2 of this fit the two coincide, since a 2-site chain has one bond and `terms=[('Z','I')]` at `chain.J = ω/2` builds exactly (ω/2)·Z₀, and the two decompositions agree on ‖M‖² and ‖M_anti‖², which do read the Hamiltonian. (Their f81_violation agreeing proves nothing: by F84 that number is Hamiltonian-independent, so it matches for any two H whatever.) The coincidence is a property of the 2-site chain, not something Section 0 tests: that gate runs on an identity-free bilinear, H = J(XX+YY). It does not survive a longer chain, where the same spelling puts Z on every site but the last (on a star it stays single-site, at (N−1)·ω/2 on the hub), nor a per-site ω profile, since one coefficient covers all terms. The script therefore hand-rolls the decomposition from the same framework primitives (`palindrome_residual`, `build_pi_full`, the Pauli-basis transform), with H_odd projected from below via X^⊗N conjugation, and GATES the hand-rolled path against `pi_decompose_M` on a terms-expressible case first: agreement is bit-exact (diff 0.0).
...
2. **Temperature-independence at the identity (F84's vacuum reading), exact:** for γ↓ = 0.007+c, γ↑ = c the velocity stays 0.0070000000 for every c up to 0.05 (individual rates up to ≈ 8× the net; the total rate b up to 15×). The symmetric thermal traffic cancels; the velocity reads only the spontaneous (vacuum) component.
...
Three corollaries worth stating precisely:

- **The number is not 1/T1.** It is the net one-way flux γ↓−γ↑ = z∞/T1, the piece that survives when the up and down thermal traffic cancels. "T1-vs-T2 discriminator" is the cold-qubit approximation (z∞ ≈ 1); leg 3's asymptote readings put the distinction at 6-42% across the two Marrakesh runs, not academic (though the heating-leg run below re-attributes most of it to the fit artifact rather than to physics).
...
- **Class scope (added 2026-07-05, fresh-context review catch):** the identity-velocity identification is exact for the local σ± + Pauli-channel family, where all of this document's statements live. It is NOT a statement about all unital channels: a tilted-axis dephasing operator such as (X+Z)/√2 (flux noise off a sweet spot) is unital and fixes the identity, yet carries Π²-odd content of √2·γ into the violation with zero identity velocity (verified from below, N=1). On free-form fits the violation therefore reads net cooling PLUS any non-Pauli-axis dephasing content; the F84 proof's lemma got a dated scope note, and the adapter states the boundary in its not-claiming list.
...
- **run 1: f81_violation(local σ± family) = 0.02603 ± 0.00024 /µs**, identity-escape RMS velocity 0.00376/µs (T1-equivalent 266 µs). The naive all-z∞=1 packaging would read 0.02839: T1 relaxometry without the asymptote overestimates the per-qubit reading by 6-16% (1/z∞ − 1) and the packaged violation by 9% on this line.
...
3. Cold scenario (p_exc = 0.01): MEET, planted recovered, γ↑ separated from zero at 1.6-3.2σ per qubit at 1% excited population; violation 0.03794 ± 0.00023 /µs (higher than warm: heating reduces the net flux, exactly F84).
...
- **P2:** the systematic branch landed exactly inside its pre-registered bands: γ↑ < 0.0002 /µs ✓ (largest joint-fit value 6.8·10⁻⁵) and up-legs flat within 3% of +1 ✓ (pointwise minimum +0.9779, q82 at 70 µs, over 320 µs). Precision note on the two γ↑ readings: under a SPLIT the shared-asymptote joint fit is mis-specified and biases γ↑ upward, so the clean reading is the up-leg's own asymptote, p_th = (1 − z∞_up)/2 = **0.83 ± 0.21 / 0.31 ± 0.27 / 0.23 ± 0.57 %** (from the unrounded stored fits z∞ = 0.98338/0.99371/0.99548); the joint-fit values are upper-bound-style. Either way p_th ≲ 1%: **Kingston's idle bath is cold.** The sub-unity asymptote is not device-universal thermal physics.
...
- **The raw-shape finding:** the down-legs are still RISING at 320 µs (q82's last interval: +0.230 → +0.336); the "asymptotes" were extrapolations. The early decay runs within ≈ 15% of the fresh calibration rate on q82 and ≈ 2× faster than calibration on q13.
- **Post-hoc, labeled as such (recipe: z∞ pinned at the up-leg fit, z0 at the measured t=0 point, unlevered shot-noise σ):** the single exponential is rejected on q82 and q13 under either error convention (χ²/dof 19.7 and 11.5 pinned; 12.6 and 6.0 under the runner's levered convention); q83's rejection is convention-dependent (6.0 pinned vs 2.9 levered, against the flag threshold 4). A two-rate mixture is accepted on all three (1.45/0.32/1.91 pinned). q82's mixture mean rate reproduces its fresh calibration T1 to 1% (206 vs 208 µs; components 57% @ 136 µs + 43% @ 687 µs, window-limited: an equally good fit gives a 216 µs mean, so the components are effective, not resolved); q83/q13 carry a small fast component (5-6% weight at 20-40 µs) plus bulk near 320-340 µs.
...
## RUN 2 pre-registration (2026-07-05, before the shot): the Marrakesh repeat

Purpose, in the outbound arc's language: underpin the S4 adapter's handover with the measurement it currently lacks. The Kingston run delivered the bias certificate but, by its own P1 rule, no valid V; this run (a) closes the attribution on the device where leg 3 read the sub-unity asymptotes, (b) tests the ORIGINAL P3 on its own qubit, and (c) extracts the first valid V via the corrected recipe. Same validated instrument, unchanged (22 circuits, the 0-320 µs grid, 8192 shots, `run_heating_leg.py`); device **ibm_marrakesh**, line **[93, 94, 95]** (the price_pair run-2 line, q94 the flagged qubit). Queue 0 pending; calibration pulled VIA API at pre-registration time (`backend.properties()`, last_update 2026-07-05 12:45:11+02:00; no CSV needed, Tom's question answered operationally): T1 = 236.9 / 227.7 / 264.3 µs, T2 = 155.5 / 82.2 / 286.1 µs, readout error 1.03 / 0.98 / 0.77 %.

A pre-shot observation that sharpens P3: **q94's calibration T1 moved from 129 µs (in-situ, 2026-07-04) to 227.7 µs today, a near-doubling in ~30 hours**, the Klimov drift signature at calibration level.

- **P1, P2:** unchanged from the original pre-registration (meeting test at 2σ per qubit; systematic branch bands γ↑ < 0.0002 /µs with flat up-legs, bath branch bands γ↑ ≥ 5σ).
...
|---|---:|---:|---:|---:|---:|
| q93 | +0.928 ± 0.038 | 2.13 | 0.994 | 2.1 ± 2.2 | 0.00451 |
...
- **P4, the first valid V.** With no SPLIT and no flags, the pre-registered P1 rule withholds nothing: **V_σ± = 0.02491 ± 0.00015 /µs** on [93, 94, 95] (RMS identity-escape velocity 0.00360 /µs), from the joint-fit rates; the error is statistical (fit covariance). Estimator provenance, said out loud: P4 pre-registered the pinned-asymptote mixture mean as the robust γ↓, but with single exponentials accepted on all three qubits (χ²/dof 0.17-2.13) the mixture mean degenerates to the single rate, so the joint rate IS the fluctuation-robust reading here. The pre-registered early-rate cross-check reads the down legs' 0-45 µs window at +5.9/+14.0/+0.6% above the joint rates, packaging to **V(early) = 0.02663 ± 0.00074 (+6.9%)**: an estimator spread of ≈ 7% accompanies the statistical error, the residue of today's mild convexity, and both numbers are recorded. In-situ γ↓ vs the same-day API calibration: +7 / −28 / −22 % (T1_eff = 222/316/339 µs vs calibration 237/228/264 µs); the ± tens-of-percent in-situ-vs-calibration scatter is the Klimov reality the two-leg in-situ protocol exists for.
