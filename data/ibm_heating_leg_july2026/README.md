...
| `heating_leg_ibm_kingston_20260705_105648.json` | **HARDWARE run 1** (ibm_kingston, job d951mhkql68s73ca3u0g, path [82, 83, 13], ≈ 1.6 QPU min) | **SPLIT on all three qubits** (13.7σ/5.1σ/6.6σ): up-legs flat at +0.98-1.00 (γ↑ = 2-7·10⁻⁵ /µs, p_th ≲ 1%, cold bath); down-legs still rising at 320 µs, non-exponential (pinned-asymptote two-rate mixture accepted, single exponential rejected). Attribution closed: the sub-unity down-leg asymptote is a leg systematic, not thermal population. Verdicts + post-hoc analysis in the bridge doc's HARDWARE RECORD; typed Confirmation `f84_heating_leg_attribution_kingston_july2026`. |
| `heating_leg_ibm_marrakesh_20260705_132609.json` | **HARDWARE run 2** (ibm_marrakesh, job d953ti5gc6cc73ffomig, path [93, 94, 95], ≈ 1.6 QPU min, pre-registered 730ff94) | **No SPLIT, no χ² flag**: up legs pointwise ≥ 0.994 over 320 µs, γ↑ ≤ 2.2·10⁻⁵ /µs, populations ≤ 0.5% (cold, with margin). P3 resolved to the mobile-defect branch: q94 (flagged 12.5 the day before) clean (0.79) after its in-situ T1 rose 129 → 316 µs overnight (same-day calibration 228 µs). First valid measurement: **V_σ± = 0.02491 ± 0.00015 /µs** (statistical; early-rate cross-check +6.9%, both recorded). RUN 2 RECORD in the bridge doc. |
