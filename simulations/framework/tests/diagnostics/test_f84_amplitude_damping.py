...
def test_F84_amplitude_damping_thermal_bath():
    ...
    # Cooling only (= F82)
    d_cool = fw.pi_decompose_M(chain,soft, gamma_z=0.1, gamma_t1=0.1)
    expected = 0.10 * np.sqrt(3) * (2 ** 2)
    assert abs(d_cool['f81_violation'] - expected) < 1e-9

    # Heating only (symmetric to cooling)
    d_heat = fw.pi_decompose_M(chain,soft, gamma_z=0.1, gamma_pump=0.1)
    assert abs(d_heat['f81_violation'] - expected) < 1e-9

    # Detailed balance: γ_↓ = γ_↑, violation must be zero
    d_balance = fw.pi_decompose_M(chain,soft, gamma_z=0.1, gamma_t1=0.1, gamma_pump=0.1)
    ...
    d_net = fw.pi_decompose_M(chain,soft, gamma_z=0.1, gamma_t1=0.1, gamma_pump=0.05)
    ...
    d_net_heat = fw.pi_decompose_M(chain,soft, gamma_z=0.1, gamma_t1=0.05, gamma_pump=0.10)
    ...
    pred = fw.predict_amplitude_damping_violation(chain,0.1, 0.05)
    assert abs(pred - expected_net) < 1e-12

    # Inverse closed form: estimate_net_cooling_from_violation
    delta_recovered = fw.estimate_net_cooling_from_violation(chain,d_net['f81_violation'])
    ...
    d_nu = fw.pi_decompose_M(chain,soft, gamma_z=0.0, gamma_t1=gt1_l, gamma_pump=gp_l)
    assert abs(d_nu['f81_violation'] - expected_nu) < 1e-9
    pred_nu = fw.predict_amplitude_damping_violation(chain,gt1_l, gp_l)
    ...
    pred_heat = fw.predict_amplitude_damping_violation(chain,[0.0]*3, [0.1]*3)
    ...
    pred_t1 = fw.predict_T1_dissipator_violation(chain,0.1)
    pred_amp_only = fw.predict_amplitude_damping_violation(chain,0.1)
