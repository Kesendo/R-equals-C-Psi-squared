...
def test_zn_mirror_diagnostic_pure_heisenberg_preserved():
    ...
    # Propagate both
    rho_a = fw.propagate_with_hardware_noise(chain,
        rho_a_0, t=1.0, terms=[('X','X'),('Y','Y'),('Z','Z')])
    rho_b = fw.propagate_with_hardware_noise(chain,
        rho_b_0, t=1.0, terms=[('X','X'),('Y','Y'),('Z','Z')])
    diag = fw.zn_mirror_diagnostic(chain, rho_a, rho_b)
...
def test_zn_mirror_diagnostic_transverse_x_breaks_zn():
    ...
    rho_wrong = fw.propagate_with_hardware_noise(chain,
        rho_wrong_0, t=1.0, terms=[('X','X'),('Y','Y'),('Z','Z')])
    rho_a_clean = fw.propagate_with_hardware_noise(chain,
        rho_a_0, t=1.0, terms=[('X','X'),('Y','Y'),('Z','Z')])
    diag_break = fw.zn_mirror_diagnostic(chain, rho_a_clean, rho_wrong)
