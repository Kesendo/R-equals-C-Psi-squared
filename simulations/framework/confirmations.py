...
class Confirmations:
    ...
    _ENTRIES = {
        ...
        'f84_heating_leg_attribution_kingston_july2026': {
            ...
            'measured_value': 'SPLIT on all three qubits (13.7σ / 5.1σ / 6.6σ): up-legs flat at z∞ = 0.983/0.994/0.996 (pointwise never below +0.978 over 320 µs) ⟹ p_th = (1−z∞_up)/2 = 0.83/0.31/0.23 % (the clean reading under a SPLIT; the joint-fit γ↑ = (6.8±2.1, 2.7±2.1, 2.1±2.0)·10⁻⁵ /µs is upper-bound-style, biased up by the shared-asymptote mis-specification), the bath is COLD; down-legs still rising at 320 µs with stretched (non-exponential) shape, so the sub-unity down-leg asymptotes (0.59-0.74) are extrapolation artifacts. Post-hoc (labeled as such): pinning z∞ at the up-leg level rejects the single exponential (χ²/dof 19.7/6.0/11.5) and accepts a two-rate mixture (1.45/0.32/1.91); q82\'s mixture mean rate reproduces the fresh calibration T1 (206 vs 208 µs; components 57% @ 136 µs + 43% @ 687 µs).',
            'hardware_data': 'data/ibm_heating_leg_july2026/heating_leg_ibm_kingston_20260705_105648.json (+ same-day simulator-validation JSONs and README in that directory)',
