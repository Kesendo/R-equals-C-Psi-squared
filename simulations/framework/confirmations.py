...
class Confirmations:
    ...
    _ENTRIES = {
        ...
        'f57_kdwell_gamma_invariance': {
            ...
            'measured_value': 'Pair A 0.6492, Pair B 0.6937 (6.4% spread despite 2.55× γ ratio)',
            ...
            'description': 'Two Kingston Bell+ pairs at 2.55× different fitted γ give K_dwell/δ within 6.4% of each other, at an absolute prefactor 0.67 against the pure-Z prediction 1.0801. An approximate two-pair comparison: pair identity and T1 vary with γ, so this neither verifies universal γ-invariance nor isolates the cause of the prefactor gap.',
        },
        ...
        'f95_angle_steering_kingston_may2026': {
            ...
            'measured_value': {
                ...
                'residual_shrinks_with_omega': 'on pair A_mid, residual 15.7° → 9.2° as Ω goes from 0.13 to 0.25 (×1.92), consistent with roughly constant natural-Kingston drift becoming smaller fraction of driven rotation',
            ...
            'framework_primitive': 'complex CΨ_com signed-sum-of-off-diagonals extension (the steered phase arg(CΨ_com)) + F25 Lindblad CΨ trajectory + F95AngleAtQuadraticZeroPi2Inheritance (θ(c;b) = arctan(√(c/b² − 1)), finite b>0; the square-root form the dwell arc length shares)',
            'description': 'First hardware verification that the complex-CΨ angle arg(CΨ_com), which shares the square-root form of F95 in its dwell arc length but is not the θ of F95 itself (experiments/CPSI_COMPLEX_PLANE.md), is actively steerable on IBM Kingston. Two Ω values (0.13 and 0.25 rad/μs) tested on two qubit pairs (A_mid mid-T2 [82,83], B_high high-T2 [13,14]) in 14 minutes total billed QPU. Three of four measurement conditions produced detectable real-axis crossings; predicted arg matches measured to within 16° across all three (best 6.8°). Confirms three open questions from CPSI_COMPLEX_PLANE.md simultaneously: (Q1) Δφ = Ω·t reproducible: crossings occur at predicted angle modulo bounded natural drift; (Q2) linear-in-Ω scaling holds: residual shrinks ~1.7× as Ω goes from 0.13 to 0.25 (×1.92), consistent with constant natural drift contribution; (Q3) active steering works: the same Lindblad+RZ model fits both Ω values on the same pair without retuning. The bra↔ket Hermitian off-diagonal of ρ on the prepared Bell-like initial state carries a well-defined complex phase, addressable on Heron r2 by per-chunk RZ injection on the steering qubit. Hardware substrate-of-evidence: the steered phase arg(CΨ_com) shares the square-root form of F95 in its dwell arc length, and this run shows that it is operationally controllable, not merely descriptive.',
        },
        'regime_uniformity_kingston_uniform_quantum': {
            ...
            'measured_value': 'truly = 0.0022, soft = 0.7409, pi2_even_nontruly = 0.0046, mixed = 0.6220. truly is 1.67× the uniform-classical Marrakesh [48,49,50] soft_break run (0.0013, the same runner script). The 13.3× against the regime-mixed Marrakesh [0,1,2] (0.0297) and the 22.1× between the two Marrakesh rows each pair a soft_break run with a framework_snapshots run, and the same-day repeats put the script difference, not the path, behind most of that factor: within framework_snapshots, minutes apart, mixed [0,1,2] reads 1.64× uniform [48,49,50] (0.0190 against 0.0116). soft sits 3.1% from the Marrakesh value 0.7646.',
        ...
        'gamma0_off_the_lever_kingston_may2026': {
            ...
            'predicted_value': 'The separate ideal two-site model with γ₀=0.05 predicts loss of transfer overshoot near J=γ₀. The four-point hardware scan can test only whether the finite-time transfer maximum lies above or below 1/2; it cannot identify γ₀ or certify critical damping.',
            'measured_value': 'q13-q14, J ∈ {0.05, 0.1, 0.2, 0.4}: max T = {0.335, 0.563, 0.703, 0.779}. Transfer overshoot is present for J ≥ 0.1 and absent at J=0.05; the grid brackets an effective response change, while gate, idle, readout, and finite-time effects are not separated.',
            ...
            'framework_primitive': 'q_basic_jscan.py ideal-model comparator + run_q_jscan.py external finite-grid hardware scan',
            'description': 'Finite-grid transfer-overshoot observation under a J scan. It brackets an effective response change between J=0.05 and 0.1, but does not isolate a local Z-dephasing rate, calibrate UniversalCarrierClaim.DefaultGammaZero, certify critical damping, or anchor the F86 Q-peak. The name is a historical locator, not a direct carrier measurement.',
        },
        'ibm_ep_onset_may2026': {
            'date': '2026-05-31',
            'machine': 'ibm_kingston',
            'job_id': 'd8dr7dfd0j8c73f4man0 (Part A, finite-time population trajectory) + d8drjbfd0j8c73f4mobg (Part B, historical EP-onset runner label)',
            'observable': 'Per-site populations ⟨n_l⟩ of a single-excitation walk on a 3-site chain (q13-q14-q15, Z-basis, no tomography); Part B revival = max ⟨n_0⟩ for t ≥ 2 μs under injected random-Z-twirl dephasing, with runner label Q_label = J/Γ',
            'predicted_value': {
                'se_walk_handover_reading': 'SE-walk population handover: finite-time revival remains near the 1/N = 1/3 reference level through Q_label=1.5 and is larger at Q_label=2.5. The spectral transition of the walk is not at this handover: the one-excitation (1,1) block of the continuous model has a single EP at Q > 0, the coherence horizon Q*(3) = √2 in the Lindblad book (CoherenceHorizonClaim), i.e. Q_label = 1/√2 ≈ 0.71, between the two lowest flown points.',
                'twirl_simulate_revival': '0.28 → 0.84 across the same Q scan (K=16 random-phase twirl simulation, seed 12345 shared with the hardware run)',
                'ideal_sector_target': 'separate ideal-model theorem: number conservation, positive dephasing, and connected hopping give asymptotic uniform 1/N in the one-excitation sector; Part A hardware does not test that asymptote',
            },
            'measured_value': {
                'Q_label_grid': [0.5, 1.0, 1.5, 2.5, 5.0, 20.0],
                'Q_lindblad_grid': [1.0, 2.0, 3.0, 5.0, 10.0, 40.0],
                'rate_book': 'random-phase σ²=2Γdt gives exp(−Γt), while jump √γ Z gives exp(−2γt); therefore γ=Γ/2 and Q_Lindblad = 2 Q_label',
                'revival': [0.2978515625, 0.3623809814453125, 0.34356689453125, 0.4898834228515625, 0.560699462890625, 0.70330810546875],
                'near_reference_through_sample': 'near 1/3 through Q_label=1.5 (0.30, 0.36, 0.34); 1/N is a reference level, not a measured floor',
                'larger_revival_samples': '0.49 → 0.56 → 0.70 at Q_label=2.5 → 5 → 20',
                'part_A_finite_time_marginals': '0.339 / 0.426 / 0.338 at 20 μs (sum 1.103; no readout/leakage correction and no asymptotic claim)',
                'high_Q_discrepancy': '0.703 measured vs 0.842 K=16 twirl simulation (raw 0.8417853730254796) at Q_label=20; consistent with accumulated circuit/gate cost, but the record contains no calibrated model assigning a unique cause',
            },
            ...
            'framework_primitive': 'CoherenceHorizonClaim (the (1,1)-block EP of the walk, Q*(3) = √2, inspect --root horizon) + EpCharacterWitness (inspect --root epcharacter, DEFECTIVE) + EpField hardware node (inspect --axis ep), with the hardware curve converted from runner labels to canonical Lindblad Q',
            'description': 'The single-excitation-walk population handover is a real-chip population-only observation. Part A (job d8dr7dfd0j8c73f4man0) records finite-time site-to-site sloshing; its final three marginals are not a normalized one-excitation distribution and do not certify convergence or an asymptotic fixed point. Part B (job d8drjbfd0j8c73f4mobg) scans the runner label Q_label = J/Γ. Its random-phase variance σ²=2Γdt gives coherence decay exp(−Γt), whereas the repository Lindblad jump √γ Z gives exp(−2γt); therefore γ=Γ/2 and Q_Lindblad = 2 Q_label. The observed bracket Q_label=1.5→2.5 maps to Q_Lindblad=3→5. It is a population handover, and it is not a spectral event. In the continuous model of the flown walk (H = (J/2)Σ(XX+YY), J = 1.5 rad/μs, jump √γ Z on every site, γ = Γ/2) the one-excitation (1,1) block has exactly one EP at Q > 0: the coherence horizon Q*(3) = √2, λ* = −2γ, algebraic multiplicity 2 and geometric 1 by exact rank (simulations/the_flow_endpoints.py; CoherenceHorizonClaim, Tier1Derived; EpCharacterWitness reads it DEFECTIVE). In runner labels that is Q_label ≈ 0.71, between the flown 0.5 and 1.0, below the bracket. The bracket is a probe-time crossover of the revival amplitude, and the flown circuit places it: the flown Trotter map (dt = 0.5 μs, Jdt = 0.75, the twirl replaced by its exact channel average), which is not the continuous model at the low end (Γdt = 1.5 at Q_label = 0.5), gives revival 0.372 → 0.453 across Q_label 1.5 → 2.5, crossing 0.45 inside the bracket, and its statistic n₀ > 0.45 at every larger Q sets in at Q_Lindblad = 4.94, 9.88, 14.82 when read at 3, 6, 9 μs; the continuous model shows the same drift at the k-th coherent return t_k = k·2π/(√2 J) (Q_Lindblad ≈ 5.65, 10.96, 16.34, the first just above the flown bracket). So the handover moves with the probe time while the EP stays at √2. The slowest mode of the Trotter map itself turns oscillatory at Q_label ≈ 0.359, below every flown point. The populations themselves measured no damping regime, coalescence, or Jordan structure. The faster hardware envelope is consistent with accumulated circuit/gate cost, but the record contains no calibrated error model assigning a unique cause.',
        },
        ...
        'cpsi_quarter_crossing_torino_feb2026': {
            ...
            'predicted_value': 't*/T₂* = 0.950 (generalized, r = T₂*/T₁ = 0.500 from T₂*(FID) = 110.7 μs of this record and T₁ = 221.2 μs); 0.858 in the pure-dephasing limit (x³+x = ½)',
            'measured_value': 't* = 114.7 μs, t*/T₂* = 1.036 (9.1% above the generalized prediction on the same T₂*); CΨ(0) = 0.885; late purity 0.740 (mean of the last five samples, 746–895 μs, still rising; not an asymptote). Qubit 52: T1 = 221.2 μs, T2_echo = 298.2 μs, T2*(FID) = 110.7 μs',
            ...
            'description': 'The first CΨ = ¼ crossing ever seen on hardware, found in IBM Torino single-qubit calibration tomography (Heron r1, q52, 2026-02-09), predating the systematic April-June 2026 campaign. A QUALITATIVE confirmation that the ¼ fold is real on a physical device, not a precision match: t*/T₂* = 1.036 = 114.7/110.7 sits 9% above the generalized prediction 0.950 on the same T₂*, and the offset moves with the T₂* fit (9.7% on the all-samples fit, 110.1 μs); the run was a calibration tomography, not a purpose-built sweep. No IBM job_id was recorded for the Torino-era runs; the data-file timestamp is the locator.',
        },
        'absorption_theorem_ratio_torino': {
            ...
            'predicted_value': 'ratio = 1 (Absorption Theorem: Re(λ) = −2γ⟨n_XY⟩, with ⟨n_XY⟩ = 1 for single-qubit coherence); at N = 1 γ* is fitted from the same coherence envelope, so the ratio is 1 by construction up to the T1 each side subtracts',
            'measured_value': 'ratio = 1.03 on the T2* baseline: excess α = 0.006960 μs⁻¹ vs 2γ* = 0.006771 μs⁻¹. The 3% is the gap between the two T1 values the sides subtract, 1.03 = [1/110.7 − 1/(2·241.4)] / [1/110.7 − 1/(2·221.2)] (population-fit T1 in the excess, calibration T1 in 2γ*), exactly 1 with one T1 on both sides. A 2.8% slow tail at the resolution limit, whose static late direction has equal X and Y parts, the pattern of a measurement (SPAM) offset (experiments/FIXED_POINT_SHADOW.md). (The 6.37 figure quoted elsewhere used the wrong T2_echo baseline.)',
            'hardware_data': 'data/ibm_tomography_feb2026/tomography_ibm_torino_20260209_131521.json',
            ...
            'description': 'Retrospective Absorption-Theorem reading of the same 2026-02-09 Torino q52 tomography run (analysis 2026-04-04): the consistency of two fits to one decay, not a test of the rate ladder. At N = 1 the coherence excess matches 2γ* as it must, since γ* is read off the same envelope (AbsorptionTheoremClaim, PROOF_ABSORPTION_THEOREM §3); the content of the theorem beyond one site, additivity of the site rates, needs N ≥ 2, and no registered ratio with an error bar reads it. Predates the systematic registry campaign; shares the Feb-9 q52 data with cpsi_quarter_crossing_torino_feb2026.',
        },
        ...
        'price_pair_locality_marrakesh_july2026': {
            ...
            'observable': 'coherence-pattern decay rates Γ(D) for all 7 subsets of a 3-qubit line (quadrature envelopes, readout-mitigated), the pair-sum price Γ(D)+Γ(D̄), the per-bond covariances c_ij (branch splitting), the pointwise shape-free law, and fitted conditional-Ramsey frequency differences (outer arm Δf02 has different gain)',
            ...
            'measured_value': 'dephasing covariances LOCAL in run 3 (all c_ij within 2σ there; one non-recurring 5.1σ outlier in run 2 downgraded, unexplained); coherent NN ZZ supported by three instruments (t² mask, 3:2:3 W fingerprint, conditional shift): run-4 fitted ζ01 = −3.92±0.14 kHz, ζ12 = −3.64±0.10 kHz; the outer-arm fitted Δf02 = +0.01635±0.11981 kHz is zero-consistent and below the pre-registered fitted-shift screen of 0.5 kHz (nominal 95% upper 0.25118). Ideal no-decay half-gain maps it to ζ02 = +0.03270±0.23963 kHz, not a measured bare coupling; a strict physical |ζ02| < 0.5 kHz bound is unresolved (geometry: docs/proofs/PROOF_PRICE_PAIR_OUTER_ZZ_GEOMETRY.md)',
            ...
            'framework_primitive': 'F89d coherence-pattern fold and F1 spectral palindrome as pure-Z structural context: for Lindblad D[Z_j] coefficients γ_Z,j the pure-Z price is 2·Σγ_Z,j. The measured P = Γ(D)+Γ(D̄) uses total fitted transverse rates, including local T1 contributions 1/(2T1_j), so it is not an exact F1 center measurement. Selected-pattern additivity follows from the local-channel model, not F70\'s partial-trace rule; F82-F84 T1 accounting; run_price_pair.py (external pipeline)',
            'description': 'The local-dephasing premise under the F-formulas, supported in the clean run-3 session: a four-run pre-registered campaign (runs 1-2 price protocol on two independent lines, run 3 the W discriminator refuting the intermediate anti-correlated-bath reading via the 3:2:3 boundary-rule fingerprint, run 4 conditional Ramsey fitted chain shifts ≈ −3.9/−3.6 kHz supporting coherent NN ZZ). Working noise model: local Z-dephasing (quasi-static + Markovian) + local T1 + coherent NN ZZ of order 4 kHz; the registered outer-arm fitted-shift screen was met, while a strict physical outer-bond 0.5 kHz bound is unresolved and one non-recurring 5.1σ covariance stays unexplained. Every wrong intermediate reading was falsified by its own pre-registered discriminator; the GHZ pattern-not-carrier immunity (XXX commutes with every Z_iZ_j) was derived before the first shot and carried the decoding.',
        },
        ...
        'f129_standing_fringe_kingston_july2026': {
            'date': '2026-07-15',
            'machine': 'ibm_kingston',
            'job_id': 'd9br4vmg26ic73dgbgk0',
            'observable': 'Ramsey fringe phase slopes of three two-triple 3-magnon superpositions on an 8-qubit line (chain [11,12,13,14,15,19,35,34], seed = the better-readout end): each arm prepares (|τ⟩+|c⟩)/√2 in the FLOQUET eigenbasis of the θ = 0.5 Trotter step (GHZ cat on the differing modes + per-arm 28-Givens network), evolves M ∈ 0..8 pure-hopping steps, inverts the network and reads the seed qubit in X/Y; weighted-LSQ slope per arm with pinned predicted-V weights. Arms: A0 = the F129 collision (1,5,7)~(2,4,8) of the n = 9 comb (clean mirror pair), A1 = (2,5,6) near dial, A2 = (1,6,7) verdict clock. ONE Batch, 64 PUBs, 16384 shots each, billed 297 s ≈ 5.0 QPU min.',
            'predicted_value': 'pre-registered (commit 5bf3b79, v2.5, 7a+7b recorded pre-shot): STANDING FRINGE CONFIRMED iff (a) A0 slope within the budgeted window center(θ̂) + [−(3σ_a + b_qs), +(3σ_a + b_zz2 + b_qs)] with center = 0.0213 + 0.1294·(θ̂ − 0.5), σ_a = 0.0034 frozen, b_zz2 = +0.0043 one-sided (the chiral ζ² anti-protection law 0.00257·(ζ/3.8 kHz)²·(τ/1.2 µs)²), b_qs = ±0.008 (quasi-static site disorder); (b) A2 separates ≥ 5σ combined with predicted sign; (c) A2 within ±(3σ + 5|ZZ_Wick|) of nominal +0.3214. Floquet predictions: A0 +0.0213 (= 0.170·θ³, the collision standing), A1 −0.0903, A2 +0.3204; nearest impostor the census permits +0.109.',
            'measured_value': 'A0 = +0.0326 ± 0.0033 (deviation +0.0114 from the θ̂ center, INSIDE the budgeted window [−0.0182, +0.0225]; +3.36σ_a purely statistically, positive as the ζ² law predicts, sized within b_zz2 + b_qs) ⟹ (a) PASS; A2 − A0 separation 61.0σ combined, predicted sign ⟹ (b) PASS; A2 = +0.3199, 0.45σ from nominal, θ̂ = 0.4992 ⟹ (c) PASS; A1 = −0.0928, 0.7σ from its prediction. STANDING FRINGE CONFIRMED; the nearest impostor lies 22.5σ above the measured A0. V(M=8) = 0.33-0.46 vs conservative model 0.215.',
            'hardware_data': 'external (AIEvolution.UI/experiments/ibm_quantum_tomography): results_f129/f129_flight_20260715_222348.json (+ f129_submit_20260715_181658.json; recovered via --recover after a local power failure, job unaffected)',
            'experiment_doc': 'experiments/IBM_F129_RAMSEY_FRINGE.md',
            'framework_primitive': 'F129 level-collision law (existence half at the smallest firing n; PROOF_F129_LEVEL_COLLISION_LAW.md); design gate simulations/f129_ramsey_fringe_design.py + compiler simulations/f129_givens_compiler.py + 7a gate simulations/f129_ramsey_7a_gate.py (estimator + verdict = the flown code path); run_f129_ramsey_fringe.py (external pipeline)',
            'description': 'F129 has a hardware sighting: the exact clean-clean level collision (1,5,7)~(2,4,8) of the n = 9 comb, prepared as a two-branch 3-magnon cat in the Floquet eigenbasis, stands still (to its computed θ³ drift and pre-registered systematic budgets) while the verdict clock winds at 61σ and the opposite-winding dial tracks its prediction at 0.7σ. The chiral mirror pair choice retired ZZ and detuning at first order (all-range, Slater-Condon branch-mixing zero); the surviving ζ² anti-protection (the mirror negates the Floquet gaps, so second-order shifts are opposite) and the quasi-static prep-basis pull were found by the 7a gate, budgeted pre-flight, and the measured +0.0114 excess landed inside them with the predicted sign. Post-flight empty round: independent hand recompute from the raw counts matches the RECORD to the digit. The concert hall on silicon: one note that does not beat.',
        },
    }
