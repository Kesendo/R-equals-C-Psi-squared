...
class Confirmations:
    ...
    _ENTRIES = {
        ...
        'price_pair_locality_marrakesh_july2026': {
            ...
            'framework_primitive': 'F89d cross-fold (dissipator half) + F1 palindrome center Σγ (COHERENCE book, pinned in the ExperimentDoc under the heading The claim, in one line: the measured price P = Γ(D)+Γ(D̄) = Σ_j 1/T2*_j on TOTAL rates, no T1 subtraction per P4, so P = 2·Σγ_F1 with γ_F1,j = 1/(2·T2*_j), the Σγ of F1 being the sum of the Lindblad D[Z] coefficients; what this entry registers is the covariance locality and ζ_ij, neither of which depends on that factor) + F70 pattern-not-carrier + F82-F84 T1 accounting; run_price_pair.py (external pipeline)',
