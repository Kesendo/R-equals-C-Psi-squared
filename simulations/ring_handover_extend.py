...
HIGH-Q light content vs the parallel-session commutant closed form 2(N-2)/N (b191df3) -- a cross-session
convergence test (does my low-Q seam's high-Q limit meet Tom's high-Q commutant?)."""
import sys
import numpy as np
sys.path.insert(0, "simulations")
from ring_handover_qh import nxy, qh_sector
...
print("=" * 90)
# N=6/8/10 by a root solve on the same <n_XY> = 1 criterion (N=6 is EXACTLY 2 = N/3);
...
print("\n" + "=" * 90)
print("(b) the (2,2) seam HIGH-Q light content (Q=200) vs the commutant forms (cross-session: b191df3)")
print("    Tom's (1,1)/(N/2,N/2) commutant = 2(N-2)/N (even). Does my (2,2) seam meet it, or differ?")
print("=" * 90)
print(f"{'N':>4} {'(2,2) <n_XY>(Q=200)':>22} {'2(N-2)/N':>10} {'2(N-1)/N':>10} {'match 2(N-2)/N?':>16}")
for N in (6, 8, 10, 12):
    dk = nxy(N, 2, 200.0)
    a = 2 * (N - 2) / N
    b = 2 * (N - 1) / N
    print(f"{N:>4} {dk:>22.5f} {a:>10.5f} {b:>10.5f} {str(abs(dk-a)<1e-2):>16}")
print("\n  (if the (2,2) seam high-Q light content != 2(N-2)/N, the seam is a DIFFERENT object than Tom's")
