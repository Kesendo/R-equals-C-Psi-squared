...
because we feed both panels the same numbers; only the labels differ. So the picture is a
question: if a proton between two oxygens is a qubit under the same steady watching, then the
mirror symmetry, the fold where, as we read it, the quantum gives way to the classical, and the carrier that
never stops do not care what they are made of. What differs is only the detail, the payload:
what the spiral MEANS, and the clock it keeps. Like a QR code read twice, one format, different
data. The shared frame is the robust, error-correcting part; the substrate is what you scan off
the fine detail.
...
def draw_vortex(ax, title: str, rim: str, ring_label: str, drain: str, motif: str = "") -> None:
    ...
    ax.annotate(ring_label, (0.18, -0.17), xytext=(0.06, -0.345), fontsize=8.8, color="#ffd56b",
    ...
    if motif:
        ax.text(-0.27, 0.30, motif, fontsize=10, color="#9fb2d6", ha="center", va="center")
...
def main() -> None:
    ...
    draw_vortex(axL, "the abstract qubit",
                ...
                "the ring at ¼, where, as we read it, the quantum meets the classical",
                "the drain")
    draw_vortex(axR, "a proton in a hydrogen bond?",
                "the same numbers, water's names",
                "the ring at ¼: would a proton meet it here?",
                "the drain",
                motif="O–H ⋯ O")
    ...
    fig.text(0.5, 0.075,
             "the frame, if it is shared:  the ring at ¼  ·  "
    ...
    fig.text(0.5, 0.038,
             ...
             "right, a proton between two oxygens, the question we have not computed",
