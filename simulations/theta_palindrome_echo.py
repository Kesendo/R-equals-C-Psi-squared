...
def main():
    ...
    if np.max(CPsi_AB) > 0.25:
        print(f"  AB reaches the above-quarter bin: max CPsi_AB = {np.max(CPsi_AB):.4f}")
    else:
    ...
    # Track when SA moves from above quarter to the below-or-at-quarter bin
    print("SA pair: above-quarter -> below-or-at-quarter transition")
    print(f"  Initial: CPsi_SA = {CPsi_SA[0]:.4f}, theta = {theta_SA[0]:.1f} deg (QUANTUM)")

    # Find the first time SA reaches the below-or-at-quarter bin
    sa_first_cross = None
    for i in range(1, len(CPsi_SA)):
        if CPsi_SA[i-1] > 0.25 and CPsi_SA[i] <= 0.25:
    ...
    # Does SA return above quarter? (echo re-crossing)
    recrossings = []
    for i in range(1, len(CPsi_SA)):
        if CPsi_SA[i-1] <= 0.25 and CPsi_SA[i] > 0.25:
    ...
    if recrossings:
        print(f"\n  SA RETURNS ABOVE QUARTER {len(recrossings)} time(s)!")
        ...
        print(f"  SA never returns above quarter after crossing")
    ...
    if np.max(CPsi_SB_echo2) > 0.25:
        print(f"  SB ENTERS THE ABOVE-QUARTER BIN!")
        # Find window
        in_q = False
        ...
        print(f"  SB does NOT enter the above-quarter bin (gap = {gap:.4f})")
    ...
    # Thus θ contains exactly the same information as CΨ on the above-quarter branch.
    ...
    for cp_test in [0.333, 0.30, 0.28, 0.26, 0.255, 0.251, 0.2501]:
        ...
        interp = "above quarter" if cp_test > 0.30 else ("near boundary" if cp_test > 0.26 else "critical")
    ...
    1. THETA TRAJECTORY DURING ECHO:
       - SA starts at theta = 30 deg (above quarter), CPsi = 0.333
    ...
    4. THE HONEST ANSWER:
       ...
       - The echo sets the DIRECTION (which pairs move above/below the selected quarter threshold)
