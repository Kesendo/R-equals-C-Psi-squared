...
def distinct_frequencies(imag_parts, tolerance=TOL_FREQ):
    """Return sorted deduplicated nonzero absolute frequencies (the array importers use)."""
...
def count_distinct_frequencies(imag_parts, tol=TOL_FREQ):
...
def main() -> None:
    ...
    for N in range(2, 7):
        ...
        total_modes, all_freqs = count_distinct_frequencies(eigvals.imag)
        ...
        for k in range(N + 1):
            ...
            n_osc = d_tot - d_real
            n_freq, freqs = count_distinct_frequencies(shell.imag)
    ...
    for N in range(2, 6):
        ...
        n_cold, freqs_cold = count_distinct_frequencies(ev_cold.imag)
        max_re_cold = np.max(np.abs(ev_cold.real))

        # Warm cavity
        L_warm = build_liouvillian(N, GAMMA, bonds)
        ev_warm = np.linalg.eigvals(L_warm)
        n_warm, freqs_warm = count_distinct_frequencies(ev_warm.imag)
        ...
        for f in freqs_cold:
            ...
            if len(diffs) > 0:
                nearest.append(float(np.min(diffs)))
                if np.min(diffs) < 0.1:
                    matched += 1
        max_nearest = max(nearest) if nearest else 0.0
        unshifted = sum(1 for d in nearest if d < 1e-6)

        log(f"N={N}: cold={n_cold} modes, warm={n_warm} modes, "
            f"matched={matched}/{n_cold} ({matched/max(n_cold,1)*100:.0f}%)")
        log(f"  Nearest warm partner: max distance {max_nearest:.7f}; "
            f"unshifted to 1e-6: {unshifted}/{n_cold}")
    ...
    for N in [3, 4, 5]:
        for topo_name, bond_fn in [("chain", chain_bonds), ("star", star_bonds), ("ring", ring_bonds)]:
            ...
            L = build_liouvillian(N, GAMMA, bonds)
            eigvals = np.linalg.eigvals(L)
            n_modes, _ = count_distinct_frequencies(eigvals.imag)
    ...
    # Cold cavity check
    log("Does every cold frequency find a warm one within 0.1?")
    for N in range(2, 6):
        bonds = chain_bonds(N)
        L_cold = build_liouvillian(N, 0.0, bonds)
        L_warm = build_liouvillian(N, GAMMA, bonds)
        _, fc = count_distinct_frequencies(np.linalg.eigvals(L_cold).imag)
        _, fw = count_distinct_frequencies(np.linalg.eigvals(L_warm).imag)
        ...
        log(f"  N={N}: {matched}/{len(fc)} cold modes found in warm spectrum "
            f"({'✓ within 0.1' if matched == len(fc) else 'some beyond 0.1'})")

    log()
    log("Verdict (reading): we read the V-Effect as a change in cavity geometry;")
    log("bond count alone sets neither the mode count nor the Q (Step 5).")
