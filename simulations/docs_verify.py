...
def verify_round4_artifacts_and_consumers():
    ...
    surfaces = {
        ...
        "experiments/PT_SYMMETRY_ANALYSIS.md": (("the final SRP class remains OPEN", "sqrt(p_x) Pi",
            ...
            "Fragile-bridge off-axis regime", "Π remains exact on both sides"),
        ...
        "hypotheses/FRAGILE_BRIDGE.md": (("derived partner gap adds no independent evidence",
        ...
        "simulations/fragile_bridge_ep_signature.py": ((
            "sits slightly below the collision",
        ...
        "simulations/results/fragile_bridge_ep_signature.txt": ((
            ...
            "near-threshold simplicity gate: nearest gap > 1e-10"),
            ("tracked axis departure", "secondary bump", "nearest-neighbour gap")),
        "hypotheses/ZERO_IS_THE_MIRROR.md": (("Past γ_crit the spectral abscissa is positive",
            "off-axis quartets appear"),
            ("tracked axis departure",)),
        "hypotheses/PAIR_BREAKING_AT_THE_HORIZON.md": ((),
            ("tracked axis departure", "tracked gain-loss instability")),
        "docs/proofs/PROOF_F86A_EP_MECHANISM.md": ((), ("tracked axis crossing",)),
        "experiments/F86_EP_THROUGH_THE_CLOCK.md": (("Only the toy 2×2 end has the stated discriminant",),
        ...
        "compute/RCPsiSquared.Core.Tests/Symmetry/TransitionBridgeF95SiblingClaimTests.cs": ((
            "two qubits per chain",),
            ()),
        "compute/RCPsiSquared.Core.Tests/F86/FragileBridgeThresholdTests.cs": ((
            "Threshold_IsADefectiveEp2OnTheRealGammaAxis", "PauliDephasingDissipator.BuildZ",
            "Svd(computeVectors: false)", "LocalGlobalEpLink.FragileBridgeThresholdGamma"),
            ()),
        "compute/RCPsiSquared.Core/F86/LocalGlobalEpLink.cs": ((
            "FragileBridgeThresholdGamma = 0.187310108345",
            "at J_bridge = 1.0 and 1.9", "two qubits per chain"),
            ()),
    ...
    fragile_tail_surfaces = {
        ...
        "docs/ANALYTICAL_FORMULAS.md": (("F19. Fragile bridge",), ()),
        "docs/GLOSSARY.md": (("sampled maximum is bracketed", "0.578 at J_bridge=10",
            "0.508 at 100", "spectral abscissa leaves zero",
            "off-axis quartets appear"),
            ("optimal (2x internal coupling)", "tracked pair leaves the imaginary axis")),
        "review/OPEN_QUESTIONS_INDEX.md": (("0.578 at J_bridge=10", "0.508 at 100"), ()),
    }
    ...
    for name, markers in (
        ("pt_symmetry_analysis", ("full irreducible SRP class remains OPEN", "off palindrome axis; operator relation remains exact",
            "That test is run in fragile_bridge_ep_signature.py (section 1)")),
        ...
        ("fragile_bridge_ep_signature", (
            "At J_bridge = 1.0 and 1.9: geometric multiplicity 1 at algebraic multiplicity 2",
            "In every row the full L holds one Jordan pair per copy",
            "gamma_crit = 0 there, and any small gain makes the bridge unstable",
            "the sampled maximum is a kink where the first (2,2) half switches spin-flip parity",
            "At both couplings the bisected gamma_crit sits slightly below the collision",
            "A single-vector K is omitted wherever the selected eigenvalue is degenerate",
            "These are samples; they establish no limit.")),
    ):
...
def verify_node_pair_current_truth():
    ...
    required = {
        "proof": ("Three spaces must be kept separate", "dim ker L_D",
                  ...
                  "Theorem 1 and Corollaries A-C are parity-free and",
                  "Corollary C requires a fixed diagonal",
                  "arbitrary zero-free real symmetric Jacobi chain", "any watched seat",
                  "local order reversal of the right principal block",
                  "three acquisitions spanning two decades", "84 current checks", "M1-M10",
                  "M9",
                  ...
                  "Corollary C (derived, same-energy iff", "r != 0,+1,-1",
                  "χ_B(x;r) = P(x)Q(x) − r² t_b² P′(x)Q′(x)",
                  "Corollary D (derived, total-count equality",
                  "N=6", "new blind energies", "physical-Heisenberg counterexample",
                  "not by itself a (1,1) decay rate", "second-order coefficient",
                  "minimum is undefined on a seat-incident bond",
                  "nonzero-frequency peripheral", "do not establish the globally"),
        "producer": ("complete_zero_frequency_mismatch", "M5 G8c", "M6 G8c",
                     ...
                     "def gate_G10", "M10 nonuniform linear-in-r bond mutation",
                     "N6 gcd and independent full-chain Krylov rank",
                     "G10 incident-bond cut escapes the zero-free common-root count",
                     "G10 fixed-diagonal fence: physical Heisenberg J bond",
                     "positive rate from minus the real dyad diagonal",
                     "count is unchanged while the chain remains zero-free",
                     "blind count is unchanged while eps != -1"),
        "claim": ("Tier.Tier1Derived", "PROOF_NODE_PAIR_RESOLVENT.md",
                  "epsilon != -1", "uniform centre-watched", "if and only if",
                  "r != 0,+1,-1", "off-centre and nonuniform", "fixed diagonal",
                  "new blind energies", "off-diagonal hopping", "inspect --root nodepair"),
        "witness": ("NodePairResolventWitness : IInspectable", "CanonicalReducedResolvent",
                    ...
                    "BigRationalMatrixCharpoly", "OffCentreBirth",
                    "NonuniformFixedEnergyCases", "JacobiPolynomial"),
        "knowledge": ("general sufficient bond theorem", "off-diagonal hopping",
                      "same-baseline-energy",
                      "off-centre arms can gain new blind energies"),
        "cli": ('new("nodepair"', "PROOF_NODE_PAIR_RESOLVENT", "uniform-centre iff",
                "off-centre", "off-diagonal-only", "new NodePairResolventWitness()"),
        "arc": ("SUFFICIENT direction", "PROOF_NODE_PAIR_RESOLVENT",
                "UNIFORM CENTRE-WATCHED", "same-baseline-energy", "r != 0,+1,-1",
                "FIXED DIAGONAL", "off-diagonal hopping scales", "N=6", "epsilon = -2",
                "all-knob fixed-energy derivative",
                "does not determine the total count"),
        "f157": ("node-pair resolvent proof", "Corollary C proves", "r != 0,+1,-1",
                 "new blind energies", "off-diagonal hopping", "inspect --root nodepair",
                 "exact N=6 off-centre birth"),
        "parent": ("node-pair resolvent proof", "sufficient bond direction generally",
                   "off-diagonal hopping moved", "same-baseline-energy iff", "New-root loci"),
        "blindsite": ("dim ker L_D=(N-1)^2+1", "=m^2+1", "dim ker A=m+1",
                      "do **not** coincide", "The Node Pair"),
        "road": ("The Node Pair", "off-diagonal hopping changes", "diagonal stays fixed",
                 "same-baseline-energy iff", "off-centre", "new-root loci"),
        "twoaxes": ("node-pair resolvent proof", "off-diagonal-only",
                    "same-baseline-energy iff", "New-root loci"),
