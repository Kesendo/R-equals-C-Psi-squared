"""F89 path-5: closed-form for the F_a (AT-locked rate 2γ) mode amplitudes.

Path-5 (N_block=6) has SE Bloch eigenvalues E_n = 4J·cos(πn/7) for n=1..6.
S_2-anti modes are n=2, 4, 6 with E_2 = 4J·cos(2π/7), E_4 = 4J·cos(4π/7),
E_6 = 4J·cos(6π/7). These are roots of a Cardano-cubic minimal polynomial
(specifically: x³ + x² − 2x − 1 = 0 has roots 2cos(2πk/7) for k=1,2,3).

Expectation: F_a closed-form amplitudes involve cubic-Cardano radicals
of {cos(π/7), cos(2π/7), cos(3π/7)} or equivalently {7-th roots of unity}.

Strategy: numerical sigs at multiple (q, N), check N-constancy after
N²(N-1) normalisation, then identify rational + cubic-radical structure.
"""
...
def main() -> None:
    ...
    Ns = [7, 8, 9, 10, 11, 12, 14, 16, 20]  # N ≥ N_block + 1 = 7
    print(f"# F89 path-5: AT-locked amplitude closed-form derivation")
    print(f"# N_block = {N_BLOCK} (path-5); qs = {qs}; Ns = {Ns}\n")
    ...
    # Filter: only F_a modes with non-trivial amplitude
    ...
    print("| Eigenvalue | freq ω/J | sigs · N²(N-1) (constant?) |")
    print("|---|---|---|")
    ...
    # Try to recognise sum as rational + cubic-radical
    print("\n## Recognising Σ F_a in algebraic extensions:")
    cos_pi_7 = sp.cos(sp.pi / 7)
    cos_2pi_7 = sp.cos(2 * sp.pi / 7)
    cos_3pi_7 = sp.cos(3 * sp.pi / 7)
    extensions = [
        (None, "rational"),
        ([sp.sqrt(7)], "Q[√7]"),
        ([sp.sqrt(2), sp.sqrt(7)], "Q[√2, √7]"),
        ([cos_pi_7], "Q[cos(π/7)]"),
        ([cos_pi_7, cos_2pi_7, cos_3pi_7], "Q[cos(πk/7)]"),
    ]
    ...
    for k, lam, sigs in nontrivial:
        ...
        for ext, label in extensions:
            try:
                ...
                if not rat.has(sp.Float):
                    print(f"# λ={lam:+.4f}, ω/J={lam.imag/J:+.4f}: sigs · N²(N-1) = {rat} ({label})")
                    break
            except Exception:
                pass
