...
HandoverFloorClaim). Both = the interior/Lebensader dressed mode's light content <n_XY> crossing 1
(= the Absorption-Theorem band-edge floor 2*gamma). Three falsifiable predictions:
  ...
  (1) THE FLOOR is exact and model-independent: the bright band-edge single-magnon coherence
      |vac><magnon| sits at rate = 2*gamma (<n_XY> = 1) for ALL Delta - it is an
  ...
  (2) Delta* = where the Lebensader rate crosses that floor: at Delta*, the slowest mode's
      <n_XY> = 1 exactly (the Lebensader rate = 2*gamma). Same condition as the Q-handover.
...
GAMMA = 0.05
FLOOR = 2.0 * GAMMA  # the band-edge Absorption-Theorem rate (<n_XY> = 1)
...
def branches(N, Delta, basis_mats, basis_nxy, gamma=GAMMA):
    ...
    Each: dict(rate, im, nxy_from_rate=rate/2g, nxy_mean). nxy_from_rate and nxy_mean are two readings of
    ...
    for s in nz[np.argsort(rate[nz])]:
        ...
        if out[kind] is None:
            out[kind] = dict(rate=float(rate[s]), im=float(abs(ev[s].imag)),
                             nxy_from_rate=float(rate[s] / (2 * gamma)),
...
def delta_star_rate(N, basis_mats, basis_nxy, lo=0.5, hi=2.5):
    ...
    (i.e. where the Lebensader light content crosses 1). Bisection on (leb_rate - FLOOR)."""
    for _ in range(40):
        mid = 0.5 * (lo + hi)
...
if __name__ == "__main__":
    for N in (4, 5):
        ...
        # (1) THE FLOOR: the band edge sits at exactly 2*gamma (<n_XY> = 1) for all Delta.
        floor_ok = True
        print("  Delta | band-edge (rate, <n_XY>, |Im|) | Lebensader (rate, <n_XY>, |Im|)")
        for Delta in [0.0, 1.0, dstar - 0.1, dstar, dstar + 0.1, 2.0]:
            b = branches(N, Delta, basis_mats, basis_nxy)
            be, le = b["band"], b["leb"]
            floor_ok &= be is not None and abs(be["nxy_from_rate"] - 1.0) < 1e-9
            bs = f"rate={be['rate']:.5f} nxy={be['nxy_from_rate']:.6f} Im={be['im']:.3f}" if be else "—"
            ls = f"rate={le['rate']:.5f} nxy={le['nxy_from_rate']:.6f} Im={le['im']:.2e}" if le else "—"
            tag = "  <- Delta*" if abs(Delta - dstar) < 1e-6 else ""
            print(f"  {Delta:5.3f} | {bs:42s} | {ls}{tag}")

        # (2) at Delta*, the slowest mode's <n_XY> = 1 (the Lebensader rate crosses the floor).
        ...
        print(f"\n  [1] FLOOR exact (band edge <n_XY> = 1 for all Delta):        {floor_ok}")
        print(f"  [2] Delta* (rate crossing, leb <n_XY>->1) = {dstar:.4f}  "
              f"(doc: N=5~1.525, N=4~1.618=phi)")
        print(f"      at Delta*: Lebensader light content = {leb_at['nxy_from_rate']:.5f}  (predicted 1.000)")
        ...
        assert abs(leb_at["nxy_from_rate"] - 1.0) < 5e-3, \
            f"N={N}: at Delta* the slowest light content {leb_at['nxy_from_rate']} != 1 (the unification fails)"
        ...
        assert abs(leb_at["nxy_from_rate"] - leb_at["nxy_mean"]) < 1e-6, "Absorption Theorem fails for the Lebensader"
        assert abs(band_at["nxy_from_rate"] - band_at["nxy_mean"]) < 1e-6, "Absorption Theorem fails for the band edge"
        print(f"  [AT] Absorption Theorem holds in XXZ: rate/2g == Sum_k k*w_k "
              f"(leb {leb_at['nxy_mean']:.5f}, band {band_at['nxy_mean']:.5f})")
        print()

    print("All asserts passed: the Delta-handover IS the band-edge-floor crossing (<n_XY> = 1), a LEVEL")
