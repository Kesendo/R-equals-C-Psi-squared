...
A(gamma) could diverge. We check, at the exact physical point:
  ...
  (3) char-poly coefficient gap (normalised L1 over the coefficients of char_L vs char_{-L-2s};
      the algebraic Delta=0 test, and NOT a sorted element-wise compare, which is not a valid
      set-equality test)
and require the two computable-at-every-N measures, greedy and OT, to declare 'broken' for every
hard pair. Measure (3) is the same multiset test read algebraically; its coefficients overflow
double range at N=5, so it corroborates at N=4 and is reported as silent at N=5 rather than
counted as a third vote. If ANY hard pair is OT-exact at gamma=0.05 while greedy says hard (or
vice versa), that is the hole.
...
def run(N):
    ...
    cp_finite = 0          # hard pairs where the char-poly gap is computable at all
    cp_broken = 0          # ... and declares BROKEN
    hard_min = (None, np.inf)
    for (t1, t2, _) in hard:
        lab = "".join(t1) + "+" + "".join(t2)
        g, ot, cp = measures([t1, t2], GAMMA, N)
        # The VERDICT rests on greedy + OT only. The char-poly gap is the same multiset test read
        # algebraically, and it overflows to nan at N=5 (the coefficients of a 4^N x 4^N char-poly
        # exceed double range), so it is counted where it is finite and reported separately.
        broken = (ot > TOL) and (g > TOL)
        hard_all_broken = hard_all_broken and broken
        if np.isfinite(cp):
            cp_finite += 1
            cp_broken += int(cp > TOL)
        if ot < hard_min[1]:
            hard_min = (lab, ot)
    ...
    print(f"  --- hard pairs: every hard broken on greedy AND OT = {hard_all_broken}")
    print(f"  --- char-poly gap (informational): finite on {cp_finite}/{len(hard)} hard pairs, "
          f"BROKEN on {cp_broken} of those"
          f"{' -- overflowed to nan, contributes nothing here' if cp_finite == 0 else ''}")
    print(f"  --- hard min OT distance (closest to spec-exact) = {hard_min[1]:.3e} ({hard_min[0]})")
    return hard_all_broken, worst_soft < TOL, cp_finite, cp_broken, len(hard)


def cp_sentence(N, cp_finite, cp_broken, n_hard):
    """Say what the char-poly measure actually did at this N, from the counts, never from memory."""
    if cp_finite == 0:
        return f"at N={N} it overflows double range on all {n_hard} and is silent"
    if cp_broken == cp_finite == n_hard:
        return f"at N={N} it is computable on all {n_hard} hard pairs and breaks on all of them"
    return (f"at N={N} it is computable on {cp_finite}/{n_hard} and breaks on {cp_broken} of those"
            f"{'  <-- DISAGREES with greedy/OT' if cp_broken < cp_finite else ''}")
...
def main():
    ...
    ok5 = run(5)
    print()
    if ok4[0] and ok4[1] and ok5[0] and ok5[1]:
        print("VERDICT(attack #3 physical): at gamma=0.05 J=1, every hard pair is broken on both")
        print("  deciding measures (greedy and OT, the rigorous multiset distance) and every soft")
        print("  pair is exact, at N=4 AND N=5. The algebraic char-poly gap is not a third vote:")
        print(f"  {cp_sentence(4, *ok4[2:])}, and {cp_sentence(5, *ok5[2:])}.")
