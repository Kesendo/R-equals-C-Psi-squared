...
Public API:
  run(topology: str, N: int, J: float, gamma: float) -> dict
"""
...
def pauli_basis_overlaps(slow_vec: np.ndarray, N: int) -> np.ndarray:
    ...
    for p in range(4 ** N):
        ...
        amp = np.vdot(P.reshape(-1), slow_vec) * math.sqrt(norm_factor)
        overlaps[p] = abs(amp) ** 2
    return overlaps


def run(topology: str, N: int, J: float, gamma: float,
        kernel_tol: float = 1e-9, verbose: bool = True) -> dict:
    if topology not in BONDS_BY_TOPOLOGY:
        raise ValueError(f"unknown topology {topology!r}; choose chain/ring/star")
    Q = J / gamma if gamma > 0 else float("inf")
    if verbose:
        print(f"\n--- {topology} N={N}, J={J:.6g}, γ={gamma:.6g}, Q={Q:.4f} ---")

    bonds = BONDS_BY_TOPOLOGY[topology](N)
    d = 1 << N
    ...
    if verbose:
        print(f"  L built ({L.shape[0]}×{L.shape[0]}), bonds={bonds}, {time.time()-t0:.2f}s")

    sectors = joint_popcount_sectors(N)
    if verbose:
        print(f"  {len(sectors)} sectors, max block size {max(len(v) for v in sectors.values())}")
    ...
    return {
        "topology": topology, "N": N, "J": J, "gamma": gamma, "Q": Q,
        "gap": gap, "im_slow": im_slow,
        "slow_sector": sec_slow,
        "n_xy_avg": n_xy_avg,
        "weight_distribution": weights_dist.tolist(),
        "per_sector_slow": [
            (sec, ev[0] if ev else None, ev[1] if ev else None, sz)
            for sec, ev, sz in per_sector_slow
        ],
    }
