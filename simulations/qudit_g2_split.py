...
#   * the 2d term  = the F121 product-mirror cap base: a per-site mirror q_l carries one lit grade
#                    c in {0, 1, 2}; the grade-1 class swap between the d DARK letters {(x,x)} and the
#                    d^2-d LIT letters {(i,j), i!=j} has rank(q_l) <= min(d, d^2-d) + min(d^2-d, d) = 2d,
#                    and for d <= 5 the all-swap pattern is the optimum, so the cap = (2d)^N. (From
#                    d = 6 a grade-0 site beside a grade-2 site beats two swaps: d(d^2-d) > (2d)^2,
#                    P_dark x P_lit at d=6, N=2 has rank 180 > 144; the qutrit is far below that.)
#                                                                                  -> 2d = 6.
...
# the cap is P(d, N) = max_m (2d)^(N-2m) (d^3-d^2)^m, equal to (2d)^N at every d <= 5
F121_CAP = {(3, 2): 36, (3, 3): 216, (4, 2): 64}                 # = 6^2, 6^3, 8^2 ; NOT 81/729/256 = d^(2N)
for (d, N), measured in F121_CAP.items():
    cap = max((2 * d) ** (N - 2 * m) * (d ** 3 - d ** 2) ** m for m in range(N // 2 + 1))
    dsq_pow = (d * d) ** N
    assert cap == (2 * d) ** N == measured and cap != dsq_pow, f"STAGE 2 GATE FIRED: cap at (d={d},N={N}) != F121 {measured}"
