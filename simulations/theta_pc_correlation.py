...
def main():
    ...
    if n_q >= 5 and np.std(best_theta[mask_q]) > 1e-10:
        ...
        out(f"\n  Best pair for theta analysis: {best_pair} ({n_q} above-quarter samples)")
        for k in range(3):
        ...
        out(f"  The sampled manifold lies entirely in the below-or-at-quarter bin (CPsi <= 1/4).")
    ...
    if n_q >= 5 and np.std(best_theta[mask_q]) > 1e-10:
        ...
        # 3-PC regression on above-quarter samples
        ...
        out(f"\n  theta-PC(3D) regression (above-quarter samples): R^2 = {r2_3pc:.4f}")
        ...
        dominant = int(np.argmax(r_abs))
        out(f"\n  On this pair/dataset, theta aligns most with PC{dominant+1} "
            f"(|r|={r_abs[dominant]:.3f}).")
        out("  The separate r=0.87 channel comparison uses a different pair/dataset;")
        out("  this regression does not explain or allocate its 24.3% residual.")
    else:
