...
applied to all 6 candidate term-sets:
  ...
  * the 2 Z-middle (XZX+XZY+YZX, YZY+XZY+YZX) RESIST the UNIFORM family (bounded away; since 2026-06-10 a
    theorem). They are nonetheless LOCAL via the period-4 golden router, which this uniform lens cannot see
    (docs/proofs/PROOF_CEILING_GOLDEN_ROUTER.md, the 2->0 step).
...
Self-validating: asserts the routable below 1e-7 and the uniform-resisting (Z-middle + I-heavy) above 1e-2.
...
# The Z-middle pair: resists this uniform family (a theorem since 2026-06-10), but is LOCAL via the
# period-4 golden router (docs/proofs/PROOF_CEILING_GOLDEN_ROUTER.md), invisible to the uniform lens here.
...
def main():
    ...
    print("\nTHE 2 Z-MIDDLE CASES (resist the UNIFORM family; local via the period-4 golden router,"
          " PROOF_CEILING_GOLDEN_ROUTER):", flush=True)
    for name, terms in NONLOCAL.items():
        best, _ = optimize(terms, restarts=70)
        print(f"  {name}: best={best:.3f}  => no UNIFORM per-site Q (the golden router routes it)", flush=True)
    ...
    for name, terms in SITEVARYING_LOCAL.items():
        ...
        assert best > 1e-2, f"{name} must resist the uniform family"
    print("\nOK: 2 route uniform (local), 2 route site-varying (local), 2 Z-middle resist the uniform"
          " family (local via the golden router). 6->4->2->0.", flush=True)
