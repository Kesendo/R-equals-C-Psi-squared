#!/usr/bin/env python3
"""FINAL resolution of the equiv2/equiv3 'mismatches': the discriminator, and its coordinates.

Section 7.5 states the criterion twice, in the two normalisations of the same block: soft iff -N is
attained in Q = sum_l Z_l (.) Z_l (spec(Q) in [-N, +N]), equivalently iff D-hat = Q - N*I is
SYMMETRIC about its centre -N (spec(D-hat) in [-2N, 0]). `omega0_block` below returns spec(D-hat),
so 'the extreme -N of Q attained' reads there as '-2N attained', NOT as '-N attained': -N is
D-hat's CENTRE. The proof says so in as many words ("this is the discriminator, not whether -N is
D-hat's minimum"). Both earlier probe generations tested a condition in the wrong normalisation.

Draw 1500 random H and keep the in-scope ones (FHF=-H, zero diagonal); discriminator = (omega=0 block symmetric about
-N), and independently (-2N attained in spec(D-hat) = -N attained in spec(Q)). Compare to TRUE
chiral-K existence (KHK=-H). Expect bit-exact agreement on both, with the ODD-CYCLE (no chiral K)
cases asymmetric. Zero mismatches => section 7.5's soft+hard legs of 'c=0 <=> bipartite' are solid
well beyond the 16 canonical pairs.
"""
...
def main():
    ...
    mism_present = 0   # phi_exists != (-2N attained in spec(D-hat)) = (-N attained in spec(Q))
    examples = []
    for _ in range(1500):
        m = rng.integers(2, 6)
        ...
        present = np.min(np.abs(s - (-2 * N))) < 1e-6
        if bip != symm:
            mism_sym += 1
            if len(examples) < 8:
    ...
    print(f"  phi_exists(bipartite) != (-2N attained in D-hat = -N in Q) mismatches: {mism_present}")
    for x in examples:
    ...
    if mism_sym == 0 and mism_present == 0:
        ...
        print(f"  <=> -N attained in Q (= -2N attained in D-hat)' over the {n_inscope} in-scope H")
        print("  that 1500 random draws yield. The")
        print("  earlier equiv2/equiv3 'mismatches' were a coordinate slip in the probe: -N is")
        print("  D-hat's CENTRE and Q's EXTREME, and only the extreme reading is section 7.5's")
        print("  condition. With both discriminators in their own normalisation, section 7.5's")
        print("  'c=0 <=> bipartite' source equivalence is solid.")
    else:
