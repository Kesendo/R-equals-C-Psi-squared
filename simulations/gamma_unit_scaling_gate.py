...
No general F14 converse is tested here. F3 reproduces the one-arm
Hamiltonian-dead Bell+/Wootters-concurrence value K=ln(4/3)/8 under equal
local Z-dephasing. The purity-times-l1 F25 coefficient is a different readout.
The |01> trajectory is a separate gamma-only-scaling negative control, not
the second arm of a universal crossing law.
...
def main(output_path=OUT_PATH):
    """Run the finite gates; explicit output paths keep imports and default CLI read-only."""
    global _lines, PASS, FAIL
    _lines, PASS, FAIL = [], 0, 0
