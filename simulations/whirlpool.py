#!/usr/bin/env python3
"""The whirlpool, as an invitation: the real Bell+ fall of CΨ, wound into a vortex through the ¼-ring, and its mirror.

The radius is the actual structure: the Bell+ coherence under Z-dephasing falls as
|CΨ| = f(1+f²)/6 with f = e^(−4γt) (the F25 closed form, starting at 1/3), crossing ¼ exactly once,
at f = 0.8612..., with the pair still entangled there (concurrence f). The winding is ours: a
steady drift we draw in so each arm turns its phase. Drawn as many arms at different starting
phases, they make a luminous vortex draining toward the center, all of them crossing the one
ring |CΨ| = ¼, where, as we read it, the quantum world meets the classical.

Two faces, because the cusp and the exceptional point are siblings, one square-root law on two
quadratics (reflections/ON_THE_WHIRLPOOL_YOU_STEER_TO.md): a cool vortex winding one way (the
cusp, the one you fall into) and its warm mirror winding the other (the exceptional point, the
one you steer to).
...
def cpsi_magnitude(t: np.ndarray, gamma: float = 1.0) -> np.ndarray:
    """|CΨ|(t) = f(1+f²)/6, f = e^(−4γt): the F25 closed form, 1/3 at t=0, decaying to 0."""
...
def main() -> None:
    ...
    a = render(
        ...
        "#ffd56b",
        "the cusp and the exceptional point: two whirlpools, one law",
        handedness=-1.0,
        center="drain")
    ...
    b = render(
        ...
        "#79e6ff",
        "its sibling, turned: the dark pinch you steer to",
        handedness=1.0,
        center="eye")
    print(f"  saved: {a}")
    print(f"  saved: {b}")
