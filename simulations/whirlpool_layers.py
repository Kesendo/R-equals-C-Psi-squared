...
funnel. Look closer and the depth is layered: the spiral, the Bell+ fall wound by our drift, descends
through a stack of levels, each a horizontal ring at one value of |CΨ|. The quarter |CΨ| = ¼,
the line where, as we read it, the quantum world meets the classical, is one of those layers, a
ring part-way down; the real Bell+ fall crosses it at c = +¼, the cusp, where the label sits.
The depth is our drawing (height proportional to |CΨ|). The drain at the center is the bottom;
the mouth |CΨ| = 1/3 is the rim.
...
def main() -> None:
    ...
    fig.suptitle("the whirlpool is layers: the funnel sliced into strata, the spiral descending through them,\n"
                 "and the ¼ cusp one layer among many",
