...
Phase 1: F6 Q-edge-gain table N=1-5, formula validation
...
def main(output_path=OUT_PATH):
    ...
    # ========================================================================
    # PHASE 1: F6 Q-EDGE-GAIN TABLE AND FORMULA VALIDATION
    # ========================================================================
    log()
    log("=" * 72)
    log("PHASE 1: F6 Q-EDGE-GAIN TABLE (Heisenberg chain, J=1.0, γ=1.0)")
    log("=" * 72)
    log()
    log("  Analytical predictions from ANALYTICAL_FORMULAS.md:")
    log("  F6 Q-edge gain V(N) = 1 + cos(π/N),  ω_k = 4J(1-cos(πk/N))")
    ...
    for N in range(1, 6):
        ...
        # Numerical F6 normalization: Q_max / (2J/gamma), within this N-site
        # Heisenberg model.  This is not a cross-N reference quotient.
        # For N=1 (no bonds), Q is undefined; display zero by convention.
        if N == 1:
            V_num = 0
        else:
    ...
    log("=" * 72)
    log()
    log("  Finite TFI frequency-bin and Q inventory as chain length changes.")
    log("  Model: Transverse-field Ising, K=20 cm⁻¹")
    log()

    log(f"  {'N':>3}  {'Regime':>10}  {'J/γ':>6}  {'EV':>5}  {'Freq':>5}"
        f"  {'Q_max':>8}  {'Q/Q(N=1)':>10}  {'cross':>5}")
    log(f"  {'─'*60}")
    ...
    for rname, J_r, gamma_r in regimes:
        ...
        for N in range(1, 6):
            ...
            # A TFI cross-N quotient relative to the N=1 row.  Its denominator and
            # generator differ from F6, so it is deliberately not called V(N).
            reference_q_quotient = r['Q_max'] / q1 if q1 > 0.001 and N > 1 else 0
            ...
            log(f"  {N:>3}  {rname:>10}  {J_r/gamma_r:>6.2f}  {r['n_ev']:>5}"
                f"  {r['n_freq']:>5}  {r['Q_max']:>8.3f}  {reference_q_quotient:>10.2f}"
                f"  {cross_str:>5}")
        log()
    ...
    log("=" * 72)
    log()
    log("1. The Heisenberg-chain F6 Q-edge gain and ω_k/Q_max formulas match the")
    log("   Heisenberg model exactly. For the physical TFI model, the")
    log("   mode count and Q-factors are similar but not identical.")
    log()
    log("2. The sampled TFI generators have different finite frequency-bin counts.")
    log("   The N=1-reference Q quotient and those counts are not the within-N F6 ratio.")
    log()
    ...
    log()
    log("4. In the sampled N=5 TFI profiles, edge sacrifice improves Q by ~2-4x.")
    log("   These rows do not establish one substrate-independent mechanism.")
    log()
