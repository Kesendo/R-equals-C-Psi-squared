#!/usr/bin/env python3
"""Historical greedy real-part matcher retained as an instrument control.
...
def main():
    ...
    print("=" * 65)
    print("HISTORICAL GREEDY REAL-PART MATCHER: Instrument Control")
    print("=" * 65)
    print("Matched-pair percentages and character errors do not test F36 or Qv transport.")

    for alpha in [0.3, 0.5, 1.0]:
        W, signs = make_balanced_dale_network(N, n_exc, 0.3, 42)
        ...
        # Historical greedy real-part matching: ignores imaginary parts and Q transport
        ...
        print(f"\n--- alpha = {alpha} ({len(pairs)} greedy real-part pairs) ---")
        ...
        for k, kp in pairs:
            r_k = rates[k]
            r_kp = rates[kp]
            # Measure character difference for these greedily selected indices
        ...
        print(f"  (0 = matching E/I fractions for these selected vectors; no transport verdict)")

        # Find the center mode (closest to E-char = I-char = 0.5)
        center_dists = np.abs(e_char - 0.5)
        center_mode = np.argmin(center_dists)
        print(f"\n  Mode with E-char closest to 0.5 (not necessarily at the spectral center):")
    ...
    for t in times:
        x_t = expm(J * t) @ delta0
        # Historical CΨ_E label: squared norm fraction of the E-component
