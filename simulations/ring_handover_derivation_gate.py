...
<n_XY> = 1 condition instead of the EP):
  ...
  Overdamped slow root:  lambda = -4g + 2*sqrt(4g^2 - J^2 q^2),  so
      light content <n_XY> = -Re(lambda)/(2g) = 2 - sqrt(4 - (Q q)^2),   Q = J/g.
  HANDOVER (<n_XY> = 1, the band-edge floor Re=-2g):
      2 - sqrt(4 - (Q q)^2) = 1  =>  (Q q)^2 = 3  =>  Q q = sqrt3  =>  Q_h = sqrt3 / q_min = N*sqrt3/(2pi).
  (the EP of the SAME mode is at Q q = 2, <n_XY> = 2 -> Q* = 2/q_min = N/pi = the SE coherence horizon;
   Q_h/Q* = sqrt3/2. The high-Q light content -> 2 = Tom's commutant 2(N-2)/N -> 2, the EP limit of <n_XY>.)

THE GATES (a firing gate is the find; do NOT loosen):
  G1  the UNIVERSAL CURVE: the numerical (2,2) slowest-mode nxy(Q) collapses onto
  ...
  G3  the HANDOVER: <n_XY> = 1 occurs at Q q = sqrt3 (Q_h * q_fit = sqrt3), AND Q_h*2pi/N -> sqrt3.
  G4  the EP / high-Q: the curve predicts <n_XY> = 2 at Qq=2; the numerical high-Q light content -> 2(N-2)/N
      (finite-N EP), consistent with the leading-order EP value <n_XY> = 2.
"""
import sys
import numpy as np
sys.path.insert(0, "simulations")
from ring_handover_qh import nxy   # the numerical (2,2) slowest-mode <n_XY>(N,k=2,Q)
...
def D_model(Qq):
    """<n_XY> = 2 - sqrt(4 - (Qq)^2), the overdamped slow root of lambda^2+8g lambda+4J^2 q^2."""
    return 2.0 - np.sqrt(np.maximum(4.0 - Qq ** 2, 0.0))


def fit_q(N, Qs):
    """Fit the single q that best maps the numerical nxy(Q) onto D(Qq). Least squares on
    light content vs 2-sqrt(4-(Qq)^2). Returns (q_fit, rms residual, table)."""
...
def main():
    ...
    print("=" * 100)
    print("DERIVATION GATE: nxy(Q) = 2 - sqrt(4-(Qq)^2) (the SE coherence-ladder dispersion);")
    ...
    for N in (8, 10, 12, 14):
        # sample Q across the overdamped band, from light content~0.3 up toward ~1.6 (below the EP/commutant)
        Q_h_guess = N * sqrt(3) / (2 * pi)
        Qs = list(np.linspace(0.6 * Q_h_guess, 1.5 * Q_h_guess, 9))
        q_fit, rms, tab = fit_q(N, Qs)
        # Q_h: bisect <n_XY> = 1 (reuse the model-free numeric)
        ...
        gate(f"N={N}: G1 curve RMS small (light content collapses onto 2-sqrt(4-(Qq)^2))", rms < 0.03, f"RMS={rms:.2e}")
        gate(f"N={N}: G3 Q_h*q_fit = sqrt3 (the handover is the <n_XY> = 1 point of the curve)",
    ...
    if GATE["fired"]:
        ...
        print("SE coherence-ladder dispersion lambda^2+8g lambda+4J^2 q^2, nxy(Q)=2-sqrt(4-(Qq)^2); the")
        print("handover (<n_XY> = 1) is at Qq=sqrt3, the EP-sibling (Qq=2 -> N/pi) of the SE coherence horizon.")
    print("=" * 100)
