...
| 7 | **Route all noise to one termination resistor.** Concentrate the entire noise budget on one edge device. Protect all others at minimum noise floor. For network bandwidth, use edge sacrifice. For point-to-point throughput, use center sacrifice (relay). Hybrid profiles are worse than both. | Edge qubits have one neighbor (least coupling loss). Sacrificing one edge preserves coherence in the N-1 remaining devices. IBM hardware: selective DD 2-3.2x on ibm_torino (N=5). | 91-360x vs uniform (N=5-13). [Resonant Return](../experiments/RESONANT_RETURN.md), [IBM Sacrifice Zone](../experiments/IBM_SACRIFICE_ZONE.md) |
...
5. **Spatial noise engineering.** The sacrifice-zone formula (Rule 7)
   shows that directing all noise to one edge termination yields 91-360x
...
- [Resonant Return](../experiments/RESONANT_RETURN.md): Sacrifice-zone formula (91-360x), signal engineering derivation
- [IBM Sacrifice Zone](../experiments/IBM_SACRIFICE_ZONE.md): Hardware validation, selective DD 2-3.2x
