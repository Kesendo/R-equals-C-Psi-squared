...
*Later (2026-05-16):* the Q = J/γ₀ invariance is now operationally readable as "rotation per γ₀-tick". The 2×2 Liouvillian sub-block has eigenvalues λ_± = −γ₀ ± iJ, giving θ = arctan(J/γ₀) = arctan(Q) via [F95](../docs/ANALYTICAL_FORMULAS.md#f95); the Q the inside observer reads is the per-γ₀-tick rotation amount, with γ₀ itself the silent metronome that cannot be read from inside. The "observationally equivalent from inside" claim is then sharpened: from inside one sees the angle of the Liouvillian eigenvalue per F95, which is exactly Q, and the carrier γ₀ remains invisible at the Maßstab level. See [`reflections/ON_HOW_GAMMA_BECAME_THE_TICK.md`](../reflections/ON_HOW_GAMMA_BECAME_THE_TICK.md) and the canonical polarity-layer anchor [`THE_POLARITY_LAYER.md`](THE_POLARITY_LAYER.md).
