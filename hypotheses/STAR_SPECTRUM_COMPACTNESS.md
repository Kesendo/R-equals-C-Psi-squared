...
**Tier 3 (reading, not yet a typed claim).** Observation surfaced from the F1 SLOW_N8 sweep on 2026-05-18 (commit 89f725e). The star topology produces an unusually compact Liouvillian spectrum at N=8, both in the imaginary range (exactly ±N/2) and in the distinct-eigenvalue count (~30× fewer unique values than the chain at the same N, γ, J). The hub geometry is the obvious suspect; the precise structural rule that gives ±N/2 as an integer bound and the order-of-magnitude degeneracy increase has not been written.
...
- Anchor data: `simulations/results/f1_n8_n9_metrics/star_N8.json` (`MaxImag`, `DistinctBinnedEigenvalueCount`); same fields in the three sister files for contrast.
- Sister Tier 3 reading from the same sweep: [F1_DISSIPATION_GAP_PATTERN](F1_DISSIPATION_GAP_PATTERN.md)
- Companion typed claim from the same sweep (closed form that did promote): [F4KernelDimensionByComponentsClaim](../compute/RCPsiSquared.Core/Symmetry/F4KernelDimensionByComponentsClaim.cs)
