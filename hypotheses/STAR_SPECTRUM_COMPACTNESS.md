...
**Tier 3 reading, partially resolved 2026-05-19.** Observation surfaced from the F1 SLOW_N8 sweep on 2026-05-18 (commit 89f725e). The star topology produces an unusually compact Liouvillian spectrum at N=8, both in the imaginary range (exactly ±σ when J = 2γ, which reads ±N/2 only at the γ = 0.5, J = 1 point this sweep ran) and in the distinct-eigenvalue count (~30× fewer unique values than the chain at the same N, γ, J). The hub geometry is the obvious suspect.

**Resolution status (2026-05-19):**
- **Reading 1 (MaxImag = σ is a hub-induced spectral cap):** **RESOLVED, and the hub reading was the wrong half**, via cross-N data + the SU(2)/Schur-Weyl derivation in [`STAR_CONFOCAL_LIMIT.md`](../experiments/STAR_CONFOCAL_LIMIT.md). Verified at N=3,4,5,6,8. The cap is `max |Im(λ)| ≤ ΔE_max(H)`, and it is saturated at every (J, γ) with uniform γ, not only at `J = 2γ`; `Im/σ = 1.0` is the `J = 2γ` specialization. The star is *not* the unique topology that saturates: chain, ring, complete and disconnected graphs all meet their own `ΔE_max(H)` exactly. What is hub-induced is the *value*, `ΔE_max(H_star) = J·N/2`, which is the minimum over connected graphs (star the unique minimiser, exhaustive at N ≤ 6).
- **Reading 2 (S_(N−1) representation theory accounts for 30× degeneracy ratio):** **SHARPENED 2026-06-02** ([probe](../simulations/star_degeneracy_repn.py)). The leaf-permutation S_(N−1) gives a clean upper bound on the distinct count. The Liouville space is `C^4` (hub, S_(N−1)-trivial) `⊗ (C^4)^{⊗(N−1)}` (leaves, S_(N−1) permuting factors), so by Schur-Weyl L block-diagonalises into blocks of size `M_λ = 4·dim S^λ(C^4)`, and `distinct ≤ Σ_{λ⊢(N−1)} M_λ = 4·Σ_{λ⊢(N−1)} dim S^λ(C^4)` (= 4400 at N=8; verified as a bound at N=3,4,5,6,8). This bound has a closed form, `Σ M_λ = 4·[x^{N−1}](1−x)⁻⁴(1−x²)⁻⁶` (the Littlewood principal specialisation Σ_λ dim S^λ(C^d)·x^{|λ|} = (1−x)⁻ᵈ(1−x²)⁻C⁽ᵈ,²⁾ at d=4, times the hub factor 4; 4400 = 4·1100 at N=8), verified four independent ways (hook-content, the binomial generating function, a sympy series, and an S₃ character computation) across N=3..16 ([test](../simulations/star_gf_closed_form_test.py)). But the exact 2 275 lies **below** this cap (≈0.52×) and is **not** a clean S_7 count: adding the U(1) `(Sz_left, Sz_right)` grading refines the blocks without lowering the sum, and the residual reduction is **cross-block value-coincidence from the non-normal dephasing spectrum** (the Re-decay ladder), not a representation-theoretic effect. The tempting `distinct = |Re|·|Im|` factorisation is refuted: `Im(λ_L) ≠ H-gaps` because L is non-normal, and `distinct/(|Re|·|Im|)` falls 0.24→0.01 across N=3..6. (No tension with Reading 1: the *set* of imaginary parts is not the set of H gaps, while the *maximum* of the two coincides. Only the extremes are forced, and they are forced by the numerical-range argument, which constrains max|Im| and says nothing about the interior.) So S_(N−1) is necessary and dominant for the compactness but not sufficient for the exact count; the clean derivable object is the `Σ M_λ` upper bound.
- **Reading 3 (integer bound at other N):** **RESOLVED** ([`STAR_CONFOCAL_LIMIT`](../experiments/STAR_CONFOCAL_LIMIT.md)). `MaxImag = J·N/2` for all (N, J) at uniform γ, verified at 25 distinct (N, Q) anchors over 29 runs (a 24-point Q-sweep, the N=8 γ=0.5, J=1 point, and four Python re-runs of the Q=2 column), typed as [`StarImMaxBoundClaim`](../compute/RCPsiSquared.Core/Symmetry/StarImMaxBoundClaim.cs) Tier 1 derived. The `MaxImag = N/2` reading at γ=0.5 was the J=2γ specialization, not a separate rule.
...
## The three original readings (2026-05-18)

Kept verbatim as the questions this doc opened with. Two of the three are closed and the third is sharpened; the current status of each is at the top of this page and in the typed-claim section below, so read these as history, not as open work.
...
**Readings 1 and 3 are now typed.** The imaginary cap `MaxImag = J·N/2` is [`StarImMaxBoundClaim`](../compute/RCPsiSquared.Core/Symmetry/StarImMaxBoundClaim.cs) Tier 1 derived (proof [`PROOF_STAR_OPTICAL_CONFOCAL_SATURATION`](../docs/proofs/PROOF_STAR_OPTICAL_CONFOCAL_SATURATION.md)). Note the historical phrasing `MaxImag = σ = Nγ` was the J=2γ coincidence; the general statement is `J·N/2` at uniform γ, independent of γ.
...
- **Sharpening of Reading 1 (the MaxImag = σ saturation):** [`STAR_CONFOCAL_LIMIT.md`](../experiments/STAR_CONFOCAL_LIMIT.md) (Tier 1 derived, 2026-05-19): full cross-N table + SU(2)/Schur-Weyl derivation + connection to OPTICAL_CAVITY_ANALYSIS point-focus limit.
