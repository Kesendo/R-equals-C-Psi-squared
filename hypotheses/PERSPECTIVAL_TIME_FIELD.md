...
**Status:** Computed (Tier 2): a sparse-Liouvillian slow-mode decomposition, nine scan families at N = 7, and a stress test across five qualitatively different initial states. [EQ-014](../review/EQ014_FINDINGS.md) closed the "closure law as first-order theorem" path: Σ_i ln(α_i) = 0 holds empirically to ±0.05 at |δJ| ≤ 0.1 but is NOT a first-order theorem. Σ f_i = lim_{δJ→0} Σ ln(α_i)/δJ is nonzero and state-dependent. PTF stays Tier 2; Tier-1 promotion via this route is closed.
**Date:** April 18, 2026
...
This is not a philosophical claim tacked onto standard physics; it is a claim about what the data privilege. The following sections show that the closure law, the thing that makes "the seven paintings add up to one mountain," is basis-independent and holds across the five states to ±0.05 in the perturbative window (±0.13 for the multi-sector |+⟩^7, one ill-posed ψ_4 fit aside); it is not a first-order theorem (EQ-014), and what survives exactly is the chiral mirror law, Σ f_i(ψ_k) = Σ f_i(ψ_{N+1−k}). The privilege resides in the closure, not in any single perspective. Painters sum to mountain. Mountain does not precede painters.
...
Concretely, under an N = 7 XY chain with γ_0 = 0.05 and the [F65](../docs/ANALYTICAL_FORMULAS.md#f65-first-order-endpoint-rate-coefficient-comb-of-the-uniform-open-xx-chain-tier-1-proven-verified-n330) bonding-mode initial state φ = (|vac⟩ + |ψ_1⟩) / √2, defect at bond (0, 1), J_mod = 1.1 (δJ = +0.1):
...
### 2.2 What it resembles, and where the resemblance stops

<!-- F14-CURRENT -->

The closure law looks like a many-observer cousin of [F14](../docs/ANALYTICAL_FORMULAS.md#f14-k-invariance-tier-2-fixed-book-scaling), and it is worth saying exactly how far the family likeness goes. F14 keeps the dimensionless dose K = γ · t_cross fixed while γ is swept, inside one fixed book: one bridge metric, one readout, a Bell+ trajectory on which the Hamiltonian does nothing. Change the bridge and K changes with it, so F14 is not a basis-free count of decoherence ticks. Here γ is not swept at all. The fit P_B(i, t) ≈ P_A(i, α_i · t) holds γ fixed and rescales each site's own clock, turning γt into γα_i t. That is a different operation, and the closure law does not extend F14; it stands on its own fits.

<!-- F14-INTERPRETIVE -->

In the painter picture the rescaled clocks invite a dose per perspective:

    K_i = γ · α_i · t,   Σ_i ln(α_i) ≈ 0.

γ stays as the atmospheric constant (there is no per-site γ_i). What we read as reallocating between observers is the effective time each perspective has spent since the event, and the log-sum-zero condition then says that the total "amount of time experienced by the chain as a whole" is left nearly unchanged by the defect (to ±0.05 in the window, ±0.13 for |+⟩^7); it is only redistributed among sites. That is our reading of the fitted α_i, not an invariant F14 certifies.

<!-- F14-CURRENT -->

### 2.3 Five different states, one closure
