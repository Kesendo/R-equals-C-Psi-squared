...
- [Fragile Bridge](FRAGILE_BRIDGE.md) - Coupled gain-loss systems have a finite stability window; instability is Hopf bifurcation, identified as Liouvillian chiral symmetry breaking (class AIII, [PT-Symmetry Analysis](../experiments/PT_SYMMETRY_ANALYSIS.md)); N-dependence non-monotonic (even N more stable). Neural sigmoid provides biological safety mechanism. Tier 2 (three independent scripts, cross-level test).
- [Resonant Return](RESONANT_RETURN.md) - Palindrome-derived spatial γ profiles outperform hand-designed profiles by 139-360x; closed-form sacrifice-zone formula beats numerical optimization; temporal modulation falsified. Tests 1-8 Tier 2; c- backward mode question Tier 5.
...
- [Heisenberg Reloaded](HEISENBERG_RELOADED.md) - The Heisenberg form is forced at Level 0 by the C²⊗C² parity selection ({II, XX, YY, ZZ} is the unique both-parity-even 2-body bilinear); atomic exchange at Level 1 inherits the form through V-Effect transitions, not generates it. IBM hardware reads Level 0 directly with Level 1+ infrastructure as carrier. Sketch (Tier 4-5) assembling PRIMORDIAL_QUBIT + V-Effect + HIERARCHY_OF_INCOMPLETENESS into the inversion of the textbook narrative. §6 hardware-data review still to be expanded.
...
- [Q As the Exchange Rate](Q_AS_THE_EXCHANGE_RATE.md) - Q = J/γ₀ is not "coupling-to-noise" or just "rotation-per-tick" but a bidirectional clock-exchange rate: between the γ₀-Clock (Carrier-Decay, Maßstab from outside) and the H-Clock (Hamiltonian-Rotation, visible from inside). Names the duality implicit in the tick reading + Two Times reflection + F86 Q-bands. Balance Q=1 is the synchron of the two clocks; F86 peak at Q≈1.5 is the slight H-favor where K_CC_pr maximizes. Tier 4 (vocabulary-cleanup grounded in F95 + UniversalCarrierClaim + project_q_middle_structure).
