...
**Tier:** 2 (structural hypothesis, grounded in F64-F77, hardware verification on Kingston, and K/Π mirror-axis numerical falsification)
**Date:** 2026-04-24
...
- **The inverse.** h = (N, k, t, basis) has a partner tuple h̃ = (N, N+1-k, t, basis) with identical mirror-pair populations under F65. The partner relation is not generated by Π itself — Π fixes each ψ_k up to a sign (-1)^(k+1) and does not swap modes — but by a second Z₂ symmetry K described in the next section.
...
In short: Π is not one of many rules. Π is the axis around which the algebra's **observables** are organised. The rules listed below are expressions of this mirror structure for specific observables (eigenvector amplitudes, decay rates, aggregate correlation).

## The second mirror: K as the partnership axis

Alongside Π the uniform chain carries a second Z₂ symmetry: the **bipartite sublattice gauge** K = diag((-1)^ℓ), a diagonal unitary that flips sign on alternate sites. Where Π organises observables, K organises the state-level partnership structure of the algebra:

- **K swaps bonding modes.** ψ_{N+1-k}(ℓ) = (-1)^ℓ ψ_k(ℓ) = (K·ψ_k)(ℓ). So K, not Π, is the operation that realises the k ↔ N+1-k involution in the handshake tuple. Whereas Π·ψ_k = (-1)^(k+1) ψ_k (fixes each mode with a sign), K·ψ_k = ψ_{N+1-k} (actual mode swap).
- **K anticommutes with the chain Hamiltonian.** For any nearest-neighbor bipartite tight-binding H = Σ_{⟨ij⟩} t_{ij} |i⟩⟨j|, (-1)^{i+j} = -1 on every bond, so KHK = -H. The relation holds independent of J-uniformity (Π would require uniform J).
...
| H-structure                             | Π | K | partner identity |
|-----------------------------------------|---|---|------------------|
| uniform J, γ₀ = const                   | ✓ | ✓ | holds            |
| non-uniform J, any γ-profile            | ✗ | ✓ | holds            |
| + on-site potential V_ℓ                 | ✗ | ✗ | breaks           |
| + next-nearest-neighbor hopping         | ✗ | ✗ | breaks           |

The partner tuple h̃ = (N, N+1-k, t, basis) therefore records a broader equivalence than Π alone would give: bonding:k and bonding:(N+1-k) yield identical mirror-pair observables not only on the uniform palindromic chain, but on any bipartite tight-binding chain with site-diagonal dephasing.

**Numerical verification.** `simulations/_pi_partner_identity.py` (2026-04-24, N=9, γ = 0 literal, single-excitation manifold): uniform J gives |Δ MI(0, N-1)| ≤ 10⁻¹⁵ across all four partner pairs {1,9}, {2,8}, {3,7}, {4,6} and the self-partner k=5 at 0; random non-uniform J ∈ [0.5, 1.5] gives the same 10⁻¹⁵ level; random on-site V gives |Δ MI| up to 3.1×10⁻¹. The PTF-analog observable Σᵢ log πᵢ(t) (single-site purity log-sum, which specialises to PTF's Σ ln α_i = 0 closure law at γ = 0) tracks the same three regimes. A γ=0.1 sanity run reproduces the structural pattern; γ acts only as a time unit.

**Why two mirrors.** Π and K are distinct Z₂ symmetries that happen to coexist on the uniform-J chain. Π is the *observable* mirror (under which MM, mirror-pair MI, F1's palindromic sum rule live). K is the *partnership* mirror (under which bonding:k ↔ bonding:(N+1-k) at the state level). The handshake algebra is organised around Π as the axis of observation and K as the axis of partner-menu folding. F67's receiver menu has size N; K-folding reduces it to ⌈N/2⌉ distinct entries.
...
**Inverse.** The K-partnership gives a candidate inverse: h = (N, k, t, basis) pairs with h̃ = (N, N+1-k, t, basis). Under F65's amplitude identity bonding:k and bonding:(N+1-k) have identical mirror-pair populations p_ℓ and identical coherence moduli, so all receiver observables match. The two tuples are operationally equivalent — not in the group-theoretic sense of h · h̃ = identity, but in the sense that the handshake does not distinguish them. K-partnership, being conditional on bipartite H alone (not on Π-invariance), is the more robust of the two mirrors for defining this equivalence.
...
5. **K-partnership breakdown on a bipartite chain.** Would invalidate the receiver-menu folding argument. Checked numerically at N=9 with γ=0 literal (`simulations/_pi_partner_identity.py`): partner identity holds to machine precision for uniform and non-uniform J, breaks only with on-site potential or NNN hopping (as predicted from KHK = -H requiring bipartite structure).
...
- Not that the inverse is group-theoretically a proper inverse. The K-partnership h ↔ h̃ gives operationally identical observables (same populations, same coherence moduli, same MI) but not h · h̃ = identity in the strict algebraic sense.
