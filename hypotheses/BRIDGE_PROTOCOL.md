...
# Bridge Protocol: A Historical Synchronization Proposal

**Status:** No channel-free protocol established; J=0 closure, not a reopening.
No post-separation message, synchronization advantage, or local detector of
the chosen scalar crossing was established. No advantage over a shared
classical schedule was demonstrated for the proposed pre-encoded protocol.
A physical J coupling can change local statistics through an ordinary
interaction; it is not a channel-free bridge.

At J=0 a trace-preserving operation on B, with its outcome not supplied to A,
leaves A's reduced state unchanged. A conditional post-measurement branch is
not an outcome available to the remote party. In the retained Bell+ example,
||Δρ_A||=0 while the joint proxy changes from 0.500 to 0.250. A projective
measurement and a chosen scalar quarter equality are different objects.
This no-signalling statement does not equate every entangled correlation
with shared classical randomness.

For the taxonomy choose a scalar C(f), then the clean Hamiltonian-dead Bell+
Lindblad book or the named retired feedback law, then solve C(f)f/3=1/4.
The two books contain six finite crossings and two never bridges. K=γt
is constant in a gamma sweep only for the named fixed readout and book;
arbitrary asymmetric site rates do not inherit that statement. None of this
identifies a physical observer, a measurement event or an experienced clock.

The separate J_bridge=0.5 comparison below distinguishes differently prepared,
coupled states using 15 observables across 5 preparations. Its t=1.27 and
purity 0.56/0.81, entropy 1.19/0.60, l1 0.33/0.92 values are finite coupled
readings, not post-separation signalling. The Star working-point paragraph
likewise concerns physical coupling. See [Bridge Closure](../experiments/BRIDGE_CLOSURE.md).

<!-- CROSSING-HISTORICAL -->

> Historical record: the complete dated construction below retains its
> timing table, proposed alphabet, agent experiment and subsequent notes.
> Handshake, click, delivery, observer-invariant K, and operational-capacity
> assertions in that construction are not current results. The no-signalling
> and fixed-book scope above governs how the record is read.
