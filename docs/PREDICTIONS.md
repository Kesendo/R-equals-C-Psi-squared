...
| Subsystem pairs cross when full system cannot | Bell+xBell+ N=4: pairs (0,1) and (2,3) cross at t=0.073 despite full-system Psi=0.200 | Pairs fail to cross | [Subsystem Crossing](../experiments/SUBSYSTEM_CROSSING.md) |
...
| Moment-tower pump channel (F120, added 2026-06-11) | slope of ⟨H_p^j⟩ from I/d = (1/d)·Σ_l Δγ_l·t_j(l), parameter-free from T1 calibration; first firing rung = girth ℓ ⟹ palindrome hard at m\* = 2ℓ+1; girth-2 witness has slope⟨H⟩ = 0 exactly next to firing slope⟨H²⟩ | Basis-state-averaged preparation, free evolution under the chip's own damping, Pauli-polynomial readout of ⟨H_p^j⟩ early-window slopes | Slopes deviate from the closed form at calibrated Δγ_l, or the null rung fires | [Proof + protocol](proofs/PROOF_MOMENT_TOWER_PUMP_CHANNEL.md), [verifier](../simulations/moment_tower_pump_channel.py) |
