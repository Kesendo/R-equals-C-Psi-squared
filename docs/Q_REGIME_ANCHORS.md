...
- **J at γ₀=0.05**: J = 0.075
- **Single-excitation flow exceptional point (a separate object near this Q):** the flow of [`THE_FLOW_BETWEEN_TWO_SINGULARITIES`](../experiments/THE_FLOW_BETWEEN_TWO_SINGULARITIES.md) has its own parameter-space singularity, the (1,1)-block EP at the coherence horizon Q*(N) (`CoherenceHorizonClaim`, Tier 1 derived: Q*(3) = √2 ≈ 1.414, Q*(4) = 1.879), where the flow's slowest mode turns oscillatory. It is a different mode from the F86/idealized **Q_EP = 2.0** anchor below and from the toy 2×2 `ExceptionalPointClock` EP. `ClockHandLadderWitness`'s default operating point Q = 1.5 sits just above Q*(3) and below Q*(4), a deliberately marginal regime for that witness, where only the N=3 rung of the clock-hand ladder still holds.
- **Hardware population handover (not a spectral Q anchor):** the same document reports a finite population scan, not an eigenmode continuation. The runner's coherence-rate book gives Q_label=1.5→2.5; because γ=Γ/2 in the canonical jump-operator convention, this is Q_Lindblad=3→5. The walk's EP Q*(3) = √2 lies below that bracket (Q_label ≈ 0.71, between the two lowest samples), and the bracket is a probe-time crossover of the revival amplitude, not a spectral event; it supplies no J_EP.
