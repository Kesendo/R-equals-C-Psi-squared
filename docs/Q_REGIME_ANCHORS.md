...
- **J at γ₀=0.05**: J = 0.075
- **Hardware population handover (not a spectral Q anchor):** [`THE_FLOW_BETWEEN_TWO_SINGULARITIES`](../experiments/THE_FLOW_BETWEEN_TWO_SINGULARITIES.md) reports a finite population scan, not an eigenmode continuation. The runner's coherence-rate book gives Q_label=1.5→2.5; because γ=Γ/2 in the canonical jump-operator convention, this is Q_Lindblad=3→5. The spectral character remains open: the scan does not locate critical damping, an EP, coalescence, or Jordan rank, and it supplies no J_EP. The toy 2×2 `ExceptionalPointClock` is a separate mathematical object.
