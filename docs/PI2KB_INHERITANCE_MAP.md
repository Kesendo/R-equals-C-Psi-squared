...
F84, plus F91, F93, F108Part3-Y, F113, F112-Y), **19 CoveredByHadamardDuality** (the
...
ratio F49c, and the k-body roll-up F85, now typed: the bit_a twin is a corollary of the
duality below, no bespoke twin owed), and **33 NeedsDerivation** (open).
...
- **Collapsible by the duality** (the majority, status `CoveredByHadamardDuality`):
  ...
  shadow-crossing Frobenius cross-term ratio), plus F85 (the k-body roll-up of that
  operator-space family, ‖M‖²_F and Spec(M) graded by bit_b parity), are
