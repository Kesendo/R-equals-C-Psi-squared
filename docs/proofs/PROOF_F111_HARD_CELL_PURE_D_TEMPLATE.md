...
This is a MECHANISM plus a measured witness, not a derivation from the letter structure: it does not yet show *why* the pure-D templates are exactly the ones that lose the anti-steady mode. The hard direction is closed independently by the windowed all-γ converse (WindowedConverseAllGammaClaim, Pascal-Gram positivity F117, no residual, 2026-06-10), which is what promoted F111 to Tier1Derived; this step is the readable picture, not the load-bearing argument. Both the classification and the −2σ witness are computed by `simulations/f111_spec_palindrome_single_term.py`.
