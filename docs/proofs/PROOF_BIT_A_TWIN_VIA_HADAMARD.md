...
- **bit_b-specific (status `BitBSpecific`):** claims whose content is intrinsically tied to Z-dephasing or to non-dephasing dissipation (amplitude damping F1T1 / F82 / F84, and the Y-dephase / break-magnitude claims F91 / F93 / F108Part3 / F113 / F112-Y) have no bit_a twin to mirror. **What decides this is the CHANNEL, not the quantity being measured**, and F113 beside F155 is the sharp case: both are closed forms for the same polarity asymmetry, yet F113 is bit_b-specific because its hypothesis names σ⁺/σ⁻ jumps, while F155 assumes no channel at all (H = A + iB with both parts arbitrary Hermitian operators) and is therefore collapsible. A claim is bit_b-specific when its own hypothesis singles out the Z basis, never merely because the quantity it computes is one a bit_b-specific claim also computes. The σ⁺/σ⁻ jumps single out the Z-basis energy ladder, and the Hadamard Z↔X relabeling maps them to (Z ± iY)/2, a different physical channel outside the standard damping family, so the symmetry does not exist. (As superoperators the σ∓ dissipators preserve bit_a: the bilinear sandwich cancels the parity of the jump operator; what they lack is a physical bit_a twin.)

## Consequence for the map

The 52 slots that were NeedsDerivation split into a collapsible majority (this theorem, `CoveredByHadamardDuality`, of which the 9 Absorption-descendants plus F83, the F49 / F49b / F80 / F81 operator-space family, the F3 / F43 / F44 Lindblad-spectral family, and the shadow-crossing Frobenius ratio F49c, and the k-body roll-up F85 are now typed, leaving 33 still marked NeedsDerivation) and a small bespoke-operator residue (genuine open work). Together with the 7 already-typed Filled twins and the 8 BitBSpecific slots, the breakdown is 7 / 8 / 20 / 33, and the bit_a side of the cubic architecture is closed in principle: one duality theorem plus a short residue list, not 52 independent derivations. The twentieth collapsible slot is F155 (2026-08-19), which was not among the 52: it is a claim minted after this theorem. It lands in case 1 by the argument this section already makes, Ad_{Q_zx} being Frobenius-unitary with Q_zx·Π_Z·Q_zx⁻¹ = Π_X, and F155's own §(g) states the resulting transport as an identity, asymmetry_X(A, B) = asymmetry_Z(hAh, hBh) for h = H^⊗N with every site rotated, gated by S10. **One clarification is worth adding here, because F155 makes it visible in a way the earlier members did not.** The quantities case 1 lists (a spectrum, an eigenspace dimension, a Frobenius norm) are functions of the OPERATOR alone, so they do not care which Π is in the room. The polarity asymmetry is a function of the PAIR, operator and palindrome convention together. Both transport under the joint conjugation this theorem is about, which turns the two together; but only for a pair-valued quantity does the ONE-SIDED turn, moving the operator while Π stays, change anything, and for F155 it does (its gate G13). So a reader meeting G13 and this theorem on the same day is not meeting a contradiction: same turn on both, nothing moves; turn one only, and a pair-valued quantity may move freely. The count lives in `PolarityCubeMap.CoveredByHadamardDualityTwinSlots` and is pinned by `PolarityCubeMapRegistrationTests`.
