...
| N = 9 Heisenberg chain | C# `LiouvillianBlockSpectrum.ComputeSpectrumPerBlock` with MklDirect bridge for blocks > LP64 ceiling | tolerance 1e-5 across 262 144 eigenvalues; max pairing distance **3.48E-13**; 0 outlier pairs | well within tolerance; ~174 eps·2σ, not bit-exact |
...
2. **Pairing precision is lost on a few pairs, and the cause is not established here**. The reported max pairing distance spans 12.6 decades across the committed anchors, from 1.13E-14 to 4.52E-02, while the MEDIAN stays at machine level in every case. The four that stand out, with median and flagged-pair count beside the max: `ring_N6_python` 4.52E-02 / 2.76E-14 / 14 of 4096; `k4_plus_disjoint_4chain_N8` 2.56E-07 / 2.16E-14 / 1746 of 65536; `ring_N4_python` 4.23E-08 / 6.55E-15 / 16 of 256; `star_N6_python` 7.41E-12 / 2.88E-14 / 8 of 4096. Every other anchor reports zero flags and a max of ~1E-14 to 4E-13. Note that the flag count is a HISTOGRAM marker, distance above 100x the median, not a count of failures.

   So the loss is localized to a handful of pairs rather than spread over the spectrum. Beyond that this item does not claim a cause, and two earlier versions of it did: the first named degeneracy density, which the anchors refute (`star_N8` has the lowest density of the seventeen at 3% and is clean, `ring_N8` at 33% is clean, `ring_N6_python` at 35% is the worst); the second named the anchor scripts' scalar sort key, which cannot be the whole story either, since the two pairing routes in play are different algorithms and the C# path (greedy nearest-neighbour with removal) and the Python path (`f1_chain_heisenberg_small_n_anchor.py`, sort both multisets by `real*1e6 + imag` and subtract positionally) both appear among the four. Deciding this needs a re-pairing of the stored spectra under an optimal assignment and a conditioning check on the affected blocks, neither of which is in the repo.

   Scope note: the 1e-6 tolerance is a C# gate (`F1GeneralTopologyN8BlockSpectrumTests`). The Python anchor scripts assert nothing, so their numbers are recorded rather than gated.
...
4. **Imaginary-axis extent is exactly the Hamiltonian spread**. Star has the smallest Im range (±4.0, an integer in units of γ at γ = 0.5); ring has the largest (±5.65), reflecting circulant-spectrum spreading. This is not a topology-specific effect: under uniform Z-dephasing every isotropic Heisenberg graph has `max|Im(λ)| = ΔE_max(H)` exactly, so the ordering across topologies is the ordering of their Hamiltonian spreads. The star's value is the minimum, `ΔE_max(H_star) = J·N/2`, derived in [`PROOF_STAR_OPTICAL_CONFOCAL_SATURATION.md`](PROOF_STAR_OPTICAL_CONFOCAL_SATURATION.md) and typed as [`StarImMaxBoundClaim`](../../compute/RCPsiSquared.Core/Symmetry/StarImMaxBoundClaim.cs) Tier 1 derived (2026-05-19).

### N = 9 chain spot-check (SLOW_N9, verified 2026-05-19 via MklDirect bridge)

The N = 9 Heisenberg chain (8 bonds, J = 1, uniform γ = 0.5, σ = N·γ = 4.5) was reached on 2026-05-19 via the MklDirect bridge in [`LiouvillianBlockSpectrum.ComputeSpectrumPerBlock`](../../compute/RCPsiSquared.Core/BlockSpectrum/LiouvillianBlockSpectrum.cs) (commit `abb2d52`). Full F1 palindromic-pairing identity {λ_k} = {−2σ − λ_k} verified across all 262 144 eigenvalues to tolerance 1e-5 (observed max pairing distance ≈ 3.5E-13, max value of `MinReal` exactly −9.0, kernel dimension exactly 10 matching the F4KernelDim Tier1Derived prediction `Π_c(|c|+1) = 9 + 1`, dissipation gap 0.027 276 matching the chain `gap × N² ≈ 2.20` empirical scaling from [F1 Dissipation Gap Pattern](../../hypotheses/F1_DISSIPATION_GAP_PATTERN.md) to 0.4 %). Wall time **3 h 24 m** on the 128 GB / 24-core dev machine; effective speedup over the (infeasible) dense N=9 Liouvillian (would be 262 144² × 16 B ≈ **1.1 TB**) is **≈ 645.9×**, exceeding the LiouvillianBlockSpectrum docstring N=8 promise of 515×.
