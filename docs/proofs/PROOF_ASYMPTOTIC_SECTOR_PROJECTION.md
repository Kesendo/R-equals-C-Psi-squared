...
- **What the argument needs: a support covering every site.** With γ = 0 at some sites, Step 2 part (a) and Step 2b have no k to spend for pairs that agree wherever γ is nonzero, so the proof stops. The `scope` run samples eight supports at N = 5 from |+⟩^5 at t = 200 and 400. Seven finite-time rows either fall by more than a factor of ten or reach the grid floor; those rows are compatible with the per-popcount mixed reference at the sampled times, not proofs of asymptotic convergence. The single-centre row stays 2.604·10⁻² away in maximum entrywise difference at both times; conserved parity gives the independent exact obstruction below.
- **A sparse centre support preserves mirror parity; it need not mix within each parity block.** Let M reverse the sites. At the fixed centre seat, [Z_k,M]=0, so the mirror-parity projectors are conserved in addition to the N+1 popcount projectors. The `scope` run of [The Blind Site](../../experiments/THE_BLIND_SITE.md) starts from |+⟩^5, which is mirror-even, and at t=400 finds maximum entrywise difference 2.604·10⁻² from the per-popcount mixed reference but 3.962·10⁻¹⁰ from the per-(popcount, parity) mixed **candidate**. That candidate differs from the per-popcount reference by exactly +1/48 at the centre population, −1/192 at each other site, and 5/192 in the mirror-partner coherence. These are facts about that input and reference, not a general asymptotic formula. A counterexample lies in the N=5, popcount-1, mirror-odd block: in the basis (|0⟩−|4⟩)/√2, (|1⟩−|3⟩)/√2, the centre Z jump is I and H is [[2,2],[2,0]]. A pure first-basis state evolves unitarily and nonstationarily; its Frobenius distance from I_odd/2 remains 1/√2, so it has no maximally mixed limit. The exact block check is in [the view comparison](../../experiments/OPERATOR_PAIR_VIEW_COMPARISON.md) and its test. Which sparse supports and preparations have a limit, and what that limit is, remains a separate question.
