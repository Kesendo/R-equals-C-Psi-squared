...
- **What the argument needs but the conclusion often does not: a support covering every site.** With γ = 0 at some sites, Step 2 part (a) and Step 2b have no k to spend for pairs that agree wherever γ is nonzero, so the proof stops. The conclusion frequently survives anyway. Measured at N = 5 on the chain from |+⟩^N over eight dephasing supports, it holds for seven and fails for one, the single centre seat. Two discriminators are used and both are stated: a residual still above the grid's N·eps floor must fall by decades between t = 200 and t = 400, and three of the seven instead sit at that floor already at t = 200 and are done. The failing one does neither: it is 2.604·10⁻² at both times.
- **Where it fails, the attractor refines rather than dissolves, and the mechanism is one commutator.** M being the site reversal, Z_k commutes with M only at the mirror-fixed seat (measured: 0 there, 2 at every other seat of an N = 5 chain). So a dephasing support inside that fixed set leaves the two mirror-parity projectors conserved, which is a constant of motion beyond the N+1 sector populations, and the limit is the maximally mixed state per **(popcount, mirror-parity)** block rather than per popcount sector. Measured at N = 5, γ on seat 2 alone: distance to the per-sector limit 2.604·10⁻², distance to the per-parity-block limit 3.962·10⁻¹⁰. The deviation is exact, not approximate: the centre seat gains exactly 1/48 of population, the four others lose exactly 1/192 each, and 5/192 survives as coherence between the mirror-partner sites. The conserved mirror-odd block is exactly the blind subspace of [The Blind Site](../../experiments/THE_BLIND_SITE.md); its `scope` run carries all of this. Whether every failure of the all-sites hypothesis refines this way, under some automorphism whose fixed set contains the support, is open.
