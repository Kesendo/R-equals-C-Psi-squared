...
- Probe 10 (`polarity_probe_pair_enumeration.py`): at N=2, exhaustive 136 Pauli-pair enumeration with fixed coefficients (1, i) preserves balance for every pair, the asymmetry being exactly 0.0 in all 136. The load-bearing part of that is the 64 pairs which carry genuine polarity content (‖M_anti‖² up to 0.32); in the remaining 72 there is no content: 64 of them are exactly 0.0 and the other 8 sit at float noise, so those rows confirm nothing on their own. With random coefficients (probe 11), the structural axis emerges.
...
- Probe 12 (`polarity_probe_z2cubed_scaling.py`): pattern scales to N=3, N=4. Over N=2, 3, 4 there are 132 breaking rows and they are precisely the cross-bit_b ones; the Z₂³-cell structure reduces to the single Z₂ axis of bit_b. The bit_b-homogeneous rows are recorded as DEGEN, not BALANCED. Note the qualifier, because it is NOT this theorem's hypothesis: that probe runs a Π²-even Heisenberg H, and it is the homogeneous c **together with** the Π²-even H that empties the polarity content (Steps 3 and 4 below), so on those rows the ratio is noise over noise and reports nothing. Under this theorem's own hypothesis, an arbitrary Hermitian H, a homogeneous c gives a vanishing DIFFERENCE over a generally nonzero content, and those readings are substantive. (Probe 12 read as 27/27, 72/72, 72/72 BALANCED until 2026-08-06, when the denominator changed from ‖M‖² to the polarity content and the probe gained an exact bit_b-homogeneity test that outranks the ratio. The breaking rows are unchanged. Its BAL verdict is now unreachable, so it anchors only the converse direction; the substantive in-scope confirmations are probes 10 and 13.)
...
- Probe (F87 connection, `polarity_probe_f87_connection.py`): all three F87 trichotomy classes (truly, soft, hard) at N=3 under standard Z-deph give asymmetry = 0 bit-exact. F112 polarity balance is insensitive to F87 trichotomy; the two are orthogonal axes on the bit_b Z₂-grading. Of its seven cases, four carry genuine polarity content and three do not, and the split is by Π²-parity rather than by F87 class: the two `truly` rows and the one Π²-even `soft` row (YZ+ZY) have a Π²-even H, so against the homogeneous Z-dephasing their M_anti vanishes as a theorem and they read DEGENERATE rather than balanced (corrected 2026-08-07). Note `soft` is not a Π²-even class: XY+YX is soft and Π²-odd and is one of the substantive rows. `truly`, by contrast, cannot contribute ON THIS PATH: every truly term-set is Π²-even by construction (#Y and #Z both even ⟹ bit_b even), so against a homogeneous c its polarity content is empty. The reason is the Π²-parity, not that M vanishes — M does not vanish, it rounds (‖M‖² = 1.5e-31 at N=3), and with amplitude damping a truly H carries a genuine ‖M_anti‖² of 8.0e-2 at N=2 and 4.8e-1 at N=3. The orthogonality reading rests on the four substantive rows, which span the hard class and the mixed one.
