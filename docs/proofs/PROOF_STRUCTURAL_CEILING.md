...
So the `N = 4` anomaly is one mechanism in two topologies: the single-excitation ladder vacates the sub-floor region at `N = 4`, leaving the half-filling sector to set the gap. Verifier Stage 2 gates `K_4 (1,1) = 1.0`, the global minimum in `(2,2)`, and `= 2 − 2/√3` to `< 10⁻⁷`.
