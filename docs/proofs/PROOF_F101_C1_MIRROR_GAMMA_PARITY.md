...
F101 is the observable-side twin of F91, which on the spectrum side says the diagonal-block eigenvalues depend only on the palindromic part of γ. F91 keeps anti-palindromic γ out of the spectrum; F101 localises the c₁ bond-mirror deviation in anti-palindromic γ and proves it odd (its parity and zero set are fixed by γ_anti alone; its magnitude also carries γ_sym, see the κ_γ section). Together they account for the full split on the γ-axis the same way F92 and F100 account for it on the J-axis.
...
**Even-power coefficients, treated honestly.** The *direct* even-power evidence is the oddness residual itself: max|D(+s) + D(−s)| isolates exactly the even-power content of D in s, and it sits at the ~1e−9 floating-point floor against the O(1) signal. That is the machine-zero verdict that only odd powers survive, and it is the primary even-power statement of F101. A separate diagnostic fits D(s) with a cubic polyfit(s, D, 3); its *constant* coefficient (≤ 8.9e−10) is genuinely at the oddness scale and is consistent. Its *quadratic* coefficient, however, runs from 1.2e−7 at N=3 up to 8.9e−7 at N=5, larger than the oddness residual at every N. This larger number is **not** a real even-power signal and must not be read as one: it is fit-noise. Recovering a quadratic coefficient from a cubic fit divides the even residual through by s², a generic 1/s² ill-conditioning; over the small s ≤ 0.03 γ-sweep this lifts the ~1e−9 oddness-scale residual up to the 1e−7-to-1e−6-scale polyfit quadratic. That within-F101 gap is the conditioning of the fit itself; it is not a clean single factor and is not pinned to any specific multiplier. Separately, the F101-vs-F100 comparison: F101's γ-sweep caps at s ≤ 0.03, four times narrower than F100's J-sweep at s ≤ 0.12, so the 1/s² amplification is about sixteen times stronger in F101, which is why F101's polyfit quadratic, in the 1e−7-to-1e−6 range, runs correspondingly larger than F100's reported ~1.1e−7 (quadratic peak at N=5). The 16× factor is this F101-vs-F100 sweep-width comparison; it is **not** the within-F101 oddness-to-quadratic gap. (F100's proof reported "even-power coefficients ... below ~1.1e−7 (quadratic peak at N=5)" for its wider J-sweep; that sentence does not transfer, and is deliberately not reused here.)
...
**Same obstruction family as F100's κ_b and F86's g_eff.** κ_γ is the same kind of object as F100's leading coefficient κ_b and as F86's coupling g_eff: a non-primitive, position-dependent residue downstream of a projection or fit. F89's path-polynomial closed form D_k does not transfer to it, for the same structural reason it does not transfer to κ_b: the orbit-polynomial reduction that closes D_k exploits Bloch-orbit symmetry and discards exactly the per-bond cross matrix elements a per-bond coefficient needs.

F100 additionally closed a first-order Lindblad perturbation route for its κ_b: a dedicated diagnostic computed the first-order eigenvalue and eigenvector-mixing data directly and showed the conjectured spectral mechanism fails (the κ-relevant content sits in the full α-rescaling-fit contraction, not in any first-order spectral expression). **F101 does not independently re-run that perturbation diagnostic for κ_γ.** No such run is claimed here. What is inherited is the structural verdict, not the diagnostic: κ_γ is a fit-residue, the same family of object, and the closed-form obstruction by inheritance from c₁ (above) is self-standing without it.

**Methodological frame.** This is F101's instance of the lesson that what survives a closed-form effort is the symmetry, not the number ([`ON_THE_Q_AXIS_AND_THE_PTF_LESSON`](../../reflections/ON_THE_Q_AXIS_AND_THE_PTF_LESSON.md)): F101's parity, D odd in γ_anti, is the Tier-1-derived symmetry that survives; κ_γ is, by construction, the residue. κ_γ stays Tier 2 empirical.
...
- **F101 (observable side):** the c₁ bond-mirror deviation D vanishes for palindromic γ and is an exactly odd function of γ_anti — its parity and zero set are fixed by γ_anti alone (its leading magnitude κ_γ still depends on γ_sym; see the κ_γ section). F101 is the *deviation* statement: the bond-mirror observable's asymmetry is born of γ_anti and vanishes without it.
...
This is the γ-axis instance of the spectrum/observable twin pairing that F100 and F92 carry on the J-axis: F92 keeps J_anti out of the diagonal-block spectrum, F100 localises the c₁/Q_peak bond-mirror deviation in J_anti. F101 mirrors F100 with the parameter substitution J → γ; the perturbed parameter's host changes from the Hamiltonian H to the dissipator D, but the R-equivariance argument is parameter-host-agnostic (PROOF_C1's two load-bearing facts hold for any parameter axis that R-conjugates by site-relabelling). F91 mirrors F92 with the same J → γ substitution.
