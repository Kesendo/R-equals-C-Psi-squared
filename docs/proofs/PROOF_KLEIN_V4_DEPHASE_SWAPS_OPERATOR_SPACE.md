...
- **F1 palindrome family is fully Klein-V₄ equivariant on the Π operators themselves.** Any quantity that depends only on the Π_d spectrum / eigenspace structure transfers between dephase letters via unitary conjugation by the appropriate element of {I, D, Q_zx, Q_yx}. **Caveat on the Lindbladian L**: only the Hadamard subgroup {I, Q_zx} lifts to Hilbert-space unitaries (Q_zx is the operator-space form of U_H^⊗N) and therefore transports Lindblad-form L between dephase representations. D and Q_yx (= H) are operator-space-only and do NOT lift; D-conjugation of a Lindblad-form L_Z is not in general another Lindblad-form L_Y. The Welle 13 cross-dephase F112 closure (`docs/proofs/PROOF_F112_CROSS_DEPHASE_VIA_KLEIN_V4.md`) handles this carefully: F112-X follows from F112-Z via Q_zx-conjugation (Route 2, Hadamard transport); F112-Y requires a direct re-run of the Welle-11 structural lemmas substituting axis_Y = bit_b (Route 1, per-axis structural re-run), not D-conjugation. Both routes are Tier1Derived universal N.
...
- **F108 cross-dephasing (closed positively by Welle 14, with mechanism distinction).** F108 Parts 1, 2, 3 (Pi5bilinear under Z-, X-, Y-dephasing respectively) ARE Klein-V₄ equivalent, but via two DIFFERENT mechanisms (proven in [F108 Klein-V₄ equivalence](PROOF_F108_KLEIN_V4_EQUIVALENCE.md), Welle 14):
  ...
  - **NEGATIVE on Q_zx and H for Π_5b at operator level:** Q_zx · Π_5b(Z) · Q_zx ≠ ±Π_5b(X) and H · Π_5b(Y) · H ≠ ±Π_5b(X) (gap = 2.0 in Frobenius distance at N = 1, 2, 3). So the operator-space Klein-V₄ action on Π_5bilinear is only PARTIAL ({I, D} subgroup); the X-deph case requires the Hilbert-space Hadamard. This is the precise refinement of the "operator-space-only D, H" / "Hadamard-lift Q_zx" distinction made earlier in this proof's Caveat: the same lift distinction matters for Π_5bilinear's cross-dephase equivariance.
