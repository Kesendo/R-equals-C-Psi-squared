...
**Status:** derived chain over ℚ(i), machine-verified end to end (eight exact gate scripts, total runtime about thirteen minutes; the one non-looped link is the textbook simple-pole change-of-uniformizer rule, §4). Since 2026-07-14 (later the same day) the §3 core identity has a one-bracket closed form (`f127_closed_form.py`), found by the Cauchy-kernel lens; it replaces only §3's argument, everything else in the chain is unchanged. Later the same day the closed form lifted to the whole of 𝔉: the global representation 𝔉 = −4·𝒪[T·cot s] (§3A, `f127_global_representation.py`), a bonus identity that makes 𝔉 explicit and the single-angle oddness manifest; it restates F127, it does not re-prove it. The overall F127 grade stays Tier1Candidate per the registry: the code-trust caveat (bespoke single implementations) applies to these gates as it does to the wall, and the two proofs discharge it for each other only partially, their trust surfaces being disjoint. The grid+CRT wall (527 deterministic items, 17 primes) remains in force as the independent certificate of the same theorem; this proof supersedes it as the primary derivation and explains it. Verifier scripts are committed with this document; the C# witness recomputes the chain's §2 lattice and §3 core identity live (`inspect --root crosstriple`, extended 2026-07-14).
**Date:** 2026-07-14
...
**Gate S2** proves, exactly and per event (288 identities): on the sheet, the event residue equals −(Π_k L_k)/4 times the corresponding (i,j) term of T(L∘x), where (L∘x)_k = L_k·x_k. Since cosine is even, Ca and Cb are invariant under every sign flip L, and the sheet condition says precisely that the flipped point satisfies Σa′+Σb′ ≡ 0. **One identity therefore covers all 32 sheets.** (This 32×9 incidence is the same bijection along which the §3A global representation pairs full terms; S2 is its residue face.)

![The 32 by 9 incidence grid: each row one sheet sign pattern, each column one (i,j) block, every cell exactly one pole event, colored by its atom class](../../visualizations/f127_sheet_lattice.png)

*The lattice, drawn (generated by [`simulations/f127_viz.py`](../../simulations/f127_viz.py)): no cell empty, no cell double, and the coloring shows how the sign pattern of L selects the (ξ, υ) class, which is the flip action of the next section.*
...
**Proof (primary since 2026-07-14: the closed form).** Write s = (Σa+Σb)/2, e₁ = Σᵢ cos aᵢ, f₁ = Σⱼ cos bⱼ, P = Π_{i,j} sin((aᵢ+bⱼ)/2), V_a = Π_{i<j} sin((aᵢ−aⱼ)/2), V_b likewise in b. Then, as an unconditional identity of trigonometric polynomials (proved exactly over ℚ(i): both sides expand to the same 864 monomials in the half-angle monomial coordinates, gate G1 of `f127_closed_form.py`):

  **T·P = ⅛ · [ 2 cos s · ((e₁−f₁)² − 2 sin²s) + sin s · (Σᵢ sin 2aᵢ + Σⱼ sin 2bⱼ) ] · V_a · V_b.**

On the variety, e₁ = 0, f₁ = 0, and s ≡ 0 mod π (the half-sum of the sheet relation), so every right-hand term vanishes term by term: T·P ≡ 0 on the variety, and pointwise T = 0 wherever P ≠ 0. On the P = 0 sublocus the pole-free witness below stands unchanged, now read as the degenerate-denominator case of the same identity. That is the whole §3 argument; no numerator assembly, no resultant, no divisibility.

Two structural remarks the closed form carries with it. First, the reason it exists: **T is a bordered determinant of the classical Frobenius cotangent kernel**, T = det[[C, U],[Wᵀ, 0]] with C_ij = cot((aᵢ+bⱼ)/2), U = [1 | sin aᵢ], W = [1 | sin bⱼ] (exact, gate G2; the Laplace expansion along the border columns is exactly the (−1)^{i+j} α_i α_j sum, and the closed form is the confluent evaluation of the Frobenius product formula det[cot(xᵢ+yⱼ)] = cos(Σx+Σy)·Πsin(xᵢ−xⱼ)sin(yᵢ−yⱼ)/Πsin(xᵢ+yⱼ), the trade's property since 1882). Second, a **sharper vanishing locus, a bonus fact about T that does not widen V or change F127's scope**: on the sheet alone, T = ±(V_a V_b/4P)·(e₁−f₁)²·cos s, a signed square of the constraint difference, so T = 0 already on {e₁ = f₁} ∧ {sheet}; the equality of the two cosine sums suffices, not their vanishing. The old resultant argument fed on Ca and Cb separately and was structurally blind to this combination. The naked falsification above is also explained in closed form: on Ca ∧ Cb without the sheet, 8·T·P/(V_a V_b) = −4 cos s sin²s + sin s(Σsin 2a + Σsin 2b), generically of order one. (Superseded in strength the same evening: the e₁ = f₁ sharpening reaches 𝔉 itself, without the sheet; see [PROOF_F128_FLIP_SUM_FACTORIZATION](PROOF_F128_FLIP_SUM_FACTORIZATION.md).)

**Proof (prior route, retained as the in-family cross-check: the resultant divisibility).** In monomial coordinates the sheet relation is z₁z₂z₃w₁w₂w₃ = 1, so w₃ = 1/(z₁z₂z₃w₁w₂) eliminates monomially, with no radicals and no coupling cost. Assemble T's numerator N in product form (nine terms, each a product of small factors; 540 terms expanded, degree 9 in z₃). Reduce N mod Qz: N ≡ r₀ + r₁·z₃. The second constraint transports to P = A·z₃² + B·z₃ + C with A = m₀², B = S_w·m₀, C = 1, m₀ = z₁z₂w₁w₂ (this z₃-quadratic is the committed gate's P, unrelated to the sine product P of the primary proof above; the letter is kept because the gate scripts and the wall's component plan use it). On a common root of Qz and P, (B − A·S_z)·z₃ + (C − A) = 0, so define E := r₀(B − A·S_z) − r₁(C − A). Then:
...
This is the fragile thing, and the closed form is its final shape: what the wall computed for ninety hours and the resultant compressed to one divisibility is, on the nose, one short trigonometric bracket. The mirror sub-case's proof mechanism (one quadratic divides one numerator) appears in the retained route doubled: two constraint quadratics, one resultant, one divisibility.

![Heat map of log10 of the absolute value of T over the two free b-angles, with the sheet curve drawn in cyan lying in a machine-zero valley](../../visualizations/f127_zero_valley.png)

*The identity, seen (generated by [`simulations/f127_viz.py`](../../simulations/f127_viz.py), which re-solves points on the drawn cyan curve and gates |T| < 1e-12 there; measured 3.5e-16): the cyan sheet curve is an exact zero valley of T on the constraint surface, while the other dark lines are ordinary sign changes, which may fall anywhere.*

## §3A The global representation (bonus): 𝔉 is the totally odd part of T·cot

Found the same day as the closed form, by asking the Liouville question (what function has exactly 𝔉's poles and residues?) and then discovering the answer holds term by term, with no analysis at all:

  **𝔉(x) = −⅛ · Σ_{L ∈ {±1}⁶, L₁ = +1} (ΠL) · T(L∘x) · cot((L·x)/2), equivalently 𝔉 = −4·𝒪[T·cot(s)],**

where (L∘x)_k = L_k·x_k, s = (Σa+Σb)/2 is the half-sum, and 𝒪 projects onto the part odd in each of the six angles separately. The proof is pure distributivity plus a 288-fold elementary identity: 𝔉's literal expansion (the cot(μ/2) prefactor times the four-cotangent kernel) is a flat list of 288 terms, each a coefficient times a product of two cotangents, and the Xh cotangent's argument is always a full ±1 form L·x; tagging each term by (canonical sheet, block) hits each of the 32×9 cells exactly once, the right side flattens to the same 288 cells, and every cell pair is equal as a rational function over ℚ(i) (gate script `simulations/f127_global_representation.py`: bijection exact, 288/288 term identities, numeric pin against the committed `cross_form`).

Three readings, with their honest scope:

1. **Gate S2 is the residue face of this identity, constants included**: taking the residue at the sheet pole ℓ = L·x → 0, cot(ℓ/2) contributes a factor 2, turning the representation's ⅛ into S2's ¼. The representation is S2's off-sheet global upgrade, reached without the atom decomposition (S1 connects the two routes); S1/S2 stay committed and cited, §4 and §5 still consume them.
2. **The single-angle oddness of 𝔉 (gates S3, S5) is manifest**: anything in the image of 𝒪 is odd under every single-angle flip. The committed sign-bijection gates stay as independent pins. (The b-transposition oddness S6 is NOT free from the representation alone; it additionally needs the closed form's V_b antisymmetry.)
3. **Scope, stated precisely**: this is a REPRESENTATION theorem, not a proof of F127. On V each T(L∘x) is generically nonzero (the flipped point does not satisfy the sheet constraint; the §3 naked-T falsification is exactly this number), so on V the identity recasts F127 as one explicit 32-term flip-sum whose vanishing is the same content §§3-6 and the wall establish. No residues, atoms, or transport are needed to STATE F127 in this form; the vanishing itself is not re-proved here, and nothing about the wall changes. (The flip-sum's own proof arrived the same evening: the character-sum collapse of [PROOF_F128_FLIP_SUM_FACTORIZATION](PROOF_F128_FLIP_SUM_FACTORIZATION.md), which factors 𝔉 as −(e₁−f₁)² times an explicit odd-projection and sharpens the locus to {e₁ = f₁}; F127 is its codimension-two special case, making that chain the third proof.)

Combined with §3's closed form for T, the whole six-angle cross form is now an explicit closed expression: the totally odd shadow of the Frobenius-kernel core times the half-sum cotangent.
...
|---|---|---|
| `simulations/f127_closed_form.py` | §3 primary: the closed form T·P = RHS exact over ℚ(i) (864 monomials), the bordered Frobenius determinant, all three corollaries numeric | ~75 s |
| `simulations/f127_global_representation.py` | §3A: 𝔉 = −4·𝒪[T·cot s], term-by-term over ℚ(i), 288 cells (bijection + term identities + numeric pin) | ~1 min |
| `simulations/f127_core_identity.py` | §3 cross-check: naked-T falsification, N assembly (540 terms), Res \| E exact, branch check | ~4 min |
