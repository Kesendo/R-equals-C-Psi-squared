...
- Connected-case upper-bound closure: [The Palindrome Inside the Palindrome](../../experiments/DEGENERACY_PALINDROME.md) Result 2 (magnetization conservation: `d_real(0) = N+1`, proven for connected Heisenberg components under Z-dephasing; this is the analytic anchor that saturates the Section 1 lower bound to equality)
...
- Child claim, the opposite corner: [the span and node-lemma proof](PROOF_BLIND_SEAT_SPAN_AND_NODE_LEMMA.md) and its `SeatCutBlindnessClaim` (F157) take ONE dephased seat instead of every site, in the single-excitation sector, and answer there the question this claim's registry bullet leaves open, which is which seat carries the γ: the kernel is `1 + dim commutant(H|_W)` with W the seat's Krylov complement. That proof also closes the narrower question the F4 seat sub-bullets end on, whether a zero-free open chain can make the component mechanism undercount; it cannot.
...
- Corroborating per-weight ker breakdown: [the weight-1 degeneracy proof](PROOF_WEIGHT1_DEGENERACY.md) § Appendix 2026-05-17 (per-weight ker(w) decomposition across topologies; the w=0 row pins the kernel-projector count to N+1 for every connected graph tested at N=3..5, corroborating the boundary upper-bound used here)
- Related: [the general-topology extension](PROOF_F1_GENERAL_TOPOLOGY.md) (the (B, D2) parameterisation of the F1 residual norm under the same disconnected / weighted graph extensions used by this proof's Section 2)
...
This proof's count, seen at a glance: the 6 modes on the Re = 0 edge are exactly N + 1 = 6, the dephased kernel for the connected N = 5 chain. The same figure is the shared anchor of the [F1 palindrome](MIRROR_SYMMETRY_PROOF.md) (the mirror about −σ) and the [absorption law](PROOF_ABSORPTION_THEOREM.md), Re λ = −2γ⟨n_XY⟩_v for each full eigenmode. Integer n_XY labels the dissipator cells, not every Hamiltonian-mixed eigenmode. Exported by `inspect --root symphony --N 5 --J 0.075 --gamma 0.05 --export`, drawn by `simulations/reel_and_projector.py`.
