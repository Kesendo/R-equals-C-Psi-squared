...
Give each site of the chain its own share of the light falling on it, and you have given it its own clock. γ is a per-site rate set from outside, the chain being looked at, and this preface reads that outside watching as light: a Tier-4 reading ([Gamma Is Light](../../hypotheses/GAMMA_IS_LIGHT.md), fenced in the sibling [Absorption Theorem](PROOF_ABSORPTION_THEOREM.md) §5), which nothing in the proof below depends on. A site that takes on more light runs at a faster time, one that takes on less runs slower. There is no single clock for the whole chain, so "how the light is spread along the chain" is really "how fast time runs at each place in it", the chain's own observer, written into the dissipation.

Now reshuffle that light across mirror-partner sites while holding each pair's total fixed: you let two observers clock each individual site differently, yet keep each pair's combined clock the same. What survives is the striking part, and the fence around it is the whole story. Block by block, nothing moves at all: the F71-refined diagonal blocks come out as the same matrices, cell for cell, so their rates and their frequencies alike are untouched. Block by block is meant strictly: the full Liouvillian does shift, in both its real and its imaginary part, because the reshuffle breaks F71 as a symmetry of L, which is the first entry in the closing "What this is NOT". What the reshuffle keeps exactly is the diagonal-block spectrum, and the trace, which depends on the total watching alone.

That split has a name here. The block rates are the chain's memory, the inheritance it carries forward, and this reshuffle is the parameter-side shadow of the quarter-turn of the mirror that "projects everything onto itself so that it does not forget" (on the γ-vector itself R₉₀ is order 2, not a quarter-turn; the Pi2-structure section below carries the Klein V₄): it holds that memory still and turns the now. So observers who clock the chain differently, site by site, still agree exactly on the diagonal-block spectrum. This proof is the parameter-side statement of that promise, showing that the surviving block of the spectrum depends on the pair-totals alone, so any rate-preserving reshuffle of the local clocks leaves the memory untouched.
...
This is the γ-parameter face of the Pi2-foundation memory mirror, whose genuine order-4 quarter-turn (i⁴ = 1) lives on the operator side (`NinetyDegreeMirrorMemoryClaim`). On the γ-vector itself the structure is Klein V₄: the palindromic mirror γ_l ↦ γ_{N−1−l} and the anti-palindromic involution R₉₀ : γ_l ↦ 2γ_avg − γ_{N−1−l} are two commuting order-2 maps. R₉₀ preserves each pair-difference and reflects each pair-sum about 2γ_avg (S_l ↦ 4γ_avg − S_l), so the anti-palindromic orbit S_l = 2γ_avg is its fixed-point set, on which the diagonal-block spectrum is constant. Anti-palindromy is a **distinct** condition from F71-palindromy, not a weaker one: the two sets of γ meet only at uniform γ, since a palindromic γ has S_l = 2γ_l, which equals 2γ_avg for every l only when γ is flat. This is the closing "What this is NOT" stated up front. The diagonal-block spectrum is the conserved inheritance.

## Statement

For the chain XY + Z-dephasing Liouvillian L on N qubits (per-site γ_l, Hamiltonian H = Σ_b (J/2)·(X_b X_{b+1} + Y_b Y_{b+1}), the convention of the probe `PauliHamiltonian.XYChain(N, J)` that every number below was computed under; Lindbladian Σ_l γ_l·(Z_l ρ Z_l − ρ)), the **eigenvalue multiset of the F71-refined diagonal-block decomposition** is invariant under any γ-distribution satisfying

    γ_l + γ_{N−1−l} = 2·γ_avg = (2/N)·Σ_l γ_l   for all l ∈ {0..N−1}

i.e. **γ is F71-anti-palindromic around its mean**. The full operator L itself generally changes (F71 is broken as L-symmetry, off-block Frobenius in the F71-refined basis is nonzero, proportional to the F71-asymmetry of γ), but the diagonal-block eigenvalues coincide. The breaking reaches the full-L spectrum as well, through the F71-cross-blocks between F71-even and F71-odd sub-sectors: both the real and the imaginary parts of spec(L) move, while the trace does not: it is −Σγ·4^N, which depends on the total watching alone, and R₉₀ preserves that total. So the conserved object is the diagonal-block spectrum, not every rate of L; this is the first entry of "What this is NOT" below.

This is a distinct condition from F71-as-symmetry (which requires γ_l = γ_{N−1−l}, palindromic), not a weaker one: the two overlap only at uniform γ, as the closing "What this is NOT" states. F1 is a different axis again and imposes no condition on γ at all. For odd N the middle site l = (N−1)/2 must equal γ_avg.
...
| F71 (palindromic mirror) | γ_l ↦ γ_{N−1−l} | 2 | conjugated, L ↦ (P⊗P)·L·(P⊗P)⁻¹ | unchanged, and so is the full spectrum (every indexed S_l is preserved) |
| R₉₀ (anti-palindromic reshuffle) | γ_l ↦ 2γ_avg − γ_{N−1−l} | 2 | F71 broken as L-symmetry | generally moves (S_l ↦ 4γ_avg − S_l); unchanged exactly on the fixed set, where R₉₀ is the identity map |
| F71∘R₉₀ (mean-reflection) | γ_l ↦ 2γ_avg − γ_l | 2 | F71 broken | generally moves (the same S_l ↦ 4γ_avg − S_l); its own fixed set is uniform γ alone |

R₉₀ is an **involution** (R₉₀² = e, NOT F71; verified in Step 7): it **preserves each F71-pair difference and reflects each pair-sum about 2γ_avg** (S_l ↦ 4γ_avg − S_l). Its fixed-point set is exactly the anti-palindromic orbit S_l = 2γ_avg, and on that set it acts as the identity, since a map does nothing on the points it fixes. So R₉₀ is not what carries the invariance, and the last two rows of the table are not a claim about spectra staying put under a group element: what makes the many anti-palindromic profiles agree is within-pair redistribution at fixed indexed S_l (Refined statement 1 below), and R₉₀'s work is to **name** that level set as its fixed-point set. Diagonal-block matrix elements in the F71-refined basis are functions of the pair-sums only (by construction of the F71-even/odd basis); cross-block matrix elements depend on pair-differences. The cross-blocks are off-diagonal in the F71-refined basis, so they do not enter the diagonal-block eigenvalues at all (the blocks are diagonalised separately); the full-L eigenvalues, which mix both, do move. R₉₀ is the order-2 parameter-side **shadow** of the operator-side 90° turn, not itself a quarter-turn.
...
- Hamiltonian: H = Σ_b (J/2) · (X_b X_{b+1} + Y_b Y_{b+1}), F71-symmetric (the chain bond set is invariant under site-mirror b ↔ N−1−b). Steps 3 to 7 use only that H is γ-independent and mirror-symmetric, so the J-normalisation is free there; it is fixed here because the sharpness table's numbers are quoted at J = 1 in this convention.
...
By Step 6 (the diagonal blocks depend only on the indexed pair-sums l ↦ S_l, which on this orbit is the constant assignment S_l = 2γ_avg), all γ-distributions in the F71-anti-palindromic orbit at fixed γ_avg yield identical F71-refined diagonal-block spectra, equal to the spectrum at uniform γ = γ_avg (a member of the orbit since uniform γ has S_l = 2γ_avg for all l trivially).
