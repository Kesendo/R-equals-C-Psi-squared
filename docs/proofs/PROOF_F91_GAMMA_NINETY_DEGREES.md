...
i.e. **γ is F71-anti-palindromic around its mean**. The full operator L itself generally changes (F71 is broken as L-symmetry, off-block Frobenius in the F71-refined basis is nonzero, proportional to the F71-asymmetry of γ), but the diagonal-block eigenvalues coincide. The breaking reaches the full-L spectrum as well, through the F71-cross-blocks between F71-even and F71-odd sub-sectors: both the real and the imaginary parts of spec(L) move, while the trace does not: it is −Σγ·4^N, which depends on that total alone, and R₉₀ preserves it. So the conserved object is the diagonal-block spectrum, not every rate of L; this is the first entry of "What this is NOT" below.
