...
Together: the diagonal entries of the open identity are real because they are squared norms, and the off-diagonal entries vanish because the inner products themselves vanish. The imaginary part is therefore zero across the entire Hermitian operator space, and F112's typed scope widens from "Hermitian H" to "any H" with no change to the bit_b-homogeneous hypothesis on the collapse operators. Scope (corrected 2026-06-20, see `docs/CAUGHT_ERRORS.md`): this widening is about the **commutator** superoperator −i[H,·] for an arbitrary matrix H, a structural ‖·‖² identity. It is NOT the physical generator of PT-symmetric / gain-loss / post-selection dynamics, which is −i(Hρ − ρH†) with H†; for H = A + iB that generator is −i[A,ρ] + {B,ρ} (anticommutator in the anti-Hermitian part), a different operator, and the balance can fail for it, content-dependently. What decides it is not this proof's finding and not new: it is [F113](PROOF_F113_COEFFICIENT_DERIVATION.md) (Tier 1 derived 2026-05-26), the commutator-drain cross-term in closed form, asymmetry = (4^N/2)·Σ_l ω_l·(γ_pump,l − γ_T1,l) for H ⊃ Σ_l (ω_l/2)·Z_l, over F113's own dissipator family (each c_k a per-site σ⁻ or σ⁺; nothing here is claimed for a jump operator outside it). That family is precisely OUTSIDE F112's bit_b-homogeneous hypothesis, which is why there is a nonzero asymmetry to have at all: σ⁻ = (X + iY)/2 is bit_b-mixed. **Naming PT and gain-loss systems in the balanced list was this note's own error, repaired 2026-08-19, and the repair is not the opposite label but the absence of one:** the verdict is the value of that sum, so the PT or gain-loss label is not a membership criterion at all. (Some classes ARE balanced as such on this generator, bond-only Heisenberg/XY chains among them, precisely because the label they carry is a statement about that sum. On the FULL Lindbladian a random per-site jump operator does break them, while X-, Y- or Z-dephasing does not, which is F112 itself.) The law is same-site bilinear, each site contributing its own product and cross-site pairs contributing nothing (F113's per-site decomposition, `experiments/F113_BREAK_MAGNITUDE_FORMULA.md`), so the sum is zero exactly when every term is zero or the terms cancel, which is a tautology and therefore complete. The term-zero half is worth spelling out because the same algebra covers physically unlike situations: H carries no single-site Z at all (a bond-only Heisenberg/XY Hamiltonian on chain, ring or star, every ω_l = 0); or the damping is detailed-balanced per site, γ_pump,l = γ_T1,l, which kills it even with a full Z-carrier present; or the carrier and the imbalanced damping simply sit on different sites, the first two being special cases of that last one. The cancelling half is a separate matter: a uniform field with gain at one end and equal loss at the other is balanced by +ω·γ meeting −ω·γ. Instances at γ = 0.1 on an open XY chain at J = 1, closed form and direct build agreeing to a few ulp, on the no-jump generator −i(H_eff ρ − ρ H_eff†), H_eff = H − (i/2)Σ_k c_k†c_k, and on the full Lindbladian alike: gain on site 0, loss on site N−1, plus a 0.4·Z₀ detuning on the Hermitian part (ω₀ = 0.8) gives asymmetry 0.64 at N = 2 and 2.56 at N = 3, a gain/loss configuration that BREAKS, and the whole value sits on the gain at the detuned site while the far, undetuned loss site contributes nothing; the same gain/loss profile with that field on every site instead gives 0, the two ends carrying +ω·γ and −ω·γ, a gain/loss configuration that is BALANCED. (Writing the same system as a non-Hermitian H = A + i(γ/2)(n₀ − n_{N−1}) is not a second measurement: that generator differs from the no-jump one by γ·Id, which is Π-invariant and cannot move the asymmetry.) (checker `simulations/f112_gain_loss_carrier_check.py`). Reading F113 on the no-jump generator is measured rather than derived: F113's theorem is stated for Hermitian H, and the transfer rests on the recycling term c ⊗ c* being asymmetry-inert, checked at N = 2, 3, 4 over random Hermitian and non-Hermitian H within that σ⁻/σ⁺ family. The sign is convention-attached (F113 pins the pairing); the magnitude, the zero-versus-nonzero verdict, and the relative sign between pump and loss are not. What this note earlier quoted as the boundary, "≈ 132/270", was a mean over a random-H ensemble that was never specified; a mean is not a boundary, and a random H generically carries Z_l content, so what that number registered was the carrier being present rather than how far the balance breaks. Still open, and F113 does not reach it: an anti-Hermitian part B outside the per-site n = (I−Z)/2 family. There the physical generator still has a bilinear A-B cross-term (measured at N = 3 for random Hermitian A and B: the anticommutator part alone gives exactly 0 and the commutator part alone gives 0 to the noise floor, together tens of units, scaling linearly in each), but what selects A is no longer F113's single-site Z. Sweeping every Pauli string as A against random Hermitian B at N = 2 and 3: every bit_b-EVEN string is balanced, exactly 0.0 in most draws and at the float noise floor otherwise, and every bit_b-odd one breaks, XY and XZ bonds included. The sorting runs one way only, which is worth stating because it is easy to over-read: even implies balanced against every B tried, while odd implies broken only for a generic B (a B proportional to the identity leaves odd A balanced too). So F113 has the wrong carrier set there rather than merely the wrong magnitude, and what the general case sorts by is bit_b parity, this proof's own axis. Gate G10 of `simulations/f112_gain_loss_carrier_check.py`. Inside the per-site family the question does not arise, since the generator is then the no-jump generator up to a scalar shift. Direction 1 refuted 2026-06-20: `hypotheses/THE_DRAIN_HAS_NO_CHIRALITY.md`. "non-Hermitian H" here always means a non-Hermitian matrix placed in the commutator, nothing more.
