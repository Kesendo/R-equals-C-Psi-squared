...
**Status:** Tier 1 (the representation and its momentum-Laplace closed form are exact; verified against direct RK4 to 1.6·10⁻⁶ and independently re-derived by two adversarial referee rounds plus an independent Lindblad ODE reproducing the tables to the digit). The representation itself is topology-blind (the derivation splits the generator at the diagonal refill and never uses the chain; only the closed form (☆) is the infinite chain's) and is verified beyond the chain: on the cracked ring with an off-diagonal observable, the circulation of [THE_CRACKED_BELL](../../experiments/THE_CRACKED_BELL.md), the ladder meets the exact superoperator to 2e-8 with O(dt²) convergence (`simulations/ring_renewal_current.py`, its renewal section: the rebirth kernels vanish exactly on the perfect ring, and the warble's dressed clock is carried entirely by the reborn generations). The asymptotic readings built on it (the survival ceiling, the prefactor, the closed refill constant) are listed as corollaries with their own, weaker labels; they cap at Tier 1 candidate.
**Date:** 2026-07-13
