...
**Builds on:**
- [the Mirror Symmetry Proof](MIRROR_SYMMETRY_PROOF.md): the April palindrome proof and the canonical per-site Π (I → X, X → I, Y → iZ, Z → iY) that this note factors.
- [the F87 windowed converse](PROOF_F87_WINDOWED_MONOMIAL_CONVERSE.md) §2: the windowed-converse spine's two involutions 𝓕 = F ⊗ F and R = I ⊗ F and their sign table.
- The F114 entry ("F114. Commutator Superoperator D-Conjugation Parity") in [the formula registry](../ANALYTICAL_FORMULAS.md): the sign law D·L_σ·D = ε(σ)·L_σ, ε(σ) = (−1)^{n_Y(σ)+1}.
- [the Z↔Y dephase-swap reflection](../../reflections/D_PI_Z_EQUALS_PI_Y.md) and [the Z↔Y dephase-swap proof](PROOF_D_PI_Z_EQUALS_PI_Y_UNIVERSAL_N.md): Welle 12's identity D·Π_Z·D = Π_Y, re-read here as the dihedral inversion relation.
- [F85 k-body generalization](PROOF_F85_KBODY_GENERALIZATION.md): the truly criterion (n_Y even AND n_Z even) that turns out to be a D₄ character cell.
- [the golden router proof](PROOF_CEILING_GOLDEN_ROUTER.md) (F116) and [the Klein-V₄ dephase-swap proof](PROOF_KLEIN_V4_DEPHASE_SWAPS_OPERATOR_SPACE.md): the mirrors that are deliberately NOT in the group (§5).

## Abstract

Two months apart, this repository built two mirror toolkits that never spoke to each other. April 2026 built the canonical palindromizer Π_Z, a per-site signed permutation of Pauli letters, and proved Π_Z·L·Π_Z⁻¹ = −L − 2Σγ ([the Mirror Symmetry Proof](MIRROR_SYMMETRY_PROOF.md)). June 2026 built the windowed-converse spine from two plain involutions, the charge conjugation 𝓕 = F ⊗ F and the ket reflection R = I ⊗ F with F = X^⊗N ([the F87 windowed converse §2](PROOF_F87_WINDOWED_MONOMIAL_CONVERSE.md)). In between, Welle 12 found a third object, the diagonal sign D = diag((−1)^{n_Y}) on the Pauli basis, and noticed it swaps dephase letters: D·Π_Z·D = Π_Y ([the Z↔Y dephase-swap reflection](../../reflections/D_PI_Z_EQUALS_PI_Y.md)).
...
Now the general statement. Work in the row-stacking (C-order) vec convention, |i⟩⟨j| ↦ e_i ⊗ e_j, where kron(A, B) acts as ρ ↦ A·ρ·Bᵀ; this matches `framework.lindblad` and [the F87 windowed converse §1](PROOF_F87_WINDOWED_MONOMIAL_CONVERSE.md). With F = X^⊗N (so F² = I and Fᵀ = F), define
...
That is exactly the canonical rule of [the Mirror Symmetry Proof](MIRROR_SYMMETRY_PROOF.md), including the essential factors i that the April search had to discover by trial: here they fall out of Y's antisymmetry (Yᵀ = −Y) meeting the Pauli product YX = −iZ. The hard-won phase is the meeting point of the two generators.
...
- the spine's Klein four-group **{I, 𝓕, R, 𝓕R}**: the two-sided and one-sided F multiplications, exactly the involution set [the F87 windowed converse §2](PROOF_F87_WINDOWED_MONOMIAL_CONVERSE.md) runs its sign table on;
...
**(a) Π_Y = Π_Z⁻¹, and Welle 12 is the dihedral inversion relation.** In any dihedral group, conjugating the rotation by a reflection inverts it: s·r·s = r⁻¹. With r = Π_Z and s = D this reads D·Π_Z·D = Π_Z⁻¹, and the verifier confirms Π_Z⁻¹ = Π_Y at dev 0.00e+00. Welle 12's identity D·Π_Z·D = Π_Y ([the Z↔Y dephase-swap reflection](../../reflections/D_PI_Z_EQUALS_PI_Y.md), universal-N proof in [the Z↔Y dephase-swap proof](PROOF_D_PI_Z_EQUALS_PI_Y_UNIVERSAL_N.md)) was the dihedral inversion relation in disguise: the Z↔Y dephase-letter swap and "running the palindromizer backwards" are the same operation. Equivalently, the two palindromizers are the two orderings of the same factors:

  Π_Z = R ∘ D (transpose, then reflect: ρᵀ·F),  Π_Y = D ∘ R (reflect, then transpose: F·ρᵀ).

**(b) 𝓕 = Π_Z² = F1².** The charge conjugation X^⊗N, registered as F1² in [the formula registry](../ANALYTICAL_FORMULAS.md), is the square of the palindromizer and the center of D₄ (§2). That it commutes with everything in the group is no longer an observation but a group-theoretic necessity.

**(c) F114 is D's row of the table.** The sign law D·L_σ·D = ε(σ)·L_σ with ε(σ) = (−1)^{n_Y(σ)+1} becomes transparent once D is recognized as the transpose superoperator: one sign from transposition reversing the commutator, one sign (−1)^{n_Y} from σᵀ = (−1)^{n_Y}·σ. The general-N statement and its derivation are owned by the F114 entry ("F114. Commutator Superoperator D-Conjugation Parity") in [the formula registry](../ANALYTICAL_FORMULAS.md); the verifier here adds an N = 5 spot check (σ = XIYIY, n_Y = 2, dev 0.00e+00), one size beyond F114's originally verified N ≤ 4.

**(d) The truly criterion is a D₄ character cell.** The diagonal Klein four-group {I, 𝓕, D, 𝓕D} is simultaneously diagonalized by the Pauli strings, and its joint eigenvalues are the pair of parities ((−1)^{n_Y}, (−1)^{n_Z}). The four eigenvalue patterns are the four Klein cells of a Pauli string, and the F85/F87 truly criterion (n_Y even AND n_Z even, [F85 k-body generalization](PROOF_F85_KBODY_GENERALIZATION.md)) is exactly the **joint-fixed cell**: σ is truly iff both diagonal mirrors fix it. At the commutator level the same condition reads through the factorization: Π_Z·L_σ·Π_Z⁻¹ equals −L_σ iff D flips L_σ (n_Y even, F114's ε = −1) and R fixes it (F·σ·F = +σ), which together force n_Z even. The verifier checks this equivalence against the framework classifier `_pauli_tuple_is_truly` for all 63 non-identity strings at N = 3: 63/63.
...
- **K₁ = Π_{l odd} Z_l** (the sublattice chirality, [ChiralKClaim](../../compute/RCPsiSquared.Core/Symmetry/ChiralKClaim.cs), [the chiral mirror rate law](PROOF_PTF_CHIRAL_MIRROR_RATE_LAW.md)): a different grading altogether. K₁ grades by site (the even/odd sublattice of the chain), while D₄ grades and moves Pauli letters uniformly at every site. K₁'s mirror action lives on the Hamiltonian (K₁HK₁ = −H for bipartite hopping), not on the letter algebra.
- **The golden router W** (F116, [the golden router proof](PROOF_CEILING_GOLDEN_ROUTER.md)): two-sided, W(ρ) ∝ P·ρ·Q with P ≠ Q golden-frame product unitaries, and non-involutive per site (q² = −(2+φ)·I). Every D₄ element is built from the transpose and one-sided or two-sided F multiplications; W is structurally a different animal, and it has to be, because it covers the n_Z-odd ceiling scope that §4f proves D₄ cannot.
- **The crossover mirror M** ([the crossover mirror proof](PROOF_CROSSOVER_MIRROR_SQRT_NINETY.md)): the square root of the 90° mirror, a continuous R_z(π/4) conjugation object (the T-gate to D₄'s Clifford-flavored S-gate). It lives on the rotation continuum that D₄ samples only at right angles.
- **F71's bond mirror** (the closure-breaking-coefficient mirror, F71 in [the formula registry](../ANALYTICAL_FORMULAS.md)): a spatial reflection of the chain (site k ↔ N+1−k), not an operator-space letter map.
- **The dephase-letter swaps Q_zx and Q_yx** ([the Klein-V₄ dephase-swap proof](PROOF_KLEIN_V4_DEPHASE_SWAPS_OPERATOR_SPACE.md)): these are the genuinely interesting boundary. The letter group S₃ permutes the three dephase letters {X, Y, Z}, and one of its transpositions is already inside D₄: the Z↔Y swap is D itself (that is Welle 12, §4a). The other two swaps move bit_a against bit_b and require the X↔Z basis permutation; they are outside ⟨R, D⟩. Adjoining them would assemble the full mirror group of the palindrome family, with expected shape **S₃-letter-action ⋉ D₄**. That completion is named here as open: this note types the D₄ core and stops at its boundary. (Seen again 2026-06-11: the completion's *reflections* now have their algebra. The transpose D extends with the antilinear unit 𝒦 to the antilinear double ⟨R, D, 𝒦⟩ ≅ D₄ × Z₂, order 16, and the two dial-inverting vertices θ and conj supply the O(2) reflection of every thickened circle while † commutes with all rotations; see [the antilinear triangle](PROOF_ANTILINEAR_TRIANGLE.md) §4. The S₃ side stays open.) Its dissipator-diagonal companion, the three dephasing diagonals {Q_X, Q_Y, Q_Z} read as one basis-S₃ orbit, is written up in [the three diagonals](../THE_THREE_DIAGONALS.md).

**Resolution of the S₃ side (2026-06-15)** ([`simulations/linear_s3_mirror_closure.py`](../../simulations/linear_s3_mirror_closure.py), self-validating at N = 2, 3, gate-first; arc `linear_s3_mirror_completion`). The linear S₃ exists as superoperators, but the order-48 expectation does not. The genuine letter-S₃ comes from the **involutive** letter-transposition Cliffords (Hadamard for X↔Z, and (Y + Z)/√2 for Y↔Z; each order 2, their product order 3): ⟨h_zx, t_yz⟩ has order 6. (A rotation such as R_x(π/2) is order 4 and instead generates the order-24 single-qubit Clifford group, so it is not the mirror-group S₃, a gauge trap worth flagging.) But this S₃ does **not** normalize the D₄: R = I ⊗ F is a one-sided multiplication, and the letter-S₃ spreads it, h_zx·R·h_zx⁻¹ = the one-sided multiplication by Z^⊗N (verified dev 0.00e+00, outside ⟨R, D⟩). So the coherence-space closure ⟨R, D, h_zx, t_yz⟩ has order 96·2^N (384, 768 at N = 2, 3), **not** a finite order-48 S₃ ⋉ D₄. This is the coherence-space complement to the addendum below: per-site ⟨r, d, h⟩ = 128, coherence-space 96·2^N, and continuous O(2) are three distinct not-48 realizations, confirming that the abstract S₃ ⋉ D₄ has no faithful finite realization here. There were never two S₃'s: the one letter-S₃ acts faithfully on the sign-blind diagonals {Q_X, Q_Y, Q_Z} (orbit 3, [the three diagonals](../THE_THREE_DIAGONALS.md)) and carries Clifford signs on the palindromizers (a larger orbit). The "named here as open" above is hereby closed; the addendum's O(2) verdict stands and is now quantified on coherence space.

**Addendum (2026-06-10, same day): the crossover mirror, placed exactly.** The crossover bullet above says M lives on a rotation continuum that D₄ samples only at right angles; three verified identities (block H of the verifier) make the placement exact. First, the right angle itself factors. Per site, in its canonical gauge (X → Y, Y → −X, I and Z fixed), the NinetyDegreeMirror is

  N90 = diag((−1)^{n_X})·t_xy = t_xy·diag((−1)^{n_Y}),

where t_xy is the pure X↔Y letter transposition and both decorations are cube characters of §7; reversing either order gives N90⁻¹, the −90° gauge, off at O(1). Careful with the name: t_xy is NOT the Klein-V₄ element q_yx of the dephase-swap proof. That q_yx realizes the Y↔X dephase swap as the X↔Z basis-index permutation through the bit_a/bit_b duality, and it is rejected as a factor of N90 at O(1); t_xy is the naive letter action, the one realization the dephase-swap S₃ never uses. The verifier closes the per-site discrete completion ⟨r, d, h⟩ (the D₄ generators plus the X↔Z basis move that carries Q_zx and Q_yx) to a monomial group of order 128 whose letter skeletons never include the bare X↔Y swap, and N90 is not in it under any global phase. So the 90° mirror is the signed avatar of the missing S₃ transposition: same letter coset, decorated by a cube sign. Second, D inverts the whole dial, not just the discrete rotation: D·Ad_{R_z(θ)}·D = Ad_{R_z(−θ)} for every θ (transposition complex-conjugates R_z, flipping Y), extending the dihedral inversion D·Π_Z·D = Π_Z⁻¹ to the continuum. Locally at the Z letter, the shape of the mirror-group completion is O(2)-flavored, a full circle of rotations with D as its reflection, not finite. Third, Ad_{R_z(π/4)}² = N90, the [the crossover mirror proof](PROOF_CROSSOVER_MIRROR_SQRT_NINETY.md) identity re-confirmed; the crossover transport sits at the half-turn point of that circle, a cos/sin mix with no permutation skeleton at all. The consequence, stated carefully: the crossover mirror M = Π·√(N90) does NOT live in the discrete S₃ ⋉ D₄ completion (the bullet above placed it outside, and that placement stands); M is the witness that the completion must thicken each letter-stabilizing transposition into the circle of rotations about its fixed letter. And the navigator's crossover axis (`inspect --root between`, the Ad_{R_z(θ)} dial, walking since 2026-06-02) is that circle traversed end to end, the continuum's first walker: the in-between of the mirror group itself.

## §6 Verification

[`simulations/mirror_inventory_d4.py`](../../simulations/mirror_inventory_d4.py), self-validating (every block asserts; the run ends with ALL BLOCKS PASS only if every identity holds). Eight blocks:
...
- **G.** The cube of characters (§7): (bit_a, bit_b, y_par) are the characters of (Ad_{Z^⊗N}, Ad_{X^⊗N}, θ), with θ∘Ad_{Z^⊗N} = (−1)^{n_X} and θ∘Ad_{X^⊗N} = (−1)^{n_Z}; all 64 strings at N = 3, exact.
- **H.** The crossover dial (§5 addendum): N90 = diag((−1)^{n_X})·t_xy = t_xy·diag((−1)^{n_Y}) per site (dev 2.2·10⁻¹⁶), the reversed orders equal N90⁻¹ (gap to N90 = 2.0), the Klein q_yx rejected at O(1) (skeleton gap 1.0); N90 outside the order-128 discrete completion ⟨r, d, h⟩ under any global phase; D·Ad_{R_z(θ)}·D = Ad_{R_z(−θ)} at five θ per site (worst dev 8.5·10⁻¹⁷) and at N = 2 on coherence space (dev 1.4·10⁻¹⁷); Ad_{R_z(π/4)}² = N90 (dev 2.2·10⁻¹⁶, the π/4 point non-monomial at entry 1/√2).
