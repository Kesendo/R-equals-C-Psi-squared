...
**Real-axis defective seeds are certified at N=5,7,9.** The exact all-odd-N theorem is instead the endpoint-nullity surplus r(0⁺)−r(∞)=N−1. Turning that surrogate into a literal finite-q count drop and then a Puiseux-1/2 defective EP2 requires two further premises, open beyond the certified sizes; N=11 already shows third-order lift-off in the endpoint bookkeeping. F86a's c=2 (n=1) block and F89's (1,2) block are the same object (GLOSSARY §q-and-Q: ‖L_F86(J)−L_F89(J/2)‖=0).
...
**Still open**, typed as `LocalGlobalEpLink`: whether a genuinely off-real-axis complex-Q defective EP exists, and beyond N=5,7,9 whether the all-odd-N endpoint surplus produces literal finite-q drops with Puiseux-1/2 defective EP2 character.
...
**Seed source:** [F89 seed existence](../../experiments/F89_SEED_EXISTENCE_REDUCTION.md) + [the codimension-1 additivity proof](PROOF_CODIM1_BY_ADDITIVITY.md): all odd N carry the nullity-surplus count; finite-q Puiseux-1/2 defective EP2 seeds are certified at N=5,7,9, with literal drop and genericity open beyond them.
