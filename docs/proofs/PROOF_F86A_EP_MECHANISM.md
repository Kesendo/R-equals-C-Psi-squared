...
**Untouched by all of this:** `t_peak = 1/(4γ₀)`; `Q_EP = 2/g_eff` as a **definition** (g_eff := σ₀); the **toy 2×2** reduction's genuine EP (the Abstract's "exceptional point where the two eigenvalues coalesce" refers to this heuristic toy, not the full block); the **separate Σγ = 0 gain-loss spectral-abscissa axis departure** (`FRAGILE_BRIDGE`, K = 403 at one finite offset), whose EP character remains OPEN; the centered-L_c **Π P-type anticommutation**. A full SRP class is not assigned.
...
This 2-level form is "PT-phenomenology-like" (EP at finite coupling, spectral flow); the centered Liouvillian L_c has the exact Π P-type anticommutation `{Π, L_c}=0`, distinct from Bender-Boettcher PT. After Π²-sector resolution Π phase-normalizes to an involution, but the complete SRP class remains OPEN. The toy EP, the SEPARATE Σγ=0 spectral-abscissa axis departure in [`FRAGILE_BRIDGE`](../../hypotheses/FRAGILE_BRIDGE.md) whose EP character is OPEN, and the full block's certified N=5,7,9 real-axis seeds are distinct objects. At arbitrary odd N only the endpoint-nullity surplus is closed; whether finite-q drops persist generically, and whether an off-real-axis complex-Q defective EP exists, remain OPEN.
...
**Separate gain-loss crossing (Σγ=0):** [The Fragile Bridge](../../hypotheses/FRAGILE_BRIDGE.md) tracks a mirror-paired departure at the real parameter γ_crit; K=403 is one finite-offset non-normality reading and exact λ ↔ −λ pairing survives, but EP character remains OPEN. It is a DISTINCT gain-loss system, not a "global instance" of the full block-L.
