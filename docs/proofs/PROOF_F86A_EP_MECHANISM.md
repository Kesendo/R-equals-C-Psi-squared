...
**Status:** Tier 1 derived. Q_EP = 2/g_eff (definitional, g_eff := σ_0), t_peak = 1/(4γ₀), bit-exact verified against full block-L numerics. The 2-level reduction itself is **heuristic**; its EP is genuine for the toy 2×2. On the full block, finite-q Puiseux-1/2 defective EP2 seeds are certified at N=5,7,9, and at N=11 the literal drop is measured and all nine loci are classified defective EP2 without a certificate. For arbitrary odd N F89 proves the endpoint-nullity surplus only; a defective seed at every larger odd N stays open. The Petermann-K magnitudes of the original sweep were grid artifacts. The distinct off-real-axis complex-Q defective EP remains an `OpenQuestion`.
...
A subtlety the proof tracks honestly: the 2-level reduction is heuristic when projected onto a higher-dimensional block. The Q_EP value (definitional, g_eff := σ_0) and t_peak survive that projection bit-exactly against the full-block decay numerics. The full block carries certified finite-q Puiseux-1/2 defective EP2 seeds at N=5,7,9 and nine loci classified defective EP2 at N=11, in narrow windows coarse Q-grids miss. For arbitrary odd N F89 proves the nullity-surplus count only; a defective seed at every larger odd N stays open. The Petermann-K magnitudes of the original real-axis sweep ("6× above FRAGILE_BRIDGE") were grid artifacts and are dropped. The block is genuinely strongly non-normal near Q_peak; the toy 2×2 EP and the SEPARATE Σγ=0 gain-loss system's threshold, itself at two qubits per chain generically a second-order exceptional point on the real γ axis, remain distinct objects; whether a defective EP also sits off the real axis at complex Q remains open (`LocalGlobalEpLink`).
...
**Real-axis defective seeds are certified at N=5,7,9 and classified at N=11.** The exact all-odd-N theorem is instead the endpoint-nullity surplus r(0⁺)−r(∞)=N−1. Turning that surrogate into a literal finite-q count drop and then a Puiseux-1/2 defective EP2 requires two further premises. At N=5,7,9 the drop is probed exactly and the character certified. At N=11 the surrogates are not the literal counts (four of the r(0⁺)=31 modes leave the axis at third order and four of the r(∞)=21 never reach it), yet the literal drop 27 → 17 = N−1 is measured and all nine loci read defective EP2, classified and uncertified. Beyond N=11 the drop as a count is open (at every resonant N the twinning degeneracy is proved over ℚ(i); the twins leaving the axis is measured at N=11 and 17 only), and so is the character. F86a's c=2 (n=1) block and F89's (1,2) block are the same object (GLOSSARY §q-and-Q: ‖L_F86(J)−L_F89(J/2)‖=0).
...
The scan's own shadow already pointed home. Strong non-normality alone does not prove a nearby EP on the physical family L(Q), but at N=5 a certified real defective seed sits at carrier Q≈1.286, about 1.9e-3 from the robust spike location Q≈1.288. That is a demonstrated caster at this N. No all-odd-N spike-to-seed conclusion follows from the endpoint-nullity count.

**Untouched by all of this:** `t_peak = 1/(4γ₀)`; `Q_EP = 2/g_eff` as a **definition** (g_eff := σ₀); the **toy 2×2** reduction's genuine EP (the Abstract's "exceptional point where the two eigenvalues coalesce" refers to this heuristic toy, not the full block); the **separate Σγ = 0 gain-loss threshold** (`FRAGILE_BRIDGE`), at generic couplings and two qubits per chain a second-order exceptional point on the real γ axis where two imaginary eigenvalues meet in a Jordan block (K = 403 is one finite-offset reading of its K ∝ 1/δ); the centered-L_c **Π P-type anticommutation**. A full SRP class is not assigned.

**Still open**, typed as `LocalGlobalEpLink`: whether a genuinely off-real-axis complex-Q defective EP exists, and, beyond N=11, whether the all-odd-N endpoint surplus produces literal finite-q drops with Puiseux-1/2 defective EP2 character (drop probed at N=5,7,9 and measured at N=11; character certified at N=5,7,9 and classified at N=11).
...
This 2-level form is "PT-phenomenology-like" (EP at finite coupling, spectral flow); the centered Liouvillian L_c has the exact Π P-type anticommutation `{Π, L_c}=0`, distinct from Bender-Boettcher PT. After Π²-sector resolution Π phase-normalizes to an involution, but the complete SRP class remains OPEN. The toy EP, the SEPARATE Σγ=0 threshold (two qubits per chain: generically an EP2 on the real γ axis) in [`FRAGILE_BRIDGE`](../../hypotheses/FRAGILE_BRIDGE.md), and the full block's real-axis seeds (certified at N=5,7,9, classified at N=11) are distinct objects. At arbitrary odd N only the endpoint-nullity surplus is closed; whether finite-q drops persist generically, and whether an off-real-axis complex-Q defective EP exists, remain OPEN.

**Local vs global.** The shared algebraic substrate is real: the same-sign-imaginary 2×2 form and the Π P-type anticommutation of the centered L_c, read at two residuals of the F1 palindrome `Π · L · Π⁻¹ + L + 2Σγ · I = 0` (Σγ = N·γ₀ vs Σγ = 0, where the gain side cancels the loss side). This relation does not assign a global SRP class; the full irreducible-sector algebra remains OPEN. The toy 2×2, the Σγ = 0 gain-loss system, and the full Σγ = N·γ₀ block are three separate EP-carriers, not one EP seen three ways: the full block's own real-axis defective seed comes from F89 (§The real-axis EP above), NOT from the real-Q Petermann-K sweep, whose peak magnitudes (max K = 1333.6 / 337.9 / 2384.7 / 795.4 across N = 5..8, data-pinning probe `compute/RCPsiSquared.Core.Tests/F86/F86PetermannProbe.cs:Probe_PetermannFineGrid_C2_VsN`) are grid artifacts retained only as a cautionary non-normality record. Whether the full block also has an off-real-axis complex-Q defective EP is open (the nearest complex-Q coalescences characterized 2026-06-21 are DIABOLIC, ‖P‖ = 1). Encoded as `compute/RCPsiSquared.Core/F86/LocalGlobalEpLink.cs` (OpenQuestion Claim; the four PetermannSpikeWitness rows are the cautionary record, not EP evidence).
...
**Symmetry anchor:** [the PT-symmetry analysis](../../experiments/PT_SYMMETRY_ANALYSIS.md) (centered L_c has a Π P-type symmetry, NOT Bender-Boettcher PT; the complete irreducible-sector SRP class is open; its Petermann reading carries the same two-half verdict as §The real-axis EP).
**Separate gain-loss crossing (Σγ=0):** [The Fragile Bridge](../../hypotheses/FRAGILE_BRIDGE.md) has its own exceptional point at generic couplings and two qubits per chain: at the real parameter γ_crit, in the first block of its Liouvillian to go unstable, two eigenvalues on the imaginary axis meet in a 2×2 Jordan block and leave the axis as a mirror pair, while exact λ ↔ −λ pairing survives; K=403 is one finite-offset reading of its K ∝ 1/δ. It is a DISTINCT gain-loss system, not a "global instance" of the full block-L.
**Empirical anchor:** [the three-bands Q-scale experiment](../../experiments/Q_SCALE_THREE_BANDS.md) Result 2 + Revision 2026-04-24.
**Seed source:** [F89 seed existence](../../experiments/F89_SEED_EXISTENCE_REDUCTION.md) + [the codimension-1 additivity proof](PROOF_CODIM1_BY_ADDITIVITY.md): all odd N carry the nullity-surplus count; finite-q Puiseux-1/2 defective EP2 seeds are certified at N=5,7,9 and classified at N=11 (literal drop measured there), with a defective seed at every larger odd N open.
