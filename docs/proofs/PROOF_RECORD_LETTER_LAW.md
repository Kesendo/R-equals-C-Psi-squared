...
7. **The fully-witnessed worlds are the stars and the complete graphs.** Call a pair (u, v) *luminous* when it holds at least one full bit at t*. At uniform coupling every ratio is 1 (odd), so by corollary 1 and the edge cases a pair is luminous in exactly two ways: **matched neighborhoods**, N(u)∖{v} = N(v)∖{u} with at least one common site (both private sets must be empty, since a uniform private watcher is odd and kills both families; this is the Bell record), or **one member hanging as the other's leaf** (the pointer record read one way, the pendant role-swap read the other; the bare dimer N = 2 is the entangled face). Hence on a connected graph with N ≥ 3, EVERY pair is luminous iff the graph is a star K_{1,N−1} or the complete graph K_N. If some leaf x hangs on a hub h, then any third vertex y makes (x, y) a non-adjacent pair with neither member the other's leaf, so it must be matched: N(y) = N(x) = {h}, and every vertex besides h is a co-leaf: the star. If the graph is leafless, the leaf family is empty and every pair must be matched; an adjacent matched pair shares its closed neighborhood, and a non-adjacent matched pair (u, v) has N(u) = N(v) ≠ ∅, so a common neighbor c makes (u, c) an adjacent matched pair with N[u] = N[c] ∋ v, contradicting non-adjacency: every pair is adjacent, the complete graph. Conversely both extremes are fully witnessed, and differently: the star is the ONE world showing all three record readings at once, across its two pair-types (mutual information is symmetric, so the hub-leaf pair is a pointer record of the hub read from the hub's side and the role-swap read from the leaf's side; every leaf pair is the m = 1 Bell weave, Y⊗Y), while K_N is a total weave with ZERO pointer content anywhere (every pair matched with m = N − 2 dressers, the letter alternating with N by (ii): K₃ Y⊗Y, K₄ X⊗X, K₅ Y⊗Y, …). Census: over ALL connected graphs at N = 4, 5, 6 (38, 728, 26704), the fully-witnessed worlds are exactly the N labeled stars plus K_N (5, 6, 7 winners), zero exceptions. Every world between the extremes necessarily keeps blind pairs: private rooms are the generic condition, and full witnessing is the property of the two extreme geometries only.
8. **The dark lattice: girth ≥ 5 + leafless ⇒ every pair dark.** By corollary 7's dichotomy a luminous pair at uniform coupling needs matched neighborhoods or a leaf. On a leafless graph of girth ≥ 5 neither exists: an adjacent matched pair with m ≥ 1 closes a triangle, and a non-adjacent matched pair has N(u) = N(v) with |N(u)| ≥ 2 (leafless), two common neighbors closing a 4-cycle. So every pair is exactly dark at t*; corollary 3's 5- and 6-cycle darkness is the smallest face of a lattice law. The infinite heavy-hex bulk (girth 12, degrees 2 and 3, leafless) holds zero luminous pairs at uniform coupling: IBM's geometry is the anti-star, all private rooms; a finite patch records only at its dangling bridges (one Heron-style cell, 12-ring + 3 bridge qubits, N = 15: 3 of 105 pairs luminous, exactly the corner-bridge pointer records). Girth ≥ 5 is sufficient, not necessary: the square-lattice bulk (girth 4, leafless) is also entirely dark, because an interior plaquette's diagonal pair keeps private watchers in the surrounding lattice (Q ≠ ∅ kills both records; a uniform 5×5 torus reads 0 luminous pairs, gated). What weaves at girth 4 are the graphs whose diagonal pairs are genuinely matched, the ISOLATED plaquette C₄ (corollary 3's X⊗X face) and the K₂,m family; embedding the plaquette in a lattice un-matches it. And aiming adds without erasing: pushing the private-watcher bonds of a chosen corner's edge witnesses to ratio 2 (even: forgiven) gives that corner a full pointer broadcast, R_perfect = deg = 3 with Law B saturated, and NO pair goes dark. The edit does not stay at the aim, though: the corner's three witnesses then also record each other (a weave around the aim), and each edited bond's far corner picks up one rotated pointer record of its own edge qubit (10 of 105 pairs luminous in the engineered cell; the full map is gated). On a dark lattice light can be aimed, but not kept private.
...
Corollaries 7 and 8 are gated separately (added 2026-07-19):

- [`simulations/qd_witness_play.py`](../../simulations/qd_witness_play.py), output [`simulations/results/qd_witness/qd_witness_play_out.txt`](../../simulations/results/qd_witness/qd_witness_play_out.txt): the K_N weave letters (K₃…K₇ each 1 full bit, letter alternating YY/XX with the dresser count), the one-bond record multiplexer, and the full fully-witnessed census over ALL connected graphs at N = 4, 5, 6 (38 + 728 + 26704 graphs, every pair of each graph measured on the full-state path; winners exactly the N labeled stars + K_N, asserted). ~10 min, dominated by N = 6.
- [`simulations/qd_heavyhex_map.py`](../../simulations/qd_heavyhex_map.py), output [`simulations/results/qd_witness/qd_heavyhex_map_out.txt`](../../simulations/results/qd_witness/qd_heavyhex_map_out.txt): the Proposition-1 pair-page engine cross-checked against the full-state path (four graphs, deviation 0.0), the 105-pair heavy-hex-cell map (luminous set asserted = the three corner-bridge ZY pointer records), the aiming play (corner 0 broadcast R_perfect = 3; the full engineered map asserted = exactly 10 luminous pairs with nothing going dark), and the square-lattice contrast (uniform 5×5 torus asserted 0 luminous pairs, the isolated-C₄ diagonal asserted 1 full bit).
