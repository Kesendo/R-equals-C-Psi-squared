...
**(i) Aliveness is watcher parity.** The pair carries exactly 1 bit of mutual information in precisely two mutually exclusive ways:
...
- **Non-integer ratios** are the generic in-between of (i): both letter channels partially lit, |c₁| ≠ |c₂| in general, the forced/free texture.
...
All statements are gated in [`simulations/qd_letter_gates.py`](../../simulations/qd_letter_gates.py), output [`simulations/results/qd_letter/qd_letter_gates_out.txt`](../../simulations/results/qd_letter/qd_letter_gates_out.txt), **70/70 at machine zero**, every prediction fixed in the code before the run:
