...
- **deg(S) = 1** (S itself a pendant): V_S ≡ 1, the pair stays coherent; for an isolated dimer at θ_w = π/2 it is maximally entangled, I = 2 bits: quantum correlation, not a classical record. A watched partner splits by the watcher's parity (even keeps I = 2 exactly, odd collapses it to 1 separable role-swapped bit, generic in between; the full account is [F136's pendant edge case](PROOF_RECORD_LETTER_LAW.md)). Law A's hypotheses exclude this deliberately.
...
The substrate is the Absorption Theorem's per-coherence closed form ([PROOF_ABSORPTION_THEOREM](PROOF_ABSORPTION_THEOREM.md)); the fragment side obeys F70's page bound ([PROOF_DELTA_N_SELECTION_RULE](PROOF_DELTA_N_SELECTION_RULE.md)): a witness is a 1-site page and carries |Δpopcount| ≤ 1 content. Registry entry: F135 in [ANALYTICAL_FORMULAS](../ANALYTICAL_FORMULAS.md). The forced/free reading (parities forced, in-betweens generic) is the arc's standing law; the inherited rules are the angles. The letter question this proof left open, WHICH operator a witness records, is closed by the [record letter law, F136](PROOF_RECORD_LETTER_LAW.md): the letter is the parity of the shared neighborhood, and the triangle case that this proof's triangle-free hypothesis set aside splits by its far bond's parity (odd → the m = 1 Bell row, even → a pointer record), both now gated.
