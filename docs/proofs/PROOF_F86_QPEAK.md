...
|---------|-----------|------|-------|
| **F86a** EP mechanism | Q_EP = 2/g_eff, t_peak = 1/(4γ₀); genuine EP of the toy 2-level rate-channel reduction | Tier 1 derived (EP location, t_peak). Full block: non-normal on the real Q axis; finite-q Puiseux-1/2 defective EP2 seeds certified at N=5,7,9, classified at N=11 (literal drop measured). All odd N: nullity-surplus count only; a defective seed at every larger odd N is open. Off-axis complex-Q EP open (`LocalGlobalEpLink`; see [§The real-axis EP](PROOF_F86A_EP_MECHANISM.md)) | [F86a EP mechanism](PROOF_F86A_EP_MECHANISM.md) |
...
|----------------|------------------------------------------------------------------------|-----------------------------------|--------------------------------------------------------------------------|
| **F86a**       | EP mechanism (t_peak, Q_EP, dressed pair, Π-centered P-type L_eff mirror) | Tier 1 derived                    | [F86a EP mechanism](PROOF_F86A_EP_MECHANISM.md)                    |
...
**Symmetry anchor:** [PT-symmetry analysis](../../experiments/PT_SYMMETRY_ANALYSIS.md) (Π is linear, not Bender-Boettcher PT; it supplies a sectorwise P relation after Π² resolution, while the full SRP class remains open).
...
**Separate gain-loss crossing:** [the Fragile Bridge](../../hypotheses/FRAGILE_BRIDGE.md) leaves the imaginary axis, at generic couplings and two qubits per chain, at a second-order exceptional point on the real γ axis: at γ_crit, in the first block of its Liouvillian to go unstable, two imaginary eigenvalues meet in a 2×2 Jordan block (in the full Liouvillian one eigenvector for the two at J_bridge = 1.0 and 1.9, where that block is its own spin-flip image), while exact λ ↔ −λ pairing survives. It is a DISTINCT gain-loss system, not a "global instance" of the full block-L. The full Σγ=N·γ₀ block is genuinely non-normal on the real Q axis and carries certified finite-q Puiseux-1/2 defective EP2 seeds at N=5,7,9. For arbitrary odd N only the nullity-surplus count is a theorem; the off-axis complex-Q EP question also stays open. See [F86a EP mechanism](PROOF_F86A_EP_MECHANISM.md).
