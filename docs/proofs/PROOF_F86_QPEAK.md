...
**Sibling proof (same coherence block, t-trajectory at fixed Q rather than Q-resonance at fixed t):** [PROOF_BLOCK_CPSI_QUARTER](PROOF_BLOCK_CPSI_QUARTER.md) carries Theorem 1 (`C_block(t=0) = 1/4` exact at the canonical Dicke initial) and Theorem 2 (`C_block(t) = (1/4)·exp(−4γ·t)` chromaticity-universal closed form). The two proofs view the same `(popcount-n, popcount-(n+1))` block from orthogonal axes: F86 holds t fixed at t_peak and varies Q to find the EP, Theorem 2 holds Q fixed (channel-uniform initial places it in the H-kernel via F73) and varies t to track the Mandelbrot-cardioid 1/4 ceiling decay. The two share the same factor-4 quadratic-discriminant algebra: F86's `4γ₀² − J²g_eff² = 0` and R=CΨ²'s `1 − 4CΨ = 0` are both `b² − 4ac = 0` instances, complementary lenses on the same structural cusp. **Hardware anchor (2026-05-08, ibm_kingston q13–q14, job d7ulfjdpa59c73b4rttg):** Theorem 1 saturated at 88.2% of the 1/4 ceiling, Theorem 2 closed-form fit R² = 0.9977 across 5 t-points; full writeup in [IBM_KINGSTON_BLOCK_CPSI_SATURATION_MAY2026](../../experiments/IBM_KINGSTON_BLOCK_CPSI_SATURATION_MAY2026.md).
...
**Sibling t-trajectory primitives (same `compute/RCPsiSquared.Core/F86/` namespace, supporting PROOF_BLOCK_CPSI_QUARTER):** `BlockCoherenceContent.Compute(rho, n)` (state-level `C_block` for any density matrix, with the `Quarter = 0.25` Theorem-2 ceiling), `BlockCpsiClosedForm.At(N, n, γ₀, t)` (chromaticity-universal closed form), `BlockCpsiTrajectory.Build` (numerical EVD-based time evolution; companion to the closed form), `IbmBlockCpsiHardwareTable` (Tier 2 verified, 32 pinned witnesses from the 2026-04-26 framework_snapshots `|+−+⟩` runs). Hardware confirmation `block_cpsi_saturation_kingston_may2026` in `Confirmations/ConfirmationsRegistry.cs` pins the 2026-05-08 Kingston run (Theorem 1 saturation 88.2%, Theorem 2 fit R² = 0.9977).
