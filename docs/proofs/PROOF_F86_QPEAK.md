...
**Refined next directions** (now in `C2HwhmRatio.PendingDerivationNote`): (a'') SVD-block per-bond effective Q_EP via V_b[2,3] magnitude, 4-mode-friendly but needs the SVD→HWHM map; (b'') **full block-L derivation, not 4-mode** (most concrete since 4-mode insufficiency is now proven structural); (c'') 6-mode basis adding σ_1 SVD pair (natural fix to A3's σ_0 degeneracy obstruction); (d'') projector-overlap lift of |u_0⟩, |v_0⟩ (A3 precondition, library-independent); (e'') symbolic char-poly factorisation at Q_EP, less promising given C2's cubic-c_3 obstruction proof.
...
**Actual c=2 progress.** The c=2 stratum has OOP scaffolding (`C2EffectiveSpectrum`, `C2KShape`, `C2HwhmRatio`, `C2UniversalShapeDerivation` in `compute/RCPsiSquared.Core/F86/`) that pins each sub-fact at its appropriate Tier label. The directional Endpoint > Interior split is empirically derived bit-equivalent with the canonical Python pipeline. **The closed-form HWHM_left/Q_peak constant per bond class remains the open analytical target.** Ranked next directions (from `C2HwhmRatio.PendingDerivationNote` and `F86OpenQuestions` Item 1', refreshed 2026-05-06 later evening after Direction (a') falsification):
  ...
  - **(a'') Most promising 4-mode-friendly direction (newly promoted 2026-05-06 later evening): SVD-block per-bond effective Q_EP via V_b[2,3] magnitude.** The bond-class signature lives in V_b SVD-block off-diagonal `V_b[2,3] = ⟨u_0 | M_h_per_bond[b] | v_0⟩` (Endpoint 0.430 vs Interior 0.953 at N=5, ratio ~0.45 stable across N=5..8), NOT in the probe-block. Direction is OPPOSITE the empirical HWHM/Q* split, so a closed form needs to derive HOW SVD-block magnitude maps to HWHM/Q* shift, likely through a per-bond effective Q_EP_eff(b) shift rather than a direct linear lift.
  - **(b'') Most concrete next step (newly promoted 2026-05-06 later evening): full block-L derivation, not 4-mode reduction.** The 4-mode reduction is now structurally insufficient: 4-mode K_b at N=5 reproduces only the ~0.673 floor (Interior) and lands Endpoint at 0.410 with off-grid Q_peak ≈ 4.91, vs empirical 0.7506 / 0.7728. The empirical HWHM lift requires modes OUTSIDE the 4-mode subspace; the closed form must derive directly from the (n, n+1)-popcount block dimensions. Harder algebra but matches the empirical witness pipeline.
  ...
  - **(c'') 6-mode basis adding σ_1 SVD pair.** Natural fix to A3's σ_0 degeneracy obstruction at even N. Adds a second SVD pair to the 4-mode subspace, may close the 4-mode insufficiency gap if the σ_1 modes carry the missing HWHM/Q* lift.
  - **(d'') Lift |u_0⟩, |v_0⟩ to projector-overlap** (per `C2InterChannelAnalytical.PendingDerivationNote`). Removes the σ_0 degeneracy obstruction at even N (Stage A3 finding below). Library-independent precondition for any cross-block-based or SVD-block-based direction; not sufficient by itself.
  - **(e'') Symbolic char-poly factorisation at Q_EP.** At the EP, two of four eigenvalues coalesce; may simplify the quartic locally. Less promising given C2's cubic-c_3 obstruction proof (Stage C2 finding below).
