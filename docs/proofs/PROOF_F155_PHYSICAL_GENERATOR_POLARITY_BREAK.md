...
*(Added 2026-08-19. **Sweep record**, because this section makes claims of its own and the gate applies to it: `docs/ANALYTICAL_FORMULAS.md` returned the F155 entry's own "Typed: not yet" paragraph, naming the carrier this section builds; `docs/proofs/` returned [PROOF_BIT_A_TWIN_VIA_HADAMARD.md](PROOF_BIT_A_TWIN_VIA_HADAMARD.md), whose criterion decides the twin verdict below, and [PROOF_F112_CROSS_DEPHASE_VIA_KLEIN_V4.md](PROOF_F112_CROSS_DEPHASE_VIA_KLEIN_V4.md), whose Q_zx transport it rests on; `experiments/` returned **nothing at all** for F155 or for the physical generator, which is still true and still owed; `fw.Confirmations` returned nothing, there being no hardware anchor for this quantity; the OpenArcs registry returned the retired `polarity_break_dephase_siblings`, whose closing line asked for exactly this carrier; `docs/GLOSSARY.md` returned no entry for the polarity asymmetry and none for the generator distinction, both of which are added in the same change; `docs/CAUGHT_ERRORS.md` returned the 2026-06-20 entry and its 2026-08-19 sequel, which is the scope error this carrier is built to make hard to repeat, and the 2026-08-06 denominator entry, which decided the tolerance below. One store answered under a LABEL the first sweep did not try: `experiments/PT_SYMMETRY_ANALYSIS.md` (Tier 2, 2026-04-01) had already concluded that Π is not a PT symmetry but a chiral one, which is corollary 7's conclusion reached from the other side, and neither document named the other until now.)*

Since 2026-08-19 the theorem is typed and live, so a reader does not have to run a Python script to see it hold.

The **claim** is `PhysicalGeneratorPolarityBreak` (`compute/RCPsiSquared.Core/Symmetry/`), Tier1Derived, with `LindbladBitBPiBreakMagnitude` (F113) as its typed parent, which is corollary 10 made structural. It carries the closed form as a static `PredictAsymmetry` over Pauli terms; the sum runs over the strings the B side carries, because the form is diagonal and a string absent from either half contributes nothing. The letter is a parameter, so §(g)'s three conventions are one method rather than three.

The **witness** is `PhysicalGeneratorPolarityBreakWitness`, reached by

```bash
dotnet run --project compute/RCPsiSquared.Cli -- inspect --root f155      # --N up to 5, default 2
```

and what it does at inspect time is run the OTHER route: the generator of §(a) is built as a 4^N × 4^N superoperator through `NoJumpGenerator.Build`, pushed through the palindrome residual at σ = 0 and the ±i refinement, and the asymmetry is read off the matrices. The closed form never builds a matrix and the measurement never consults the closed form, so the two meet rather than one confirming the other.

The configuration count depends on N and is pinned by a test rather than quoted here: four at N=1, eight at N=2, five at odd N above 1, six at even N above 2, since the disjoint-supports case needs two sites, the PT dimer and its detuned partner are N=2 objects, and the cancelling zero needs equally many gain and loss ends. They do not all discriminate equally, and the honest breakdown is worth stating. At N=2, three reach a **nonzero** value: the F113 reduction, the positive control at exactly −4^(N+1), and the detuned dimer. Three reach zero through an **empty sum** and are, on the closed-form side, one branch three times, though they differ on the measured side, where ‖M_anti‖² is 20, 272 and 16: B = 0, disjoint odd supports, and the PT dimer at its own exceptional point. One reaches zero through a **cancellation**, which is the only zero here the closed form reaches by subtraction rather than by an absence; the measured route could have returned nonzero on any of the four. One is **silent**, the F113 configuration read under Π_X, where both halves are individually zero.

**The tolerance is a law, and it takes two numbers.** Each reading reports |residual| / (eps·‖M_anti‖²), how closely the routes agree in units of the measured route's own noise, and |closed form| / (eps·‖M_anti‖²), whether that route could have refuted the closed form at all. The second is not decoration. The asymmetry is bilinear in (A, B) while the noise floor is quadratic in whichever half is larger, so pulling the halves apart, A by s and B by 1/s, holds the true value fixed while the floor grows like s²: measured on a dense N=2 fixture, at s = 10⁸ the matrix route returns exactly 0.0 against a closed form of 17.5168, a relative error of 1, while the agreement ratio sits at a comfortable 0.178. Nothing is wrong with that ratio; it is answering a different question. A reading whose signal is at or under its own noise is therefore marked NOT INFORMATIVE rather than passing quietly, and the two numbers together predict the surviving precision, the relative error being the ratio divided by the signal-to-noise (at s = 10⁵: 0.154 / 1.78·10⁵ = 8.7·10⁻⁷, measured). A BALANCED configuration is the one case where the signal is zero and the reading is refutable anyway, since the measurement could have returned a nonzero asymmetry and did not; there the agreement ratio decides alone. The bound of 8 that the headline count uses is inherited slack from the Python gate's `EPS_RATIO_BOUND`, not a derived constant, and it is the predictive law rather than that ceiling which the sweep tests.

The denominator is ‖M_anti‖² and not ‖M‖², which is what the asymmetry is a difference OF. This repo retired the larger denominator once already on that ground ([CAUGHT_ERRORS](../CAUGHT_ERRORS.md), 2026-08-06), and here it is not merely looser but wrong in kind: the asymmetry is provably blind to σ, since the 2σ·I term lies in Ad_Π's +1 eigenspace which M_anti annihilates. Gated at N=2 on a sixteen-string fixture, the asymmetry and ‖M_anti‖² are bit-identical at σ = 0, 0.05, 1 and 17.3 (17.516800000000011 and 113.949) while ‖M‖² runs from 494.8 to 18458.7 over the same four.

**Which zeros are bit-exact is reported, not asserted.** At N=2, six of the eight readings are exact and the worst agreement ratio is 0.757576; at N=3, four of five are exact and the worst is 0.831169. Those two numbers are what the named command prints, and they will move if anything upstream does.

On the cube map F155 sits on the bit_b axis with the `CoveredByHadamardDuality` twin verdict, which differs from its own parent's `BitBSpecific`. That is not a contradiction: F113's hypothesis names σ⁺/σ⁻ jumps, which single out the Z-basis energy ladder, while F155 assumes no channel at all, so its bit_a image is the Hadamard transport §(g) states and gate S10 measures. One thing should be said plainly rather than left for a reader to notice: that criterion, "what decides is the channel and not the quantity being measured", was **sharpened in this same change**. It is a reading of [PROOF_BIT_A_TWIN_VIA_HADAMARD.md](PROOF_BIT_A_TWIN_VIA_HADAMARD.md)'s two exception classes rather than a test that stood there in those words before. What licenses the twin itself is that proof's case 1 unchanged, Ad_{Q_zx} being Frobenius-unitary and conjugating Π_Z to Π_X; §(g) then states the resulting transport as an identity and S10 measures it. The only clarification F155 adds there, now recorded in that proof, is that the asymmetry is a function of the operator AND the convention, where case 1's other members are functions of the operator alone. That is why G13's one-sided turn moves this number and moves none of theirs, and it is not a new admission criterion.

`NoJumpGenerator` (`compute/RCPsiSquared.Core/Lindblad/`) and the new `PolarityCoordinates.Decompose(superoperator, N, σ, letter)` overload are the two pieces that make the object expressible in the Object Manager. Neither the generator nor a general superoperator route was unreachable before, `PalindromeResidual.Build` having always taken an arbitrary matrix; what was missing is a **typed, named carrier and a one-call route**, since both chain-facing polarity entry points build a Lindbladian out of a Hermitian H and so cannot express this generator. The generator's doc comment says in full what it is not.
