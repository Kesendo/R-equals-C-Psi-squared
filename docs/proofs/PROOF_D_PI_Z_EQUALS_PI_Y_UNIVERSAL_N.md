...
- **F112 non-Hermitian extension (Welle 11)** is stated for Z-dephasing and consumed by the `LindbladBitBPiBalance` NonHermitianExtension docstring; the D · Π_Z · D = Π_Y identity proven here is the structural reason the Welle 11 Z-dephase closure transports to Y-dephasing under D-conjugation without re-proof (forward-looking corollary; the explicit citation into PROOF_F112_NONHERMITIAN_UNIVERSAL_N.md and the `NonHermitianExtension` inspectable is a Welle 12 Task 3 follow-up).
