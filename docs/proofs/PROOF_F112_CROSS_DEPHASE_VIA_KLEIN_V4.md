...
F112 was proved for Z-dephasing. The natural next question was whether the same polarity-balance identity holds for X-dephasing and Y-dephasing as well. After the Klein-V₄ dephase-swap group landed in Welle 12, the answer looked structurally suggested: the V₄ relates the palindrome operators Π_Z, Π_X, Π_Y (precisely, by the regular V₄-action on the four oriented operators {Π_X, Π_X⁻¹, Π_Y = Π_Z⁻¹, Π_Z}; see (d)). But two things travel under a swap element and they are not the same: the dephasing CHANNEL travels only when the element moves the dissipator's axis (Q_zx and Q_yx do, Z ↔ X, Q_zx as a Hilbert-space lift; D does not, it fixes every letter dissipator), while the IDENTITY travels whenever the element maps the hypothesis class onto itself and one palindromizer onto another, which D does too: D·Π_Z·D = Π_Y = Π_Z⁻¹, so F112-Y is F112-Z read through the inverse mirror, asymmetry_Y = −asymmetry_Z for every superoperator (F155). The job was to write the two routes that actually close it, and to spell out how the hypothesis on the collapse operators rotates with the dephase axis.
...
2. **Klein-V₄ conjugation transport (Route 2), one-sided.** The Welle 12 Task 2 result establishes the Klein-V₄ subgroup `{I, D, Q_zx, Q_yx}` of `U(4^N)` with `D · Π_Z · D = Π_Y` and `Q_zx · Π_Z · Q_zx = Π_X` (each operator unitary, so Frobenius norms and asymmetry values are preserved exactly). **What is preserved is the value under a JOINT turn**, the operator and the palindrome convention conjugated together; turning the system while holding Π_Z fixed is a different move and does move the number, which is [F155](PROOF_F155_PHYSICAL_GENERATOR_POLARITY_BREAK.md)'s gate G13. The two facts sit one line apart in any reader's head and are easy to read as a contradiction, so: same turn on both, nothing changes; turn on one only, everything may. **Q_zx** transports the identity between dephase letters as a Hilbert-space unitary: it is the Hadamard lift, so it rotates the dephasing axis Z → X, and conjugating the F112-Z identity by Q_zx yields F112-X (mapping the bit_b hypothesis on c to bit_a); Q_yx = Q_zx·D rotates the same axis (both carry L_Z to L_X and fix L_Y) but lands the mirror on Π_X⁻¹, the other orientation. **D does not yield a Y-dephasing channel**: D is the transpose superoperator, which conjugates Π_Z → Π_Y but leaves the dephasing axis on Z (D · L_Z · D⁻¹ is still a Z-dephasing Lindbladian); the crossed (L_Z, Π_Y) instance it produces IS F112-Y as the Statement defines it, L_d being d-independent there. F112-Y is supplied by Route 1 instead (it shares the bit_b axis with Z); see (d).

**Diagnostic consequence.** F112 is now a universal Lindblad-channel polarity-balance identity: for any of the three F1 palindrome conventions (Z, X, Y), any Hamiltonian H read as the commutator generator −i[H, ·] (see the Scope note in (a); the physical generator −i(Hρ − ρH†) is a different object), any axis-adapted homogeneity hypothesis on c, the asymmetry vanishes bit-exactly for any N. The `polarity_coordinates_from_L` primitive can be run under any dephasing convention with the same diagnostic interpretation. Asymmetry ≠ 0 localises the failure: (i) c violates the axis_d-homogeneous hypothesis, (ii) L not in Lindblad form, or (iii) the conventions are crossed (computing M_d with Π_d' for d' ≠ d).
...
**Scope (commutator generator only).** As with the non-Hermitian parent ([F112 non-Hermitian extension](PROOF_F112_NONHERMITIAN_UNIVERSAL_N.md), corrected 2026-06-20, sharpened 2026-08-19), "non-Hermitian H" here means the **commutator** superoperator −i[H, ·] for an arbitrary matrix H, a structural ‖·‖² identity. It is NOT the physical generator of PT-symmetric / gain-loss / post-selection dynamics, −i(Hρ − ρH†) = −i[A, ρ] + {B, ρ} for H = A + iB; that anticommutator part can break the balance, and what decides whether it breaks is the content of (A, B), not the dephase letter. The parent's boundary for Z is [F113](PROOF_F113_COEFFICIENT_DERIVATION.md)'s closed form, carried by H's single-site-Z content. **The X- and Y-dephase analogue is not registered, and this note used to overstate what was known about it** (repaired 2026-08-19): the figures it quoted, asymmetry ≈ 240–1500 at N = 2, 3 with a "mean ≈ 54/200 over random fixed-norm configs", were means over an ensemble that is specified nowhere, and it cross-referenced them to the parent as "the same boundary", which they are not. A mean over random configurations is not a boundary in any case: a random H generically carries whatever the carrier turns out to be, so such a number records that the carrier is present, not how far the balance breaks. Which (A, B) content carries the X- and Y-dephase break **was closed on 2026-08-19** by §(g) of [the F155 proof](PROOF_F155_PHYSICAL_GENERATOR_POLARITY_BREAK.md): for the physical generator the X law is supported on the bit_a-odd strings with weight (−1)^#X, and the Y one is the Z law with a global minus, an exact consequence of Π_Y = Π_Z⁻¹ that holds for every superoperator. What remains open is the same question for the FULL Lindbladian, recycling term included, which is a different object and is carried by no arc yet. The two generators coincide exactly when H is Hermitian.
...
This means Route 2 within the V₄ gives F112-Z → F112-X (the Hadamard lift) and F112-Z → F112-Y by D (D·Π_Z·D = Π_Y on a D-invariant hypothesis class), while no V₄ element carries a Z-dephasing channel to a Y-dephasing one. The quarter turn R_x(π/2)^⊗N, a Hilbert-space unitary outside the V₄, does: it carries Π_Z to Π_Y and L_Z(H, c) to L_Y(R H R†, R c R†) as exact signed permutations and preserves bit_b on every string, so F112-Y is a Route-2 corollary of F112-Z as well. Route 1 (direct re-run of the Welle-11 lemmas in the bit_b axis with d = Y phase substituted for d = Z phase) is the independent second proof. The two routes are complementary: Route 1 covers all three d ∈ {X, Y, Z} structurally; Route 2 provides the insight that the Hadamard-conjugated Z-dephase config IS a natural F112-X config, and the quarter-turned one an F112-Y config.

In particular, **D-conjugation moves the identity and not the channel**. D · L_Z · D⁻¹ is a perfectly valid Z-dephasing Lindbladian (= L_Z with H → −H^T); conjugating the F112-Z identity by D therefore gives the crossed (L_Z-style, Π_Y) instance, a Z-dephasing channel paired with the Y-palindrome, and that crossed instance IS F112-Y as the Statement defines it, since L_d does not depend on d and Π_Y = Π_Z⁻¹ makes asymmetry_Y = −asymmetry_Z for every superoperator (F155). Route 1 (the direct axis re-run of Welle-11) confirms it independently, and it is immediate there: Y shares the bit_b axis with Z, so any bit_b-homogeneous c that satisfies F112-Z satisfies F112-Y directly (verified asymmetry_Y(L_Z) ≤ 3e-14).
...
| Mixed bit_b, homogeneous bit_a, complex coefs (e.g. X+iY site) | **0** | non-zero | non-zero |
| Homogeneous bit_b, mixed bit_a, complex coefs (e.g. Y+iZ site) | non-zero | **0** | **0** |

Each row's pattern matches the per-d hypothesis: F112-d gives 0 whenever c is axis_d-homogeneous; the non-zero cells are for complex-coefficient c. The zeros are the claim and they are bit-exact; the non-zero entries carry no magnitude here, deliberately (removed 2026-08-19). The verifier draws a random H and a random rate per trial, so any magnitude quoted from it is a property of that draw, and this document retires two other numbers of exactly that kind in its Scope note.
...
2. **Klein-V₄ equivariance is partial but real:** the Hadamard subgroup {I, Q_zx} of {I, D, Q_zx, Q_yx} cleanly transports F112-Z to F112-X (Route 2). The D and H = Q_yx involutions are operator-space-only (no Hilbert-space unitary lift). D does not rotate the dephasing axis of L: it is the transpose, and D · L_Z · D⁻¹ stays a Z-dephasing Lindbladian (H → −H^T), never a Y-dephasing one, yet it carries the F112-Z identity to F112-Y (D·Π_Z·D = Π_Y = Π_Z⁻¹ on a D-invariant hypothesis class); Q_yx = Q_zx·D does carry L_Z to L_X (both Q's do, and both fix L_Y), landing the mirror on Π_X⁻¹. No element of the V₄ carries a channel between Z and Y; the quarter turn R_x(π/2)^⊗N outside the V₄ does, and Route 1 (direct axis re-run) proves F112-Y independently; it is immediate there because Y shares the bit_b axis with Z.

3. **The `Pi2KleinV4DephaseSwapGroup` typed Claim's significance docstring was softened in Welle 15** (it states that D and H = Q_yx are operator-space-only, that D fixes every letter dissipator and transports nothing between letters, that H = Q_yx carries L_Z to L_X as Q_zx does with the mirror landing on Π_X⁻¹, and that no element reaches L_Y). The reading it now carries:
   - F1-family identities that depend only on (a) the Π-eigenvalue / Π²-eigenvalue spectrum and (b) Pauli-basis support structure transfer between dephase letters via Route 1 (direct axis re-run); these include F112-d, F108-d (closed in the F108 proof), and any other F-formula whose proof reduces to F38 + matrix-support disjointness.
   - Identities depending on the Lindblad-form L itself transfer within the V₄ via either Q (Route 2), between (Z, X) only, the mirror landing on Π_X or Π_X⁻¹; the Clifford quarter turns outside the V₄ reach (Z, Y) and (Y, X) as well.
   - D and H retain mathematical value as Π-intertwiners and witness the Klein-V₄ structure on dephase letters; neither is a Hilbert-space unitary, D transports no L, and H does transport L_Z to L_X.
...
The D involution (Welle 12's Z↔Y swap) is the transpose superoperator: it conjugates Π_Z → Π_Y but does not rotate the dephasing axis (D · L_Z · D⁻¹ is still Z-dephasing, = L_Z with H → −H^T, a valid Lindbladian, not a "category mistake", just not a Y-dephasing channel). So D-conjugation of L_Z gives L_Z again; F112-Y needs no transport, being F112-Z read through Π_Y = Π_Z⁻¹ (asymmetry_Y = −asymmetry_Z for every superoperator, F155), with Route 1 and the quarter turn as independent confirmations.
