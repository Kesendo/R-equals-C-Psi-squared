...
2. **Klein-V₄ conjugation transport (Route 2), one-sided.** The Welle 12 Task 2 result establishes the Klein-V₄ subgroup `{I, D, Q_zx, Q_yx}` of `U(4^N)` with `D · Π_Z · D = Π_Y` and `Q_zx · Π_Z · Q_zx = Π_X` (each operator unitary, so Frobenius norms and asymmetry values are preserved exactly). **What is preserved is the value under a JOINT turn**, the operator and the palindrome convention conjugated together; turning the system while holding Π_Z fixed is a different move and does move the number, which is [F155](PROOF_F155_PHYSICAL_GENERATOR_POLARITY_BREAK.md)'s gate G13. The two facts sit one line apart in any reader's head and are easy to read as a contradiction, so: same turn on both, nothing changes; turn on one only, everything may. Only **Q_zx** transports an identity between dephase letters: it is the Hadamard Hilbert-space lift, so it rotates the dephasing axis Z → X, and conjugating the F112-Z identity by Q_zx yields F112-X (mapping the bit_b hypothesis on c to bit_a). **D does NOT yield F112-Y**: D is the transpose superoperator, which conjugates Π_Z → Π_Y but leaves the dephasing axis on Z (D · L_Z · D⁻¹ is still a Z-dephasing Lindbladian), so it produces only F112-Z in a crossed (L_Z, Π_Y) disguise, not F112-Y. F112-Y is supplied by Route 1 instead (it shares the bit_b axis with Z); see (d).
