...
- L_d = -i[H, ·] + Σ_k γ_k · (np.kron(c_k, c_k^*) − ½·{c_k^† c_k, ·}) for a standard Lindblad-form (GKSL) Liouvillian. The anticommutator term is Π²-even and contributes only to M_zero, so it does not affect the asymmetry (it is present in the verifier; omitting it changes nothing).
