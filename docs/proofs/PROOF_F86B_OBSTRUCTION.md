...
**Retracted / partially closed (2026-07-02).** The `2·√(2(c−1))` form and its "converges monotonically from below to 2.0" reading were **retracted 2026-05-08 as a finite-N artifact**: the ratio σ_0/√(2(c−1)) at c=2 keeps climbing past 2.0 (N=9 → 2.014, N=10 → 2.017, N=11 → 2.020, [`SigmaZeroChromaticityScaling.cs`](../../compute/RCPsiSquared.Core/F86/SigmaZeroChromaticityScaling.cs)), so it does not converge from below. The **c=2 case is closed by F86e** ([`SigmaZeroCommutatorNormClaim.cs`](../../compute/RCPsiSquared.Core/F86/SigmaZeroCommutatorNormClaim.cs), Tier-1 derived, closure 2026-05-21): σ_0(c=2) = ‖[Π_HD1, M_H]‖ is a commutator / Schur-multiplier norm with the non-elementary asymptote σ_0(∞) ≈ 2.8629 (not 2√2 ≈ 2.828), γ-independent. **Only c ≥ 3 stays open** ([`F86OpenQuestions.cs`](../../compute/RCPsiSquared.Core/F86/F86OpenQuestions.cs) Item 5; the true asymptote follows from F89's cyclotomic Φ_{N_block+1} structure, see [`PROOF_F90_F86C2_BRIDGE.md`](PROOF_F90_F86C2_BRIDGE.md) Item 5).

These three items are tractable with existing infrastructure (`coherence_block`, the SVD-of-V_inter construction, OBC sine-mode algebra), but each is multi-page algebra. Item 1 is the path to the closed form for HWHM_left/Q_peak; its Direction (b'') (full block-L derivation) is numerically achieved Tier-1 via the F90 bridge identity ([`PROOF_F90_F86C2_BRIDGE.md`](PROOF_F90_F86C2_BRIDGE.md)), reducing the remaining analytical work to F89's AT-locked F_a/F_b cyclotomic structure plus the intra-channel dispersion lift of the rank-1-bridge model (the inter-coupling "octic residual" lift suspicion is refuted, two-dial scout 2026-06-11). The empirical envelope (c, N, γ₀ checks above) is now Tier-1 grade.
...
3. **Cubic-c_3 char-poly obstruction (Stage C2).** The 4×4 effective characteristic polynomial at c=2 has a non-rational c_3 coefficient (cubic in Q with no clean root structure). We rigorously ruled out any rational-coefficient `(λ²−aλ+b)(λ²−cλ+d)` factorisation by symbolic match against the c_3 evidence. The Tier2 outcome for `C2EffectiveSpectrum` is therefore **evidence-based** ("we proved no such split exists in this family"), not "we couldn't find one". Symbolic char-poly factorisation at Q_EP itself (Item 1' direction (e'') above; retired with the rest of the 2026-05-06 (a''-e'') taxonomy when the F90 bridge identity superseded the letter-direction approach 2026-05-11) may still help locally, but the global quartic does not factor.
...
### Corollary (F90 bridge): D_k closed, g_eff did not follow

The F90 bridge identity ([`PROOF_F90_F86C2_BRIDGE.md`](PROOF_F90_F86C2_BRIDGE.md), Tier 1 derived) identifies F86 c=2 K_b with the F89 path-(N−1) per-bond Hellmann-Feynman derivative. This corollary originally (2026-05-14) read F86's g_eff and F89's `D_k = odd(k)² · 2^E(k)` ([`PROOF_F89_PATH_D_CLOSED_FORM.md`](PROOF_F89_PATH_D_CLOSED_FORM.md)) as one wall seen from two sides, both blocked on the OBC sine-sum / cyclotomic structure of `⟨ψ_k|σ⁺σ⁻|ψ_l⟩`. That reading was corrected the next day, and the true outcome is asymmetric. On 2026-05-15 D_k was promoted to Tier-1-Derived: the F_a-eigenvector ansatz + Chebyshev expansion + orbit-polynomial reduction derive `(P_k, D_k)` algebraically, closing the three previously-open gaps (poly-degree term, k-self 2-adic, deep-2-power bonus) without the unexecuted sine-sum identities the 2026-05-14 reading had named as the wall. D_k is closed. But the predicted transfer (close D_k and g_eff closes too) was tested directly (`simulations/f86_geff_via_f90_bridge_probe.py`, 2026-05-15) and refuted: D_k closes *via* the orbit-polynomial reduction, which exploits the Bloch-orbit symmetry and discards the per-bond cross matrix elements `⟨v_n|M_h_per_bond[b]|v_{n'}⟩`; g_eff is a per-bond Hellmann-Feynman-in-J_b quantity built from exactly those discarded cross elements. The very step that closes D_k strips g_eff's information: obstruction L4 (reduced-model insufficiency) once more. So the two are not one wall. D_k was reducible by orbit symmetry and is now closed; g_eff is per-bond, lies outside the orbit-reduced category, and remains the irreducible residue. The six obstruction lemmata L1–L6 and the diagnosis below never depended on D_k's status; they stand unchanged. ∎(corollary)
...
The Liouvillian spectrum under uniform Z-dephasing has exactly two primitives, and the Absorption Theorem ([`PROOF_ABSORPTION_THEOREM.md`](PROOF_ABSORPTION_THEOREM.md)) names them. **γ₀** carries the real part: `Re(λ) = −2γ₀·⟨n_XY⟩` exactly, because L_H is anti-Hermitian and contributes nothing to Re(λ). The underlying dissipator cells are quantized in units of 2γ₀, while a Hamiltonian-mixed full eigenmode can have fractional ⟨n_XY⟩. **y** carries the imaginary part: for the modes the F90 bridge ties this proof to, the OBC Bloch dispersion `4cos(πn/(N+1))`, set by L_H alone.
...
([the chiral mirror rate law proof](PROOF_PTF_CHIRAL_MIRROR_RATE_LAW.md)) ran the same algebra
...
**Companion investigation:** [The Atmosphere and the Cancelled Formulas](../THE_ATMOSPHERE_AND_THE_CANCELLED_FORMULAS.md), g_eff as the un-atmospheric residue.
...
**Scripts:** [`eq022_b1_step_d_extended_verification.py`](../../simulations/eq022_b1_step_d_extended_verification.py) (N=8 data that falsified the closed-form conjectures), [`eq022_b1_step_g_two_level_decomposition.py`](../../simulations/eq022_b1_step_g_two_level_decomposition.py), [`eq022_b1_step_h_slowest_pair_basis.py`](../../simulations/eq022_b1_step_h_slowest_pair_basis.py), [`eq022_b1_step_i_svd_inter_channel.py`](../../simulations/eq022_b1_step_i_svd_inter_channel.py) (SVD of V_inter; established the EP-partner subspace, σ_0 ≈ 2√2 asymptotic, probe ⊥ EP partners).
