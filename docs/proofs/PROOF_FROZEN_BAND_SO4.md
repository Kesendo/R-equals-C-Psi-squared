...
**Verification:** [`simulations/eta_ceiling_reduction.py`](../../simulations/eta_ceiling_reduction.py) (must print "eta-ceiling reduction gate: ALL GREEN", 236 checks, about ten seconds; `--deep` adds the larger rungs and the wider sweeps, 286 checks in about four minutes, and carries F143 to N = 40) and, for Proposition 5.3 and the exceptional couplings, [`simulations/exceptional_couplings.py`](../../simulations/exceptional_couplings.py) (must print "exceptional couplings gate: ALL GREEN"; that run states its own check count; `--deep` adds N = 7, the whole-block enumeration at N = 6 and the N = 7 rung)
...
The frozen band was found one block at a time: a diagonal line of blocks all carrying the same undisturbable decay rate, plus two lines beside it, and a ladder that climbs the diagonal one excitation at a time. This document says the three lines are not three facts. They are one object seen from three angles, and the object is an old one: the chain under dephasing is the Hubbard model wearing a disguise, and a Hubbard model has two independent SU(2) symmetries rather than one. The disguise is worth naming precisely, because the two halves of a Hubbard model land in two different halves of a Liouvillian. The turning supplies only the hopping; what plays the part of the on-site repulsion is the dephasing, since at a uniform rate on a diagonal block (p, p) it reads −4γ(p − D̂), and the coefficient of D̂ is the +4γ that a Hubbard model calls U. The borrowed word ends there, because the two ledgers run opposite ways: in a Hamiltonian, U·D̂ is an energy a ground state avoids, while here −4γp + 4γD̂ is a flat bill of 4γ per excitation with 4γ handed back for every doubly occupied site. A doubly occupied site is a site where the two faces of the coherence agree (a doubly empty one agrees too, and on a fixed block the two kinds differ in number only by the constant N − 2p), and agreement is exactly what the arriving light passes through without separating. So the on-site term is neither a repulsion nor a penalty: it is a **discount for agreement**, and the band's diagonal frozen modes have bought every discount but one: the frozen root is the bill of the single disagreement left standing. So this is not a Hubbard Hamiltonian with a dissipator attached. It is a generator whose kinetic term is the chain and whose interaction term is the light's price schedule, which is why the two SU(2)s are exactly the ones a Hubbard model has and why the whole question below is about the double occupancy. The ladder already known is the first of them. The second is what steps sideways, off the diagonal onto the two neighbouring lines, and that is the whole reason the band is three lines wide and not one.
...
**Lemma 2.1 (neither ladder moves the disagreement set, at any profile).** Φ and S⁺ commute with the dissipator for every rate profile γ_l, and not only for the uniform one.
...
**And "finitely many" is not a hedge.** The exceptional set is NOT empty. At N = 5, at the uniform watching point and J² = 3/2, the blocks (2,2) and (3,3) carry ⌊N/2⌋ + 1 frozen modes, as an exact rank over ℚ. At N = 5 the whole band has since been enumerated at every coupling at once, exactly: three exceptional couplings, on those two blocks and on no other, each a SIMPLE root of the deflated determinant and so raising the count by exactly one. At N = 6 the block (2,2) carries six, again by exact enumeration of the whole block and again simple. At N = 7 the number is 11, and there the exact polynomial is the one of the ℓ = 2 singlet subspace rather than of the whole block, so the count is exact while completeness and the multiplicity are not read. At the couplings that block carries, the extra mode is an η multiplet seeded at ℓ = 2 that is a spin SINGLET, so it occupies the diagonal rungs ℓ through N − ℓ and no side line, and neither the band statement nor the count on the two side lines moves. A block (p, p) sees only the rungs ℓ ≤ min(p, N − p), so those are not all the exceptional couplings: at N = 6 the block (3,3) carries four more, exactly, seeded at ℓ = 3. That the extra mode stays on the diagonal is established over all couplings at N = 5, where the whole band was enumerated, read at one coupling at N = 6, and read numerically over the whole band at N = 7; above that it is measured at no N. What IS now derived, from the spin ladder rather than from a scan, is the weaker statement the completeness question actually needs: a side line can never carry an exceptional coupling the diagonal block beside it misses, since within one p + q the block is the su(2) weight and mult(0) ≥ mult(m). That much holds at every N and every coupling. Turning it from a statement about absolute counts into one about EXCESSES needs the two generic depths to agree, which they do at N ≥ 6 by Corollary 7.3 with Proposition 5.3 from above and Proposition 3.2 from below, and the second of those imports the band statement for a general side line, so the sharpened form carries the same import the paragraph below names. The extra mode may therefore still step sideways for all this says; it cannot step sideways ALONE. [THE_EXCEPTIONAL_COUPLINGS](../../experiments/THE_EXCEPTIONAL_COUPLINGS.md) carries the certificates; gate [`simulations/exceptional_couplings.py`](../../simulations/exceptional_couplings.py) pins them. So no argument can promote this section's bound to every coupling, and the generic statement above is the sharp one.
...
Three readings are worth keeping. The first is that **the ceiling belongs to the second SU(2)**. Section 2 introduced the spin ladder to carry the corner's frozen modes sideways, which is a floor; here the same operator returns as the leading exact deficit in the disagreement, and a state maximising the double occupancy must be one it annihilates, the two chiral conditions of Corollary 7.3 holding besides. The band's two ladders turn out to hold the two ends of the same statement, one the floor and one the ceiling. The second is that N = 5 is visible in the bound rather than around it: ℓ(N − ℓ) = N + 1 has, as [ETA_CEILING_REDUCTION](../../experiments/ETA_CEILING_REDUCTION.md) records, the single solution N = 5 at ℓ = 2, so the one N the criterion cannot settle is the one where the floor lands exactly on the frozen value. The third is that **the identity's primitive variable is the agreement**: Proposition 7.2 is an equality for M·⟨v, D̂ v⟩ with ℓ(ℓ+1) as the top and three squares as the exact deficit, so what the section proves first is a ceiling on how much agreement a lowest-weight state can hold, ℓ(ℓ+1)/M per unit norm, and the floor on the disagreement is that ceiling restated through 𝒦 = ℓ − D̂, since ℓ − ℓ(ℓ+1)/M = ℓ(N − ℓ)/(N + 1). In that variable the frozen question "does 𝒦 = 1 occur" reads "can a rung-ℓ multiplet reach agreement ℓ − 1", and for N ≥ 6 with 2 ≤ ℓ ≤ ⌊N/2⌋ the agreement ceiling already sits below it.
...
**Which couplings are exceptional.** Proposition 5.3 closes the step this list used to open with: the depth is exactly ⌊N/2⌋ for all but finitely many couplings at every N ≥ 6, and the exceptional set is not empty, so nothing stronger is available. What is not proved is the exceptional set itself. Its size is exact per N and has no law; that EVERY exceptional mode is an ℓ = 2 spin singlet holds at N = 5, where the whole band was enumerated at every coupling at once and where no rung above 2 exists, and it fails as a general statement: at N = 6 the block (3,3) carries four further couplings, exactly, whose extra mode is η-lowest-weight at rung 3, and a block (p, p) sees only the rungs ℓ ≤ min(p, N − p), so the counts 3, 6, 11 read on the block (2,2) are the rung-2 counts. The extra modes stay on the diagonal at every rung read so far, so the band statement is untouched. Whether the point is semisimple or defective is unread here, where [PROOF_R90_FROZEN_DIVISOR](PROOF_R90_FROZEN_DIVISOR.md) §9 answers defective for its own object, the corner on the R90 locus, whose jump is algebraic where this one is geometric. [THE_EXCEPTIONAL_COUPLINGS](../../experiments/THE_EXCEPTIONAL_COUPLINGS.md) carries the open list.
...
**The off-diagonal equality.** Lemma 2.5 derives the floor ⌊N/2⌋ on (2,0) and (0,2). That those blocks carry no MORE than that is the ceiling on the off-diagonal lines, and it stays measured, as [XY_FROZEN_BAND](../../experiments/XY_FROZEN_BAND.md) states for them. One step toward it landed on 2026-08-08 and is recorded in open item 4 of [THE_EXCEPTIONAL_COUPLINGS](../../experiments/THE_EXCEPTIONAL_COUPLINGS.md): within one p + q the block IS the spin weight, so mult(0) ≥ mult(m) bounds every off-diagonal count by the diagonal one at EVERY coupling. Combined with Proposition 5.3's diagonal ceiling that gives the off-diagonal ceiling generically at N ≥ 6, and it is written here as a step and not as the closure, because the floor it must meet is Proposition 3.2's for a general side line and that one imports the band statement.
