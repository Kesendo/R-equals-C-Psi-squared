...
- **Section 5**, the large-J reduction, and every even order vanishing by transpose parity. Proposition 5.1 is an upper bound at large J and claims nothing more; the step from there to generic coupling is marked in the text as asserted and not argued.
...
That is an upper bound at large J and nothing more, which is all the proof gives and all the proposition claims. **Reaching generic J from it is a separate step and is not proved here**: eigenvalue multiplicity is upper semicontinuous in the parameters and the floor holds at every J, so a bound valid for all large J ought to be a bound at generic J with the floor making it an equality, but that sentence is an expectation rather than an argument. It is the informal bridge, and it is not what carries the weight. The rigorous per-N form of this upper bound is the exact rank certificate of [ETA_CEILING_REDUCTION](../../experiments/ETA_CEILING_REDUCTION.md): a full column rank over GF(q) forces a full rank over ℚ, which settles the non-existence outright and needs no limiting argument. Section 5 supplies the object on which that rank is read; it does not close the bound by itself. Section 7 replaces the rank read: the non-existence it certifies rung by rung is proved there for all N and all ℓ ≥ 2 at once.
...
**What it does not buy.** Extending that from large J to generic J is the semicontinuity step of Section 5, which is not proved. So the ceiling as a statement about arbitrary coupling still rests on that bridge, or on the per-N rank certificate where the certificate reaches. Corollary 7.3 removes the per-N computation from the input; it does not remove the bridge from the argument.
...
**The ceiling away from large J.** Section 7 makes the depth exactly ⌊N/2⌋ in the large-J regime at every N ≥ 6. Carrying that to generic coupling is the upper-semicontinuity step named after Proposition 5.1, and it is asserted rather than argued; where the exact rank certificate of [ETA_CEILING_REDUCTION](../../experiments/ETA_CEILING_REDUCTION.md) reaches it settles the same question without a limit, but that reach is finite. This is the gap that carries the most weight for a statement about a physical chain at a given coupling, and it is the one a reader consulting this list should see first.

**The equality in the law.** Corollary 7.3 proves min 𝒦 ≥ ℓ(N − ℓ)/(N + 1), which is the direction the ceiling needs. That the bound is ATTAINED, so that the measured law is an equality, is separate: at ℓ = 2 the maximizers are known in closed form and are checked per N by gate V9, and for ℓ ≥ 3 attainment stays measured. [ETA_CEILING_REDUCTION](../../experiments/ETA_CEILING_REDUCTION.md) carries the evidence and the extremal vectors.
