...
eigenmode accessible to single-excitation states has rate alpha_1, and
a slower mode exists at rate alpha_2 < alpha_1 with odd n_XY parity,
then alpha_1 is an exact ceiling for single-excitation protection.
No optimizer, no ansatz, no trick within the SE family can reach alpha_2.
...
For a Pauli string P = sigma_1 x sigma_2 x ... x sigma_N, define:

    n_XY(P) = number of indices k where sigma_k in {X, Y}
...
An **SE operator** is any operator of the form rho = sum_{j,k} c_{jk} |e_j><e_k|,
i.e., any operator supported on the SE x SE block of the full Hilbert space.
...
- L_H(rho) = -i[H, rho] is the Hamiltonian part
- L_D(rho) = sum_k gamma_k (Z_k rho Z_k - rho) is the Z-dephasing part

**L_D preserves n_XY exactly.** The dissipator acts on Pauli strings as
L_D(P) = gamma_k (Z_k P Z_k - P). Conjugation by Z_k flips the sign of
...
of X and Y factors):
    ...
    <e_k| P |e_j> = 0     for all j, k in {0, ..., N-1}
...
the Pauli decomposition of any SE operator sum_{j,k} c_{jk} |e_j><e_k|
...
Let rho_0 = |psi><psi| be any pure SE initial state. Its Pauli
decomposition lies in V_even (Part 3). The overlap of rho_0 with any
Liouvillian eigenmode v_m is:

    c_m = Tr(L_m^dag @ rho_0)

where L_m is the left eigenvector of mode m.

**Left eigenvectors respect the block structure.** Since L is block-diagonal
on V_even + V_odd (Part 2), L^dag is also block-diagonal on the same
decomposition: for any u in V_even and v in V_odd, <L^dag u, v> = <u, Lv> = 0
because Lv is in V_odd and u is in V_even. Therefore L^dag maps V_even to
V_even and V_odd to V_odd. The left eigenvectors of L, which are the right
eigenvectors of L^dag, inherit definite parity. In particular, if the right
eigenvector v_m lies in V_odd, the corresponding left eigenvector L_m also
lies in V_odd.

Now: rho_0 lies in V_even and L_m lies in V_odd. Since V_even and V_odd are
orthogonal in the Hilbert-Schmidt inner product (they are spanned by disjoint
sets of orthonormal Pauli strings):

    c_m = Tr(L_m^dag @ rho_0) = 0     exactly.
...
any graph topology, any site-dependent Z-dephasing profile gamma_k,
any N.
...
**amplitude damping** (T1 decay), whose jump operator sigma_- = (X - iY)/2
has n_XY = 1 (odd parity). Since real hardware has both T1 and T2 noise,
this theorem applies strictly only to the pure-dephasing component.
On hardware where T2 >> T1 (dephasing-dominated regime), the selection
rule holds approximately; on hardware where T1 ~ T2, it does not.
...
depends on N, topology, and gamma profile.
...
N=2-6, Chain/Star/Ring/Complete topologies, four gamma profiles each.
...
**Absorption Theorem (AT):** The AT gives Re(lambda) = -2*gamma*<n_XY>.
...
Re = -2*gamma are weight-1 Pauli operators (n_XY = 1, odd parity).
...
- [Absorption Theorem Proof](PROOF_ABSORPTION_THEOREM.md) - Re(lambda) = -2*gamma*<n_XY>
