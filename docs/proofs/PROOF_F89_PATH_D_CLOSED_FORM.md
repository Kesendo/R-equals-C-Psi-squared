...
The "+1" is therefore exactly the congruence m² + 3 ≡ 4 (mod 8) for odd k, or (m/2)² + 1 ≡ 2 (mod 8) for 4|k: a single power of 2 in an explicit integer bracket, which is what caps the cancellation at min(v2,2). These bracket parities are elementary and hold for all k; only the two closed forms for L̃ remain as bit-exact-verified (k=3..51) rather than derived from the trace. The remaining item, top-degree dominance (i), is resolved next.

### Top-degree dominance: proven for odd k and 4|k (2026-06-04)

Part (i) is the claim that v2(D_k) = max_d(−val₂(Q_d)) is realised at the top coefficient d = FA−1, so that v2(D_k) = 2(FA−1) − val₂(L̃). It is a k ≥ 5 theorem: at k = 3, 4 (FA = 2) the constant term carries the denominator instead (D_3 = 9, D_4 = 4 stay correct, just not via the leading coefficient), harmless base cases.

Two node-identity collapses, sharing the trigonometric core of the L̃ lemma, reduce it. Work in u = 2c, where Φ_u is the monic integer orbit polynomial (Washington) and the cleared integer numerator is Nint(u) = (4 − u²)²·Ã(u)²·B̃(u). Set Rm = Nint mod Φ_u (integer); top-degree dominance is equivalent to val₂(Rm_d) ≥ 4 for every d ≥ 1 (the d = 0 corner is separate and slack).

- **Lemma 1 (rigorous):** (4 − u²)·Ã(u) ≡ ε·m·(2 + u) (mod Φ_u), ε = 2 (odd k), 1 (even k), a collapse from degree 2k to degree 1. At each node, (4 − u_j²)·A(c_j) = 4 sin θ_j·S₁(θ_j) = ε·m·(2 + u_j), with S₁(θ_j) = m·cot(πj/m) the same sum as the "+1" lemma; since ε·m·(2+u) has degree 1 < FA and agrees with (4−u²)Ã at every node of the separable Φ_u, it is the reduction. So Rm = ε²·m²·W with W = (2+u)²·B̃ mod Φ_u, and dominance becomes val₂(W_d) ≥ 3 (odd k, 4|k), ≥ 2 (k ≡ 2 mod 4), for d ≥ 1.
- **Lemma 2 (rigorous):** (4 − u²)²·B̃ ≡ b₀ + b₂u² (mod Φ_u), a degree-2 collapse from the node identity 16 sin²θ·S₂(θ) = quadratic, with v2(b₀) = 3, v2(b₂) = 1.

**Closure for odd k and 4|k.** Here Φ_u(0) = ±1, so u is a unit in ℤ₂[u]/Φ_u. With (2 − u)² ≡ u² (mod 4) and u invertible, every d ≥ 1 coefficient of W lands in 8·ℤ₂, hence val₂(W_d) ≥ 3 and dominance is proven. This covers the two classes that carry the whole D_k = k²·2^{polydeg} (odd k) and the cap-2 cancellation (4|k).
