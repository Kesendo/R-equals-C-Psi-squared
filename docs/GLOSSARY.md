...
The symbol `CΨ` has two implemented quantum conventions and one algebraic role.
They must be named at the point of use. The active C# owners
`DensityMatrixTools.ComputeCPsi` and `Symphony.Cpsi` use purity; the pairwise
lens experiments use Wootters concurrence. The recurrence accepts the chosen
scalar product as its parameter but does not make the two diagnostics equal.
...
of the letters is retired. See [The CΨ Lens](THE_CPSI_LENS.md).

**In plain language:**

- **Purity** and **concurrence** are different inputs. Purity measures mixedness of
  a density matrix; concurrence is a two-qubit entanglement monotone. Neither may
  be substituted for the other when copying a crossing time or closed form.
- **Ψ (coherence)** is the normalized l1 sum of off-diagonal density-matrix
  entries in the stated basis, so it is basis-dependent.
- **CΨ = ¼** is the discriminant-zero parameter of `u²−u+CΨ=0`.
...
| Symbol | Definition |
|--------|------------|
| **θ** | arctan(sqrt(4c - 1)) for the real recurrence parameter `c=CΨ>1/4`. Angular distance of the recurrence's complex fixed-point pair from the 1/4 discriminant boundary; it does not make either real-valued CΨ diagnostic complex. |
...
|---------------|------------|
| **Π** | Conjugation operator. Per-site action: I→X, X→I, Y→iZ, Z→iY. Within its Hamiltonian/channel scope it satisfies `Π·L·Π⁻¹ = −L−2Σγ·I` and transports the full complex eigenvalue `λ` to `−λ−2Σγ`, including generalized eigenspaces and multiplicity. This is one member of the P1 family. A second family P4 (I↔Y, X↔Z) supports additional terms (XZ, ZX). XY/YX terms require non-uniform (alternating) operators. See [Mirror Symmetry Proof](proofs/MIRROR_SYMMETRY_PROOF.md), [Non-Heisenberg Palindrome](../experiments/NON_HEISENBERG_PALINDROME.md). |
...
| **Palindromic pairing** | The **full complex** eigenvalue multiset is closed under the linear map `λ ↦ −λ−2Σγ`, including multiplicity, within the Hamiltonian/channel family admitting the F1 palindromizer. Equivalently `μ=λ+Σγ` maps to `−μ`. Complex-conjugate closure is separate; composing it with F1 gives `λ ↦ −conj(λ)−2Σγ`, fixed on the line `Re λ=−Σγ`, while linear F1 has only the point `λ=−Σγ` fixed. Proven for Heisenberg/XY/Ising/XXZ with local single-axis dephasing on arbitrary graphs. DM uses a different alternating map and is covered on bipartite interaction graphs, whatever their edge orientation. Depolarizing noise is a breaking control. The direct spectral census reaches N=8. |
...
- **Π (Pi)** is the explicit operator that proves F1 within its stated Hamiltonian and channel scope. It transports the whole complex spectral value `λ`, not merely a decay rate. It does not by itself supply complex conjugation, stability, or a spatial wave.
...
- **Σγ (total noise)** is the sum of the local Z-dephasing coefficients. In an F1-compatible generator it fixes the affine shift of the mirror. Changing its distribution across sites can still change the spectrum and transport, so profile comparisons hold the total fixed when they intend to isolate placement. Outside F1's scope, a spectral mean does not supply a palindrome.
...
- **Palindromic pairing** means that an allowed generator carries every complex eigenvalue to its affine negative. The N=2 through N=8 computation checks that scoped statement; it does not promote every open-system spectrum to a palindrome. Keeping only real parts throws away the frequency condition F1 also imposes.
...
## The light content, and what "dark" names

A mode's **light content** ⟨n_XY⟩ is the mean number of {X, Y} letters it carries, a
number in [0, N]. The **Light and lens** row above defines it; this section only names it
and says which words may stand for it.

**The quantity and the ratio are two things, joined by a hypothesis.** At a UNIFORM rate
γ, Theorem 1 of [the Absorption Theorem](proofs/PROOF_ABSORPTION_THEOREM.md) gives
Re(λ) = −2γ·⟨n_XY⟩, so there the ratio −Re λ/(2γ) reads the light content off the
spectrum. Under a per-site profile Theorem 2 applies instead, Re(λ) = −2 Σ_l γ_l·light_l(v),
the quotient by any single 2γ is not a light count, and it is not bounded by N. The proof
says so about exactly this elision: a restatement that drops the word "uniform" is not a
shorthand for Theorem 1, it is a different claim.

**More light, shorter life.** ⟨n_XY⟩ is what the dephasing charges for, so a mode carrying
more of it is gone sooner, and a mode carrying none is immortal.

**Do not name that quantity a darkness.** It measures the light, so under that noun the
immortal mode is the one with "darkness 0", while the repo's own comparative reads the axis
the other way: "the darkest `[H,A]=0` commutant coherence" is a MINIMUM of ⟨n_XY⟩
(`StructuralCeilingWitness.CommutantDarkest`, the smallest nonzero eigenvalue of N_XY on the
ad_H kernel), which is the longest-lived end. A reader meeting both in one file cannot tell
which end is meant. "Above" and "below the floor" need the same care: some sites read them
in Re units, where below means a larger ⟨n_XY⟩, and others in ⟨n_XY⟩ units, where it means a
smaller one.

**"Dark" is a spent word, and this section does not own all of it.** These senses are all
live and they are not one object:

| Sense | Where |
|---|---|
| a MODE carrying little light, the long-lived end | "the darkest `[H,A]=0` commutant coherence"; the third of the eleven axes listed in [the neural palindrome note](neural/ALGEBRAIC_PALINDROME_NEURAL.md) |
| the F135/F136 **record class**, an exact zero in the record laws below | the sense [The Seat That Cuts](../experiments/THE_SEAT_THAT_CUTS.md) calls "the glossary's sense" |
| the **lens** letters {I, Z} themselves, against the light {X, Y} | `CrossoverMirrorSqrtNinetyClaim.cs` (`Dark = { 0, 2 }`), `chain_gap_sector_diagnostic.py` |
| a site with γ_l = 0, a seat with no light on it | fenced at its own door in [The Motion and the Missing Phase](../experiments/THE_MOTION_AND_THE_MISSING_PHASE.md), together with two of the others |

The list is not a census, and the comparative is not uniform either:
[Naked vs Chain](../experiments/NAKED_VS_CHAIN_BENCHMARK.md) calls the bonding mode's
palindromic partner the "dark twin" although it carries the COMPLEMENTARY, larger light
content and decays nearly as fast as a naked qubit. Name the end you mean rather than
trusting the word.

**Glyph note.** `Symphony.LightContent(ρ)` computes this same number on a STATE,
Σ popcount(i⊕j)·|ρ_ij|² / Σ |ρ_ij|². It is ⟨n_XY⟩ for any operator, not only for an
eigenmode, because the Pauli strings of XY-weight k span exactly the matrix units with
popcount(i⊕j) = k. What does not carry over is the RATE: a state on a trajectory is a
superposition of modes, so its light content obeys no Absorption-Theorem identity.

**In plain language:** one number says how much of a mode stands in the light, and the more
of it there is the sooner the mode is gone. Dark is the far end of that number, where the
watching finds almost nothing to read and the mode lasts. The word is borrowed by several
other things in this repo, so it is worth checking which one is meant.
...
| Term | Definition |
|------|------------|
| **Concentrator** (formerly "sacrifice zone") | A specified edge-heavy Z-dephasing profile tested for peak created Sum-MI under a fixed total-noise budget. It is not a profile-independent optimum and does not imply that the edge carries no relevant information. |
| **Concentrator formula** | `gamma_edge = N*gamma_base − (N−1)*epsilon`, `gamma_other = epsilon`. This parameterizes the edge-heavy profile; its advantage belongs to the stated chain, objective, and epsilon-floor comparison. |
...
- **Concentrator** is an edge-heavy noise profile that scored well for one transport observable. It does not literally absorb heat or protect an interior; those are readings laid over the measured Sum-MI trajectory.
- **ENAQT** studies how an environment can change transport. The repo's large ratios compare one epsilon-floor spatial profile with selected baselines using peak created Sum-MI; they are not directly comparable to every efficiency figure in that literature, and no priority claim follows from this glossary.
...
| **Fragile Bridge** | Coupled gain-loss system (one side decays, the other amplifies, Σγ_total = 0). In the executed sweep of two coupled two-qubit chains, the spectral abscissa leaves zero and off-axis quartets appear; Π supplies λ ↔ −λ and Hermiticity preservation supplies λ ↔ λ*. At generic couplings the threshold is a second-order exceptional point on the real γ axis: in the first block of L to go unstable, two eigenvalues on the imaginary axis meet in a Jordan block and leave the axis as a mirror pair. At J_bridge = 1.0 and 1.9 that block is its own spin-flip image and the full L has one eigenvector for the two, read from the rank of L − λI at the located collision; at other couplings the collision can appear in two or four blocks at once, copies under the spin flip and under the chain reflection combined with a one-sided spin flip. At five exact couplings (J_bridge = 3/4, √5/2, 4/3 and two cubic roots near 1.0294 and 3.3289) an even and an odd frequency of one block already coincide without gain, and the threshold is zero, with no exceptional point: the bridge is unstable at every gain sampled there, from 10⁻⁶ to 2. The sampled maximum is bracketed at J_bridge/J ∈ [1.8, 2.0]. In the large-coupling sample γ_crit·J_bridge decreases from 0.578 at J_bridge=10 to 0.508 at 100 and has not converged; existence and value of an asymptotic limit remain open. See [Fragile Bridge](../hypotheses/FRAGILE_BRIDGE.md), [PT-Symmetry Analysis](../experiments/PT_SYMMETRY_ANALYSIS.md). |
...
| Value | Source |
|-------|--------|
| **1/4** | Discriminant of the fixed-point equation. Below 1/4: real fixed points exist. Above: complex. Algebraically exact within the iteration. The fixed-point quadratic of the Mandelbrot map z ↦ z² + c carries that same discriminant, so 1/4 is also the cusp of the main cardioid on the real axis ([F97](proofs/PROOF_F97_CARDIOID_HALF_FIXED_POINT.md)). What forces the value is the Rényi order: for R = C(Ψ+R)^α the fold threshold is (α−1)^(α−1)/(α^α·Ψ^(α−2)), which depends on the state at every α except 2, and at α = 2 it is 1/4. The uniqueness is in the state-independence, not in the value; other orders reach 1/4 too, at particular Ψ. |
...
| **15.5 bits** | Local linearized Gaussian-channel diagnostic at one `N=5` operating point and assumed 1% feature noise. It is conditional on the selected features, power constraint and linearization; it is not demonstrated global capacity or arbitrary-profile recovery. The directly tested noiseless alphabet has four profiles. |
