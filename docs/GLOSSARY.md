...
- **CΨ** is concurrence times coherence. This single number captures the overall "quantumness" of the system. ¼ is where its own iteration changes character. In its Mandelbrot form, u² − u + CΨ = 0, the fixed-point equation of z ↦ z² + c with c = CΨ, the two fixed points always add up to 1 and multiply to CΨ, and two numbers adding to 1 can multiply to at most ¼, reached when both are ½; so below ¼ the pair is two real numbers and above ¼ it is complex. Why the recursion has that shape at all, and so why the threshold is ¼ rather than something else, is a separate argument: see the ¼ row of the constants table below. Crossing ¼ is not the instant a system stops being quantum. A Bell⁺ pair under a local Markovian channel does fall through ¼ once and stay below ([Monotonicity](proofs/PROOF_MONOTONICITY_CPSI.md)), and under Z-dephasing its trajectory, its coherences and its populations all pass through the point smoothly ([Exclusions](EXCLUSIONS.md)). But a pair sitting inside a chain is another matter: at strong coupling its own CΨ crosses ¼ repeatedly, in both directions, coherence pumped back into it by the rest of the chain. ¼ is a level that can be re-entered, not a door that shuts.
