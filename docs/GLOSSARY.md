...
| **Palindromic pairing** | The **full complex** eigenvalue multiset is closed under the linear map `λ ↦ −λ−2Σγ`, including multiplicity, within the Hamiltonian/channel family admitting the F1 palindromizer. Equivalently `μ=λ+Σγ` maps to `−μ`. Complex-conjugate closure is separate; composing it with F1 gives `λ ↦ −conj(λ)−2Σγ`, fixed on the line `Re λ=−Σγ`, while linear F1 has only the point `λ=−Σγ` fixed. Proven for Heisenberg/XY/Ising/XXZ with local single-axis dephasing on arbitrary graphs. DM uses a different alternating map and is covered only when the oriented interaction graph is compatibly bipartite (the chain is verified). Depolarizing noise is a breaking control. The direct spectral census reaches N=8. |
...
| **Fragile Bridge** | Coupled gain-loss system (one side decays, the other amplifies, Σγ_total = 0). In the executed two-two-qubit sweep, the spectral abscissa leaves zero and off-axis quartets appear; Π supplies λ ↔ −λ and Hermiticity preservation supplies λ ↔ λ*. Exceptional-point character remains OPEN because no branch continuation or strict threshold coalescence/Jordan certificate has been executed. The sampled maximum is bracketed at J_bridge/J ∈ [1.8, 2.0]. In the large-coupling sample γ_crit·J_bridge decreases from 0.578 at J_bridge=10 to 0.508 at 100 and has not converged; existence and value of an asymptotic limit remain open. See [Fragile Bridge](../hypotheses/FRAGILE_BRIDGE.md), [PT-Symmetry Analysis](../experiments/PT_SYMMETRY_ANALYSIS.md). |
