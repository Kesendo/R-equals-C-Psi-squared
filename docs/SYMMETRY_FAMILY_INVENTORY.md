...
| 20 | HardCellYInversionPattern | YParity (term-level Z₂) | F87-hard pairs only in diagonal Klein cells (Aspect A closed-form via F108 Part 1+2+3 + F107 + F109 + F87 dissipator-resonance) with Y-inversion structural pattern (dominant y_par in hard cell equals dephase letter's own y_par; Z/X→0, Y→1) and k-purity sharpening (42:8 @ k=3 → 228:0 @ k=4 with Y-inversion preserved) | Tier1Candidate; F110, added 2026-05-25. Seventh YParity-axis Claim; completes the y_par-axis classification of the F87 trichotomy together with F107 (truly) and F109 (mother soft). Aspect A closed-form, Aspect B+C empirically anchored at F103/F105/F106; closed-form derivation of 42:8 / 228:0 ratios open. F111 closes Aspect B at k=N=4 via the Pure-D Template Rule (Tier1Candidate) | `HardCellYInversionPattern` (added 2026-05-25) |
| 21 | HardCellPureDTemplate | YParity (term-level Z₂) | at k=N=4 in diagonal Klein cell (D.BitA(), D.BitB()) for dephase D, pair is F87-hard iff at least one term is a "pure-D template" (length-4 string with only D and I letters). Per-cell decomposition: 36 Pure-Pure (HARD) + 192 Pure-Mixed (HARD) + 300 Mixed-Mixed (SOFT) = 528 pairs, 228 hard. Implies F110 Aspect B at k=4 as immediate corollary (pure-D templates have y_par=y_par(D) by construction) | Tier1Candidate; F111, added 2026-05-25. Eighth YParity-axis Claim, sharpens F110 Aspect B. Subclaim (a) pure-D=hard heuristically derived via dissipator-commute mechanism; subclaims (b/c/d) empirical. Subclaim (d) Mixed+Mixed=soft is the blocking subclaim for Tier1Derived promotion (Task 1 BLOCKED after exhausting Path 1/2/3 operator search). Empirical anchor: 1584 pair classifications across 3 dephases match the rule, zero exceptions | `HardCellPureDTemplate` (added 2026-05-25) |
...
| 23 | LindbladBitBPiBreakMagnitude | BitB (Π² = (−1)^bit_b) | closed-form magnitude for the F112 polarity-asymmetry counterexample when F112's typed scope is violated by the canonical Z-drive × amplitude-damping interference: for Lindblad-form L with Hermitian H containing single-site Z-drives Σ_l (ω_l/2)·Z_l plus any bit_b-homogeneous additions (each F112-balanced) and dissipator c containing σ⁻_l rate γ_T1,l + σ⁺_l rate γ_pump,l per site, asymmetry = (4^N / 2) · Σ_l ω_l · (γ_T1,l − γ_pump,l) bit-exactly. Structural origin in [Z, σ⁻] = −2·σ⁻ (proportional to the non-Hermitian σ⁻ itself, producing Π-eigenspace ±i imbalance); X- and Y-drives give Hermitian commutators that remain F112-symmetric, hence contribute 0. Per-site additivity from same-site locality [Z_l, σ⁻_m] = −2·σ⁻_m · δ_{lm}. Helpers: `PredictAsymmetry(ω_l, γ_T1,l, γ_pump,l, N)`, `PredictAsymmetryUniform(ω, γ_T1, γ_pump, N)` | Tier1Derived at N=2, 3, 4 (bit-exact via constructive parameter sweep + per-site / cross-site / sign-flip / detailed-balance / non-uniform-rate verification, `simulations/_f113_break_formula_derivation.py`); F113, added 2026-05-26. Tier1Candidate for general N: the (1/2)·4^N coefficient is N-universal in form but rigorous algebraic derivation from Π-eigenspace structure is open. Sister to F112 on the same bit_b axis: F112 covers in-scope balance (asymmetry = 0), F113 the out-of-scope counterexample magnitude; together complete polarity-axis description of the standard Lindblad family. Ctor parent: F112 (LindbladBitBPiBalance). BitBSpecific BitATwin slot (Z-axis single-site drives intrinsically bit_b, no meaningful bit_a analog). σ⁻ / σ⁺ convention note documented in inspectables: F113 derivation script's SIGMA_MINUS is a raising-style matrix, C# T1Dissipator uses standard physics σ⁻ (lowering); asymmetry magnitudes match bit-exactly, signs flip between the two conventions. Hardware fingerprinting: Welle 2 f95 fit (ω=0.13, γ_T1≈0.001, N=2) gives 16·0.13·0.001 = 2.08e-3, matching Kingston bit-exact (experiments/F112_HARDWARE_LENS_KINGSTON.md) | `LindbladBitBPiBreakMagnitude` (added 2026-05-26) |
