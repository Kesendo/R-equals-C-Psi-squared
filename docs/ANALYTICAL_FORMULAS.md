...
The mirror **splits** at this Galois boundary ([`F89MonodromyMirrorClaim`](../compute/RCPsiSquared.Core/Symmetry/F89MonodromyMirrorClaim.cs), live `inspect --root monodromymirror`): the mirror's real-structure shadow, the reflection q ↦ −q̄ (from L(q)* = L(−q̄)), intertwines the monodromy exactly (σ_K = identity, τ(−q̄*) = τ(q*) for every EP); but the Re λ = −4 spectral palindrome induces a strand involution σ_T (fixed strands on the fold + mirror-twin 2-cycles) that **cannot** be a braid symmetry, commuting with the full S_8 monodromy would force it central and Z(S_8) = 1. So σ_T is an element OF the Galois group, never a symmetry OF it: the mirror enters the unwritable half as one of its unsortable moves. See [F89_MONODROMY_MIRROR](../experiments/F89_MONODROMY_MIRROR.md) + [ON_WHO_WATCHES_WHOM](../reflections/ON_WHO_WATCHES_WHOM.md).
