...
**The cross-fold is integrability-INDEPENDENT (the (q,Δ) extension, Tier 1 derived 2026-06-30).** The same antiunitary similarity holds for the FULL interacting XXZ block L(q,Δ) at *every* anisotropy Δ, not just the integrable XY one:

    L_(1,N−2)(q̄, Δ)  =  −P · conj(L_(1,2)(q, Δ)) · Pᵀ  −  2N·I

machine zero for N = 4..9 at every q (real and complex) and every Δ. The reason is a parity: the Δ·ZZ frequency term (the only addition over the XY block, a diagonal −i·q·Δ·(zz(ket) − zz(bra)) with zz(c) = Σ_bond ⟨c|Z_bZ_{b+1}|c⟩) is **even** under the global bit-flip that the bra-complement performs, because Z_bZ_{b+1} ↦ (−Z_b)(−Z_{b+1}) = Z_bZ_{b+1}, so zz(b̄) = zz(b). The bra-complement therefore carries the ZZ term across the fold unchanged, exactly as it carries the XY structure. This sharply separates two questions the arc kept adjacent: the **diabolics themselves die** under Δ (integrability-protected, they flip defective, the arc's Δ-test / Move 2), but the **pairing structure does not**: a diabolic and its cross-fold partner turn defective in lockstep, the two blocks staying antiunitary-similar at every Δ. The discriminant is **bit-flip parity**: a bit-flip-*odd* perturbation breaks the fold. A longitudinal Z-field Σ_k w_k Z_k has fieldEnergy(b̄) = −fieldEnergy(b) (each Z_k flips sign), so its diagonal frequency is *not* preserved by the bra-complement and the residual is O(1), not machine zero (verified: residual ≈ 5.46 at N=6). So the cross-fold is a **structural/algebraic** property of the Z-dephasing Liouvillian, not a free-fermion (integrability) artifact: it survives any bit-flip-even bond coupling (ZZ anisotropy, longer-range ZZ) and breaks only under a bit-flip-odd one (a longitudinal field). This corrects the arc's tentative handover guess (which expected the AT-rate flip vs the ZZ-invariance to *break* the fold at Δ≠0); the invariance is exactly what makes it survive. **Live:** the Δ=0 and Δ=0.7 residuals plus the field control are children of `inspect --root crossfold`; the Core gate is `WeightCoherenceBlockTests` (the `Build(n, wKet, wBra, q, Δ)` overload). **Source:** [F89_PATH_K_DIABOLIC](../experiments/F89_PATH_K_DIABOLIC.md) (the cross-fold section).

**F89d is one leg of a Klein four-group, and docks onto the F1 palindrome Π (Tier 1 derived 2026-06-30).** The same antiunitary similarity holds at **every** ket weight, not just wKet=1, and it is one of two antiunitary legs of a Klein four-group of bit-flip similarities on the (wKet, wBra) coherence-block lattice (verified machine zero at all weights, N=5,6,7, all q, all Δ):

- **Bra leg P** (flip the bra index, right-multiplication ρ·F, F = X^⊗N): (wKet, wBra) → (wKet, N−wBra), antiunitary, `L(wKet,N−wBra)(q̄,Δ) = −P·conj(L(wKet,wBra)(q,Δ))·Pᵀ − 2N·I`. This is F89d.
