...
Equivalently, with no bonus term (2026-06-04): **D_k = 2^{max(0, ⌊(k-5)/2⌋)} · k² / 2^{min(v₂(k), 2)}.** The identity v₂(k) + max(0, v₂(k) − 2) = 2·v₂(k) − min(v₂(k), 2) folds the k-self and deep-bonus terms into one cancellation cap: the eigenvector-norm k² loses at most a factor of 4. The "deep-2-power bonus" is an artifact of pulling odd(k)² out front, not a separate 2-adic mechanism. The cap itself is reduced (two independent routes, 2026-06-04) to one isolated integer-valuation lemma: the chain is rigorous modulo the 2-adic valuation of a single Chebyshev coefficient (the leading reduction step adding exactly one factor of 2), see [`PROOF_F89_PATH_D_CLOSED_FORM`](proofs/PROOF_F89_PATH_D_CLOSED_FORM.md) § "Why the cap is 2^min(v₂,2)". Verified bit-exact k=3..300, equivalence proved k=3..600 (`simulations/_f89_dk_clean_form.py`), reduction chain bit-exact v2=0..6 (`simulations/_f89_capA_mastertable.py`, `simulations/_f89_capB_chain.py`).
