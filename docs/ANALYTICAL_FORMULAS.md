...
- Distinct from the T1/depol siblings below, whose per-site kernels are not proportional to I. Whether their residuals keep a cross-site `(Σγ)²` term is a matter of where σ sits, not of the channel: at each channel's own spectral mean (σ = Σγ for depolarizing, σ = Σγ/2 for T1) the per-site trace vanishes and only `Σγ²` remains, `(16/9)Σγ²` and `3Σγ²`; the T1 formula below is recorded at σ = Σγ_Z, where `(Σγ)²` appears.
...
- ‖M(T1)‖²_F = 4^(N−1)·[3·Σγ²+4·(Σγ)²]  (H-independent, γ_Z-independent; bit-exact N=2..5), at the F1 shift σ = Σγ_Z (zero for pure T1). The norm is a parabola in σ with its minimum at T1's spectral mean, σ = Σγ_Z + Σγ/2 (Σγ the T1 rates), where it is 4^(N−1)·3·Σγ²: the (Σγ)² term is the squared diagonal mean, as for depolarizing noise.
...
with hopping 2J.
The block eigenvalues match to < 1e-12. Separately, the raw-SFF producer
reports an FFT candidate within 1% of omega_1 at N=2-4 and N=6, but does not
identify that candidate at N=5 or N=7
([Spectral Form Factor](../experiments/SPECTRAL_FORM_FACTOR.md)).
...
in the dissipator's Pauli-cell spectrum, which is a ladder with rung spacing
2γ. A full-Liouvillian eigenmode instead pays the expectation of that cell
cost: Hamiltonian mixing can make ⟨n_XY⟩ non-integer and place its real part
between the cell rungs.
...
Full period of the k=1 frequency in D10's (0,1) coherence block. Its partner
at -omega_min, at the same rate 2γ, is its complex conjugate in the (1,0)
block (L commutes with ρ ↦ ρ†), so the pair contributes `2*cos(omega_min*t)`
to the frequency trace amplitude of the rate-2γ band. F1 carries that band to
its mirror at 2σ − 2γ with the same cosine, since λ ↦ −λ − 2σ also negates
the frequency. The complete SFF squares the full amplitude
and also contains doubled and cross frequencies. The period grows as
~N^2/(pi*J) for large N. The finite-N producer finds an FFT candidate within
1% at N=2-4 and N=6, but not among its reported candidates at N=5 or N=7.

**Valid for:** Heisenberg chain, the (0,1) coherence block.
**Replaces:** numerical evaluation of the D10 k=1 period; the FFT association remains a sampled diagnostic.
**Source:** [Spectral Form Factor](../experiments/SPECTRAL_FORM_FACTOR.md)

### F42. Finite-N raw-density-scale comparison (Tier 2, measured N=2-7)

    f = sort({ |Im(lambda)| : |Im(lambda)| > 1e-10 }, multiplicity retained)
    S_raw = 2*pi / mean(diff(f))
    reported ratio = t_Pi / S_raw

`S_raw` retains repeated frequencies. Duplicating every entry changes the mean
adjacent gap and therefore changes `S_raw`, even though the normalized SFF is
unchanged at every time. It is a descriptive scale of this finite multiset,
not a physical Heisenberg time. The ratios below report only the executed
finite-N arrays; they do not establish a short/long-time boundary or an
N→∞ law.

| N | t_Pi | S_raw | t_Pi/S_raw |
|---|------|------|----------|
| 3 | 3.14 | 61.3 | 0.051 |
| 5 | 8.22 | 497 | 0.017 |
| 7 | 15.9 | 5810 | 0.003 |

**Valid for:** the raw, non-unfolded frequency multisets exported for the N=2-7 uniform Heisenberg chains under Z-dephasing. Multiplicity and the producer's frequency cutoff are part of the definition.
**Replaces:** a physical-timescale reading with the finite-N raw-scale report the producer actually computes.
**Source:** [Spectral Form Factor](../experiments/SPECTRAL_FORM_FACTOR.md)

### F43. Palindrome-paired band SFF (Tier 1, proven D09)

    I' = {2*N*gamma - d : d in I}
    K_freq(I, t) = K_freq(I', t)

The full-spectrum palindrome maps `lambda=-d+i*omega` to
`-(2*N*gamma-d)-i*omega` with algebraic multiplicity preserved. Therefore
reflected decay-rate bands have frequency multisets related by sign and equal
...
frequencies are zero, so their normalized frequency SFF is the constant 1 for
every time (unnormalized `(N+1)^2`): a delta spike at zero frequency, not an
impulse at time zero. The count N+1 at both ends is F158's two-end count,
dim ker L = dim ker(L + 2σ) = N+1 on this chain, read there by exact ranks
(the typed join is code-level: `F43BandSffPairingPi2Inheritance.EndpointMultiplicity`
returns `PalindromeTwoEndCountClaim.CanonicalChainCount`). A finite-width producer
bin may include additional modes and must report its own count. At `gamma=0`
the endpoints coincide at zero and merge into the generally larger commutator
kernel, so the N+1 endpoint count is not asserted there.

**Valid for:** mirrored decay-rate bands under the F1 palindrome; the average-light reading assumes uniform local Z-dephasing. The N+1 endpoint count assumes a connected chain in the stated F23/F4-kernel scope and strictly positive uniform dephasing.
**Replaces:** the fixed-XY-weight-sector interpretation of the numerical rate bins.
**Source:** [Spectral Form Factor](../experiments/SPECTRAL_FORM_FACTOR.md)

### F44. Algebraic pair-rate log identity (Tier 1, proven D08)

    ln(d_fast / d_slow) = 2 * artanh(Delta_d / (2*Σγ))

For each finite ordered positive-rate palindromic pair (d_fast, d_slow) with
`Delta_d = d_fast-d_slow`, `0 <= Delta_d < 2*Σγ`, and
`d_fast + d_slow = 2*Σγ`. The strict upper endpoint excludes `d_slow=0`,
where the logarithm diverges. The exact API and its linear approximation use
this same finite domain.
Linear approximation: ln(d_fast/d_slow) ~ Delta_d / Σγ,
whose algebraic linear coefficient is 1/Σγ.

This follows only from d_fast + d_slow = 2*Σγ. It is not a Crooks
fluctuation theorem: no forward/reverse protocol probabilities, work,
free-energy change, or temperature are defined. The separately computed
<exp(-Delta_d)> ~ 0.93 is an ad hoc transform of dimensional rate
differences, not a Jarzynski test.
...
### F64. Effective gamma from cavity mode exposure (Tier 1-2, analytical; verified N=3,4 chain + N=5,7 across five topologies)
...
**General N.** Diagonalize the N×N tridiagonal single-excitation Hamiltonian. Find the eigenvector with the smallest |a_B|² among those with nonzero |a_S|². No layered composition: the formula is a global eigenvector property, not a product of per-layer factors. Multiplicative stacking fails at N=4 (ratio 0.04 to 62); the H-eigenvector formula is first-order accurate (ratio 1.0000 ± 0.0003 at the tested γ_B/J).

**Replaces:** time-domain exponential fit for γ_eff extraction.
**Valid for:** any graph topology (chain, star, ring, complete, tree), uniform or non-uniform per-bond J, XX+YY or Heisenberg single-excitation, Z-dephasing on any single site B. The H-eigenvector reading is first-order in γ/J and degrades as γ/J grows (relative error ∝ (γ/J)², F65 "Perturbative nature": ≈ 4·10⁻³ at γ₀/J = 0.05, N=5); the L_coh-eigenvector form (EQ-015) is exact at every γ/J, so what breaks in the bad cavity is the H-eigenvector *reading*, not the rate law.
...
The two poles are palindromic partners under the conjugation Π. Their exact endpoint eigenspaces are the x=0 and x=N ends of F43's reflected average-light bands. The single-excitation sector (F65) never reaches either pole for N ≥ 3; both poles live in the extreme XY-weight subspaces (w = 0 and w = N). Both claims of that last sentence are endpoint-B statements; see the Scope correction below.
...
**Hardware signature.** Under Kingston-grade Z-dephasing (T₂ ∈ [240, 320] μs, γ = 1/(2T₂)) the F69 optimum crosses CΨ = 1/4 monotonically at t* ≈ 22.4 μs. A single 2-qubit tomography at t = 0 distinguishes GHZ_3 (0), W_3 (0.123), and F69 (0.320) as three separable points, no timing needed.
...
**Valid for:** any Hamiltonian with \[H, R\] = 0 (uniform coupling on a symmetric graph), any dissipator with \[D, R_sup\] = 0 (uniform or R-symmetric dephasing), any exactly R-invariant initial state. For initial states that are R-symmetric only up to coherence signs (even-k ψ_k + vac), the sign flip must additionally be a local symmetry of L: \[L, U_sup\] = 0 with U = (−1)^n̂ = ∏ Z_i. XY hopping, Z-dephasing and amplitude damping all satisfy this; a uniform transverse field satisfies \[H, R\] = 0 but not \[L, U_sup\] = 0, and the mirror fails at linear order in the field (see the proof's Scope section and [`c1_mirror_scope_probe.py`](../simulations/c1_mirror_scope_probe.py)). Purely kinematic.
...
**Verified:** ψ_1+vac at N = 3, 4, 5, 6 (residuals < 2·10⁻⁹, largest 1.32·10⁻⁹ at N = 6) and ψ_2+vac at N = 4, 5, 6 (residuals < 5·10⁻¹⁰). Source: [`eq021_obc_sine_basis.py`](../simulations/eq021_obc_sine_basis.py), [`c1_veffect_scaling_small.py`](../simulations/c1_veffect_scaling_small.py).
...
**Corollary: the lit site (γ₀ is the F73 term's only rate; J enters as a curvature).** The sum is conserved against H; one of its terms is not, and the way it leaks answers, for this term, whether the waves bring a second rate of their own beside γ₀. Light one site: |α⟩ = |1_l⟩, realised as the pure state ψ = (|vac⟩ + |1_l⟩)/√2, whose (vac, 1_l) coherence is the probe's. Read site l in two ways, its F73 term p_l(t) = 2·|(ρ_l)_{0,1}(t)|² and its purity P_l(t) = Tr(ρ_l(t)²). Then, for every H in F73's class,

    p_l(0) = 1/2,  P_l(0) = 1
    p_l'(0) = P_l'(0) = −2γ₀
    p_l''(0) = P_l''(0) = 8γ₀² − V_l,   V_l = Σ_{j≠l} |⟨1_j|H|1_l⟩|²

and at γ₀ = 0 both readings lose (V_l/2)·t² + O(t⁴). On the XXZ graph family H = Σ_b J_b (X X + Y Y + Δ_b Z Z) + Σ_i h_i Z_i (Pauli convention, single-excitation hopping 2J_b) this is V_l = 4·Σ_{b∋l} J_b², so

    loss(t) = 2 · Σ_{b∋l} J_b² · t² + O(t⁴),   = 2 · deg(l) · J² · t² at uniform J,

on any graph, at every Δ_b and every longitudinal field. In the (XX + YY)/2 hopping form J_b (XX + YY)/2 + Δ_b Z Z of the F73 sweep script the hopping is J_b and the same law reads (1/2)·deg(l)·J²·t²; the convention-free statement is V_l/2.

*Derivation.* By the proof above x(t) = e^{−2γ₀t}·U_SE(t)·e_l, so p_l = (1/2)·e^{−4γ₀t}·S(t) with S(t) = |⟨1_l|e^{−i(H_SE − E_vac)t}|1_l⟩|². S is even in t (the amplitude at −t is the conjugate of the amplitude at t), and S(t) = 1 − t²·V_l + O(t⁴) with V_l = (H_SE²)_{ll} − (H_SE)_{ll}² = Σ_{j≠l}|(H_SE)_{jl}|², the energy variance of |1_l⟩. Differentiating the product gives the two derivatives above. For P_l: P_l = n_l² + (1 − n_l)² + p_l with n_l(t) = ⟨1_l|ρ_l(t)|1_l⟩ the excitation weight of site l, and n_l = S/2 + O(t³) (γ₀ enters n_l only from third order, through the (SE, SE) coherences that the first order creates). With n_l(0) = 1/2 and n_l'(0) = 0 the population part n_l² + (1 − n_l)² has vanishing first and second derivatives at t = 0, so P_l shares p_l's. Δ_b and h_i are diagonal in the computational basis: they shift (H_SE)_{ll} and E_vac and never an off-diagonal element, and a variance is blind to the diagonal. That is the whole Δ-independence.

*Reading.* For the F73 term the answer holds at every t, not only at t = 0: p_l = (1/2)·e^{−4γ₀t}·S(t) exactly, with S free of γ₀, so γ₀ is its only rate and H only modulates it, through S, a finite sum of phases. J enters first at t², as a curvature, and the loss does not accumulate: S(t) = |Σ_k |⟨ψ_k|1_l⟩|²·e^{−iE_k t}|² is a finite sum of phases, hence almost periodic, and returns arbitrarily close to 1. What leaves the lit site stays in F73's conserved sum (the other sites' terms). So in this term the waves bring no second rate beside γ₀; they bring a recurring curvature whose size counts the lit site's bonds. The purity says less: it agrees with p_l through the third derivative at t = 0 and first departs at the fourth, P_l''''(0) − p_l''''(0) = 12·V_l², because its population part is not γ₀-free: n_l'''(0) = 4γ₀·V_l, the (SE, SE) coherences that the first order creates being damped at 4γ₀ rather than by the scalar 2γ₀ of the (vac, SE) block. The corollary says nothing about the purity's rates beyond t = 0.

*Where it holds.* Exact as Taylor coefficients at t = 0 (no short-time approximation in them; "short time" refers only to reading the loss off its leading order), for any Hermitian H with \[H, N_total\] = 0, uniform γ₀, any graph, any N. U(1) is sufficient, not necessary: on the gated cases a transverse field on any site, the lit one included, leaves both second derivatives standing, and a pairing term G·(XX − YY) on the lit site's bond (J_x ≠ J_y) turns that bond's J_b² into (J_b − G)², which breaks the law for every G other than 0 and 2J_b.

*Sweep before writing.* F73's entry and proof (this entry; no separate proof file), the F73 claim `F73SpatialSumPurityClosurePi2Inheritance` and its tests, MirrorWorld's F73 pin (`Formulas.F73_SpatialSumClosure`, `Marginal`), the Diagnostics half (no F73 witness), `docs/CAUGHT_ERRORS.md` (the internal-origin non-exhaustiveness entry of the incompleteness review, which concerns the dissipator's origin and not this reading), `fw.Confirmations` (no F73 entry), the glossary (J-blind receivers read beside F73; the light/lens row) returned no statement of the lit-site loss. Adjacent and not the same object: the OpenArcs arc `noise_origin_after_candidate_two`, which states the closed-system side (trace(L_H) = 0 for every H, so a unitary source has no decay), while the corollary is an open-system statement for this one term, where H adds no rate because uniform dephasing is a scalar on the (vac, SE) block; `docs/PREDICTIONS.md` (the row "Discriminate a specified memory model from Markovian alternatives") with [Algebraic Exploration](../experiments/ALGEBRAIC_EXPLORATION.md)'s Markovian control, a single qubit with H = Y under dephasing, which lies outside F73's class and moves the slope as well as the curvature (ξ′(0) = 3/2 and ξ″(0) = −7, against −1/2 and 0 at H = 0), whereas inside the class the slope stays −2γ₀ and H only curves, by V_l; both say that curvature alone is not a memory certificate; [The Motion and the Missing Phase](../experiments/THE_MOTION_AND_THE_MISSING_PHASE.md)'s leakage p_out = ε²t² (a detuned-bond leak of an encoded state, the same energy-variance mechanism in a different setting), and [Mathematical Findings](../experiments/MATHEMATICAL_FINDINGS.md) §8's Tr(ρ²) = 0.5 + 0.5·cos²(3Jt), an unspecified-state closed-system subsystem purity whose short-time curvature the corollary does not claim to reproduce.
...
- Lit-site corollary, exact over ℚ(i) (the derivatives are 𝓛ρ₀ and 𝓛²ρ₀ on the full 2^N density matrix, compared with ==): chain N = 4 (degrees 1, 2), star N = 5 (centre degree 4, leaf degree 1), complete graph N = 4 (degree 3), each at Δ ∈ {0, 1, 2, −7/3} and γ₀ ∈ {0, 1/20}; an inhomogeneous ring N = 4 with a different J_b and Δ_b on every bond and longitudinal fields, every site, γ₀ ∈ {0, 1/7}; V_l read off H equals 4·Σ_{b∋l} J_b² in every case. The F73 term equals (1/2)·e^{−4γ₀t} times its γ₀ = 0 series through sixth order (chain, star, complete graph at N = 4, Δ = 2, γ₀ ∈ {1/20, 3/7, 1/3}), while the purity agrees with the term through k = 3 and departs at k = 4 by 12·V_l², and n_l'''(0) = 4γ₀·V_l. The pairing mutation at G ∈ {1/2, 3, −1} fails the law on both readings and equals 8γ₀² − 4(J − G)² exactly; G = 0 and G = 2J pass; a transverse field on every site passes. [`F73LitSiteCurvature`](../compute/RCPsiSquared.Core/Symmetry/F73LitSiteCurvature.cs), [`F73LitSiteCurvatureTests`](../compute/RCPsiSquared.Core.Tests/Symmetry/F73LitSiteCurvatureTests.cs); the live value (star centre, P_l''(0) = −16) sits on the F73 claim's inspect node.
...
**Pytest lock:** `test_F81_pi_conjugation_of_M` (algebraic check) + `test_F81_pi_decompose_M_method` (cockpit primitive) + `test_F81_violation_T1_diagnostic` (T1 diagnostic) + `test_F81_identity_bearing_odd_terms_enter_H_odd` (identity-leg terms reach H_odd) + `test_F81_pi_decompose_M_materialises_its_terms` (a generator must not decompose the zero Hamiltonian) + `test_F81_single_letter_terms_are_refused_not_silently_dropped` (a body count the builders cannot place raises). C# twins: `IdentityBearingOddTerm_EntersHOdd_SoTheIdentityHolds` and its counterpart `IdentityBearingTrulyTerm_StaysOutOfHOdd`.
...
**Mechanism (T1 dissipator structure).** Single-site T1 acts on Pauli basis as: I → +γZ, X → −γ/2 X, Y → −γ/2 Y, Z → −γZ. Under Π² conjugation (signs (-1)^{bit_b}: I,X → +, Y,Z → −), only the (Z, I) entry flips sign. So D_{T1, local, odd} has matrix element +γ at (Z, I) and zero elsewhere. Multi-site: 4^(N−1) such "rest of qubits unchanged" entries per site, summed orthogonally over sites. That sign is the one place here where a convention shows: the framework's lowering operator is σ⁻ = (X+iY)/2 = [[0, 1], [0, 0]], taking |1⟩ → |0⟩. The other operator, (X−iY)/2 = [[0, 0], [1, 0]], which is the textbook σ⁻ when |0⟩ is read as spin-up rather than as the ground state, gives −γ instead. The other three rows and the closed form above are convention-independent, the closed form because it reads only the magnitude, which is why a norm check cannot see this sign at all. Derived entry by entry in [PROOF_F82_T1_DISSIPATOR_CORRECTION](proofs/PROOF_F82_T1_DISSIPATOR_CORRECTION.md).
...
**Mechanism (Step 2 of proof, why factors 4 and 8 differ).** The F49 chain Frobenius identity gives ‖M‖² = Σ_k 2^(N+2)·n_YZ(k)·‖H_k‖²_F·𝟙\[non-truly\], where n_YZ(k) counts Y/Z letters in Pauli pair k (= 0 truly, 1 Π²-odd non-truly, 2 Π²-even non-truly). Read the sum per Π²-class, not per term: the quantity that enters is the norm of the class's combined sub-Hamiltonian, which differs from the per-term sum whenever two terms of one class land on the same Pauli string (possible once a term carries an identity leg, or on a topology whose bonds revisit a site). Substituting the per-class n_YZ values gives the 4·2^N (Π²-odd) and 8·2^N (Π²-even non-truly) coefficients. Geometrically, these reflect the Frobenius-inner-product behavior ⟨Π·L·Π⁻¹, L⟩_F: anti-aligned (truly), Frobenius-orthogonal (Π²-odd non-truly), aligned (Π²-even non-truly).
...
**Valid for:** any 2-body H on any topology supported by F49 (chain, ring, star, complete K_N), Z-dephasing, any γ_z ≥ 0, any N ≥ 2. Terms carrying an identity letter (a bilinear with an identity leg, such as (Z, I)) are included: the Π²-class is the parity of #Y and #Z, which an identity letter does not change, and the per-class Frobenius norms are read off the built sub-Hamiltonians either way. Whether such a term is a uniform site field depends on the topology; see the [F81](#f81) entry for what the bond graph actually builds.
**Verified:** 11 mixed configurations × N ∈ {3, 4, 5} on chain, plus 4 configurations × {ring, star, K_4} at N=4, all machine-precision residual. Identity-bearing terms checked separately at N ∈ {3, 4} on chain, ring and star against the numerical `pi_decompose_M`, which reaches ‖M‖² without the Π²-classifier and is therefore the reference that can disagree on it. Both its ‖M‖² and its ‖M_anti‖² are classifier-free, since M_anti is (M − ΠMΠ⁻¹)/2 and the classifier touches only L_H_odd. What it does share is the Hamiltonian builders, so a builder bug would move both. The F49 sibling `predict_residual_norm_squared_from_terms` is asserted too, but as a consistency tie only: it shares classifier and builders alike and agrees bit-for-bit, so it can catch a divergence between the two copies and nothing else.
...
**Pytest lock:** `test_F83_pi_decomposition_anti_fraction_closed_form` (12 cases spanning pure-odd, pure-even-non-truly, all-truly and mixed, × 3 N-values + γ-independence) + `test_F83_predict_pi_decomposition_full_closed_form` (full norm-triple match against numerical `pi_decompose_M` + Pythagoras + special cases at canonical r values) + `test_F83_identity_bearing_terms_are_in_scope` (identity-leg terms, cross-tied to the numerical route) + `test_F83_predictors_materialise_their_terms` + `test_F83_single_letter_terms_are_refused_not_silently_dropped`. C# twin: `IdentityBearingTerms_AreInScope_NotSilentlyZero`.
...
|--------|---------|------|------|
| F86a | EP mechanism: t_peak, Q_EP, dressed pair, Π-centered P-type L_eff mirror | Tier 1 derived | §F86a; `TPeakLaw.cs`, `QEpLaw.cs`, `LEffMirrorAxisClaim.cs` |
...
universal across c, N, n, and bond position. Higher-k toy-model EPs decay faster (1/(8γ₀), 1/(12γ₀), ...) and are masked by the slowest. **At the empirically sampled Q_peak the Dicke probe has more weight in H-mixed modes** than on the large-Q plateau. This is a Tier-2 empirical weight redistribution; it does not identify the full block's Q_peak with the two-level toy EP, nor isolate a causal complex-conjugate eigenvalue pair. (Specific W values W_peak ≈ 0.99 / W_plateau ≈ 0.31 at Q=20 are unverified anchors retained by `DressedModeWeightClaim`, not universal constants; empirical W_peak ranges 0.832 [N=4 c=2] to 0.9996 [N=9 c=3] per [the Q-scale three-bands study](../experiments/Q_SCALE_THREE_BANDS.md) lines 95–123; per-(c, N) closed form and any full-block EP mechanism remain open.)
...
**Verified:** N=3 36-enum (14 hard / 19 soft / 3 truly); N=3, 4, 5 120-enum (15 truly / 46 soft / 59 hard, N-stable); Marrakesh hardware Δ(soft − truly) = −0.722 (2026-04-26); Marrakesh F83 4-class signature 2026-04-30 (ibm_marrakesh job `d7pol1e7g7gs73cf7j90`).
...
- **F89c**: the structural lemma, dissipator cell cost 2γ₀·n_diff plus the full-block column-flip relation that supplies the spectral pair-sum (below).
...
**Structural lemma F89c (Tier 1 derived).** For a computational-basis coherence |A⟩⟨B|, the **dissipator's diagonal cell cost** is 2γ₀·n_diff: its Pauli decomposition has n_XY = n_diff because each site is purely {I,Z} or purely {X,Y}. This is not in general the real part of a full-Liouvillian eigenvalue. For a full eigenmode v, the Absorption Theorem instead gives Re λ = −2γ₀·⟨n_XY⟩_v, which may be fractional after Hamiltonian mixing. For (k+1)-qubit blocks: k = 1 is the unique case where every populated coherence cell has n_diff = 1 (DE = |11⟩ contains both block sites, so no no-overlap (SE,DE) pairs exist); k ≥ 2 has cell costs {2γ₀,6γ₀} from n_diff ∈ {1,3}. Hamming complement sends those cell costs as k ↔ N−k. The full-eigenvalue pair-sum 2γ₀N is stronger and follows only after the exact column-bit-flip block relation, including its Hamiltonian part, is established (verified bit-exact at path-2 across all 9 pairs). Hence the all-isolated topology (1)^m is the **unique** single-rate-envelope-plus-single-frequency case. Derivation, per-sector eigendecomposition, Hamming-complement bijection table: [F89_TOPOLOGY_ORBIT_CLOSURE](../experiments/F89_TOPOLOGY_ORBIT_CLOSURE.md) F89c section.
...
machine zero for N = 4..9 at every q (real and complex) and every Δ, because the Δ·ZZ frequency is **bit-flip-even** (zz(b̄) = zz(b)), so the bra-complement carries it across the fold unchanged. Under a sampled Delta no (SE,DE) crossing is certified at N=4..7, and every zero `XxzCoherenceBlock` certifies inside its seed's isolation disk (on the full reflection-even block) is a Jordan EP2; the exact similarity carries each certified character across: each EP2 and its cross-fold partner turn defective in lockstep (read live at N=7, Delta=0.02). The similarity transports a certified character; it does not turn a small sampled gap into one. The discriminant is bit-flip parity, so a bit-flip-*odd* longitudinal field breaks the fold (residual O(1), the negative control). Full argument and live/gate anchors (`inspect --root crossfold`, `WeightCoherenceBlockTests`): [F89_PATH_K_DIABOLIC](../experiments/F89_PATH_K_DIABOLIC.md) §"The cross-fold survives XXZ anisotropy".
...
**Character: DIABOLIC (semisimple), NOT a defective EP (Tier 1 derived).** The two eigenvalues coalesce (a genuine double root) but the eigenvectors stay independent: g1 = g2 = 2, rank(L − λ_EP·I) = n−2, departure-from-normality dep ≈ 0, the 2×2 restriction L|₂D = λ·I (scalar, no Jordan coupling), ‖P‖ ≈ 3.88 finite (obliquely embedded / non-normal, but NOT a Jordan block; ‖P‖ measures the embedding, not the character). The exact factor (3q⁴+q²−1)² locates the degeneracy and, after the one colliding pair is isolated, supports an analytic-crossing reading. It does **not** prove semisimplicity: that verdict comes from the twin-scalar restriction and is checked live by [`EpCharacter`](../compute/RCPsiSquared.Core/Numerics/EpCharacter.cs) (`inspect --root f89octic`). This is the **diabolic** member of the EP-character trilogy, in contrast to the **defective** coherence-horizon √-EP, whose isolated transverse pair has a simple discriminant zero and nonzero Jordan coupling.
...
**WHY diabolic (Tier 2 hypothesis).** At Delta=0 the exact N=4 character fact is the twin-scalar restriction at q_EP: H_eff (= 2iJ·I, from a 4-fold free-fermion multiplet, since DE energies are sums of SE energies) and the dephasing (= −4γ·I at the overlap-½ midpoint) both restrict to scalars on the 2D coalescing span, so L|₂D = λ_EP·I, with no off-diagonal Jordan coupling. Free-fermion additivity is the proposed explanation for the H-half, but the general protection principle remains interpretive and conditional. Under a sampled XXZ anisotropy (Delta = 0.02 to 0.5) the crossing splits into two Jordan EP2s at real q on Re λ = −4, certified on the full reflection-even block (alg 2, geo 1, each a simple zero of the pair discriminant); this measures a change of character but does not isolate integrability as its cause, since a diabolic also unfolds into EP2s under a generic perturbation. No commuting-symmetry separation supplies the Delta=0 result (site-reflection R, the overlap↔no-overlap involution, and the chiral Σ all fail to separate the two modes). See [hypotheses/DIABOLIC_BY_INTEGRABILITY.md](../hypotheses/DIABOLIC_BY_INTEGRABILITY.md).
...
The diagonal-block spectral content is preserved on the anti-palindromic locus, and the asymmetry is carried by the F71-cross-blocks. These block quantities are basis-relative once F71 is broken: they are not eigenvalues of L, so they are not the decay rates of anything, and the physical rates do move. Analogous to F81 Π-decomposition: M_anti = L_{H_odd} is the antisymmetric part captured by Π-conjugation; the γ-anti-palindromic part plays the analogous role in γ-parameter space.
...
**Tier outcome <Tier 1 derived>:** algebraic proof complete (2026-05-12, see PROOF_F91 § Algebraic proof). The proof's sharper conclusion: the F71-refined diagonal-block matrix elements of L = −i[H, ·] + D are linear functionals of γ that depend only on the **indexed F71-pair-sums, the assignment l ↦ S_l = γ_l + γ_{N−1−l}**, never on individual γ_l or pair-differences D_l = γ_l − γ_{N−1−l}; pair-differences appear only in the F71-cross-block off-diagonal entries (which do not enter diagonal-block eigenvalues). The dependence is on the assignment and NOT on the bare multiset {S_l}: at N=6, γ_A = [0.7, 0.2, 0.5, 0.3, 0.6, 0.4] and γ_B = [0.4, 0.4, 0.55, 0.55, 0.4, 0.4] share Σγ and the multiset {1.1, 0.8, 0.8} yet their diagonal-block spectra differ by 8.69. The anti-palindromic class S_l = 2γ_avg ∀l is the fixed-point set of R_{90} : γ_l ↦ 2γ_avg − γ_{N−1−l}, and on that set R₉₀ acts as the identity, so what makes its members agree is within-pair redistribution at fixed indexed S_l, not the action of R₉₀. The originally claimed F91 (90°-invariance) is the corollary of that indexed-pair-sum law. Empirical witness at N=4, 5, 6 across the five γ-profiles above remains the bit-exact verification.
...
For the N-site XY chain (Δ=0, open ends, arbitrary bond profile `J_b`) under local Z-dephasing (arbitrary rates `γ_j`; the SETTING, not a blanket scope for the entry: the intertwiner identities below are profile-general, the edge-normality lemma is UNIFORM-γ only and false without it, fenced where it is stated), define with the Jordan-Wigner **site** fermions `c_l` (strings included) the site-summed spectator
...
**W is the η-pairing ladder, and F142 is where its name lives.** Reading the bra index as a second fermion species turns `Σ_l c_l† ρ c_l` into `Σ_l c†_{l↑}c†_{l↓}`, so the site-summed spectator IS Yang's η-pairing, unstaggered; the identity above is F142's Lemma 2.2 with the Φ case of its Lemma 2.1. F142 also carries the SECOND ladder, `S⁺(ρ) = Σ_l (−1)^l d†_l ρ d†_l`, stepping sideways rather than up. That is this arc's §6 map V with the stagger restored: V fails on the Hamiltonian part, S⁺ does not, and the price is scope, since S⁺ needs ΣhΣ = −h and so covers only the pure-hopping opposite-parity part of the H class named above, on-site potentials and disorder excluded.
...
**Containment orbit corollary (the diamond, containment direction ONLY).** The orbit of (1,2) under {one climbing W-step, the transpose (p,q̃)↔(q̃,p), the Klein full flip X^⊗N, the F89d cross-fold q̃↦N−q̃ at q̄} reproduces the census transport sets exactly: the |p−q̃|=1 band united with its cross-fold image, with the **parity law** that a diagonal core (p,p) is reached iff **|2p−N| = 1**, i.e. iff N is odd, and sizes **4N−12 (N even) / 4N−8 (N odd)** (N=4: the confined 4-orbit; N=5 and N=6: two different 12-sets, the N=6 one without (3,3)). Every band sector carries the shared eigenvalue λ and every fold-image sector its cross-fold partner −λ̄−2N; a Jordan chain is transported wherever the starting locus carries one, known at the locus's grade, certified at N=5,7,9 and classified at N=11 (the census's two families of six at N=5; byte-identity across a family holds at the real loci because the coalescing eigenvalue is real there, while at N=4 the locus eigenvalue is complex and the 4-orbit is accordingly conjugate-related, not byte-identical); the EXCLUSION half (no braid outside) is DERIVED at N=5 (2026-07-03): the boundary blocks by normality (at UNIFORM γ, the edge-normality lemma's hypothesis; this half does not extend to a rate profile), the four interior non-member cores by the fold-resultant certificate pair (the identity-composition (1,1)×λ_A certificate + remainder R1's corner-fold certificate) at every branch locus q ≠ 0 of both parities, propagated by the Klein full flip and the composed holomorphic (1,4)-fold spec(1,4)(q) = −spec(1,1)(q) − 2N (gate `RemainderR4InteriorExclusionTests`, Category R4INTERIOR); beyond N=5 it stays census-evidence, since 2026-07-04 through N=9 and (2026-07-06) N=11 in full (the step-3 σ_min shell census: membership = this diamond at all 7 certified N=9 seeds + all 9 classified N=11 loci, shared-λ separation ×3.7e8..×4.6e11 (N=9 and N=11 combined), the six N=11 blocks the LP64 dense wall deferred now resolved by the Tasks 5-6 sparse σ_min two-instrument path, `sparse-witness` members (rigorous from-above bound) + `sparse-invit` cores (estimate), `PASS (witness-assisted; classified defective; uncertified)` at every N=11 locus, members at the Jordan depth ~(gap/2)²; instrument `SectorShellCensus`, results in the census experiment). The census's first light also DERIVED the R-parity grading of the maps: **R·W = (−1)^{p+q̃}·W·R** (the JW-string reflection sign), so the carried value's R-parity ALTERNATES along the band chain (seed parity at odd band index p, flipped at even p; fold/transpose preserve it; diagonal climbs commute strictly).

**Edge-normality lemma (where the sharing must stop; UNIFORM γ only).** On the edge blocks (0,1), (1,0), (N−1,N), (N,N−1) every coherence has `n_diff = 1`, so at a uniform rate `A = −2γ·I` and `L(q) = −2γ·I + q·C` is a NORMAL pencil at real q (C anti-Hermitian): no Jordan block can live on an edge. Uniformity is the hypothesis and it carries the lemma: under a per-site profile `A = −2·diag(γ_j)` is still normal by itself but no longer scalar, and the conclusion is FALSE, not merely unproven, a defective EP being exhibited on this block at N = 4, 5, 6 by turning one rate ([PROOF_EDGE_BLOCK_DEFECTIVE_UNDER_PROFILE](proofs/PROOF_EDGE_BLOCK_DEFECTIVE_UNDER_PROFILE.md) §(e), with §(b) leaving a repeated eigenvalue on a path no form but a Jordan block). Do not read that as a profile generically defectivizing the block: what a profile does generically is make it NON-NORMAL and SPREAD the shared rate over the γ-window, while the defectiveness sits on a codimension-one hypersurface, measure zero in rate space, and sitting on it needs the rates set finely enough that the square-root split stays small. Do not carry a single tolerance out of here: the proof's constant is read on ONE of its seven witnesses, and across all seven the implied requirement spans more than an order of magnitude, so a device number has to be derived for the case at hand. A universal fell; a common phenomenon was not found. What replaces the exclusion under a profile is a LOCATION, and it holds AT REAL q, where the Hermitian part of the pencil equals A: the defective eigenvalue must sit strictly inside the γ-window `[−2γ_max, −2γ_min]`, which is a point at uniform γ and returns this lemma as its zero-width case. At complex q the window moves instead (the moved Bendixson, proof §6). Interior boundary, now DERIVED at real loci by the **rate-window lemma** (proof §6, gate item 8): at real q every block eigenvalue obeys `Re λ = v†Av/v†v ∈ [−2·n_max, −2·n_min]` (Bendixson bound in n_diff units, i.e. at uniform γ = 1; the edge lemma is the zero-width case THERE, and the γ-window of the previous paragraph is a different window, an edge block having zero n_diff width at every profile while its A is scalar only at a uniform rate). The corner block (p_c+1,p_c+1) has window `[−2(N−3), 0]` while `Re λ_B = −Re λ_A − 2N` sits below it whenever `Re λ_A > −6` (measured margins 1.381/2.208 at the two N=5 real loci, 1.115 at N=7), so λ_B is absent from the corner spectrum and W kills the core's whole generalized eigenspace, `(L_corner−λ)^m` being invertible: the measured `‖Wx₁‖ = 1.7e-15`, `‖Wx₂‖ = 2.5e-15` at q* = 0.620878, plus the out-of-sample locus 2 (`9.6e-16`). At the seed locus the window also derives the full N=5 exclusion (interior four window-excluded, boundary blocks rung-pinned at uniform γ; condition `Re λ_A ∈ (−6,−4)`). The N-uniform strictness `Re λ_A(N) > −6` is DERIVED at real loci by the **window-edge lemma** (a defective EP cannot sit at either edge of its block's rate window: edge ⟹ joint `A,C` eigenvector ⟹ semisimple, the classical numerical-range boundary fact, and the arrows run one way only, the contrapositive being what the argument consumes; the converses are false, a semisimple eigenvalue being free to sit strictly inside; so `Re λ_A ∈ (−6,−2)` for all N ≥ 4, and the shrinking seed-locus margins 1.381→1.115 never reach zero, a strict real-part inequality needing no uniform floor). The complex-q loci are CLOSED at N=5: near-axis by the moved rate window (H(q) = A + Im(q)·K), every remaining branch locus by the fold-resultant certificate, both parities (proof §6).

**Theorem A (the two regimes of silent crossings).** (i) AT-locked crossings: two AT-locked eigenvector families are analytic through a crossing and remain independent, so their codim-1 coincidences are semisimple. The 528 crossings in one N=6 box are diabolics of this AT-locked kind, not residual A2 diabolics. (ii) Residual coalescences: twin-scalar restriction on the isolated 2-plane is sufficient for semisimplicity. Free-fermion additivity supplies the H-scalar restriction only under an independently certified single-multiplet descent; additivity does not establish that premise. The D-scalar restriction is separate. Both restrictions are established directly at the N=4 point. At N=5, `TwinScalarDHalfTests` measures scalar D and full restrictions to 5e-10…4e-9 on the sampled genuinely-complex-q representatives, but does not certify their multiplet descent; the pure-imaginary-q family is semisimple by Hermiticity (‖L−Lᴴ‖<1e-14). The residual mechanism therefore remains conditional and Tier 2. The test is inner-product-sensitive and runs in the HS-orthonormal coherence basis, not the non-orthonormal ×2-cleared orbit basis.

**Delta-tightness.** Loss of the W/H intertwining identity is an algebraic statement: the XXZ interaction adds quartic terms, so the free-fermion cancellation no longer supplies that identity; the D-part still intertwines. The W argument therefore no longer guarantees the diamond's shared spectrum. At N=4/N=5/N=6/N=7 the Delta=0 crossings are Diabolic (alg=geo=2, a double zero of the pair discriminant); at every sampled positive Delta no crossing is certified, and every zero the certifier locates inside its seed's isolation disk is a Jordan EP2 (alg 2, geo 1, a simple zero), certified on the full reflection-even block (N=4 at Delta=0.02: real q = 0.6569356380 and 0.6602489409, both on Re λ = −4). The F89d cross-fold survives by its independent bit-flip identity. The exact W/H boundary and the certified finite-N Delta response are separated in [PROOF_CODIM1_BY_ADDITIVITY](proofs/PROOF_CODIM1_BY_ADDITIVITY.md) §8.

**Tier labels** (per the proof's Status line): Theorem B and the containment corollary Tier 1 derived (exact operator identities, gate machine-zero); the exclusion half DERIVED at N=5 (boundary normality AT UNIFORM γ + the fold-resultant certificate pair on the interior four, propagated by Klein + the composed holomorphic (1,4)-fold; gate `RemainderR4InteriorExclusionTests`; beyond N=5 census-evidence), the interior-core kernel death derived at all real loci and, via the moved window + the certificate, at all complex loci (N=5); Theorem A's AT-locked half Tier 1 derived, its residual half a conditional twin-scalar statement (mechanism Tier 2, proven at the N=4 point). Gap byte-identity across sectors: DERIVED (implied by the full-spectrum holomorphic fold spec(3,3)=−spec(2,3)−2N, an isometry on eigenvalue gaps; §7, gate `HolomorphicFoldIdentityTests`). The holomorphic diamond fold itself is now a Tier-1 corollary of F89d + transpose + W (§7), closing remainder 1's last analytic entry.
...
which steps sideways off the diagonal onto the two neighbouring lines.

**Both ladders are F125's, under other names.** Φ is F125's site-summed spectator W, the same map under a different letter: that entry calls the chain's Jordan-Wigner fermion c where this one calls it d, so its W(ρ) = Σ_l c_l† ρ c_l is Σ_l d†_l ρ d_l verbatim. The doubled-species form Σ_l c†_{l↑}c†_{l↓} above is the same object again, and note that c means the chain fermion in F125 and a doubled species here. Its Theorem B is Lemma 2.2 below with the Φ case of Lemma 2.1. And S⁺ is that document's §6 map V = Σ_l c_l†(·)c_l† with the stagger restored: V fails the Hamiltonian part, S⁺ does not, and the (−1)^l is the whole difference. What F125 supplies in return is the sharp transport premise, a Jordan chain moving exactly when its eigenvector avoids ker Φ. Measured beside them, typed 2026-08-09 as `SidewaysSpinLadderClaim` (parent F125's `SpectatorIntertwinerClaim`; live witness `inspect --root sideways`): the F125 fold family is exactly the interiors of the two S⁺ chains at p + q̃ = N∓1, each of interior length N−2 carrying spin ℓ = (N−3)/2, and the S⁺ transport norms are the Clebsch-Gordan coefficients √(ℓ(ℓ+1) − m(m+1)) with no free parameter, derived from the closed form and measured at N=7 as 2, √6, √6, 2 and at N=9 as √6, √10, √12, √12, √10, √6 (each predicted before its run, the arc entry the record; N=9 walked past the dense wall by LU shift-invert, `SidewaysSpinLadderSparse`). The two η chains carry the same spin by the count alone, fold + band = 4N−8 = 4(N−2) at odd N being the multiplet-dimension accounting; their transport norms are untested. Read the word band carefully across the link: this entry's band is |p − q| ∈ {0, 2} with j_η = N/2 − 1, F125's is |p − q̃| = 1 with spin (N−3)/2, and they are different sets. (open arc `sideways_spin_ladder`; the chain walk is gated at four N-groups, by [`simulations/eta_ladder_chain.py`](../simulations/eta_ladder_chain.py) and by `SidewaysSpinLadderGateTests` in C# at N=5,7, by `SidewaysSpinLadderSparseTests` at N=9, and at the even N=4 by [`simulations/eta_ladder_chain_n4.py`](../simulations/eta_ladder_chain_n4.py) + `Gate_N4` (all four complex-λ loci, band and fold freight a conjugate pair on the shared 4-orbit, ℓ = 1/2; the arc entry carries the port's calibration), with Φ on the chain rungs as the exact-zero control throughout; the full rung sweep for BOTH ladders beyond the chains is measured and not gated, `simulations/eta_ladder_blocks.py`).

Both
commute with the dephasing at any rate profile (F141), and

    [K, S⁺] = 0   ⟺   Σ h Σ = −h,   Σ := diag((−1)^l)   (real symmetric h, Lemma 2.3's hypothesis),
...
### F154. The conditional locus saturation law: where every reflected site-density contrast compresses to zero, which is a theorem on the all-scalar territory and derived on the N = 8, Δ = 0 mixed spaces, the R₉₀-locus spectrum lies in its size-class-centre interval and, at γ̄ > 0, a multi-class block reaches both ends exactly when the extreme classes have pure vectors in colliding eigenspaces (subject to the stated nondegenerate-space census); at N = 11, Δ = 0 the contrast fires on `(1,1)` and the identity fails there, while an independent `(1,1)` interval theorem holds and both ends of that block sit in its scalar zero-frequency room, where the identity is a theorem (registered 2026-08-15; N=11 extension 2026-09-23)
    ...
    s_min(block) = |p − q| and s_max(block) = min(p + q, 2N − p − q):
        ...
        Δ = 1, N = 8), and DERIVED on the N = 8, Δ = 0 mixed spaces by the
        mixed-space reflection law (PROOF_MIXED_SPACE_REFLECTION_LAW);
        at Δ = 1, N = 6 the ω = 0 extreme carriers satisfy the identity,
        while the breaking interiors are contained by the separate
        mirror-transversal certificate for every physical locus profile;
        the convex hull of the block-wide spectrum equals that interval, with
        spread 2γ̄·(s_max − s_min), on a MULTI-CLASS block at γ̄ > 0
...
theorem. Its breaking interiors are contained for every physical locus
profile by the corner reduction and finite closed-form certificate in
[THE_MIRROR_TRANSVERSAL_CERTIFICATE](../experiments/THE_MIRROR_TRANSVERSAL_CERTIFICATE.md),
including the six tight corner spaces. The SU(2) resonance carries
...
On these `(1,1)` spaces the one-legged mirror contrast cancels, so
C_l = −2Π_Ω(|ll⟩⟨ll|−|mm⟩⟨mm|)Π_Ω, m=N−1−l: the contrast is the signed
agreement/double-occupancy overlap. This is the physical primitive whose
uniform-rate zero-frequency Gram form appears in
...
**What it gives on the covered rows at γ̄ > 0.** The strong-coupling Re-extremes and spread of a locus block come
...
64·eps·‖A‖/gap, breaks gated from below); the Δ = 1, N = 6
breaking-space containment is
[THE_MIRROR_TRANSVERSAL_CERTIFICATE](../experiments/THE_MIRROR_TRANSVERSAL_CERTIFICATE.md)
with gate [`simulations/locus_containment_gate.py`](../simulations/locus_containment_gate.py);
the mixed-space derivation is
[PROOF_MIXED_SPACE_REFLECTION_LAW](proofs/PROOF_MIXED_SPACE_REFLECTION_LAW.md)
with gate
...
The Krylov line is the DEFINITION and the primitive: ranks computed exactly in
GF(p) on the integer sector matrix at two primes, no eigensolver and no tolerance.
Their maximum is a lower bound on the rational rank, so the resulting blind count
is an upper bound; two-prime agreement alone does not certify rational equality.
The uniform-chain count is exact by its separate node/gcd derivation. The
committed Python route uses 2³¹−1 and 2⁶¹−1, agreeing everywhere and checking
the Heisenberg law through N = 21; MirrorWorld's `BlindSeat` uses 998244353 and
1004535809 through the world's shared `ModP`, taking the larger rank since a rank
mod p can only drop, its run mode re-checking laws against rank at N = 60, 120,
200, past the wall.
...
mode `blind N`, 39 from-below tests including a dynamic one-seat-lit Cone pin on a blind state that MOVES, a blind eigenvector being a fixed point that pins nothing;
...
`inspect --root blind` (`SeatCutBlindnessWitness`, the count read by two GF(p)
Krylov ranks as a rational upper bound and the span by a second, independent elimination,
