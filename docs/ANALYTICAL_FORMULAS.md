...
The asymptotic decay rate is 4γ₀ universal across all m (matches F73's vac-SE rate exactly). The m-dependence enters only through a periodic correction with frequency 4J that vanishes at cos(4Jt) = 1 (period π/(2J)). The empirically observed "clustering at t ≈ 20" for J = 0.075 is the in-phase moment t ≈ π/(2J) ≈ 21, NOT a slow-mode equilibrium. Derivation: the Lindbladian factorises over disjoint blocks plus bare sites; in each 2-qubit block the populated coherences sit in the (vac, SE)_B and (SE, DE)_B-overlap sectors only, both decaying at rate 2γ₀ per coherence (one site differing in the basis-state pair). The 6γ₀ no-overlap-SE-DE channel cannot be populated on a 2-qubit block alone (requires single-excitation site outside the doubly-excited pair), so the asymptotic rate is universally 2γ₀ per coherence (4γ₀ on |·|²). Per-block H_B-eigenstate phase tracking gives the cos(4Jt) interference between vac-SE and SE-DE-overlap contributions to (ρ_l)_{0,1}.
...
**Structural lemma F89c: why all-isolated is the unique clean case (Tier 1 derived).** L_super decomposes over computational-basis coherence sectors. The (vac, SE)_B sector of any (k+1)-qubit block has uniform per-coherence rate 2γ₀ (every basis pair has n_diff = 1) and yields a clean sub-block of L_super with eigenvalues E_k − 2iγ₀ for H_B SE eigenstates k. The (SE, DE)_B sector has rates ∈ {2γ₀, 6γ₀} (overlap-count-determined): for 2-qubit blocks (k = 1) the unique DE state |11⟩ contains both sites so every SE state overlaps it, **no no-overlap pairs exist**, and the (SE, DE) sub-block also has uniform rate 2γ₀. For (k+1)-qubit blocks with k ≥ 2 no-overlap SE-DE pairs exist and L_super^{(SE, DE)} has mixed dephasing rates coupled by H_B, producing eigenvalues in [2γ₀, 6γ₀] continuum (e.g. path-2 3-qubit block has rates {2γ₀, 2.556γ₀, 2.889γ₀, 3.112γ₀, 3.444γ₀, 4γ₀, 6γ₀}; the (SE, DE)_B sub-block alone contributes {2.0, 3.112, 3.444, 4.0, 6.0}γ₀, while the additional fractional rates 2.556 and 2.889 come from H_B mixing in the (SE, SE)_B sector). **Column-bit-flip pair structure (Absorption Theorem corollary)**: per [PROOF_ABSORPTION_THEOREM](proofs/PROOF_ABSORPTION_THEOREM.md), every Pauli-string eigenvalue is Re(λ) = −2γ₀·n_XY. A computational-basis coherence |A⟩⟨B| has n_XY = n_diff(A, B) (each site decomposes as pure {I, Z} when bits agree, pure {X, Y} when they differ), so per-coherence rate = 2γ₀·n_diff. The column bit-flip ρ[a, b] → ρ[a, bar(b)] uses the Hamming complement identity n_diff(a, b) + n_diff(a, bar(b)) = N to map rates 2γ₀·k ↔ 2γ₀·(N − k); rate-pairs sum to 2γ₀·N = the spectral maximum (Absorption Theorem section 4.1). For (k+1)-qubit blocks with k+1 = 3 (path-2), bar(SE) = DE elementwise (popcount complement at N=3) so the column flip maps the (SE, SE) sub-block to the (SE, DE) sub-block; L_super (SE, SE)_B and (DE, DE)_B sub-blocks have bit-exact identical eigenvalues, and (SE, DE)_B eigenvalues are exactly {6γ₀ − λ : λ ∈ (SE, SE) eigenvalues} (every Hamming-complement pair sums to 2γ₀·N = 6γ₀ exactly; trace check trace(SE,SE)/9 + trace(SE,DE)/9 = (−24γ₀ − 30γ₀)/9 = −6γ₀, so mean(rates)_SS + mean(rates)_SD = 6γ₀). Verified by per-sector eigendecomposition in [`_f89c_cross_sector_pair_structure.py`](../simulations/_f89c_cross_sector_pair_structure.py). Therefore the all-isolated topology (1)^m is the **unique** topology where S(t) has a single-rate-envelope-plus-single-frequency closed form. Pi2-Foundation a_{−1} = 4 anchors cleanly precisely because there's a single dephasing rate to anchor. Verified by numerical Liouvillian eigendecomposition at k = 1, 2, 3 in [`_f89c_liouvillian_eigenstructure.py`](../simulations/_f89c_liouvillian_eigenstructure.py).
