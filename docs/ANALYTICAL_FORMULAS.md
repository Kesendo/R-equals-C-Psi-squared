...
The same-sign-imaginary off-diagonal structure is what admits an EP at finite J·g_eff = 2γ₀ (verified numerically: opposite-sign +iJg, −iJg gives discriminant 4γ₀² + J²g_eff² with no EP; same-sign gives 4γ₀² − J²g_eff² with EP). This is "PT-phenomenology-like" (EP at finite coupling, spectral flow); the centered Liouvillian L_c sits in **class AIII chiral** per [PT-Symmetry Analysis](../experiments/PT_SYMMETRY_ANALYSIS.md) (Π linear, Π⁴=I, {Π, L_c}=0), distinct from Bender-Boettcher PT (classical PT requires anti-linear operators). Q_EP = 2/g_eff is the genuine EP of the toy 2×2 rate-channel reduction (a SEPARATE object, not "the instance of" the full-block classification; in isolation its eigenvalues coalesce at centre −4γ₀·k and carry no λ↔−λ pairing). The chiral-breaking transition in [The Fragile Bridge](../hypotheses/FRAGILE_BRIDGE.md) is a DISTINCT genuine EP, the SEPARATE Σγ=0 gain-loss system (centre 0, exact λ↔−λ pairing), and it sits on the REAL γ axis at γ_crit itself: Re λ ∝ √δ over four decades and the leading mode's Petermann factor diverges as 1/δ. The full Σγ=N·γ₀ block-L carries its own third kind: F89's real-axis defective seeds at scattered q\* (existence forced at every odd N by the nullity count; defectiveness census-verified through N=11, the codim-2 β-exotic genericity still open for all N; the retraction-era grid could not see them, see [the F86a EP-mechanism proof](proofs/PROOF_F86A_EP_MECHANISM.md) §The real-axis EP). Away from the seeds the block is non-normal without coalescence. What stays OPEN in [`LocalGlobalEpLink`](../compute/RCPsiSquared.Core/F86/LocalGlobalEpLink.cs) (OpenQuestion) is the DISTINCT off-real-axis complex-Q EP structure and its FRAGILE_BRIDGE connection specifically.

**2026-05-06 / retracted 2026-06-21.** Local-vs-global EP relationship (now `LocalGlobalEpLink` **OpenQuestion**, demoted from Tier2Verified by the F86a-retraction): the shared algebra is read at Σγ = N·γ₀ vs Σγ = 0 (two residuals of F1 `Π · L · Π⁻¹ + L + 2Σγ · I = 0`), but the prior reading, that the real-Q Petermann-K sweep at c=2 N=5..8 (`compute/RCPsiSquared.Core.Tests/F86/F86PetermannProbe.cs:Probe_PetermannFineGrid_C2_VsN`) is a real-axis hit of FRAGILE_BRIDGE's complex-γ EP, does NOT survive artifact-free re-verification. At the sweep's grid the full block shows no eigenvalue coalescence on the real Q axis (gaps ~0.25–0.35 at the sampled points); it is genuinely NON-NORMAL there (cond(V) = 48.7 / 50.9 / 268.5 at N = 5 / 6 / 7, confirmed artifact-free via the Riesz spectral-projector norm ‖P‖ on a simple eigenvalue), with the Petermann factor large but FINITE. RETRACTED as grid artifacts: the peak magnitudes (K = 1333.6 / 337.9 / 2384.7 / 795.4 swing 2–4× over ΔQ = 1e-3), the "N=7 spike ≈ 6× above FRAGILE_BRIDGE's K = 403", and the "2-4× odd/even parity-asymmetry confirming σ_0 R-even/R-odd-degeneracy". The genuine EPs established at the time of the retraction were the toy 2×2 reduction and the SEPARATE Σγ = 0 gain-loss system (FRAGILE_BRIDGE, K=403); F89 then established the third kind (census 2026-07-04, reconciled into F86a 2026-07-07): the block's own real-axis defective seeds at odd N, sitting in windows the sweep's dQ ≈ 0.029 grid could not resolve (`F86aSeedMaskingTests`: at the N=9 seed q\* = 0.849011 the pair is a Jordan block at gap ~2.7·10⁻³, yet ~0.35 apart one grid step away). Encoded as `compute/RCPsiSquared.Core/F86/LocalGlobalEpLink.cs` (OpenQuestion, scope reconciled 2026-06-27 and 2026-07-07: what remains open is the DISTINCT off-real-axis complex-Q EP structure and the FRAGILE_BRIDGE connection, not the block's real-axis defectiveness at the seeds); the four sweep rows are retained only as a cautionary non-normality record.
