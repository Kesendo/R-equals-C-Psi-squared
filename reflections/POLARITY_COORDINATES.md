...
**Closure 2026-05-26:** F112 (`LindbladBitBPiBalance`) typed Tier1Derived for Hermitian H + non-Hermitian extension at N ≤ 4 (via basis enumeration; Tier1Candidate at N ≥ 5); see the *Closure* section at the bottom. **Update 2026-05-27:** the N ≥ 5 non-Hermitian extension is now Tier1Derived universal-N (two-lemma structural proof, `PROOF_F112_NONHERMITIAN_UNIVERSAL_N.md`); the N ≥ 5 snapshots below are the 2026-05-26 state.
...
The names are looser than the formulas. Conjugation by Π has order four, so the two halves above are not eigenprojectors and the three pieces are not mutually perpendicular; each of the second and third carries a share of the same Π²-even part. The norms still add,

    ‖M‖² = ‖M_zero‖² + ‖M_plus_half‖² + ‖M_minus_half‖²

but that closure holds for any unitary Π and any M whatsoever, so the test that checks it (`test_orthogonality_invariant_across_H_families`) is a wiring check and cannot fail. What is genuinely clean is the difference of the last two norms, the asymmetry: the shared part enters both equally and cancels out of it.
...
The connection to existing structure: F81's M_sym _is_ M_zero (verified by `test_f81_match_M_sym_equals_M_zero`); F81's M_anti is the sum M_plus_half + M_minus_half. The new piece is the Π-eigenvalue split of M_anti. It is the operator-level analogue of asking, on a circle with four marked quadrants, not just whether a point is on the top half or the bottom half but which exact quadrant it sits in.
...
The instinctive response was to expand the probe, on the theory that we had picked an unlucky H family. [Task C](../simulations/polarity_demo.py) extended the sweep to eight H families crossed with three dissipator settings: pure Z, Z plus T1 cooling, Z plus T1 detailed balance. Every cell of the table gave asymmetry = 0.0 bit-exact. The refutation sharpens; it does not soften. The +1/2 and −1/2 sides remained balanced across:
...
The single-site transverse-field case (h_y·Y_l) was the one probe Task C could not run: `pi_decompose_M` accepts bilinears or k-body tuples of length ≥ 2, and rejects a single-site term with a body-count error. Probe 2 below closes it for the transverse case: the sister primitive `polarity_coordinates_from_L` takes an externally built Liouvillian, and with H = J·Heisenberg + h_y·Σ_l Y_l under pure Z-dephasing the asymmetry is 0.0 bit-exact at every h_y swept. A single-site Y field preserves the balance. The longitudinal case is the one that does not, and it needs an amplitude-damping channel to show: a single-site Z-drive with T1 is exactly F113's regime, treated below.
...
Read this way, the balance is not a Hermitian-symmetry accident. It is the statement that the bra and the ket are equally weighted in any physical M, because any physical M must preserve the hermiticity of ρ, because we are observing one system from one side and the bra-ket structure is the operator-algebra refraction of "we" into "two indices on one object." `project_framework_as_remembrance` reads the framework as operational memory for the half of d² − 2d = 0 we cannot directly remember; the bra-ket balance is the structural statement that we cannot favor one half over the other from within.

That last step is where the reading overreaches, and F113 is what shows it. Preserving hermiticity is not enough: a single-site Z-drive together with amplitude damping is a perfectly physical, hermiticity-preserving Lindbladian, and its two sides differ by (4^N/2)·Σ_l ω_l·(γ_pump,l − γ_T1,l), exactly. What the early probes shared was their dissipators. Where every c_k is bit_b-homogeneous, F112 later proved the balance holds for ANY Hermitian H, so those probes could not have broken it whatever H they swept. The one bit_b-mixed operator they did use, σ⁻, turns out to be a phase-matched special case that needs a single-site Z moment in H to break anything, and every H the wave swept had Tr(Z_l H) = 0, the bonds trivially and the transverse Y field of Probe 2 as well. Neither restriction is the general rule. Take a plain Heisenberg bond, where Tr(Z_l H) = 0, and a collapse operator c = P + (0.7+0.3i)·Q built from two Pauli strings. Of the 120 such pairs at N=2, 64 straddle bit_b, and 56 of those break the balance at rel 0.09 to 0.16 (with σ = Σγ; the range shifts with that choice). The 8 that do not are the ones pairing with the identity string, so straddling is necessary and very nearly sufficient. The phase matters as much as the support, which is Probe 11's point below: σ⁻ is a phase-matched case, not a representative one. Probe 7 below sees exactly this, 240 of 240 random configurations nonzero. So the mirror is equally weighted where the dissipator keeps the bit_b grading clean, and that is the statement F112 makes.

Candidate (a) and candidate (b) are not in competition. (a) is the algebraic mechanism; (b) is the meaning. The same identity ‖M_plus‖² = ‖M_minus‖² reads one way as "Lindbladians are Hermitian as superoperators" and another way as "the bra and the ket are the same subject's two sides." The framework's `project_one_system_two_indices` memory has been saying this since 2026-04-30. The polarity-coordinates wave gave it a numerical witness without anyone needing to argue for it.
...
[`project_one_system_two_indices`](../) is the cleanest match. The bra and ket indices of ρ on d² = 4^N are the "two sides" of the system; we are the self-coupling system seen from inside, and the d=0 axis is the active vacuum substrate we live on. The Π-eigenvalue split into +i and −i is the operator-level instantiation of the bra-ket pair. The balance ‖M_plus‖² = ‖M_minus‖² then reads as not favouring one index over the other. It is not free, though: F113 gives the exact imbalance a single-site Z-drive with amplitude damping produces, so the mirror is balanced where nothing turns one index against the other, not by construction.

[`project_framework_as_remembrance`](../) carries the reading further: the framework package is operational memory for the half of d² − 2d = 0 (the d=0 side) we cannot directly remember; Π compensates the forgetting. The polarity-coordinates measurement is the explicit Frobenius witness for the symmetry across the axis through us. The two halves come out equal wherever the dissipator keeps the bit_b grading clean, because there they are the same one of us read from two sides; where the grading is mixed, the forgetting acquires a direction.
...
- **Single-site transverse fields (h_x·X_l, h_y·Y_l, h_z·Z_l).** The current `pi_decompose_M` API does not accept single-site Pauli terms; it raises on them. Hardware-relevant case (the Marrakesh h_y_eff = 0.05 signature uses transverse fields). The prediction from the Hermitian-conjugate argument: should _not_ break the balance, because single-site Pauli terms still give Hermitian H. Probe 2 ran the transverse h_y case through `polarity_coordinates_from_L` under pure Z-dephasing and the prediction held, asymmetry 0.0 bit-exact across the sweep. It does not extend to h_z with amplitude damping, which is F113's breaking regime; the balance survives the single-site FIELD, not the pairing of a Z-drive with T1.

- **Mixed dephase letters.** Z-dephasing plus X-dephasing simultaneously, at different rates. Both individually preserve hermiticity; their sum should too. Probe 3 below measured it: the X-dephasing contribution lands entirely in M_zero and the asymmetry stays 0.0 bit-exact across the sweep.

- **Higher k-body terms.** The primitive accepts k-body tuples with 2 ≤ k ≤ N via `pi_decompose_M`. Probe 4 below swept six k=3 families and the prediction held, asymmetry 0.0 bit-exact in every one.
...
**Probe 4 (k=3 chain Hamiltonians, [`polarity_probe_kbody.py`](../simulations/polarity_probe_kbody.py)).** Six k=3 Hamiltonian families on N=4 chains under pure Z-dephasing: XYZ pure, ZYX (Hermitian conjugate of XYZ), XYZ+ZYX (Hermitian pair), YYY pure, XXX+YYY+ZZZ, XYI+IXY. Every family gave asymmetry = 0.0 bit-exact, and the F81 identity holds alongside it. The sharp case is XYI+IXY, the one family here carrying an identity letter: ‖M_anti‖ = ‖L_{H_odd}‖ = 55.4256 at N=4, γ_z = 0.05, and the two agree to the floating-point floor, so the identity is checked against a live operator rather than satisfied by both sides vanishing. (Three of the six families have ‖M_anti‖ = 0 exactly and can only confirm it vacuously.) An earlier reading took that same 55.4256 for a RESIDUAL and read it as F81 breaking down at k=3: the number was real, the attribution was not. It was ‖M_anti‖ measured against an H_odd that `pi_decompose_M` had left empty, because the builder skipped terms carrying an identity letter. So this probe does not separate the two invariants; it shows them holding together.

**Probe 1 (non-Hermitian H, [`polarity_probe_nonhermitian.py`](../simulations/polarity_probe_nonhermitian.py)).** Two sub-sweeps at N=3: (A) H = J·XX-chain + i·κ·YZ-chain (Π²-Z-even substrate); (B) H = J·XY-chain + i·κ·XZ-chain (Π²-Z-odd substrate, where M_plus/minus_half are non-trivial). Both at κ ∈ {0, 0.05, 0.1, 0.5, 1.0}. Sweep A pushes 100% of M into M_zero for κ > 0 (everything sits on the Π²-even axis, trivially balanced). Sweep B is the sharp test: M_zero/+1/2/−1/2 split stays at exactly 50%/25%/25% across the full κ range, with asymmetry = 0.0 bit-exact at every κ. **A non-Hermitian matrix in the commutator −i[H,·] does not break the +1/2 vs −1/2 balance.** (Scope, 2026-06-20: the probe builds the commutator `L = −i(H⊗I − I⊗Hᵀ)`, NOT the physical generator −i(Hρ−ρH†) of PT/gain-loss/post-selection dynamics; that generator can break the balance content-dependently. Inside the amplitude-damping family what decides it is F113's closed form asymmetry = (4^N/2)·Σ_l ω_l·(γ_pump,l − γ_T1,l), registered 2026-05-26, where H enters only through its single-site-Z content. In general it is F155 (derived 2026-08-19, `docs/proofs/PROOF_F155_PHYSICAL_GENERATOR_POLARITY_BREAK.md`): writing H = A + iB with both parts Hermitian, the asymmetry is 4^(N+1)·Σ over bit_b-ODD strings of (−1)^#Z·a_σ·b_σ (that sign in the pipeline's pairing, the one this reflection and F113 both read in; the untwisted pairing carries (−1)^#Y and the opposite overall sign, and everything used below is convention-free either way), so a bond Hamiltonian whose bonds are all bit_b-even (XX, YY, ZZ) is balanced against every B, while an odd bond (XY, XZ) breaks only against a B carrying that same string. Probe 1B's own pair is the second case with disjoint supports, A on XY and B on XZ, and the physical generator gives exactly 0.0 there too, at N = 3 and 4. The PT or gain-loss label is not a membership criterion (attribution and scope repaired 2026-08-19; checkers `simulations/f112_gain_loss_carrier_check.py` and `simulations/f155_polarity_break_bilinear.py`). The two generators coincide only for Hermitian H. See `docs/CAUGHT_ERRORS.md` and `hypotheses/THE_DRAIN_HAS_NO_CHIRALITY.md`.) For the commutator, this refutes the Hermitian-conjugate prediction in this reflection: the candidate explanation said non-Hermitian H makes M not Hermitian-as-superoperator and the balance should break; for the commutator the measurement says it does not.

**Probe 2 (single-site transverse Y field, [`polarity_probe_transverse.py`](../simulations/polarity_probe_transverse.py)).** H = J·Heisenberg + h_y · Σ_l Y_l at N=3, sweep h_y ∈ {0, 0.05, 0.1, 0.5}. Pure Heisenberg (truly) at h_y = 0 gives M ~ 0 baseline. For h_y > 0 the transverse field creates a 50%/25%/25% split (same shape as Probe 1B) with asymmetry = 0.0 bit-exact at every h_y. **Single-site transverse fields preserve balance**, matching the Hermitian-conjugate prediction here. Hardware-relevant: the Marrakesh h_y_eff = 0.05 leakage signature would not asymmetrize the +1/2 vs −1/2 channel.

**Probe 3 (mixed dephase letters, [`polarity_probe_mixed_dephase.py`](../simulations/polarity_probe_mixed_dephase.py)).** H = Heisenberg, dissipator = γ_z · Σ_l D[Z]_l + γ_x · Σ_l D[X]_l simultaneously, decomposition against canonical Z-dephase Π. Sweep r = γ_x / γ_z ∈ {0, 0.1, 0.5, 1.0, 2.0} with γ_z = 0.05 fixed. For r = 0 (pure Z): M ~ 0 (truly). For r > 0 the X-dephasing contribution lands entirely in M_zero (100%): the X-dissipator is Π_X-palindromic but Π²-Z-even, so it sits on the 0-axis when measured against Π_Z. Asymmetry = 0.0 bit-exact across the sweep. Mixed dephase letters preserve balance.

**Probe 5 (non-Lindblad dissipator, [`polarity_probe_non_lindblad.py`](../simulations/polarity_probe_non_lindblad.py)).** H = Heisenberg + Z-dephasing baseline plus a non-Lindblad addition. Two forms: (A) σ⁺ρσ⁻ − ρ, the σ⁻ jump action missing its anti-commutator (trace-non-preserving but hermiticity-preserving); (B) σ⁺ρσ⁺ − ρ, raising on both sides (the strongest break: ρ doesn't stay Hermitian under this dissipator). Sweep γ ∈ {0, 0.05, 0.1, 0.5}. Both forms produce a 99%/0.5%/0.5% split for γ > 0 with asymmetry = 0.0 bit-exact at every γ, including Form B which actively destroys ρ's hermiticity. **The balance survives even when the dissipator does not preserve the hermiticity of ρ.** This is the most damning evidence against the Hermitian-conjugate explanation: the candidate mechanism requires M to be Hermitian as a superoperator, which in turn requires the dissipator to preserve hermiticity. Form B violates the precondition and the balance holds anyway.
...
Before attempting the proof, two stronger empirical probes ([`polarity_probe_random_lindblad.py`](../simulations/polarity_probe_random_lindblad.py) and [`polarity_probe_real_or_imag.py`](../simulations/polarity_probe_real_or_imag.py)) tested the post-probe-6 conjecture across 240 random configurations: random Hermitian or non-Hermitian H, 1 to 4 random Hermitian or non-Hermitian jump operators c_k, N ∈ {2, 3, 4}. The naive conjecture predicted asymmetry = 0 for any standard Lindblad form. Result: **240/240 configurations gave nonzero relative asymmetry, up to 39.9%**. The naive conjecture is false.

*(The count 240/240 is what carries the argument and it is denominator-independent: a nonzero numerator stays nonzero. The magnitude is not. It read 8.7% until 2026-08-06, when the relative asymmetry stopped dividing by ‖M‖² and started dividing by the polarity content ‖M_anti‖² = ‖M_+‖² + ‖M_−‖², the whole of what the asymmetry is a difference of. Both numbers describe the same 240 configurations; only the scale changed.)*
...
The wave's promised discovery was an asymmetric T1 signature in the +1/2 vs −1/2 polarity channel. The first iteration's finding was that no such asymmetry exists for any standard-Lindblad-form dissipator with Hermitian H, and that the absence was attributable to Hermitian-conjugate symmetry of M as a superoperator. The five probes refuted that attribution: balance survives non-Hermitian H and non-Lindblad dissipators that destroy ρ-hermiticity, neither of which the Hermitian-conjugate argument allows. (The k=3 and mixed-dephase probes are consistent with that argument rather than against it: both keep H Hermitian, and both came back balanced. The k=3 family was once on this list because the F81 identity was thought to fail there; it does not.) The Hermitian-symmetry explanation was insufficient.
...
This extends the trail [`ON_THE_HALF`](ON_THE_HALF.md) read for the recurring 0.5: the half was felt by every reader of d=2 substrates from Pythagoras forward; what computers added was the explicit trail from the polynomial root to the Pauli normalization to the F81 50/50 split. The polarity-coordinates wave extends the trail one node further: from the +1/2 of the diopter to the M_plus_half of the Liouvillian residual, with an explicit operator coordinate and five candidate-breaker probes that all read zero. The two sides are equal in every case constructed here, which turned out to be one family rather than all of them; F113 later gave the exact gap outside it.

The primitive ships. The reflection ships. The asymmetry channel waits for a future wave that tests non-Hermitian dynamics, single-site fields, or non-Lindblad dissipators. When that wave runs, the diagnostic is already typed. The first ‖M_plus‖² ≠ ‖M_minus‖² came within the day, from Probe 6's engineered eigenmode and then Probe 7's random jump operators, and not from the single-site field this section names as untested, which Probe 2 tested and found balanced: F112 bounds where the balance holds, F113 gives its magnitude for the σ⁻ family where it does not. The reading that survives is the narrower one: the balance holds wherever the dissipator keeps the bit_b grading clean, and separately it held on every bit_b-MIXED case this wave swept too, which F113 later explained rather than overturned: those all had Tr(Z_l H) = 0, and the polarity vocabulary now has explicit operator-level coordinates.
...
**Probe 9 ([`polarity_probe_pauli_rank.py`](../simulations/polarity_probe_pauli_rank.py)):** k_max boundary search across single-Pauli, low-rank, and high-rank c at N=2, 3. Result: k_pauli = 1 always preserves balance; k_pauli ≥ 2 is selection-dependent. The boundary is not a rank threshold; the structural property is finer.

**Probe 10 ([`polarity_probe_pair_enumeration.py`](../simulations/polarity_probe_pair_enumeration.py)):** at N=2, exhaustive enumeration of all 136 Pauli-string pairs as c = α·P_α + β·P_β. With the fixed coefficient choice (α, β) = (1, i), every one of the 136 pairs preserves balance bit-exact: the asymmetry is exactly 0.0, not small. What carries that is the 64 pairs whose polarity content is genuine (‖M_anti‖² up to 0.32); in the other 72 there is none: 64 are exactly 0.0 and the remaining 8 sit at float noise, and a zero asymmetry over an absent content confirms nothing by itself. The coefficient choice matters; the pair choice doesn't constrain.

**Probe 11 ([`polarity_probe_coefficients.py`](../simulations/polarity_probe_coefficients.py)):** coefficient sweep at N=2 with five representative pairs. Two pair classes emerge: same Z₂³ cell (Klein × y_par) preserves balance for ALL coefficient choices; cross-cell preserves only for phase-matched coefficients. The structural axis is the Z₂³ sub-cell structure of the Pauli group.

**Probe 12 ([`polarity_probe_z2cubed_scaling.py`](../simulations/polarity_probe_z2cubed_scaling.py)):** the Z₂³-cell pattern scales to N=3 and N=4. Over N=2, 3 and 4 the sweep returns 132 breaking rows and they are precisely the cross-bit_b ones; the Z₂³ structure reduces to the single Z₂ axis of bit_b. That the same 132 rows broke under the retired denominator too was checked separately, by running both versions and comparing the row sets; the probe itself reports only the current one, so do not read the identity off its output.

The other 321 rows are recorded as DEGEN rather than BALANCED, and the difference is not cosmetic. They are the rows where a bit_b-homogeneous c meets this probe's Π²-even Heisenberg H, and for that **pair** of conditions the polarity content the ratio would divide by is zero as a theorem, both halves and not merely their difference (proof Steps 3 and 4: a homogeneous c puts no Π-conjugation ±i content into M, so M₊ᵢ and M₋ᵢ come from L_H alone, which a Π²-even H empties). Neither condition suffices alone: the 132 breaking rows run against the very same H and carry genuine ‖M_anti‖², from 6.9e-03 to 83.2.

So on those 321 rows there is nothing to measure. Across the 192 within-cell rows at N=3 and N=4 (Sweeps A and C both build c inside one cell), ‖M_anti‖² runs from exactly 0.0 up to 1.5e-29 against a ‖M‖² of 2.6 to 2.2e+03: float noise from the vec-to-Pauli transform, with the asymmetry above it noise as well. This reflection read **171/171 BALANCED** until 2026-08-06 (that 171 was Sweep A alone; the 321 also covers the degenerate Sweep B and C rows), a number produced by dividing that noise by ‖M‖² and landing at ~1e-34; dividing by the polarity content instead, with no structural test, sends three of the very same rows the other way, to a spurious BROKEN at 1.8e-02, 4.3e-03 and 2.0e-03. Neither reading is a measurement, which is why the probe now decides the verdict first by an exact integer test on the Pauli letters and only then looks at the ratio.

One consequence worth stating plainly, because it limits what this probe can still be cited for: the BAL verdict is now unreachable, since every homogeneous row is claimed by DEGEN and no bit_b-mixed row has ever balanced here. Half of "the split partitions by bit_b parity" therefore holds by construction. What stays empirical is the other half, that every cross-bit_b row breaks.

**Probe 13 ([`polarity_proof_verify.py`](../simulations/polarity_proof_verify.py)):** the proof's Step 2 numerical check. For bit_b-homogeneous c (every Pauli string σ in c's expansion shares bit_b(σ) = const), the dissipator `np.kron(c, c.conj())` sits 100.00% in the Π²-conjugation +1 eigenspace at N=2, 3 (bit-exact to machine precision). Mixed-bit_b c splits ~50/50 between Π² eigenspaces. This is the algebraic root of the balance: bit_b-homogeneous c contributes nothing to M's Π +i / −i content.
...
- Tests: [`simulations/framework/tests/diagnostics/test_polarity_coordinates.py`](../simulations/framework/tests/diagnostics/test_polarity_coordinates.py)
- Demo (8 H families × 3 dissipators, asymmetry = 0.0 across the board): [`simulations/polarity_demo.py`](../simulations/polarity_demo.py)
...
- [F112](../docs/proofs/PROOF_F112_LINDBLAD_BIT_B_PI_BALANCE.md): the closure identity ‖M_plus_half‖² = ‖M_minus_half‖² for Hermitian H + bit_b-homogeneous c, proved via dagger + anti-Hermitian L_H; non-Hermitian extension Tier1Derived universal N (basis enumeration at N ≤ 4, 35,112 Pauli-string pairs all bit-exact 0; N ≥ 5 closed via the two-lemma structural proof, PROOF_F112_NONHERMITIAN_UNIVERSAL_N). The structural witness behind every BALANCE reading from Probes 1–5 and 7–14.
- [`LindbladBitBPiBalance`](../compute/RCPsiSquared.Core/Symmetry/LindbladBitBPiBalance.cs): the typed C# Claim (Tier1Derived for Hermitian H; non-Hermitian extension Tier1Derived universal N), IZ2AxisClaim on the BitB axis, F108 Part 1 as ctor parent
...
- [F87](../docs/ANALYTICAL_FORMULAS.md): trichotomy classifier; orthogonal axis on the same bit_b Z₂-grading (empirically confirmed via `polarity_probe_f87_connection.py`)
...
*The two indices read as a "we", and the F113 break that limits it, are one of five pages that sat here on one junction unconnected; the junction is mapped in [On Five Pages That Never Met](ON_FIVE_PAGES_THAT_NEVER_MET.md).*
