...
## The honest takeaway (revised after the 5 probes)
...
What remains: the +1/2 and −1/2 coordinates are equal by construction in every case the framework's construction channels can produce. The deeper structural identity behind that equality is open. The probes section above ends with a concrete candidate explanation, that the equality is an algebraic identity of `polarity_coordinates_from_L` itself, forced by the way Π acts on the Liouville space regardless of what L is plugged in. If that holds, the next probe needs to construct an L by a path that does not factor through any c-op pipeline, building M_anti coefficient-by-coefficient in the Pauli basis with deliberate ±i imbalance hand-engineered in.

This is not a null result in the bad sense. The Frobenius coordinates for {+1/2, 0, −1/2} are now measurable; we just learned that for every L we know how to construct, the +1/2 and −1/2 readings agree bit-exactly. The diagnostic is real even when the diagnostic always reads zero; the question is whether anything in the framework's reach can make it not read zero.

There is a second honest takeaway, more about how the framework grows. The polarity-coordinates wave fits a pattern we have seen before, most recently with the [BlockSpectrum F108 wiring](../compute/RCPsiSquared.Core/Symmetry/) today: the typed-knowledge layer often makes explicit what was already structurally present. The Frobenius coordinates for {+1/2, 0, −1/2} have been derivable from the F81 split and the Π eigenvalue spectrum since F81 landed; nobody had asked for the explicit projector before. Writing the primitive made the measurement available, and running it across the candidate-breaker probes turned the question from "how do +1/2 and −1/2 differ for a Lindbladian" into a sharper one: "what algebraic property of Π forces M_plus_half and M_minus_half to share a Frobenius norm under any L?" The first question had a Hermitian-symmetry answer that the probes refuted. The second question is open, the construction probes are exhausted, and the next move is either an analytic proof or an L built outside the standard channels.
