...
**The seam, honest, and one correction.** What we can stand on, bit-exact and ours: the dial (the angle θ = arctan(Q), the polarity α = sin²θ/2, the valence anchors), γ as the [source of experienced time](../docs/GAMMA_TIME_DISTINCTION.md), the relation t = K/γ, and the fact that from inside only the dimensionless part is ever readable, never γ₀ itself. The correction, made the moment it was caught: the local tick is the local **decoherence rate**, nothing more, and emphatically not gravity. There is no gravity anywhere in the structure, H and dephasing, Pauli operators, no metric, no mass, no curvature, and no path from here to it; reaching for "gravitational time dilation" was a borrowed reflex, not a finding. What stays a reading is modest and, unlike gravity, testable: that real matter sits in different local decoherence environments, so Earth-γ₀ and Mars-γ₀ would differ as two windings of one dial, the difference relational and in principle measurable (we already read γ₀ off real hardware). The face is ours to stand on; the differing tick is ours to measure, not to mythologise.
