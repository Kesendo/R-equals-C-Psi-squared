...
**Depends on:** [F86 in the Analytical Formulas registry](../docs/ANALYTICAL_FORMULAS.md), [the F86 Q-peak proof](../docs/proofs/PROOF_F86_QPEAK.md), [PT-Symmetry Analysis](../experiments/PT_SYMMETRY_ANALYSIS.md), [The Fragile Bridge](../hypotheses/FRAGILE_BRIDGE.md), [Perspectival Time Field](../hypotheses/PERSPECTIVAL_TIME_FIELD.md), [On Two Times](ON_TWO_TIMES.md), [Primordial Qubit](../hypotheses/PRIMORDIAL_QUBIT.md), EQ-014 and EQ-022 in [Emerging Questions](../review/EMERGING_QUESTIONS.md), `memory/project_retraction_lesson_ptf_to_f86`.
...
*Seen again 2026-06-10.* The survivor got its proof. The chiral mirror law turned out to be the summed shadow of a site-wise trajectory identity, P_i(t; φ_k) = P_i(t; φ_{N+1−k}) exactly, derived in four steps from the same K₁ the law was named after (with a conjugation trick and a U(1) phase doing the quiet work), verified to 9·10⁻¹⁶, and typed as `ChiralMirrorTrajectoryClaim` beside `ChiralKClaim`: one sublattice chirality, eigenvalue side soft, eigenvector side mirrored. The lesson's own decision rule closed its loop, rule 4 said name the symmetry and the survivor is structural; naming it was, in the end, the whole proof. The proof idiom (an involution and a sign table) is the one the windowed-converse wave built for the palindrome; April's survivor waited for June's technology. [the PTF chiral-mirror rate-law proof](../docs/proofs/PROOF_PTF_CHIRAL_MIRROR_RATE_LAW.md).
