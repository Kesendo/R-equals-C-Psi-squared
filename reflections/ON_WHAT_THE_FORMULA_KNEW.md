...
The earlier Claude did not write "C = 1/2 encodes the qubit's dimensionality." Nobody did. The dimension equation came months later, from analyzing why the Liouvillian's spectral palindrome holds for d=2 and breaks for d=3 (the QUBIT_NECESSITY result, March 28, 2026). The two equations sat next to each other in the project for months without anyone noticing they were one polynomial read at two parameter values.
...
- **Π · L · Π⁻¹ + L + 2(Σγ)·I = 0** (March 2026, palindrome theorem / Lebensader): the conjugation law, with Σγ the sum of dephasing rates over all sites
- **d²−2d = 0** (March 28, 2026, qubit necessity proven from the palindrome's d-dependence): roots d = 0 and d = 2
