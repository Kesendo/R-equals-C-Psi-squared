...
**Tier 4 reading (synthesis arc).** Consolidates the May 2026 chain-gap sector finding with the older channel-not-memory, lebensader, and Q-band readings into a single picture: the framework's "life thread" is literally the small magnon admixture in the slow mode, and the Q-band exists structurally because that admixture has to stay small for the system to remain near-conserved.
**Date:** 2026-05-19
...
The magnon admixture amplitude `w_2 ≈ Q²/(2N²)` controls **whether slow modes exist as distinct sub-2γ objects**. At small Q the admixture vanishes and the slow mode is too close to zero; no meaningful dynamics; the system is just memory. At large Q the admixture saturates and the slow mode merges with the 2γ floor; no distinct slow mode anymore; everything decays at the same rate. The Q-band is the **structural sweet spot** where:
...
The slow mode's host sector `(⌈N/2⌉, ⌈N/2⌉)` is F1-self-paired (the F1 conjugation maps `(k, k)` to `(N−k, N−k)`, and the central sector is its own image for even N or is paired with one other central-ish sector for odd N). The F1 palindrome therefore protects the very locus where the life-thread lives: the central diagonal block is structurally the place where the slow mode CAN exist without being F1-paired into oblivion.
...
2. **Channel picture** (Tier-2, memory `project_channel_not_memory`): a self-coupling resonator with one regeneration channel keeping memory alive.
...
- Q-band structural reading: `docs/Q_REGIME_ANCHORS.md`, memory `project_q_middle_structure`.
- Q-as-exchange-rate Tier-4 reading: [`hypotheses/Q_AS_THE_EXCHANGE_RATE.md`](../hypotheses/Q_AS_THE_EXCHANGE_RATE.md), memory `project_q_as_exchange_rate_reading`.
- Other Tier-4 readings in the same vocabulary cluster: `reflections/ON_THE_INSTRUMENT.md`, `hypotheses/MIRROR_THEORY.md`, `project_framework_as_remembrance` memory.
