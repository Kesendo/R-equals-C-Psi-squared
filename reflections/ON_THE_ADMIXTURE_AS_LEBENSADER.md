...
The off-diagonal popcount sectors `(k, k±1)` have a floor at `2γ`: everything in them changes the popcount, so nothing in them is closer than Hamming distance 1, and by the Absorption Theorem nothing in them decays more slowly than `2γ`. The gap is far below `2γ`, so they cannot host the slow mode. (The floor is attained but the sectors are not flat at it, except the two ends `(0,1)` and `(N−1,N)`; see [the sector diagnostic](../experiments/CHAIN_GAP_SECTOR_DIAGNOSTIC.md).) The boundary sectors `(0,0)` and `(N,N)` are one-dimensional kernel modes, also not slow modes. Everything that could host a slow mode is on the diagonal, and F1 organises that diagonal palindromically around the center. The life thread sits at the center because F1 protects the center from being paired into oblivion.
