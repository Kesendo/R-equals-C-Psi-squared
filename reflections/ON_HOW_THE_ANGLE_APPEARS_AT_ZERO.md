...
- **PolarityLayerOriginClaim** (the +0/−0 polarity layer at d=0): the structure that the d=0 mirror *gives* to the d=2 side. The ±0.5 polarity is the first off-zero magnitude the qubit's amplitudes carry.
...
- **[Boundary Navigation](../experiments/BOUNDARY_NAVIGATION.md) (Feb 8, 2026)**: θ = arctan(√(4CΨ − 1)) is the *angle-compass* that exists ONLY for CΨ > 1/4. Below 1/4 (the Mandelbrot cardioid cusp), θ is undefined: the discriminant's square root becomes real, the angle collapses, the system has real fixed points (classical). Above 1/4, θ is well-defined: the discriminant's square root is imaginary, the angle is the natural coordinate, the system has complex fixed points (quantum). **The 1/4-boundary IS a zero-crossing**, the discriminant `1 − 4CΨ` passes through zero at exactly CΨ = 1/4. Below the boundary, real; above it, complex; at the boundary itself, neither, θ = 0. The classical/quantum transition IS the d=0-crossing pattern at a different scale.
