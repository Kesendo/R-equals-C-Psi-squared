...
gamma is external to any system described by a Lindblad equation for that system. INCOMPLETENESS_PROOF certifies that the system is open; the framing below reads that as gamma entering from outside, with no inverse path back. And it nests: whatever generates gamma for system S has its own gamma from the next outer layer (IBM qubit <- control microwaves <- lab EM environment <- cosmic background <- ...).
...
(b) **Read the incompleteness proof as layer-specific rather than universal.** INCOMPLETENESS_PROOF establishes that the system is open and eliminates no internal origin; the recursion needs the stronger reading that gamma cannot originate from within the d(d-2)=0 framework. Does this mean "from within any instance of the framework at any scale", or only "from within the specific system under study"? The distinction matters: the first reading forces recursion, the second does not.
...
**(i) INCOMPLETENESS_PROOF scope: it proves openness, not an outside.** Its trace identity holds for any Lindblad generator, at every layer: a palindrome centred away from zero certifies an open system. Its candidate survey eliminates no internal origin: [Pi^2, L] = 0 constrains the noise's form (Candidate 1), Candidate 2 is open, and the rest say what cannot exist rather than clearing an existing source. So there is no universal "noise cannot originate from within" for a recursion to inherit, and where the noise comes from the proof leaves open, with the reason that nothing inside the Lindblad formalism could close it.
...
2. **Central result of the four probes:** "J/gamma is the single dimensionless control parameter of the nested Lindblad system." Every dimensionless inside reading depends on the coupling/dephasing ratio, not on J or gamma separately. Keep the Hamiltonian books distinct: the carrier convention calls `Q_carrier = J_carrier/gamma` for `H=(J_carrier/2)(XX+YY)`, while F7 uses `H=J_F7(XX+YY+ZZ)` and has mean modal quality factor `2J_F7/gamma`. These are not two definitions of one symbol in one convention.
...
| Check | Result | Method |
|-------|--------|--------|
| (i) INCOMPLETENESS_PROOF scope | Proves openness; no internal origin eliminated | Full re-reading, this session |
...
INCOMPLETENESS_PROOF Section 6, Option 2 leaves room for this: gamma_0 as axiomatic, like c. THE_BRIDGE_WAS_ALWAYS_OPEN does not require further layers. The Inside-Outside Correspondence probes confirm: gamma is a pure scale factor, the physics is in Q = J/gamma_0.
...
**2. CΨ = 1/4 (fold boundary).** Anchor [F16](../docs/ANALYTICAL_FORMULAS.md): R = C(Ψ+R)². Internal structure: F18 fold threshold Σγ_crit/J = 0.00249 (|+⟩^N product, N-flat over N = 2-5) / 0.00038 (Bell at N = 2; GHZ starts below the fold from N = 3, CΨ(0) = 1/(2^N − 1), so it has no threshold): the minimal two-region band on the Σγ/J axis in the corpus; F25 Bell+ Z-deph closed form with K = 0.0374; F26/F27 per-channel K values (K_Z = 0.0374, K_X = K_Y = ln(2)/8 = 0.0866433…); F47 the Bures path-metric coefficient's coordinate-shape second derivative *at* the fold; F56/F57 cusp critical-slowing and dwell scalars; F35 dual-perspective lifetime ratio T1/T2 (Π-side decay rides T1, direct decay rides T2); F69 GHZ+W sextic saddle above 1/4 at N=3, generalised in EQ-016 closure to the central-Dicke-triple slice with cpsi_∞ ≈ 0.4313 and x_opt ≈ 0.2647 (root of an irreducible degree-6 polynomial).
...
**4. ⟨n_XY⟩ sum rule = N (Absorption Theorem).** Anchor [AT](../docs/ANALYTICAL_FORMULAS.md): Re(λ_k) = −2γ·⟨n_XY⟩_k. Internal structure: continuous axis ⟨n_XY⟩ ∈ [0, N], integer basis-cell costs at J=0, fractional fill via H-mixing at J>0; palindromic weight swap ⟨n_XY⟩_fast + ⟨n_XY⟩_slow = N (F1 corollary); F2 Heisenberg (0,1) coherence block dispersion ω_k = 4J(1 − cos(πk/N)); F2b XY single-excitation dispersion E_k = 2J·cos(πk/(N+1)); F3's 2γ and 2(N−1)γ are pure-sector band edges; D6 identifies the full spectral gap with 2γ only above its N-dependent coupling threshold, while weak-coupling mixed modes can decay more slowly. The connection to Q_SCALE is through the continuous eigenmode expectation, not a discrete eigenmode-rate ladder (Deep-dive 1 of MIDDLES_CARTOGRAPHY_FIVE).
...
> **Forward note (2026-06-21):** the "exceptional point" in this entry is the **surviving** toy 2×2 channel reading: `Q_EP = 2/g_eff` as a definition, `t_peak = 1/(4γ₀)`, the toy 2-level effective's genuine EP, and Q_peak as the maximum-mixing point. All of that **stands**. What was later built on top of it and then **retracted 2026-06-21** is the reading that tied this to a **defective exceptional point on the real Q axis of the full block-L** (the local↔global "same EP", the "6× above FRAGILE_BRIDGE" Petermann evidence): the full (n,n+1) block has **no eigenvalue coalescence on the real Q axis** (eigenvalues simple), only genuine large-but-finite non-normality. See the `[RETRACTED 2026-06-21]` entry below and `compute/RCPsiSquared.Core/F86/LocalGlobalEpLink.cs` (now OpenQuestion) / `PROOF_F86A_EP_MECHANISM.md` §The real-axis EP. **Further corrected 2026-07-07:** that "no real-axis coalescence" was itself an over-correction; F89 locates a real-axis defective seed on the full block at every odd N (census-defective through N=11), which the coarse-grid scan missed. See `PROOF_F86A_EP_MECHANISM.md` §The real-axis EP.
...
**PT-like phenomenology, full class OPEN.** The same-sign-imaginary two-level pattern has an EP at finite J and resembles PT spectral flow, but Π is linear whereas Bender-Boettcher PT is anti-linear. For the centered full generator, Π becomes an involutive sectorwise P symmetry only after Π² reduction; the remaining irreducible symmetry algebra has not been completed. The toy local EP and the separate Fragile-Bridge threshold, itself at two qubits per chain generically a second-order exceptional point on the real γ axis, are therefore not licensed instances of a computed global AIII class, and their stronger algebraic relationship remains open.
...
### EQ-022 (b1) Update 2026-05-06: Local-vs-Global EP connection \[RETRACTED 2026-06-21\]

> **RETRACTED 2026-06-21 (F86a-retraction).** The "real-axis hit" reading below does NOT survive artifact-free re-verification. An artifact-free Riesz spectral-projector norm found the full c=2 (n, n+1)-coherence block has **no eigenvalue coalescence on the real Q axis**, its eigenvalues stay simple (gap ~0.25–0.35), so there is no real-axis EP for the local F86 instance to "hit". The block is genuinely non-normal there (large but FINITE Petermann), exactly `experiments/PT_SYMMETRY_ANALYSIS.md`'s baseline. The Petermann-K magnitudes (K = 1333.6 / 337.9 / 2384.7 / 795.4), the "≈ 6× above FRAGILE_BRIDGE", the within-parity growth rates, and the 2-4× odd/even asymmetry are **grid artifacts** (K swings 2–4× over ΔQ = 1e-3). `LocalGlobalEpLink` was demoted **Tier2Verified → OpenQuestion**; the four PetermannSpikeWitness rows are retained only as a cautionary non-normality record. The toy 2×2 reduction has a genuine EP; the SEPARATE Σγ = 0 gain-loss system (FRAGILE_BRIDGE) has its own, at generic couplings and two qubits per chain, a second-order exceptional point on the real γ axis at its threshold, where K = 403 is one finite-offset reading of K ∝ 1/δ. Whether the full block has an off-axis defective EP is open (the nearest complex-Q coalescences found 2026-06-21 are themselves diabolic, ‖P‖ = 1). The historical entry below is preserved as a record of the (since-corrected) 2026-05-06 claim. **Further corrected 2026-07-07:** the "no real-axis EP, eigenvalues simple" conclusion was itself an over-correction. F89 proves the full (1,2) block carries a real-axis defective seed at every odd N (census-defective through N=11); the 2026-06-21 grid missed a √-EP window ~20-30× narrower than its step. See [PROOF_F86A_EP_MECHANISM](../docs/proofs/PROOF_F86A_EP_MECHANISM.md), the real-axis EP section. (The distinct off-real-axis complex-Q question stays open.)
...
The toy two-level EP, the F89 full-block seeds and the Fragile-Bridge threshold (two qubits per chain: generically an EP2 on the real γ axis) are distinct objects in different systems. No analytic continuation connecting them and no global AIII classification has been established; the retained Petermann magnitudes are grid-dependent non-normality readings, not evidence for that identification or for a parity scaling law.

**Encoded as:** `compute/RCPsiSquared.Core/F86/LocalGlobalEpLink.cs` (since demoted to **OpenQuestion**; was Tier2Verified Claim, registered in F86KnowledgeBase). 4 PetermannSpikeWitness entries retained as a cautionary non-normality record.

**Pendant Tier1Derived promotion target \[moot since the retraction\]:** complex-γ analytic continuation in `LindbladPropagator`, OR closed-form K(N) at the EP. Named in `LocalGlobalEpLink.PendingDerivationNote`.

**Status \[superseded by the 2026-06-21 retraction note above\]:** Local-vs-global-EP structural question now **OpenQuestion**, not closed. EQ-022 (b1) Item 1' c=2 stratum stays at Tier1Candidate (closed-form HWHM_left/Q_peak constant unchanged, independent of the EP-link retraction).
...
**F86↔PTF relationship:** PTF's K₁ mirror law is a Hamiltonian sublattice-chirality statement, while F86's Q-rotation is a two-level effective-model statement. The common clock `t_peak = 1/(4γ₀)` does not identify K₁ with the full Liouvillian Π or put both results in one computed AIII class. The Fragile-Bridge threshold is separate, at two qubits per chain generically an exceptional point of its own on the real γ axis; any stronger inheritance among these three objects remains OPEN pending the full sectorwise algebra and an explicit intertwiner.
...
The 0.5 baseline is `QubitDimensionalAnchorClaim` (1/d at d=2); the ±r/2 polarity content is `PolarityLayerOriginClaim` (Layer 2 explicit). The F86 bond-class split is therefore **Locus 6 of inheritance-through-layers**: inherited from the polarity-layer pair, not a new symmetry to derive analytically. \[The EP-side (Locus 5) `LocalGlobalEpLink` Tier2Verified referenced here was **retracted 2026-06-21** → OpenQuestion. The toy F86 reduction has its own EP; the separate FRAGILE_BRIDGE system has another, at two qubits per chain generically a second-order exceptional point on the real γ axis at its threshold, and no shared-EP algebra joining the two is established. The symmetry-side closure (Locus 6, this entry) is unaffected. F89 separately certifies narrow real-axis defective seeds; see `PROOF_F86A_EP_MECHANISM.md` §The real-axis EP.\]
...
**Status:** open, narrowed (2026-07-22). The full April-26 run chain (three same-day runs, now committed in [data/ibm_cusp_precision_april2026/](../data/ibm_cusp_precision_april2026/README.md)) settles three sub-questions: the γ override was a same-day in-situ fit (run 2, wide window 2.26–39.6 μs, γ_fit = 15.04/ms; the window covered the April-16 crossing region and found it dead); the collapse hit at least TWO pairs that morning (run 1, pair (149,150): γ_fit = 18.0× its echo-γ), so it is not unique to pair (14,15), though the multiplier is pair-dependent (9× vs 18×); and the April-16 vs April-26 delay protocols are equivalent (both bare native delay, no DD; the qiskit-experiments tomography transpile preserves μs delays, checked offline with current library versions on a simulated Heron r2 backend, not the April versions on Kingston). Remaining: a day-level free-evolution (T2*-class) rate change invisible to Hahn-echo T2; no Ramsey T2* exists for Kingston in any committed record.
**Pointer:** same-day in-situ Ramsey T2* alongside a dense CΨ run (the Torino Run-3 lesson, now for a pair) on 2+ pairs; then cross-backend (Marrakesh + Fez). If the pair T2*-sum predicts the multiplier, the gap is the ordinary T2*/echo gap amplified by day noise; if not, the next question is what does (T1 ratio? CZ error? TLS proximity?).
