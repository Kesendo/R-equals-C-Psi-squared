...
- **Three-class decomposition completeness.** Are Classes 1-3 exhaustive over the J-blind set, or do other mechanisms exist? Direction 4 from the refinement TASK restructures here: necessity per class is distinct from necessity in the union. Empirical attack: random F71-symmetric-state sampling outside all three classes, checking for zero J-Jacobian to numerical precision. **Status 2026-04-28: partially closed for F71-symmetric *product* states** by [_eq024_three_class_completeness.py](../simulations/_eq024_three_class_completeness.py): 100 random samples, all 100 outside-class, capacity range [7.54, 12.41] bits, no fourth-class candidate. Strong empirical support for completeness within product states. Non-product F71-symmetric (20-dim Hilbert subspace, 38 real params) extension lifted as new sub-question 1b in [J_BLIND_RECEIVER_CLASSES](../experiments/J_BLIND_RECEIVER_CLASSES.md).
