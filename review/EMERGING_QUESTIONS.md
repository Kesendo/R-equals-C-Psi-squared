...
The toy two-level EP, the F89 full-block seeds and the Fragile-Bridge spectral-abscissa axis departure are distinct objects; EP character of the last remains OPEN. No analytic continuation connecting them and no global AIII classification has been established; the retained Petermann magnitudes are grid-dependent non-normality readings, not evidence for that identification or for a parity scaling law.
...
**F86↔PTF relationship:** PTF's K₁ mirror law is a Hamiltonian sublattice-chirality statement, while F86's Q-rotation is a two-level effective-model statement. The common clock `t_peak = 1/(4γ₀)` does not identify K₁ with the full Liouvillian Π or put both results in one computed AIII class. The Fragile-Bridge spectral-abscissa axis departure is separate and its EP character is OPEN; any stronger inheritance among these three objects remains OPEN pending the full sectorwise algebra and an explicit intertwiner.
