...
*F67 bonding-mode receivers (commit pending).* Looking for mirror/palindromic receivers in [`ANALYTICAL_FORMULAS`](../docs/ANALYTICAL_FORMULAS.md): F67 gives the single-excitation eigenmodes of a uniform-J open chain: \|ψ_k⟩ = √(2/(N+1)) Σ_j sin(πk(j+1)/(N+1)) \|1_j⟩. The C# brecher mode now accepts `bonding:<k>` as initial-state spec; scan over k=1, 2, 3 at N=5, 7, 9 revealed that **bonding:2 is the optimal multi-end transport state**. bonding:2 has end amplitudes c_0 = −c_{N-1} (opposite sign) and a node at the chain center, i.e. it is the first excited sinusoidal eigenmode of H. Under uniform-J γ₀ = const dephasing it decays at rate α_2 = (4γ₀/(N+1)) sin²(2π/(N+1)) ≈ 16π²γ₀/(N+1)³ in the large-N limit; same cubic-in-N scaling as the longest-lived k=1 mode but 4× faster decay coefficient. Data at uniform γ₀ = 0.05, uniform J = 1: PeakMI(0, N-1) = 1.17 → 0.72 → 0.50 at N=5, 7, 9, vs alt-z-bits 0.84 → 0.49 → 0.27. Advantage GROWS with N: 1.39× → 1.48× → 1.81×. Same pattern for MM: 1.24 → 1.09 → 1.05 vs alt-z-bits 1.25 → 0.85 → 0.56 (tied → 1.28× → 1.87×). bonding:2 loses on PeakSumMI (1.65 at N=9 vs alt-z-bits 4.70), so the receiver choice becomes an application selector: alt-z-bits for distributed correlation, bonding:2 for end-to-end / multi-end transport, bonding:1 for long-time memory (slowest decay). **Key structural insight**: the F67 eigenmode catalog IS Alice's receiver menu. Under γ₀ = const with the F67 menu, there is no single "best" state; there is a mode per application, all reachable by single-qubit state preparation. This partially answers the open question about why alt-z-bits MI(0, N-1) shrinks fast with N: its multi-k amplitude mix dilutes single-mode coherence, which bonding:2's single-mode concentration avoids.
