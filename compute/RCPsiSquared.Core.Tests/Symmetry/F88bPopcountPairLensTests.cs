...
public class F88bPopcountPairLensTests
{
    private readonly ITestOutputHelper _out;

    public F88bPopcountPairLensTests(ITestOutputHelper output) => _out = output;

    [Fact]
    public void Tier_IsTier1Derived()
    {
        var lens = new F88bPopcountPairLens(N: 6, np: 1, nq: 2);
    ...
    public void HdEqualsN_Pi2OddIsZero()
    {
        ...
        var lens = new F88bPopcountPairLens(N: 4, np: 1, nq: 3);
    ...
    {
        // The c=2 stratum at N=6 (n_p=1, n_q=2) is generic α=0.5; F88b lens predicts
        ...
        // predict bond-position — it pins what F88b says about the c=2 coherence block.
        var lens = new F88bPopcountPairLens(N: 6, np: 1, nq: 2);
    ...
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => new F88bPopcountPairLens(N, np: 0, nq: 0));
    }
    ...
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => new F88bPopcountPairLens(N, np, nq));
    }
    ...
    public void Reconnaissance_C2_Pi2OddAcrossN5To8()
    {
        ...
        // F88b's pin on each (N, hd) entry as a Schicht-1-fact for downstream consumers.
        _out.WriteLine("  N | n_p | n_q | kind     | α       | s       | π²-odd(HD=1) | π²-odd(HD=3)");
        _out.WriteLine("  --|-----|-----|----------|---------|---------|--------------|--------------");
        foreach (int N in new[] { 5, 6, 7, 8 })
        {
            var lens = new F88bPopcountPairLens(N, np: 1, nq: 2);
