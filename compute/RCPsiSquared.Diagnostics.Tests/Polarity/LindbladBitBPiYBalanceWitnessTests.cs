...
public class LindbladBitBPiYBalanceWitnessTests
{
    ...
    [Fact]
    public void Witness_Heisenberg_pure_Y_DEGENERATE_Matches()
    {
        var w = LindbladBitBPiYBalanceWitness.StandardSet(MakeChain())[0];
        Assert.Equal("Heisenberg_pure_Y_DEGENERATE", w.WitnessName);
        ...
        Assert.True(w.IsStructurallyDegenerate,
            "Heisenberg_pure_Y: Pi^2-even H with homogeneous c carries no polarity content");
    ...
    [Fact]
    public void Witness_YZ_ZY_pi2even_Y_DEGENERATE_Matches()
    {
        var w = LindbladBitBPiYBalanceWitness.StandardSet(MakeChain())[1];
        Assert.Equal("YZ_ZY_pi2even_Y_DEGENERATE", w.WitnessName);
        // Structural, not a float reading: decided from the Pauli letters, so it holds
        // at every N. The float IsDegenerate only reaches exact 0.0 at N = 2.
        Assert.True(w.IsStructurallyDegenerate,
            "YZ_ZY_pi2even_Y: Pi^2-even H with homogeneous c carries no polarity content");
        Assert.Equal("DEGENERATE", w.ExpectedVerdict);
        Assert.Equal("DEGENERATE", w.ActualVerdict);
        Assert.True(w.Matches,
            $"witness '{w.WitnessName}' expected {w.ExpectedVerdict}, got {w.ActualVerdict} " +
