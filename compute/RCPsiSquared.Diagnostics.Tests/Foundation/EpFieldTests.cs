...
public class EpFieldTests
{
    ...
    [Fact]
    public void DefectivenessChild_PeaksAtOneAtTheEp()
    {
        // overlap = min(x, 1/x) ≤ 1, reaching ~1 near Q_EP.
        ...
        Assert.True(curve.Y.Max() > 0.97, "overlap should approach 1 near the EP");
    }
    ...
    public void RotationChild_IsZeroBelowEp_PositiveAbove()
    {
        ...
        Assert.True(curve.Y[0] < 1e-6, "θ ≈ 0 well below the EP");
        Assert.True(curve.Y[^1] > 10.0, "θ has lifted off well above the EP");
    }
