...
/// <summary>The REAL COUNT-CHANGE CENSUS: locate the real loci on the (1,2) block at odd N where the
/// containment corollary's defective-EP input sits, and classify each one. The instrument is the
...
///
/// <para>PRE-COMMITTED CRITERIA: a count-change point at real q in [0.2, 3] whose AT-aware
/// defective-anywhere reading on the raw (1,2) block is Defective is CLASSIFIED a defective locus;
/// it is a certified seed where an exact certificate also reaches (N=5,7,9, the gcd-layer certificate
/// of experiments/F89_BETA_EXOTIC_GENERICITY.md). The nine N=11 loci are classified and uncertified;
/// RealDefectiveSeedsTests recomputes that classification at every one of them.
...
public sealed class RealSeedCensusTests
{
    ...
    public void N11_TheProbe_ClassifiedLoci()
    {
        var all = ScanBoth(10, out var even, out var odd);
        Print("N=11 R-even", even);
        Print("N=11 R-odd", odd);

        var classified = all.Where(p => p.IsDefective).ToList();
        _out.WriteLine("");
        _out.WriteLine(classified.Count > 0
            ? $"VERDICT N=11: {classified.Count} real loci classified defective on (1,2) in the window; " +
              "classified, not certified (no exact layer certificate reaches N=11)."
            : "VERDICT N=11: no count-change locus with a defective reading found in the window (window-scoped).");
