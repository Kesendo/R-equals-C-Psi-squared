...
/// the full irreducible-sector symmetry algebra remains open. Methodology inherited from IntegrabilityBreakingCsr:
/// pool per-spectrum z's, cluster-bootstrap whole independent spectra for random ensembles, attach no sampling CI
/// to deterministic grids, use finite-size-matched references, and use OffReal once conjugation symmetry is broken.</summary>
public class FillingThresholdCsrTests
{
    ...
    private static string Interval(IntegrabilityBreakingCsr.CsrReading r) =>
        r.Uncertainty == IntegrabilityBreakingCsr.UncertaintySemantics.SpectrumClusterBootstrap95
            ? $"[{r.CiLo:F3},{r.CiHi:F3}] from {r.IndependentSpectrumCount} spectrum clusters"
            : "no sampling CI";
    ...
    public void Reconnaissance_FillingLadder_N6()
    {
        ...
        foreach (double delta in new[] { 0.0, 1.0 })
        {
            ...
            _out.WriteLine("  (wk,wb) dim |  W   | ⟨|z|⟩ [whole-spectrum cluster 95% CI / no sampling CI] | ⟨cosθ⟩ | Npool | ref P/G ⟨|z|⟩ | ref G ⟨cos⟩");
            foreach (var (wk, wb) in ladder)
            {
                ...
                foreach (double w in ws)
                {
                    ...
                    _out.WriteLine($"  ({wk},{wb}) {dim,4} | {w,4:F1} | {r.MeanAbs:F3} {Interval(r)} | " +
    ...
    public void Reconnaissance_DenseSizeScaling()
    {
        ...
        _out.WriteLine("  N (wk,wb) dim  | ⟨|z|⟩ [whole-spectrum cluster 95% CI] | ⟨cosθ⟩ | perSpec | Poisson z/cos | GinUE z/cos");
        foreach (var (n, wk, wb, R) in cases)
        {
            ...
            _out.WriteLine($"  {n} ({wk},{wb}) {dim,4}  | {r.MeanAbs:F3} {Interval(r)} | " +
    ...
    /// <summary>The N=8 capstone of the size trend: dense (4,5)=3920 with a spectrum-cluster CI (R realizations) and a
    ...
    public void Reconnaissance_DenseN8_Full()
    {
        ...
        Assert.Equal(IntegrabilityBreakingCsr.UncertaintySemantics.SpectrumClusterBootstrap95, dilute.Uncertainty);
        Assert.Equal(60, dilute.IndependentSpectrumCount);
        Assert.Equal(0.676, dilute.CiLo, 3);
        Assert.Equal(0.685, dilute.CiHi, 3);
        Assert.Equal(IntegrabilityBreakingCsr.UncertaintySemantics.SpectrumClusterBootstrap95, dense.Uncertainty);
        Assert.Equal(4, dense.IndependentSpectrumCount);
        Assert.Equal(0.716, dense.CiLo, 3);
        Assert.Equal(0.719, dense.CiHi, 3);
        _out.WriteLine($"N=8, Δ={delta}, q={q}, W={W}");
        _out.WriteLine($"  dilute (1,2) 224 : ⟨|z|⟩={dilute.MeanAbs:F3} {Interval(dilute)} ⟨cosθ⟩={dilute.MeanCos:+0.000;-0.000} (N={dilute.ZCount})");
        _out.WriteLine($"  dense  (4,5) 3920: ⟨|z|⟩={dense.MeanAbs:F3} {Interval(dense)} ⟨cosθ⟩={dense.MeanCos:+0.000;-0.000} (N={dense.ZCount})");
