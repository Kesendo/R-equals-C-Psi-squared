...
public abstract record SweepDimension<T> where T : Claim
{
}

public abstract record SweepDimension : SweepDimension<Claim>
{
    /// <summary>F73 closed-form sweep. The closed form depends only on N and HamiltonianClass
    /// (chain default); ChainOnly is a flag list that reflects the spec's "γ_T1 grid"
    /// surrogate, but the closed form is γ-independent so each ChainOnly entry produces the
    /// same numeric output: this proves γ-independence is a structural property of the
    /// closed form rather than a mere numeric coincidence.</summary>
