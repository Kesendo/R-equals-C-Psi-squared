...
public static class KnowledgeRegistryFactory
{
    ...
    public static ClaimRegistry BuildDefault()
    {
        ...
        return new ClaimRegistryBuilder()
            ...
            // Openness from the trace (Tier1Derived, INCOMPLETENESS_PROOF): a generator with non-negative
            // rates is closed iff trace(L) = 0, so a palindrome centred away from zero certifies an OPEN
            // system. The 5-candidate elimination that once read "therefore external" does not hold and the
            // noise ORIGIN is open. Parents PolynomialFoundationClaim + QubitDimensionalAnchorClaim (Pi2
            // family, above).
            ...
            // F157, the blind seat (Tier 1 derived, 2026-08-24; the span half proved 2026-08-25): with
            // the Z-dephasing on ONE seat j, the single-excitation blind count is
            // blind(j) = N - dim Krylov(e_j) = deg gcd(chi(H), chi(H struck at j)), an identity by Cramer
            // with no hypothesis beyond the real symmetry H_SE has, closing on the uniform open chain to
            // (gcd(2j+1, N) - 1)/2 with the ZZ term and gcd(j+1, N+1) - 1 without, and forced by PARITY
            // alone at every odd seat of every odd XY chain. The stationary manifold is the commutant of
            // the algebra generated by H and the seat projector; one cyclic vector forces that algebra to
            // End(K) + C[H|_W], so dim ker L_SE(j) = 1 + dim commutant(H|_W), which equals 1 + blind(j)
            // exactly when H restricted to the KRYLOV COMPLEMENT is simple. The Jacobi node lemma supplies that
            // on any zero-free chain in either book, so the zero bond is not the discriminator: [1,1,0,1,1]
