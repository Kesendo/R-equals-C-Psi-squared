...
public sealed class TrichotomyWitness : IInspectable
{
    ...
    /// on the canonical convention (H = (Q/2)·H_unit, per-site γ-profile). Public canonical convention is
    ...
    public static (int PCol, int PRow, double Rate) SurvivorSector(
        TopologyKind topo, int n, double canonicalQ, IReadOnlyList<double> gammaProfile)
    {
        int bestPc = 0, bestPr = 1;
        double best = SectorReductionWitness.SectorSlowest(n, canonicalQ, gammaProfile, 0, 1, topo);
        for (int p = 1; p <= n - 1; p++)
        {
            double r = SectorReductionWitness.SectorSlowest(n, canonicalQ, gammaProfile, p, p, topo);
    ...
    /// <summary>H = (Q/2)·H_unit = ChainSystem(N, J=Q, XY).BuildHamiltonian(): the SAME normalization
    /// SectorReductionWitness.QHUnit uses, so the block here is the survivor block whose rate
    ...
    /// the full-Liouvillian <see cref="PostEpFlowField.GlobalSlowestRateDeviation"/> at its own canonical
    /// Q_lo=3/Q_hi=2000; only N=5 attaches the birth/sterile surface label. For the sector-reduced path (N>6, or ring/star,
    ...
    {
        const double qLo = PostEpFlowField.BirthCanalProbeQLow;
        const double qHi = PostEpFlowField.BirthCanalProbeQHigh;
        if (topo == TopologyKind.Chain && n <= 6)
        {
            ...
            return flow.GlobalSlowestRateDeviation;
        }
    ...
    /// canonical Hamiltonian Q=1 (the carbon J=1), uniform γ = 1/Q. <see cref="SurvivorSector"/> and
    /// <see cref="SectorSlowest"/> use H=(Q/2)·H_unit, so to read the SAME block
    /// Survivor read, pass canonical Q=1 and the uniform 1/Q profile. The carbon-mapping gate
    /// (CarbonBlock_RateMatchesSurvivor) pins this: CarbonSlowestRate == Survivor.Gap to 9 digits.</summary>
    private const double CarbonHamiltonianQ = 1.0;
    ...
    /// way, as <see cref="IncompletenessSurvivorWitness.Survivor"/> (canonical Hamiltonian Q=1, uniform γ=1/Q). Used
    ...
    public static double CarbonSlowestRate(TopologyKind topo, int n, double Q, int pc, int pr) =>
        SectorReductionWitness.SectorSlowest(n, CarbonHamiltonianQ, CarbonProfile(n, Q), pc, pr, topo);

    /// <summary>|Im| / phase-rigidity of the SINGLE slowest non-kernel mode of the carbon (pc,pr) survivor
    /// block (canonical Hamiltonian Q=1, uniform γ=1/Q — bit-identical to <see cref="IncompletenessSurvivorWitness.Survivor"/>'s
    ...
    {
        var h = QHUnit(n, CarbonHamiltonianQ, topo);
