...
| `F87Z2CubedRefinement` (Symmetry/) | **F103**: Z₂³ refinement of F87 trichotomy at k_body=3 with N=4 empirical anchor (294 Z₂³-homogeneous Pauli pairs). Five sub-statements: truly y_par=0-pure (300 classifications), hard diagonal 42:8 with Y-inversion, diagonal soft 13:13, mother (0,0) soft y_par=1-pure, off-diagonal soft Pattern B (proportional) + Pattern C (y_par=1-pure). Second YParity-axis Claim on `PolarityCubeMap`. Tier 1 derived (empirical anchor; PROOF_F103). |
