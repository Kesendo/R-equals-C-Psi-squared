...
| `MotherSoftYParityOnePurity` (Symmetry/) | **F109**: Mother sector Klein (0,0) soft pairs under any single-letter dephase channel are y_par=1 pure. Closed-form derivation chain: Klein (0,0) same-parity constraint (#X+#Y even AND #Y+#Z even ⟹ all three counts share same parity) + F107 per-dephase truly criteria (all three even = truly) ⟹ non-truly = all three odd ⟹ #Y odd ⟹ y_par = 1. Step 5 (Π²-even non-truly ⟹ SOFT, not hard) depends on the open F108 Part 1 (block-restricted palindrome lemma on Π²-eigenspace decomposition); empirically verified across 5346+ pairs. Explains the y_par=0 mother-soft count of zero observed across F103/F105/F106 (1026 mother-soft classifications). Sixth YParity-axis Claim on `PolarityCubeMap`. Tier 1 derived modulo Step 5 dependency (PROOF_F109). |
