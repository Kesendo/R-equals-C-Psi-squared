...
| `F86.PolarityInheritanceLink` | **F86 ↔ polarity-layer-pair meta-claim** (Tier 2 verified): the c=2 bond-class split (Q_peak 2 ± 0.5, HWHM/Q* 1/2 + r·(1/2)) inherits structurally from the polarity-layer pair {−0.5, +0.5} at d=2 named in `Pi2KnowledgeBase` (`PolarityLayerOriginClaim` + `QubitDimensionalAnchorClaim` + `HalfAsStructuralFixedPointClaim`). The decomposition Q_peak ≈ 2 + r and HWHM/Q* ≈ 1/2 + r·(1/2) holds across c=2 N=5..8 with mean Q_peak = 2.04 ± 0.06 and Interior r_HWHM ≈ 0.502 (close to `HalfAsStructuralFixedPoint` 1/2 anchor). Properties: `PolarityRootAnchor` (parent Pi2 claims), `ParallelLocusReference` (Locus 5 EP-side closure pointer), `Witnesses` (4 pinned `PolarityWitness` rows from the c=2 N=5..8 `C2HwhmRatio` pipeline at γ₀=0.05), `IsAnalyticallyDerived` (false), `PendingDerivationNote` (Tier1Derived promotion path: (α) per-bond projection of K-resonance state onto polarity Bloch axis at t_peak, OR (β) Locus 5 EP-rotation tan θ = Q/Q_EP combined with Locus 6 polarity inheritance to fix r as function of g_eff(N, b)). Tier2Verified honesty: empirical decomposition pinned + structural inheritance from Pi2 claims; per-bond r(N, b) closed form NOT derived. **Locus 6 of inheritance-through-layers** (symmetry-side closure); companion at Locus 5 is `LocalGlobalEpLink` (EP-side closure) |
