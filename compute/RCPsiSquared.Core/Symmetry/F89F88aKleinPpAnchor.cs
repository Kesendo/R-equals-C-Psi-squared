...
/// <summary>F89 ↔ F88a/F88b bridge: F89's bond Hamiltonian (XX and YY per bond) sits
/// entirely in F88a's Klein <b>Pp</b> cell (Π²_Z = Π²_X = +1), and F89's initial state
/// ρ_cc = (|S_1⟩⟨S_2| + |S_2⟩⟨S_1|)/2 is the F88b popcount-pair coherence configuration
/// (n_p=1, n_q=2). Both anchors place F89 within F88a + F88b typed taxonomy without ambiguity.
///
/// <para>The two F89-side ingredients (Hamiltonian + initial state) each land in
/// specific F88a cells; together they pin F89's evolution to the Pp/Pm sub-algebra
...
///   <see cref="F88bPopcountPairLens"/>(N, 1, 2) classifies it:
///   PopcountMirror at N=3 (1+2=3=N → α=0); KIntermediate at N=4 (n_q=2=N/2 →
///   closed-form α); Generic at N≥5 (α=1/2).</item>
/// </list>
///
/// <para>This is the typed-knowledge counterpart to the F88a + F88b cross-reference in
/// the F89 entry of <c>docs/ANALYTICAL_FORMULAS.md</c>: F89's structural identity
/// with F88a's Pp-Klein-cell + F88b's popcount-pair-lens framework, no separate proof needed
/// (both Pi2 and F88a/F88b are upstream Tier-1-Derived; this bridge propagates their
...
/// Hamiltonian is Pp-resident); F88b's popcount-pair α anchors then directly feed
...
public sealed class F89F88aKleinPpAnchor : Claim
{
    ...
    /// <summary>F88b popcount-pair lens for F89's (S_1, S_2) Dicke-coherence initial
    /// state on an N-qubit chain. Returns the typed Tier-1-Derived F88b lens with
    /// its closed-form α anchor.</summary>
    public static F88bPopcountPairLens InitialStateLens(int N) =>
        new F88bPopcountPairLens(N, F89InitialStateNp, F89InitialStateNq);

    public F89F88aKleinPpAnchor(KleinFourCellClaim kleinFourCell, F89TopologyOrbitClosure f89)
        : base("F89 ↔ F88a/F88b bridge: F89 bond Hamiltonian (XX, YY) is F88a-Pp-Klein-cell-resident; ρ_cc initial state is the F88b popcount-(1, 2) coherence pair, classified by F88bPopcountPairLens(N, 1, 2) as PopcountMirror at N=3, KIntermediate at N=4, Generic at N≥5",
               Tier.Tier1Derived,
               "KleinFourCellClaim Pp cell (operator-level), F88bPopcountPairLens (state-level α anchors), F89TopologyOrbitClosure (F89 bond-graph orbit closure), docs/ANALYTICAL_FORMULAS.md F88a + F88b + F89")
    {
    ...
    public override string DisplayName => "F89 bond (Pp) + ρ_cc (popcount-(1, 2)) anchor in F88a/F88b";

    public override string Summary =>
        $"XX, YY both F88a-Pp (Π²_Z = Π²_X = +1); ρ_cc = popcount-(1, 2) coherence; F88bPopcountPairLens(N, 1, 2) " +
    ...
    protected override IEnumerable<IInspectable> ExtraChildren
    {
        get
        {
            ...
            yield return new InspectableNode("F89 initial state ρ_cc",
                summary: $"(|S_{F89InitialStateNp}⟩⟨S_{F89InitialStateNq}| + |S_{F89InitialStateNq}⟩⟨S_{F89InitialStateNp}|)/2; F88b popcount-pair coherence with (n_p, n_q) = ({F89InitialStateNp}, {F89InitialStateNq})");
            yield return new InspectableNode("F88b lens classification per N",
