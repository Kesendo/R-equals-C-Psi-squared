...
/// <summary>F2 closed form (Tier 1, proven D10; ANALYTICAL_FORMULAS F2):
///
/// <code>
///   ω_k = 4·J · (1 − cos(π·k / N)),      k = 1, ..., N−1
///
///   N−1 distinct frequencies on the Heisenberg chain's (0,1) coherence block
///   (D10 Step 6: the block, not the larger Pauli w=1 sector, which is not L-invariant)
///   Tight-binding hopping 2·J
/// </code>
///
/// <para>F2 is the registry-formal entry for this dispersion.
...
///
/// <para><b>Distinction from F2b:</b> F2 is the (0,1) LIOUVILLIAN coherence
/// block (the |0><j| between the ferromagnet and the single excitations) for the
/// HEISENBERG Hamiltonian, dimension N with N−1 oscillating modes, argument
/// π·k/N. F2b is the single-excitation HAMILTONIAN sector for the XY
...
/// neighbour tight-binding on N sites. Pi2-Foundation anchoring
/// is composition through W1Dispersion + Pi2DyadicLadder.</para>
///
/// <para>Anchors: <c>docs/ANALYTICAL_FORMULAS.md</c> F2 +
...
public sealed class F2W1DispersionPi2Inheritance : Claim, IZ2AxisClaim
{
    ...
    public F2W1DispersionPi2Inheritance(Pi2DyadicLadderClaim ladder, W1Dispersion w1)
        : base("F2 (0,1) coherence block dispersion ω_k = 4J·(1−cos(πk/N)), k=1..N−1; 4 = a_{-1}, 2J hopping = a_0; N−1 distinct frequencies",
               Tier.Tier1Derived,
    ...
    protected override IEnumerable<IInspectable> ExtraChildren
    {
        get
        {
            yield return new InspectableNode("F2 closed form",
                summary: "ω_k = 4J·(1−cos(πk/N)) for k=1..N−1; Heisenberg (0,1) coherence block; tight-binding hopping 2J on N sites; D10 derivation");
