...
public static class ConfirmationsRegistry
{
    private static readonly IReadOnlyList<Confirmation> _all = new[]
    {
        ...
        new Confirmation(
            ...
            HardwareData: "data/ibm_cusp_precision_april2026/cusp_precision_ibm_kingston_20260426_115939.json",
        ...
        new Confirmation(
            ...
            PredictedValue:
                "The complex-CΨ phase arg(CΨ_com), which shares the square-root form of F95 in its arc length but is not the F95 angle θ, is actively steerable via per-chunk RZ(Ω·Δt) injection on the bra-ket Hermitian off-diagonal. " +
        ...
        new Confirmation(
            ...
            Observable: "coherence-pattern decay rates Γ(D) for all 7 subsets of a 3-qubit line (quadrature envelopes, readout-mitigated), the pair-sum price Γ(D)+Γ(D̄), per-bond covariances c_ij (branch splitting), the pointwise shape-free law, and fitted conditional-Ramsey frequency differences (outer arm Δf02 has different gain)",
            PredictedValue:
                "F89d fold at the ρ level: Γ(D)+Γ(D̄) = Σγ_j pattern-independent under LOCAL dephasing (all c_ij = 0); " +
                "deviations decode by the pre-registered table (common-mode / single-bond / ZZ boundary rule / shape misfit)",
            MeasuredValue:
                "dephasing covariances LOCAL in run 3 (all c_ij within 2σ there; one non-recurring 5.1σ outlier in run 2 " +
                "downgraded, unexplained); coherent NN ZZ supported by three instruments (t² mask, 3:2:3 W fingerprint, " +
                "conditional shift): run-4 fitted ζ01 = −3.92±0.14 kHz, ζ12 = −3.64±0.10 kHz; the outer-arm fitted " +
                "Δf02 = +0.01635±0.11981 kHz is zero-consistent and below the pre-registered fitted-shift screen " +
                "of 0.5 kHz (nominal 95% upper 0.25118). Ideal no-decay half-gain maps it to ζ02 = +0.03270±0.23963 kHz, " +
                "not a measured bare coupling; a strict physical |ζ02| < 0.5 kHz bound is unresolved " +
                "(geometry: docs/proofs/PROOF_PRICE_PAIR_OUTER_ZZ_GEOMETRY.md)",
            ...
            FrameworkPrimitive: "F89d coherence-pattern fold and F1 spectral palindrome as pure-Z structural context: " +
                "for Lindblad D[Z_j] coefficients γ_Z,j the pure-Z price is 2·Σγ_Z,j. The measured P = Γ(D)+Γ(D̄) " +
                "uses total fitted transverse rates, including local T1 contributions 1/(2T1_j), so it is not an exact " +
                "F1 center measurement. Selected-pattern additivity follows from the local-channel model, not F70's " +
                "partial-trace rule; F82-F84 T1 accounting; run_price_pair.py (external pipeline)",
            Description:
                "The local-dephasing premise under the F-formulas, supported in the clean run-3 session: a four-run pre-registered campaign (runs 1-2 the price " +
                "protocol on two independent lines, run 3 the W discriminator refuting the intermediate anti-correlated-bath " +
                "reading via the 3:2:3 boundary-rule fingerprint, run 4 conditional Ramsey fitted chain shifts ≈ " +
                "−3.9/−3.6 kHz supporting coherent NN ZZ). Working noise model: local Z-dephasing (quasi-static + " +
                "Markovian) + local T1 + coherent NN ZZ of order 4 kHz; the registered outer-arm fitted-shift screen was met, " +
                "while a strict physical outer-bond 0.5 kHz bound is unresolved and one non-recurring 5.1σ covariance stays " +
                "unexplained. Every wrong intermediate reading was falsified by its own pre-registered " +
