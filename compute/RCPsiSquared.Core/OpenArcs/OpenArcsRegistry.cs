...
public static class OpenArcsRegistry
{
    private static readonly IReadOnlyList<OpenArc> _all = new[]
    {
        ...
        new OpenArc(
            ...
            NextStep: "the parked FROST_CIRCLE seam (docs/carbon/FROST_CIRCLE_AS_THE_CLOCK_FACE.md twin, own genre, only when the structure stands); the proof note (N=2 block closed-form; general N>=3 off-gap argument open, now with a Q-floor as well as the N-ceiling); the ring dihedral-lock sibling (Im_max = Delta_E_max); the 0.99750 fine-structure pull",
            Status: OpenArcStatus.Open),
