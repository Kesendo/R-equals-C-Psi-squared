...
public static class OpenArcsRegistry
{
    private static readonly IReadOnlyList<OpenArc> _all = new[]
    {
        ...
        new OpenArc(
            ...
            NextStep: "FROST_CIRCLE seam FIRST STONE laid (2026-06-12, merged): the carbon coherent<->incoherent threshold IS our Q-floor, verified bit-for-bit and given our label - the Coherence Horizon Q*(N). Live witness CoherenceHorizonWitness / inspect --root horizon bisects Symphony.Clock.Omega and computes Q*(2,3,4,5)=1 / sqrt2 / 1.8785 / 2.3722, equal to carbon's sqrt2/1.879/2.372 (FROST_CIRCLE) under J<->|beta|; ANALYTICAL_FORMULAS F2b corollary carries it. N=2 (Q*=1) is the EP base rung the polyene layer can't reach; the N=2,3 band-edge coincidence (Q*=2cos(pi/(N+1)) ONLY there) explains carbon's 'sqrt2 exact at N=3, rest awaiting a clean form' puzzle. STILL OPEN: the closed form of Q*(N) (the freezing mode just below Q* is overdamped AND half-lit, <n_XY>~1/2 not dark - a verified clue that REFUTED a 'dark-storage-undercuts-bright-readout' guess); the full FROST_CIRCLE reflection in its own genre. Also still: the proof note (N=2 block closed-form; general N>=3 off-gap argument open, with a Q-floor as well as the N-ceiling); the ring dihedral-lock sibling (Im_max = Delta_E_max); the 0.99750 fine-structure pull",
            Status: OpenArcStatus.Open),
