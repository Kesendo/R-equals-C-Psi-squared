...
public static class OpenArcsRegistry
{
    private static readonly IReadOnlyList<OpenArc> _all = new[]
    {
        ...
        new OpenArc(
            ...
            NextStep: "WORKED 2026-08-29, and what it produced is smaller and elsewhere than intended. "
                + "THE INTENT was to write the gamma/gamma_0/Q separation and move the origin question to Q. "
                + "TWO SURFACES WERE WRITTEN AND BOTH WITHDRAWN BEFORE COMMIT, by three independent reviews: a "
                + "reading page (docs/THE_UNIT_HAS_NO_ORIGIN.md) and an F-number with a proof page (F159, the "
                + "scale law). Both fell for one reason, and it is this repository's own top rule biting: what "
                + "they claimed as new stood committed already. Q_SCALE_THREE_BANDS:83 has owned the identity "
                + "L(lambda*J, lambda*gamma_0) = lambda*L(J, gamma_0) as Tier 1 since 2026-04-22; "
                + "THE_GENESIS_OF_AN_OSCILLATION owns the unit reading ('gamma_0 is the fixed constant, the "
                + "substrate unit, not measurable from inside the system (only Q is)'); and INCOMPLETENESS_PROOF "
                + "states that scaling gamma alone moves Q AND that this is what GAMMA_TIME_DISTINCTION measures, "
                ...
                + "elimination and does not need it. "
                + "A THIRD SURFACE WAS WRITTEN AND WITHDRAWN, and this one was withdrawn by its own author before "
                + "any reviewer saw it, which is the only reason it is worth recording. It corrected "
                + "CROSSING_TAXONOMY:169, which generalizes K = gamma*t_cross to 'any observable of a Lindblad "
                + "system whose Hamiltonian is gamma-independent', on the strength of a |01> counterexample whose "
                + "K spread 7.3%. TWO THINGS KILLED IT. The gate computed its own CPsi as purity * purity(rho_A); "
                + "the repository's CPsi is purity * (l1 off-diagonal) / 3, so the counterexample measured a "
                + "different observable than the document it was correcting. Recomputed in the repo's own book, "
                + "Bell+ gives K = 0.03735 at every gamma, spread EXACTLY 0.000%, and 0.03735 is F25's own printed "
                + "value, which is the check that the object is now right. And |01> has CPsi(0) = 0.0, i.e. it "
                + "STARTS BELOW the threshold and then crosses it 10, 4 and 2 times at the three rates, so it has "
                + "no well-defined first crossing and no K at all. The counterexample was an instrument that could "
                + "not have returned the other answer. A reviewer then offered (|00>+|11>+|01>)/sqrt(3), which "
                + "does start above the threshold and is not H-dead; its 79.95% spread reproduces from below, but "
                + "it crosses the threshold 11, 5 and 5 times, so it fails the same test and was not used either. "
                + "WHAT LANDED INSTEAD NEEDS NO CROSSING AT ALL, which is why it is safe: the generalization rests "
                + "on a PREMISE, 'the dissipator is linear in gamma and H is independent of gamma, THEREFORE all "
                + "observables depend on tau = gamma*t', and that step is false. The scaling symmetry is JOINT, so "
                + "holding J while sweeping gamma moves Q, and GAMMA_TIME_DISTINCTION Part 3 measured exactly that "
                + "failing in March, deviations to 0.861, on the same chain. What makes the step true for the K "
                + "values is the STATE: Bell+ is H-dead, so its observables really are functions of tau alone, and "
                + "its K reproduces ln(4/3)/8 = 0.03596 at every gamma to 0.00% spread. CROSSING_TAXONOMY's "
                + "derivation and F14's 'Valid for: any Lindblad system, any bridge metric' are scoped accordingly "
                + "now. A LEAD, NOT A RESULT, and deliberately not written into either document: a random search "
                + "over 300 pure states (numpy default_rng(7), H = XX+YY+ZZ at J=1, gamma in {0.02, 0.05, 0.10}) "
                + "returned SIX states that DO pass the single-crossing test and are not H-dead, with K spreads "
                + "11.85, 5.57, 5.14, 4.99, 4.35 and 0.30 percent, and the smallest spread belongs to the state "
                + "with the smallest commutator (0.091), which is the consistency one would want. That says K "
                + "itself moves off the H-dead sector. It is NOT landed because the crossings were located on a "
                + "fixed 15001-point grid and the convergence check under grid refinement timed out; run that "
                + "first, then the two documents can say K moves rather than only that their derivation does not "
                + "reach. "
                + "STILL OPEN AND NOT WRITTEN ANYWHERE: the framework-level successor question, where Q comes "
                + "from. substrate_q_provenance has asked it per-substrate since 2026-08-22 and was never read as "
                + "the successor; that framing lived only on the dropped page, so decide where it belongs rather "
                + "than re-deriving it. ALSO STILL OPEN, one real defect this work's sweep found and did not fix: "
                + "GAMMA_IS_LIGHT's 'Observed (Tier 2-3)' bullet carries the photon-shot-noise device mechanism "
                + "without the qualifier the same file's prose carries three times. Note "
                + "for whoever resumes: a microscopic system-plus-bath derivation (item 2 below) returns a bath and "
                + "a rate IN THAT BATH'S UNITS, so it moves the question one level down rather than closing it; "
                + "that is an argument for doing it for what it does settle, not for expecting it to settle this. "
                + "ALSO CORRECTED ON THE WAY, and recorded in CAUGHT_ERRORS as the ledger's first OBJECT error on "
                + "this letter: the inventory of gamma as 'four objects' is wrong twice. gamma_0-as-unit and "
                + "gamma_0-as-framework-constant are one object under two hats (PRIMORDIAL_GAMMA_CONSTANT says so "
                + "itself); Sigma-gamma and gamma_avg are one scalar in two normalizations, Sigma = N*avg, the q/Q "
                + "shape with N as the silent factor; and the inventory OMITTED Sigma-gamma, which is the scalar the "
                + "palindrome actually runs on. Three alleged tensions between the gamma documents were all "
                + "refuted, two of them already reconciled by Q_BELONGS_TO_NO_SUBSTANCE. One real defect remains "
                + "unfixed: GAMMA_IS_LIGHT's 'Observed (Tier 2-3)' bullet carries the device mechanism without the "
                + "qualifier the same file's prose carries three times. THEN the sweep below. "
                + "ZERO FIRST, and it is a sweep rather than a question: the withdrawal reached the "
                ...
                + "termination rested on it: REPAIRED 2026-08-29, the termination does not need the withdrawn "
                + "premise because a unit does not regress, and that file's own 'What remains open' already said "
                + "so),hypotheses/PAIR_BREAKING_AT_THE_HORIZON.md:22/63/171 (a Tier "
                ...
                + "enumeration. CAUGHT_ERRORS has cited QUBIT_NECESSITY sections 8a, 8d and 10 as the authority "
                + "for that since 2026-06-22, and they are not: all three are about non-dephasing "
                + "dissipators at d > 2, and the words measurement and classical field do not occur in that "
                + "document at all. The non-exhaustiveness stands on its own, and the axis has NO source in "
                + "the repo, which makes it more open than the ledger made it look. It is written into the "
                + "proof now; the axis itself is untouched. ALSO PARKED HERE, because it was found on the "
        ...
        new OpenArc(
            ...
            ParkedAt: "MEASURED, AND THE FIRST MECHANISM CANDIDATE IS ALREADY DEAD, which is the most useful thing "
                ...
                + "THE CANDIDATE THAT SPLIT THE FAMILIES: that the failing rows are the configurations "
                + "invariant (or anti-invariant) under the graph's own site swap. Tested directly on P3 with "
                + "equal magnitudes, comparing only what the operator sees (a site with no field letter "
                + "contributes no magnitude, and a first version of this test compared those too and returned a "
                + "meaningless 0): of the 78, just 6 are swap-anti-invariant, 0 are swap-invariant, and 72 are "
                + "neither. So the bare reflection is NOT the operator anywhere, and for the 72 nothing of this "
                + "shape is. For the SIX it is a different story, settled on 2026-08-26 below: the swap DRESSED "
                + "with an axis rotation, guessed here from what those rows look like (a swap composed with "
                + "something acting differently on the X sites and the Y site, e.g. deph .X., field XYX, "
                + "h = (30,30,-30) mapping to (-30,30,30)), turns out to exist as an exact commuting operator on "
                + "that side. The count above reproduces exactly; what needed splitting was the reading laid over "
                + "it. THE NAME OF THIS ARC WAS WRONG ONCE ALREADY: it opened as "
                ...
                + "2026-08-26: THE NAMED CANDIDATE SPACE IS EMPTY, AND EXISTENCE TURNS OUT TO BE FREE. Five findings, "
                + "each carrying its own evidence grade, because they are not all graded alike. (1) PRIOR WORK THIS ARC "
                + "DID NOT KNOW IT HAD. Three gitignored scouts of 2026-08-04 had already built most of this search; "
                + "they left no artifact, and their only surviving verdict was one sentence inside the second "
                + "docstring. Re-run they reproduce: 0 on both exception families, 4 hits on an allowed control, the "
                + "canonical Pi with two phase choices each. A local scout is not the repo answering, and what a "
                + "session does not carry back does not exist. The same failure is LIVE IN THIS BLOCK until its own "
                + "scouts are ported, which is why that port is item ONE of NextStep and not a tidying afterthought. "
                + "(2) THE SPACE IS EMPTY OVER A STRICTLY LARGER SPACE. Every dressing of the named kind is a MONOMIAL "
                + "matrix in the Pauli-string basis, so enumerate the permutation part and SOLVE the phases instead of "
                + "enumerating them: any per-site letter bijection at each site composed with any site permutation, 6 * "
                + "24^3 at N=3, then eps propagated over the support graph from eps_a L_ab == M_{phi(a),phi(b)} eps_b, "
                + "with M = -L - 2*sigma. That admits every non-factorizing phase vector, so it contains the space this "
                + "arc named, both 2026-08-04 scouts, and the product they never crossed. Exact over three primes "
                + "against the committed build_L, with the Pauli-basis transform checked by W^-1 L_comp W == L_pauli, "
                + "which none of the scouts did. The positive control returns the canonical Pi_Z, one permutation part, "
                + "I to X and Y to Z at every site; two negative controls return none; the exception rows return NONE. "
                + "NOTE WHAT IS AND IS NOT RESTRICTED, because item THREE of NextStep turns on it: the DIAGONAL leg is "
                + "free, eps ranging over every invertible diagonal, so any chiral sign pattern is already inside. What "
                + "is restricted is the PERMUTATION part, to site-local. (3) WHY THE AUTOMORPHISM PART COULD NEVER HAVE "
                + "WORKED. This arc owns the argument; it needs no basis and no diagonal machinery. The identity-string "
                + "column of L is exactly zero, so S carries the identity into ker(-L - 2*sigma) = ker(L + 2*sigma); "
                + "every algebra automorphism fixes the identity string, and so does the transpose, which is an "
                + "ANTIautomorphism and must not be listed among them; that would force L to hold eigenvalue -2*sigma "
                + "on the identity, false for sigma > 0. So the site reversal dressed with signed axis permutations "
                + "cannot be S at any gamma > 0 on any row, and the bare reflection failing on 72 of the 78 was not a "
                + "near miss. The same reads off the diagonal in the Pauli basis, where L = A + D with A antisymmetric: "
                + "a = b leaves D_a == -D_{phi(a)} - 2*sigma, and with ONE dephasing site D takes two values and phi "
                + "must exchange them. That two-class phrasing belongs to this row family, not to the general law: with "
                + "m dephasing sites the reflection permutes m+1 classes about -sigma. PRECEDENT RATHER THAN OWNERSHIP, "
                + "and the distinction matters because a returning session will open the citation. F118 "
                + "(PROOF_PI_FACTORS_AS_R_TIMES_D section 4f) asserts in a bare parenthesis, unproved and used only to "
                + "cut D4 down to four elements, that any palindromizer must exchange the dead letters with the lit "
                + "ones. F121 (PROOF_QUDIT_PARTIAL_PALINDROME section 6) PROVES a strictly narrower statement, for "
                + "per-site PRODUCT mirrors intertwining the DISSIPATOR palindrome, premises that this arc site "
                + "permutations and full-L palindrome do not meet, and its own section 7 exhibits non-product "
                + "intertwiners beating the cap. So the shape is theirs and the argument is this one. F118 section 4f "
                + "is itself a repair surface: it asserts without proof what this now proves for its own case. What "
                + "F118 section 3 does own outright is that the string-MOVING leg carries the entire -2*Sigma*gamma. "
                + "(4) EXISTENCE IS FREE, AND THE EVIDENCE GRADE SPLITS ACROSS THE ROWS. Read on the MINIMAL "
                + "polynomial, g = f / gcd(f, f-prime) then g(L) == 0, and not on a discriminant. PROVED: the 72-family "
                + "row, the K3 copy and the allowed control measure 64 distinct eigenvalues of 64, and a squarefree "
                + "characteristic polynomial at ONE prime proves squarefreeness over Q(i), the discriminant lying in "
                + "Z[i]; diagonalizability and similarity follow. CERTIFIED ONLY: the named row measures 30 of 64 and "
                + "the two non-pairing controls 61 of 64, and there diagonalizability rests on g(L) == 0 mod p at three "
                + "primes, which is not a decision, because the radical mod p need not be the reduction of the radical "
                + "over Q(i). Both failure modes of that certificate push toward DIAGONALIZABLE, so the error runs "
                + "TOWARD the conclusion, the reverse of the one-sidedness this repo states wherever it reads a GF(p) "
                + "rank. The headline row is the certified one, so treat this half as unfinished rather than as "
                + "measured. Where diagonalizability IS proved, diagonalizable plus equal characteristic polynomials "
                + "implies similar, and equal characteristic polynomials IS the palindrome verdict, so an invertible S "
                + "with S L S^-1 == -L - 2*sigma exists there, found by no search and explaining nothing. The "
                + "F1PalindromeIdentity breaks-for node already carries the reason: the F138 sweep decides the SPECTRUM "
                + "while F1 is the OPERATOR identity, which also sees the Jordan structure, and a row where the "
                + "spectrum pairs anyway says nothing about whether Pi conjugates L there. The repo name for that gap "
                + "is the SOFT BREAK. WHAT IS NOT KNOWN AND MUST NOT BE REACHED FOR. The Jordan structure at N=3 WITH "
                + "an on-site field is unmeasured territory here. The coherence-horizon square-root EP is not a fence "
                + "for it: that object is an XY chain, field-free, uniform bonds, and PROOF_CHAIN_GAP_DOMINANCE records "
                + "that the ZZ term removes that family at N=3 exactly. A rational-versus-irrational escape is not a "
                + "fence either; the repo retracted that argument twice, and PROOF_R90_FROZEN_DIVISOR carries an "
                + "irrational AND a rational exceptional coupling at N=3, the rational one with a size-three Jordan "
                + "block. The repo own mechanism is the WINDOW: a square-root EP splits as sqrt of |q - q*| and is "
                + "invisible outside a window of order 1e-3, which is what retracted F86a. So the verdict is a verdict "
                + "at the sampled rational points. And pin the book before comparing any Q: this sweep runs H = "
                + "J*(XX+YY+ZZ), the doubled Pauli-J book, so its J/gamma = 20 is Q = 40 in the horizon units, while "
                + "the horizon Q* = sqrt(2) is q about 0.707 in this sweep units. (5) THE TWO FAMILIES DIFFER, AND THE "
                + "BURIED CANDIDATE IS ALIVE IN THE SMALLER ONE. On the 6-family the surplus symmetry exists as an "
                + "operator and is derived rather than guessed: the Heisenberg bond is invariant under any global "
                + "rotation, so conjugation by U = Z_0 Z_1 Z_2 sends H(h) to H(-h) for an X field, and the site "
                + "reversal R does the same because the two field sites carry opposite signs, so V = R.U fixes H; it "
                + "fixes the dissipator too, the sign U puts on X_1 being invisible to A(.)A. Measured entry-wise mod p "
                + "at three primes: [V, L] is exactly 0 on the named row and on its K3 copy, and NONZERO at generic "
                + "magnitudes, nonzero when the equal pair is end-plus-middle, and nonzero for the bare reversal alone. "
                + "On a 72-family row it is nonzero. SCOPE, and this is the sharp edge: exactly ONE V was tested, with "
                + "U hard-coded. The 72-family measurement therefore says THIS V does not commute there, NOT that no "
                + "surplus symmetry exists; no search over dressings was run on that side. For the six it does carry: "
                + "the six clause2-not-orthogonal rows are exactly the six swap-anti-invariant rows of the count above, "
                + "and they are the letter-relabelling and sign orbit of the row tested, so the symmetry reaches all "
                + "six by that orbit rather than by six measurements. What the earlier count got right: the count "
                + "itself reproduces exactly. What its reading missed: the conclusion drawn from it holds for the 72 "
                + "and not for the 6. V is still not the answer, and for a reason of kind rather than of fit, it "
                + "COMMUTES with L instead of reflecting it, and no commuting operator sends lambda to -lambda - "
                + "2*sigma. Do not upgrade that into a claim that it delivers the degeneracy: commuting does not imply "
                + "degeneracy, and how much of the 30-of-64 V accounts for is not computed. K3 is unaccounted "
                + "separately, its automorphism group being larger. REPAIR WORK THIS SWEEP UNCOVERED. Two sit outside "
                + "this arc and are recorded rather than done: PROOF_F111_HARD_CELL_PURE_D_TEMPLATE asserts, in a "
                + "non-load-bearing aside, that spectrum-level palindromy is realized by some similarity with existence "
                + "guaranteed by the palindromic spectrum, which is the inference that (4) shows is not free in "
                ...
                + "THIRD is NOT outside: MIRROR_SYMMETRY_PROOF still says the palindrome holds exactly when, carries no "
                + "converse fence anywhere, and its Evidence paragraph asserts zero exceptions in either direction, "
                + "which Stage C contradicts for clause 1 as well. F138 names those Scope paragraphs as its own Proof "
                + "anchor, so the registry header and the cited proof now say opposite things. That one is this arc "
                + "territory and is item TWO of NextStep. HOW THE 2026-08-26 NUMBERS WERE PRODUCED, and how far they "
            ...
            NextStep: "READ docs/proofs/PROOF_PALINDROME_TWO_END_COUNT.md FIRST, dated 2026-08-28/29. THE ARC'S "
                + "CENTRAL QUESTION IS ANSWERED AND THE ANSWER IS NOT AN OPERATOR. The palindrome holds IFF "
                + "dim ker L = dim ker(L + 2*sigma), both directions proved: the near kernel is the commutant of "
                + "<H, A_l> and the far one is the same space with the jump sign flipped (a Cauchy-Schwarz "
                + "equality case in the HS norm), both eigenvalues are semisimple, so the char-poly identity "
                + "forces the two nullities to agree, and equal nullities force an invertible U with [U,H] = 0 "
                + "and U A_l U^-1 = -A_l. So the S this arc asked for since 2026-08-03 exists exactly when two "
                + "integers agree, and finding it is never necessary. Registered as F158, typed as "
                + "PalindromeTwoEndCountClaim with the live witness inspect --root twoend, gated at "
                + "simulations/f138_rank_criterion.py. FOUR ITEMS OF THIS ARC ARE THEREBY SETTLED AND ONE "
                + "CAUTION SURVIVES. Finding (4)'s caution that a free unstructured S explains nothing is "
                + "RESPECTED rather than tripped over: the two-end count is not an existence claim, it is often "
                + "UNEQUAL, and where it is equal the operator is forced rather than found. Finding (5)'s V is "
                + "Ad_U = Pi squared, so filing it as the wrong KIND of object is superseded: a commuting weak "
                + "symmetry is frequently the SQUARE of the reflector. Finding (2) stands unchanged and is why "
                + "the answer was missed, one-sided multiplication by a SWAP not being monomial in the Pauli "
                + "basis. Item TWO below (repair MIRROR_SYMMETRY_PROOF) IS DONE, 2026-08-29: its 'holds exactly "
                + "when' is now 'holds when' with the converse failure and its counts beside it, and F138's "
                + "Proof field is narrowed to say the Scope paragraphs carry the SUFFICIENT direction as a "
                + "measured census. The circle that made this necessary is worth recording: the proof cited "
                + "F138 for the law while F138 cited the proof for the proof, and neither end derived it. "
                + "AND THE EXCEPTIONS NOW HAVE AN OPERATOR IN CLOSED FORM, which retires F138's exhibition "
                + "sentence: on P3 with an XX+YY bond, no field and one dephasing axis per site, exactly the "
                + "two three-axis assignments with Z on the MIDDLE site pair anyway, and there U = YYZ + ZXX "
                + "(axes XZY) and U = XXZ + ZYY (axes YZX), each Hermitian, each squaring to 2*1, det 16, with "
                + "the reflector identity verified entry by entry at residual exactly 0. The bare site "
                + "reflection was refuted correctly and read wrongly: U is a SUM of two Pauli strings, neither "
                + "a single string nor a signed site permutation, so its refutation was never evidence that "
                + "nothing exists. Clause 1 cannot see this because the exceptions are about WHERE the odd axis "
                + "sits, not how many axes there are. WHAT IS STILL OPEN, and it is short. (a) Whether F158 "
                + "should become F138's Proof anchor, dropping the clauses to a corollary: F158 decides the "
                + "same setting exactly, since a dephasing axis n.sigma is Hermitian and squares to 1 and "
                + "on-site fields sit inside H, so every configuration F138 sweeps is inside its class. "
                + "DELIBERATELY NOT TAKEN on the day F158 landed, the reason being sequence rather than doubt: "
                + "a Tier-1 row that predates F158 should not depend on a result one day old that has already "
                + "needed one repair round. Take it after F158 has been read by someone who was not in the room. "
                + "(b) The mechanism on the clause-2 72-family, still not shown; the operators above are "
                + "exhibited on two named families only. (c) Lemma 3's Case 2 recursion, which could be a "
                + "direct argument. (d) Item ONE below, the port of the 2026-08-26 scout numbers, is now "
                + "PARTLY done: the criterion, the palindrome verdict and the exception operators are all in "
                + "committed scripts and a live witness, while the monomial search and the surplus commutator "
                + "are still gitignored. EVERYTHING BELOW IS THE ROUTE AND IS KEPT, because it records what was "
                + "tried; read it as history now rather than as a worklist. The 2026-08-26 layer: "
                + "It exhibits a reflector on the named row, Pi(rho) = U rho^T with U = SWAP_02 . Z_0 Z_1 Z_2, and "
                + "gives a criterion scored in both directions against the exact verdict over 140861 rows: the "
                + "char-poly palindrome holds iff there is an invertible U with [U,H] = 0 and U A_l U^-1 = -A_l at "
                + "every dephasing site. Three things there change this entry rather than extend it. The V of finding "
                + "(5) is Ad_U, which is Pi squared, so the reading that files it as the wrong KIND of object is "
                + "superseded: a commuting weak symmetry is frequently the SQUARE of the reflector, and this arc had "
                + "the square in hand. Finding (2) stands unchanged and is why the answer was missed, one-sided "
                + "multiplication by a SWAP not being monomial in the Pauli basis, so the exhaustive verdict and the "
                + "exhibited operator are consistent. And finding (4) is beside the point rather than wrong: existence "
                + "is free where diagonalizability is proved, but the operator found there is STRUCTURED and was scored "
                + "in BOTH directions, which free existence never is. Everything below stays in force, item ONE most of "
                + "all, because that page is gated by a committed script and is still not carried by a witness.  "
                + "ONE, AND NOTHING ELSE LANDS BEFORE IT: PORT THE EVIDENCE. Every number in the 2026-08-26 block above "
                + "lives in gitignored scouts, which is precisely the failure that item (1) measures. Three pieces are "
                + "load-bearing and all three type naturally: the exact GF(p) palindrome verdict on a named row, the "
                + "monomial search with its positive control, and the surplus commutator. A live witness in Core with "
                + "its claim, in the genre of the existing palindrome witnesses, is what makes any of the above "
                + "citable. TWO: REPAIR the MIRROR_SYMMETRY_PROOF Scope and Evidence paragraphs, and fence the F138 "
                + "exhibition sentence. The proof still carries the iff withdrawn on 2026-08-03 while being named as "
                + "the F138 Proof anchor; the F138 header, the F1 registry entry and the F1PalindromeIdentity "
                + "breaks-for node were all fenced that day, and the proof is the surface that was missed. Then the "
                + "F138 sentence saying no operator explaining the exceptions has been exhibited needs to separate "
                + "EXISTS, settled and free on the rows where diagonalizability is proved, from EXPLAINS, open, and to "
                + "record that the DRESSING of the bare reflection does exist on 6 of the 78. THREE: TAKE THE COMMITTED "
                + "THEOREM BEFORE INVENTING A COMPUTATION. reflections/ON_THE_SOFT_BREAK records, as answered in "
                + "2026-06, that a soft break is exactly one whose Hamiltonian hopping graph is BIPARTITE in the "
                + "dephasing basis, rescued by the chiral sign-flip sending one side of that bipartition to minus "
                + "itself, with the windowed converse, non-bipartite implies hard, proved in "
                + "PROOF_F103_F87_Z2_CUBED_REFINEMENT. That is a predictor of the right KIND for the 72-family, where "
                + "(5) has no symmetry to offer and where a chiral sign-flip is not a symmetry of the configuration at "
                + "all. Run it on a 72-family row and on the named row before anything below. Carry this arc sharpening "
                + "into it: the chiral leg is DIAGONAL, and the monomial search eps already ranged over every "
                + "invertible diagonal, so if the theorem carries here then the class-swapping PERMUTATION is what must "
                + "be non-site-local. FOUR, only if THREE leaves the structured question open: widen the S search past "
                + "site-locality. The permutation part need not factorize over sites, and the class-swap constraint "
                + "plus the support pattern of A make the full monomial group decidable by backtracking rather than by "
                + "enumeration. A miss there is a much stronger statement than item (2), which is scoped to site-local "
                + "permutation parts. FIVE, the centered-polynomial reading, and take it as a graded object rather than "
                + "as a verdict. p(x) == p(-x - s) says the centered q(y) = p(y - s/2) is EVEN, checked from below on "
                + "the named row: 0 of 32 odd coefficients nonzero at y - s/2, and 32 of 32 at y + s/2, so the sign is "
                + "not free. The phenomenon is then the simultaneous vanishing of 32 coefficients. Carry the two end "
                + "field values as SIGNED symbols rather than as magnitudes, because the signs are (+,+,-) and the two "
                + "conventions name different loci; the row pairs at (30, -30), so every odd coefficient must carry the "
                + "SAME factor and the locus is h0 + h2 = 0 in the signed convention. Codimension is how many of the 32 "
                + "conditions are independent, a rank to compute rather than a coefficient to name. And such a locus "
                + "explains one row parameter family, not 78 rows; the families are read separately. FOUR THINGS THIS "
                + "ARC MUST NOT DO. It must not soften the sufficient direction, which is intact everywhere and is what "
                + "F138 is for. It must not be answered by widening the sweep: more rows at one magnitude tuple say "
                + "nothing, and that habit hid the magnitude axis for a day. It must not restate the F138 converse as "
                + "though the scope were a footnote. And it must not be answered by exhibiting an unstructured S, which "
                + "(4) shows is free where diagonalizability is proved; a free answer is a sign that the question was "
                + "mis-posed, not that it is closed. THE NAMED ROW, assembled once so that it need not be gathered from "
                + "four places: N=3, graph P3 with edges (0,1) and (1,2), each bond J*(XX+YY+ZZ) at J = 1, dephasing "
                + "along X on site 1 alone at gamma = 1/20, an on-site X field on sites 0 and 2 of equal magnitude "
                + "30/100 with signs (+,+,-), site 1 fieldless. Prior work in order: the F1PalindromeIdentity "
                + "breaks-for node, reflections/ON_THE_SOFT_BREAK including its 2026-06 addendum, "
                + "PROOF_PI_FACTORS_AS_R_TIMES_D sections 3 and 4f, PROOF_QUDIT_PARTIAL_PALINDROME section 6, Stage C "
                + "and Stage F of simulations/results/f138_clause_two_sweep.txt, and the arc f138_clause_two_sweep for "
                + "the traps this territory is full of.",
            Status: OpenArcStatus.Open),
        ...
        new OpenArc(
            ...
            ParkedAt: "2026-08-22, THE REAL PART NOW HAS A CLOSED FORM ON THE R90 LOCUS, which this "
                + "entry previously said it had nowhere: on the (0,1) block source 2 of "
                + "experiments/WHAT_THE_R90_LOCUS_BUYS.md (TWO hypotheses, gamma anti-palindromic AND H "
                + "reflection-symmetric under the same reflection) gives the pairing lambda -> "
                + "-conj(lambda) - 4*gbar, which PRESERVES Im and reflects Re about -2*gbar, so ANY "
                + "eigenvalue whose Im is shared with no other is its own image and has Re = -2*gbar "
                + "EXACTLY. The statement is PER EIGENVALUE and sharper than the collapse: pinning "
                + "arrives mode by mode as degeneracies lift, so a block carries exactly-pinned modes "
                + "while its own spread is wide (three of seven pinned to 1e-15 at spread 0.87, N=7), "
                + "and there is NO single J onset, only the last degeneracy lifting. ONE DIRECTION: "
                + "the converse is FALSE, a star at uniform gamma being pinned while N-2 of its modes "
                + "share Im = 0, though only TRIVIALLY so: at uniform gamma the diagonal is already "
                + "scalar, so that witness says nothing about the pairing, and whether a NON-uniform "
                ...
                + "than exempted; what is added here is only the finite-J, per-eigenvalue statement. NOT novel wholesale and the "
                + "credits matter: the IDENTITY is that note's source 2 (gated by "
                + "simulations/two_sources_gate.py), the COLLAPSE is its measured table, and F152 owns "
                + "the generator; what is added is only why the collapse is exact on THIS block. It is "
                + "NOT the route that note rejects (palindrome plus the one-sided Absorption bound, "
                + "which merges flat with on-the-floor); nothing here mentions the floor, and the axis "
                + "sits 0.875 off it on the profile measured. It is NOT "
                + "THE_SPREAD_IS_A_RESONANCE's X^N cancellation either, which needs an index at N/2 "
                + "that (0,1) never has: the two mechanisms partition rather than compete. "
                + "PREVIOUSLY, AND STILL TRUE OFF THE LOCUS: "
                + "with a gamma PROFILE the block operator is M = -2iJ*Laplacian - 2*diag(gamma), "
            ...
            NextStep: "RESUMPTION POINT (heading written 2026-08-02; APPENDED TO THROUGH 2026-08-22 without moving, so read the dated items below rather than this line). WHERE TO START AS OF 2026-08-22: class B, which now has both an instrument (the two block builders take a profile) and a law to point at (see WHAT CLASS B SHOULD POINT AT). Class A is owner-blocked on one site and cannot be advanced by a worker. The psi_opt IBM-profile overlap gate named further down is a SEPARATE thread, not the resumption point. FOUR THINGS ARE DONE and are not to be " +
                ...
                "FIRST-A, WHAT THE COUNTING SCOUT FOUND ON TOP OF IT (2026-08-20, AFTER the proof landed at 5ee4a14; SCOUT LEVEL, the scripts are local and uncommitted, so nothing below is gated in the repo and the numbers are recorded here so they are not re-found). The question was how MANY EPs a line through rate space meets, and the answer turned into a structure. "
                + "(1) THE INSTRUMENT. On the (0,1) XY block the discriminant of the characteristic polynomial is a POLYNOMIAL in the turned rate with rational coefficients once the profile and J are rational, of degree exactly 2*(N-1) (measured N=4..8). So STURM counts the crossings exactly, with no grid, no eigensolver and no threshold. That is strictly stronger than the count-change scan PathKMonodromyScout runs on (1,2), which declares two blind spots in its own docstring (a tangency and a semisimple crossing); Sturm has neither. Three cheaper instruments were tried first and all three produced confident nonsense: a float sign scan (the discriminant near a double root IS rounding, so the counts came out non-monotone in the parameter), an exact sign scan on a GRID (exactness cured the signs, not the sampling, and its 'threshold' was simply where the feature grew wider than the spacing), and a square-root fit to that artefact. "
                + "(2) EVERY EP IS A STEP OF +-2 in the number of REAL eigenvalues, so the endpoints of a line fix the NET count, hence its parity and a floor, and never the total. The excess is round trips. A round trip is a WINDOW in the rate inside which one conjugate pair has come to rest on the real axis: it does not turn there, it only decays. Consequence for the proof that just landed: its seven witnesses are NOT seven of the same thing. At N=5 the three of them are the two EDGES of one such window plus a separate transition, and they carry a direction (a frequency dying, or being born). "
                + "(3) THE WINDOW IS BORN AT A CUSP, NOT A FOLD. Its width grows like d^(3/2) past the threshold, constant to three digits over four decades, at N=5, 6 and 7 (constants 3.11, 3.02, 3.39; the near-agreement is unexplained). A fold would give 1/2. "
                + "(4) AND AT THE CUSP SITS AN EP3, exactly. The cusp value is algebraic; its minimal polynomial divides the resultant of the two subdiscriminants, whose common vanishing IS a triple root of the characteristic polynomial (gcd equals the minimal polynomial itself at N=5, 6 and 7, no tolerance anywhere). With Lemma A holding the geometric multiplicity at one, that is a SINGLE JORDAN BLOCK OF SIZE THREE. It is the first sighting of the general case the proof's own corollary states and never exhibited. "
                + "(5) A DEGREE LAW, and it reduces. The cusp locus has total degree 6*(N-2), verified N=4..8, unchanged across sixteen choices of WHICH two sites are turned and across four base profiles including one with denominator 97; the splitting into irreducible factors moves, the SUM never does. It is not a mystery: 6*(N-2) = 3*(d-2) with d = 2*(N-1) the discriminant degree above, so all the N-dependence enters through d, which is 2*(N-1) because the characteristic polynomial is LINEAR in a rate. The resultant degree 8*N^2-20*N+10 was predicted from N=4..7 and landed at N=8. "
                + "(6) THE MULTIPLICITY SIGNATURE SEPARATES RATES FROM BONDS, which is the same message as gate G9 one level up. Turning a rate gives the classical signature, multiplicity 3 for a cusp and 2 for two separate double roots. Turning a BOND doubles both to 6 and 4, because the characteristic polynomial sees a coupling only through J^2 and the discriminant is then EVEN in it, so every cusp is a +- pair. A first reading of that run said bonds carry no cusps at all, which was wrong and is recorded because the wrong version is the tempting one. "
                + "(7) THE GAIN-SIDE CUSP, chased 2026-08-20 and NOT what the first note called it. At every N a cusp sits at a NEGATIVE rate, gain rather than loss. The first note read the near-agreement across N (-0.18785, -0.18693, -0.18717) as a constant and said GAMMAFOLD TERRITORY. Both halves were wrong. It is not a constant: it moves with J and NOT proportionally (the ratio to J runs 1.46 to 2.86 as J is scanned), it moves with the neighbouring rates, and on a profile of 0.5/0.7/0.9 it is not there at all. What it IS is LOCAL: on a fixed near-profile the value converges exponentially in N, alternating, each site shrinking the change by about a factor ten (-0.184545372, -0.184090312, -0.183909012, -0.183950976, -0.183945604, -0.183946125 at N = 4..9), so it is a well-defined object of the SEMI-INFINITE chain that the far tail cannot feel. The N-agreement the first note admired was that locality, not universality. AND IT IS ADJACENT TO GAMMAFOLD, NOT INSIDE IT. GammaFold s is a WHOLE-PROFILE flip and no per-site involution exists in the object; the structural fact that costs nothing is M(-gamma) = -conj(M(gamma)), so the full flip maps the EP set to itself, but it moves all N rates while this point has ONE negative among N-1 positive, so its image leaves the family. The per-site involution this note asked MirrorWorld for WAS BUILT, 2026-08-20 (GammaFold.SiteTurn / SiteTurnGroup, 16 tests, run mode gammafold), and it answers both halves of the ask with neither answer the expected one. ON THE PROFILE the single turn is UNCONDITIONAL: exact, an involution, the N of them commuting, generating (Z/2)^support, an unwatched site contributing no factor. So the move this cusp needs IS available there, which the sentence above wrongly denied. ON THE RATE AXIS, where GammaFold's two mirrors live, it has a shadow only conditionally: a rate is -2 times a subset sum, so s_l is a function of the rate iff the profile RESTRICTED TO ITS SUPPORT has distinct subset sums; whole-profile dissociation is sufficient and not necessary (a zero rate moves by 4*0 and may collide freely), and UNIFORM gamma is the extreme failure, the spectrum collapsing to N+1 values with no single site addressable. AND THE GROUP DOES NOT CLOSE: s and s0 are reflections of the axis, slope -1 apiece, while the shadow of s_l is PIECEWISE the identity and a translation by 4*gamma_l, no reflection at all, and (s_l o s0)^2 is not even TOTAL on the spectrum. The infinite dihedral does not grow; a partial piecewise action sits beside it. The EP question stays outside, as predicted. TWO TRAPS THE BUILD WALKED THROUGH, both worth carrying: the criterion must be decided over the INTEGERS, and not for the tempting reason (1/10 + 2/10 = 3/10 does not slip past a float test, since the stored doubles for those decimals have distinct sums and both routes agree) but because distinct exact sums can ROUND TOGETHER, so a float test invents collisions; and the object's own default profile, written 0.2 + 0.1*l, is INTENDED as an arithmetic progression, whose sums collide, while what it STORES is 0.30000000000000004 at site 1, one ulp off, which makes the stored profile dissociated after all. A float reading of each question had returned the opposite. "
                + "(8) THE NEAREST PRIOR OBJECT, and it must be cited whenever (4) is: PROOF_R90_FROZEN_DIVISOR section 9 already exhibits a JORDAN BLOCK OF SIZE THREE under a gamma profile, at N=3 on gamma = (1/25, 0, -1/25) with J* = +-2/75, kernel dimensions (3, 4, 5). It is a DIFFERENT object on five counts and the differences are the interesting part: it is the (1,1) frozen divisor on the R90 locus, not the (0,1) edge block; Heisenberg, not XY at Delta = 0; tuned in J at a fixed profile, not in a RATE at fixed J; geometric 3 against algebraic 5, where ours is geometric 1 against algebraic 3; and it needs a zero-mean MIXED-SIGN profile, i.e. gain, while the cusps of (4) sit at rates that are all POSITIVE and therefore physical dephasing. What the two SHARE is the shape, and section 9 states it in words that fit this arc exactly: an index that fixes a geometric count at every coupling, and a finite set of couplings where the algebraic count runs ahead of it. Here the index is Lemma A. Section 12 of that proof then leaves open the counting question, and the degree law of (5) is an answer to it of that type, on a different block. Neither result may be called new without naming the other. " + "WHAT IS OWED BEFORE ANY OF (1)-(7) IS CITED ELSEWHERE: a committed gate, and the usual empty-session rounds. The repo holds no counting law of this shape anywhere (a counting law for the real exceptional couplings is an open item on the F140/F143 list), so this is new surface and must not be leaned on until it has been attacked. " +
        ...
        new OpenArc(
            ...
            NextStep: "THE SCOUT'S CUT IS THE QUEUE, and it is sharper than a list of laws. Every law " +
                ...
                "other. RUN 2026-09-02, experiments/THE_BLIND_SEAT_ON_THE_ROAD.md, gate " +
                "simulations/blind_seat_on_the_road.py: the detuned bond walked, the two kept apart. " +
                "F157's count splits by SEAT rather than by summand (a reflection-fixed seat keeps it " +
                "at every coupling; every other seat loses it off a finite locus), and at odd N = 5..17 " +
                "the u locus is the Delta locus plus the ring ends at all 77 seats, at 44 of the 66 even " +
                "ones, breaking at the other 22 exactly where the u locus is not negation-closed. That " +
                "hands the u axis F157's committed closed form, since the Delta side is generated by " +
        ...
        new OpenArc(
            ...
            NextStep: "ONE REMAINS; the first is DONE (2026-08-23) and carries a same-day ANNOTATION that reverses one of its own conclusions, plus the check that annotation called for, run the same day. " +
                ...
                "MirrorWorld Formulas.cs. BOTH ITEMS ARE BUILT (2026-08-25). " +
                "The typed layer: SeatCutBlindnessClaim " +
                ...
                "gate simulations/blind_seat_span_proof.py (9 gates, about 19 s). It " +
                "carries the Jacobi node lemma (no eigenvector of an unreduced Jacobi " +
                "matrix has two consecutive zeros, hence none vanishes at an end; the " +
                "spectrum is simple; and an eigenvector vanishes at seat j exactly when " +
                "its eigenvalue is shared by the two leftover principal submatrices), " +
                "and it returns the span identity SHARPER than it was handed over. The " +
                "kernel is the COMMUTANT of the algebra generated by H and the seat " +
                "projector; one cyclic vector forces that algebra to End(K) + C[H|_W], " +
                "so dim ker L_SE(j) = 1 + dim commutant(H|_W), which equals 1 + blind(j) " +
                "exactly when H is simple ON THE KRYLOV COMPLEMENT. That decides all " +
                "73 failing (graph, book, seat) triples of the twenty-graph table. The " +
                "zero-bond reading decides 12 of them and misses in BOTH directions, " +
                "leaving 61 failures unexplained and calling 20 holding triples broken " +
                "(counted in the run, part `commutant`): the fence was never about the " +
                "zero bond, and [1,1,0,1,1] versus [1,0,1] is the pair that says so. " +
                "The commutant framing itself is NOT new and is credited in the proof: " +
                "three surfaces state it on 2026-08-24, all in the single-excitation " +
                "sector: docs/CAUGHT_ERRORS.md, THE_SEAT_THAT_CUTS section 5 (which " +
                "claims it in that page's own ownership table) and the F4 seat " +
                "sub-bullets, each with dim ker = sum m_i^2. What was open is that with " +
                "ONE seat the blocks are forced and can be named; the proof also closes " +
                "the question those two surfaces left open, whether a ZERO-FREE OPEN " +
                "CHAIN can make the component mechanism undercount, which it cannot, " +
                "every m_i being 1 there. The fence-free statement was already " +
