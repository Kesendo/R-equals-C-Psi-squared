...
public static class OpenArcsRegistry
{
    private static readonly IReadOnlyList<OpenArc> _all = new[]
    {
        ...
        new OpenArc(
            ...
            NextStep: "FIX FIRST (bug, found live 2026-06-12 at gamma=0.9): the witness's dial node computes theta(N=2) from the RAW OmegaMem below the Q=2/sqrt3 crossover (shows 29.05 deg = arctan(J/2gamma) instead of the honest arctan(sqrt(Q^2-1)) = 25.84 deg; gap verified still exactly 2gamma there) - the pull node already applies the crossover honesty, the dial node must use the same closed-form hand. Then: the parked FROST_CIRCLE seam (docs/carbon/FROST_CIRCLE_AS_THE_CLOCK_FACE.md twin, own genre, only when the structure stands); the proof note (N=2 block closed-form; general N>=3 off-gap argument open); the ring dihedral-lock sibling (Im_max = Delta_E_max); the 0.99750 fine-structure pull",
            Status: OpenArcStatus.Open),
