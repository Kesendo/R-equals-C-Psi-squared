...
public static class InspectCommand
{
    ...
    public static readonly IReadOnlyList<RootCatalogEntry> Catalog = new RootCatalogEntry[]
    {
        ...
        new("fillcsr", "the F89 Door-C finite-size filling comparison at canonical Delta=1 plus disorder. Nonzero Delta breaks free-fermion additivity, but uniform XXZ remains Bethe-integrable; random longitudinal Z disorder at Delta=0 remains quadratic (Anderson/free fermions); generic random field plus Delta!=0 is the interacting disordered nonintegrable test. These knobs can break reflection/conjugation/cross-fold without each breaking Hamiltonian integrability. The finite executed CSR evidence contrasts dilute (1,2) and dense (wKet,wBra) near N/2 under the same disorder: dense angular repulsion trends toward GinUE (≈−0.09/−0.13/−0.16 at N=6/7/8 = 43/56/67% of the size-matched GinUE angle), while dilute ⟨cosθ⟩ remains near 0. This is not a universal filling or thermalization theorem. Class A reference uses unequal weight (p,p+1) and the measured disordered conjugation-match ≈0. Live: general WeightCoherenceBlock + random field → MathNet EVD → complex spacing ratio, pooled per-spectrum with finite-size-matched refs. The sequel to --root galoischaos",
        ...
        new("branchpalindrome", "the F89 octic branch locus is a palindrome (the typed home F89BranchLocusPalindromeClaim): the EP/diabolic collisions are mirror-symmetric about Re λ = −σ = −4, FORCED by the F1 palindrome carried antiunitarily on the (SE,DE) block (T L(q) T⁻¹ = −L(q̄)−2σ, a same-q fold at real q; all-q holomorphic identity F8(λ,q)=F8(−λ−8,−q); spectral action λ↦−λ̄−2σ). Two-sided gate live: the octic closes under the antiunitary mirror, not the linear one; every EP on the line or in a mirror pair, no orphan. Semisimplicity follows from the twin-scalar restriction at the N=4 point, not the mirror alone; the sampled XXZ Delta control stays on-line yet defects, not an all-N cause. The q-direction sibling of --root galoismonodromy; reading reflections/ON_WHO_WATCHES_WHOM.md",
