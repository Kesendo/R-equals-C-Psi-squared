...
public static class InspectCommand
{
    ...
    public static readonly IReadOnlyList<RootCatalogEntry> Catalog = new RootCatalogEntry[]
    {
        ...
        new("crosstriple", "the F127 cross-triple orthogonality, live slice: an independent C# transcription of the six-angle cross form evaluated in GF(p) at deterministic Conway-Jones variety points (both Tonelli roots each side, all four combinations must give 0) with an off-variety NONZERO control per point. The second implementation the F127 code-trust caveat asks for, at sample scale; the proof object stays the 527/527 grid+CRT wall (grid_proof_sweep.py --assert). Sub-second, fully deterministic",
