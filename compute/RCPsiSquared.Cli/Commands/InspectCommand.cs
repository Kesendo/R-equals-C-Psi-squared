...
public static class InspectCommand
{
    ...
    public static readonly IReadOnlyList<RootCatalogEntry> Catalog = new RootCatalogEntry[]
    {
        ...
        new("branchpalindrome", "the F89 octic branch locus is a palindrome (the typed home F89BranchLocusPalindromeClaim): the EP/diabolic collisions are mirror-symmetric about Re λ = −σ = −4, FORCED by the F1 palindrome carried antiunitarily on the (SE,DE) block (T L(q) T⁻¹ = −L(q)−2σ for all q; octic identity F8(λ,q)=F8(−λ−8,−q); spectral action λ↦−λ̄−2σ). Two-sided gate live: the octic closes under the antiunitary mirror, not the linear one; every EP on the line or in a mirror pair, no orphan. The line is the palindrome's gift, the diabolic's silence is integrability's (separate, XXZ Δ≠0 stays on-line yet defects). The q-direction sibling of --root galoismonodromy; reading reflections/ON_WHO_WATCHES_WHOM.md",
            _ => new BranchLocusPalindromeWitness(), RequiresN: false),
        new("monodromymirror", "how the F89 palindrome reaches the octic monodromy: the mirror SPLITS at the Galois boundary. C-K: the q↦−q̄ reflection (from L(q)*=L(−q̄)) intertwines the monodromy exactly (σ_K = identity, every EP carries the same braid as its mirror), the branch-locus palindrome lifted from the seams' positions to their braids. C-T: the Re=−4 spectral palindrome induces a genuine strand involution σ_T (fixed strands on the fold + mirror-twin 2-cycles) but CANNOT be a braid symmetry, commuting with the full S_8 monodromy would force it central and Z(S_8)=1; so σ_T is an element OF the Galois group, never a symmetry OF it. The braid-level sibling of --root galoismonodromy and --root branchpalindrome; the from-below ground of reflections/ON_WHO_WATCHES_WHOM.md",
