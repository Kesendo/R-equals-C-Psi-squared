...
public static class InspectCommand
{
    ...
    public static readonly IReadOnlyList<RootCatalogEntry> Catalog = new RootCatalogEntry[]
    {
        ...
        new("branchpalindrome", "the F89 octic branch locus is a palindrome (the typed home F89BranchLocusPalindromeClaim): the EP/diabolic collisions are mirror-symmetric about Re λ = −σ = −4, FORCED by the F1 palindrome carried antiunitarily on the (SE,DE) block (T L(q) T⁻¹ = −L(q̄)−2σ, a same-q fold at real q; all-q holomorphic identity F8(λ,q)=F8(−λ−8,−q); spectral action λ↦−λ̄−2σ). Two-sided gate live: the octic closes under the antiunitary mirror, not the linear one; every EP on the line or in a mirror pair, no orphan. Semisimplicity follows from the twin-scalar restriction at the N=4 point, not the mirror alone. Only N=4 Delta=0 is certified Diabolic 2/2; N=4/N=5/N=6 positive Delta proposals remain Uncertified and carry no Jordan-character conclusion. The q-direction sibling of --root galoismonodromy; reading reflections/ON_WHO_WATCHES_WHOM.md",
        ...
        new("crossfold", "The (SE,DE)↔(SE,w_{N−2}) cross-block fold is an exact antiunitary matrix similarity. The branch-locus palindrome's bra bit-flip ρ[a,b]→ρ[a,b̄] maps the (w1,w2) block to (w1,N−2), and the witness checks L(1,N−2)(q̄) = −P·conj(L(1,2)(q))·Pᵀ − 2N·I to machine zero for N=4..9. It conditionally transports a Jordan character only after independent coincidence/character certification. N=4 Delta=0 is certified; N=5/N=6 positive Delta proposals are Uncertified. Sampled small gaps beyond N=4 are similarity reads, not defect, lift, or diabolic certificates. Reading experiments/F89_PATH_K_DIABOLIC.md and F89_BRANCH_LOCUS_PALINDROME.md",
