...
public static class InspectCommand
{
    ...
    public static readonly IReadOnlyList<RootCatalogEntry> Catalog = new RootCatalogEntry[]
    {
        ...
        new("quarter-uniqueness", "UNIQUENESS_PROOF, the typed argument behind the chosen ¼ normal form. Within the assumed normalized recurrence/power family, purity motivates α=2 but does not derive the recurrence; physical selection remains open. In that family the fold threshold CΨ*_α=(α−1)^(α−1)/(α^α·Ψ^(α−2)) is state-independent only at α=2, where the Ψ^(α−2) factor drops out and it equals ¼, and the α=2 fixed-point discriminant D=1−4CΨ has its zero at ¼. Sweeps α at two probe states (spread=0 only at α=2) + the discriminant across CΨ; elementary arithmetic, exact. --psi-low / --psi-high set the probe states",
            ...
            RequiresN: false),
        new("noise-origin", "INCOMPLETENESS_PROOF, the noise-origin candidates and what each is worth: the 5-candidate elimination does NOT hold (candidates 2 and 3 lost their evidence 2026-08-29), so an internal origin is OPEN. What is exact is the trace identity, a palindrome centred away from zero certifies a non-unitary generator, i.e. OPEN rather than EXTERNAL. Candidate 5 (the dimension algebra d²−2d=0 ⟹ d∈{0,2}, d=1 and d≥3 excluded) is recomputed live; candidate 1 is the typed F63 [Π²,L]=0 constraint; candidates 2-4 (single-qubit decay, now open; the bath's infinite regress; d=0's property-lessness) are surfaced from the proof, the heavier process-tomography compute deferred. --max-d sets the dimension sweep",
            c => new NoiseOriginExclusionWitness(
                c.Parser.OptionalDouble("max-d") is { } m ? (int)m : NoiseOriginExclusionWitness.DefaultMaxD),
            RequiresN: false),
        new("n5diabolic", "F164, the N=5 real-q A₂ locus: these are the only real-q points in the N=5/N=6 A2 inventories. q is the settable parameter and lambda is the resulting spectral eigenvalue, not a second control (q = ±1.1292509708747671, λ = −4.7919603651796410). The double eigenvalue is SEMISIMPLE by Galois conjugacy rather than a contour reading. Current N=5 truth: 24 imaginary-q loci are directly Hermitian-certified; F164 certifies the complete 26-member R-odd orbit, overlapping 12 Hermitian loci and adding 2 real-q plus 12 nonreal-q loci. Thus 38 distinct loci are exact-certified and only 20 nonreal-q R-even loci remain numerical-only; the classifier still reads all 32 nonreal-q loci. An empty exactRankCertificates array is classifier-local provenance. At N=6 all 266 exact rational tBox.real intervals exclude zero, rather than a display midpoint deciding the axis. Recomputes the sector split 50 = 26 + 24, the coupling book pinned by spectrum/operator reconstruction, the real-symmetry residual, the nullity-2 reading, and the direction law: RVR=-V implies P_- V P_-=0 independent of the base point; reflection symmetry of the base is needed only for invariant subspaces. This is not an all-N theorem and no metrological claim follows. The irreducibility of A₂_O is consumed from simulations/o2b_gcd_certificate.py, not redone here",
            c => new RouteBN5RealQSemisimpleWitness(),
            RequiresN: false),
        new("n6unfolding", "F163: N=6, 266 complex-q A2 loci and local end-bond epsilon/epsilon/epsilon^2 unfolding. " +
            "Rebuilds the exact 90=45+45 pencil, joins the certified inventory, and recomputes the mod367 cycle and mod101 effective matrices. " +
            "The complete bounded-CRT factorization is consumed certificate input; complex parameter geometry, not an all-N or physical measurement claim",
            c => new RouteBN6A2UnfoldingWitness(), DefaultDepth: 2, RequiresN: false),
        ...
        new("protection", "the qudit mirror-protection scaling law: the per-site product mirror protects P(d, N) / d^{2N} = (2/d)^N of the coherence space for d ≤ 5, falling as the N-th power of 2/d; = 1 ⟺ d=2 (the qubit is the unique full-mirror dimension); from d = 6 the cap exceeds (2d)^N (180 > 144 at N = 2). Rates stay d-independent.",
        ...
        new("crosstriple", "the F127/F128/F129/F130/F133/F134/F139 family, live: the F127 GF(p) variety slice + core identity + closed form + sheet lattice, the F128 flip lemma (exact ℤ) + factorization + sharper locus, the F129 exact ℤ[ζ_2n] level-collision census (n ≤ 60) + family-inventory sum tie, the F130 exact collision decoupling (every proof cell + controls), the F133 symplectic closed form of W (sin-s lemma + read-off spot + GF(p) certificate), the F134 two-row reflection law (table + read-off + the l = 2 fence), and the F139 seam identity deriving F134 from F133 (the wall as a Chebyshev divisor: Φ_k = S₁₀·(−1)^k P_k + R_k, deg R_k ≤ 4+k, exact ℤ on an own five-variable engine). Each check carries its own discriminating control; the proof objects stay the 527/527 grid+CRT wall and the committed f12*/f13* gates. Seconds to ~a minute (the F133 halves build), fully deterministic",
        ...
        new("galoischaos", "Galois vs spectral chaos: does the H_B-mixed half (chain Galois S_8/18/32/53, no radical closure) read as dissipative quantum chaos (GinUE) at fixed q? A clean NULL — it reads Poisson-like/sub-Poisson on the structured Delta=0 XY frequency lattice. This does not classify the Liouvillian as integrable. Algebraic chaos (Galois over the q=J/γ field) ≠ spectral chaos (RMT at fixed q); the former does not imply the latter. Live: the shared (SE,DE) block → MathNet EVD → complex spacing ratio (Sá-Ribeiro-Prosen), whose own GinUE reference confirms it CAN see chaos. The sector-resolved test the global RMT scan left open; the q-parametric monodromy (--root f89octic discriminant/EP loci) is where the Galois structure does live. The spectral-statistics sibling of --root topowritability",
            _ => new GaloisSpectralChaosWitness(), RequiresN: false),
        new("fillcsr", "the F89 Door-C finite-size filling comparison at canonical Delta=1 plus disorder. Nonzero Delta breaks the underlying Hamiltonian's free-fermion additivity, but uniform XXZ remains Bethe-integrable; at Delta=0 the random-field XY Hamiltonian remains quadratic (Anderson/free fermions), which does not classify the Z-dephasing Liouvillian as a quadratic generator; generic random field plus Delta!=0 is the interacting disordered nonintegrable test. These knobs can break reflection/conjugation/cross-fold without each breaking Hamiltonian integrability. The finite executed CSR evidence contrasts dilute (1,2) and dense (wKet,wBra) near N/2 at the same disorder distribution but in separately sampled realization ensembles: dense angular repulsion trends toward GinUE (≈−0.09/−0.13/−0.16 at N=6/7/8 = 43/56/67% of the size-matched GinUE angle), while dilute ⟨cosθ⟩ remains near 0. This is an ensemble comparison, not a within-realization causal intervention or a universal filling/thermalization theorem. GinUE comparison only: unequal weight (p,p+1) and one measured disordered conjugation mismatch do not determine the full sector symmetry algebra or irreducible SRP class, which remain open. Live: general WeightCoherenceBlock + random field → MathNet EVD → complex spacing ratio, pooled per-spectrum with finite-size-matched refs. The kernel retains finite-precision cluster representatives; its counts and CSR are tolerance-dependent and not an exact degeneracy census. The sequel to --root galoischaos",
            _ => new FillingThresholdWitness(), RequiresN: false),
        new("galoismonodromy", "where the F89 octic's Galois structure lives spectrally: the q-parametric MONODROMY (the sequel to the --root galoischaos fixed-q null). As q=J/γ loops the complex plane the 8 octic roots braid, and that braiding IS the Galois group. G2 live: a loop around q_EP≈0.659 returns the identity, corroborating single-valued sheets but not deciding semisimplicity (an analytic defective pair can also have identity eigenvalue monodromy). The simple zeros of P_10 carry transpositions that generate Gal(F_8)=S_8. Built on the trusted 12×12 block + validated tracker",
            _ => new GaloisMonodromyWitness(), RequiresN: false),
        new("branchpalindrome", "the F89 octic branch locus is a palindrome (the typed home F89BranchLocusPalindromeClaim): the EP/diabolic collisions are mirror-symmetric about Re λ = −σ = −4, FORCED by the F1 palindrome carried antiunitarily on the (SE,DE) block (T L(q) T⁻¹ = −L(q̄)−2σ, a same-q fold at real q; all-q holomorphic identity F8(λ,q)=F8(−λ−8,−q); spectral action λ↦−λ̄−2σ). Two-sided gate live: the octic closes under the antiunitary mirror, not the linear one; every EP on the line or in a mirror pair, no orphan. Semisimplicity follows from the twin-scalar restriction at the N=4 point, not the mirror alone: the sampled XXZ Delta control stays on-line yet defects, read live (at Δ = 0.02 the q_EP crossing splits into two Jordan EP2s at real q, both on Re λ = −4, each alg 2, geo 1), a local control and not an all-N cause. The q-direction sibling of --root galoismonodromy; reading reflections/ON_WHO_WATCHES_WHOM.md",
        ...
        new("crossfold", "Move 4, answered: the (SE,DE) diabolics PAIR across the (SE,DE)↔(SE,w_{N−2}) cross-block fold, because that fold is an EXACT antiunitary similarity. The branch-locus palindrome's bra bit-flip ρ[a,b]→ρ[a,b̄] (F89c lemma, n_diff(a,b̄)=N−n_diff) maps the (SE,DE)=(w1,w2) block to the (SE,w_{N−2})=(w1,N−2) block; the witness builds both blocks + the bra-complement permutation P and checks L(1,N−2)(q̄) = −P·conj(L(1,2)(q))·Pᵀ − 2N·I to machine zero (N=4..9, every q, every Δ). STRONGER than foldcross's spectrum match: an antiunitary similarity preserves Jordan structure, so a certified coalescence at (q,λ) maps to one at (q̄,−λ̄−2N) with identical gap and character; the character itself comes from XxzCoherenceBlock's certificate, not from a small sampled gap. N=4 is the degenerate partner=self within-block self-fold; for N≥5 the partner is a different block and the N=4 on-line 'zeros' become cross-block mirror partners. Reproduces the N=7 real-q diabolic pairing live (q*=1.1264485133, λ*=−4.9418574904, certified Diabolic 2/2 at Δ=0 ↔ −9.0581425096), and reads an N=7 Δ = 0.02 EP2 and its partner turning defective in lockstep. The Move-4 grounding of the diabolic_over_higher_n arc; reading experiments/F89_PATH_K_DIABOLIC.md and F89_BRANCH_LOCUS_PALINDROME.md",
            _ => new CrossFoldSimilarityWitness(), RequiresN: false),
        new("gauge", "the BIPARTITE GAUGE and its commutation criterion: why a coherence block's R-sectors are real matrix families on one parity and conjugate-SWAPPED on the other, decided by the single character (p+q)(N−1). 𝒟 = diag((−1)^(σ(a)+σ(b))), σ the sum of OCCUPIED SITE INDICES; writing L(q) = A + q·C with A the real AT diagonal and C the pure-imaginary hopping, a nearest-neighbour hop flips the bipartite parity on one side, so 𝒟L(q)𝒟 = L(−q) at every complex q and, at real q, 𝒟L𝒟 = conj(L). Both are ±1 sign rearrangements, so the residual is EXACTLY 0.0 and not a machine zero. The content is the other half: σ(rev(c)) = w(N−1) − σ(c), so the reflection rescales every gauge sign by ONE global factor (−1)^((p+q)(N−1)), hence 𝒟 commutes with R iff that character is even and anticommutes otherwise, never anything between. Commuting ⟹ 𝒟 restricts to each R-sector, the sector is a real family and its spectrum conjugation-closed; anticommuting ⟹ the gauge swaps the two sectors, so what it closes is the PAIR, the spectrum of one being the conjugate of the other, and not either sector on its own. It SUBSUMES the path-k arc's odd-N effect (at p+q=3 the criterion reads 'N odd'; at odd N every block commutes). SUFFICIENT, not necessary. Gauge and fold are independent levers: (0,3)/(2,3)@N=6 are fold-fixed yet anticommute, (1,3)@N=4 commutes without being fold-fixed. Completes PROOF_CODIM1_BY_ADDITIVITY §7's open clause, 'P, Q, 𝒟, T all commute with R up to a block-global sign', which never says when the sign is −1. Scope read live: Δ=0 and no longitudinal field, each adding an imaginary diagonal a sign matrix cannot reach. Gate BipartiteGaugeCriterionTests; reading experiments/F89_PATH_K_DIABOLIC.md and the arc sideways_spin_ladder",
            _ => new BipartiteGaugeWitness(), RequiresN: false),
        ...
        new("f155", "F155 live (proof PROOF_F155_PHYSICAL_GENERATOR_POLARITY_BREAK.md, typed PhysicalGeneratorPolarityBreak): the polarity break of the PHYSICAL (no-jump) generator rho -> -i(H rho - rho H+) for H = A + iB with both parts Hermitian is ONE diagonal bilinear form, asymmetry = 4^(N+1) * sum over bit_b-ODD Pauli strings of (-1)^#Z * a_sigma * b_sigma. DIAGONAL: distinct strings never pair, so only strings carried by BOTH halves contribute and bit_b-odd content in A is necessary and NOT sufficient. F113 is the Z-diagonal special case (corollary 10), which is the parent edge. The witness runs TWO routes against each other at inspect time: the closed form is exact arithmetic on Pauli coefficients and builds no matrix, the measurement builds the 4^N x 4^N generator and reads the asymmetry off the palindrome residual at sigma = 0. Eight configurations at N=2 (four at N=1, five at odd N above 1, six at even N above 2, since the disjoint-supports case needs two sites, the PT dimer is an N=2 object, and the cancelling zero needs equally many gain and loss ends), incl. the positive control A = B = Z_0 at exactly -4^(N+1), the textbook PT dimer balanced at its OWN exceptional point (the quantity is a detuning x gain-imbalance meter, not a PT meter), and the F113 configuration read under Pi_X, which is SILENT rather than balanced (both +-i norms individually zero). The tolerance is a LAW and it takes TWO numbers: |residual| / (eps * ||M_anti||^2) says how closely the routes agree, and |closed form| / (eps * ||M_anti||^2) says whether the measured route could have refuted them at all, since the asymmetry is bilinear in (A, B) while the noise floor is quadratic in the larger half. A reading whose signal is under its own noise is marked NOT INFORMATIVE rather than passing quietly. Which zeros are bit-exact is REPORTED rather than assumed. Default N=2; --N up to 5. NOT the commutator -i[H, .] of F112 and NOT the full Lindbladian; that conflation is the error docs/CAUGHT_ERRORS.md records in its 2026-06-20 entry and that entry's 2026-08-19 sequel",
            c => new PhysicalGeneratorPolarityBreakWitness(c.Parser.HasFlag("N") ? c.N : 2),
            RequiresN: false, HonorsOptionalN: true),
        ...
        new("decoder", "reading power measured live: Fisher information vs Q per readout basis - resolution grows with the Q-factor and is monotone on the seven-point sampled grid. Q=1 is the lowest sampled endpoint, not an EP; the relevant N=4 EP Q*=1.87874 is unsampled, so there is no EP verdict",
        ...
        new("seedcount", "the endpoint-nullity surplus theorem live: r(0⁺) − r(∞) = N − 1 (odd N) on the (1,2) " +
        ...
        new("label", "the label layer, typed: the dephasing routes by the letter it holds - the 4^N " +
            ...
            c => new HeldLetterRoutingWitness(c.Parser.HasFlag("N") ? c.N : 3,
                                                 c.Parser.OptionalDouble("gamma") ?? 0.05),
            RequiresN: false, HonorsOptionalN: true),
