...
if (args.Length > 0 && args[0] == "blind")
{
    ...
    Console.WriteLine("  too, and the dark states are a UNION of two subspaces, which no single count reports.");
    Console.WriteLine();
...
Console.WriteLine($"  F18 fold threshold Sg_crit/J: product={Formulas.F18_FoldThresholdProduct} (N-flat over N=2..5), "
                  + $"GHZ={Formulas.F18_FoldThresholdBellN2} at N=2 only; "
                  + $"CPsi(0)=1/(2^N-1) clears 1/4 nowhere else (N=3: {Formulas.F18_GhzInitialCPsi(3):F5})");
...
Console.WriteLine($"  F55 K_death=ln10={Formulas.F55_KDeath:0.000} (99% absorption, above Q*_gap(N)); immortal modes=N+1={Formulas.F55_ImmortalModes(4)} (N=4);  F56 asymptotic cusp K(eps=1e-4,tol=1e-12) = {Formulas.F56_CriticalSlowingAsymptotic(1e-4, 1e-12):0.000}");
...
Console.WriteLine($"  F94+F96 Born-deviation table (|0+0+>, N=4 Heisenberg ring, pair (0,2); setup-specific): Delta_00 = +(4/3)Q^2K^3, Delta_01 = Delta_10 = {Formulas.F96_SubdominantSlopeSingle():0.000}K, Delta_11 = {Formulas.F96_SubdominantSlopeDouble():0.000}K; qudit c(d)=4(d+2)(d-1)/(3d^2): c(3)={Formulas.F94_QuditCoefficient(3):0.000}, peak c(4)={Formulas.F94_QuditCoefficient(4):0.000} (sym3/sym5 recomputed exactly in tests)");
Console.WriteLine($"  F72 site-purity block split: Tr(rho_i^2) = 1/2 + z^2/2 (DD, DeltaN=0) + (x^2+y^2)/2 (CC, |DeltaN|=1), no cross term, |DeltaN|>=2 invisible to a site; e.g. (x,y,z)=(0.6,0,0.8) -> {Formulas.F72_SitePurity(0.6, 0, 0.8):0.000} (pinned on an all-sector dense rho at N=5 in tests)");
Console.WriteLine($"  F73 spatial-sum closure on the vac-SE probe: sum_i 2|(rho_i)_01|^2 = (1/2)e^(-4g0t) exactly, H-blind (F74's n=0 mono-chromatic end); g0=0.05, t=20: {Formulas.F73_SpatialSumClosure(0.05, 20):0.000000e+0} (Restless-pinned in tests: XY, XXZ, ring)");
