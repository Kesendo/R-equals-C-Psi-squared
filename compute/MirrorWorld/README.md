...
| `Warble.cs` | **the warble** (built 2026-08-31 beside `experiments/THE_CRACKED_BELL.md`; the bell-founder's word for the beat of a cracked bell, and bell here is the instrument, not Bell of the Bell states): crack the RING's wrap bond to J' = J(1-delta) via `Cone.SetBond` on `Topology.Ring` and every m ↔ N−m traveling-wave pair splits by the SAME 4·delta·J/N -- a point defect is flat in mode space, every mode hears the crack. Launch the perfect ring's traveling wave (the `Cone` complex seed) and its sense of circulation I(t) = Σ Im ρ[a,a+1] dies at T_zero ≈ πN/(8·delta·J) and is fully REVERSED at T_rev ≈ πN/(4·delta·J): the walk-time step's discarded O(delta) reflection, resonantly accumulated by the closed ring until it is the whole signal. On THIS block (the (1,1) Haken-Strobl page) the clock is gamma-DRESSED: the zero crossing advances under the watching (20.30 → 19.35 → 16.48 at N=8, delta=0.15, gamma = 0/0.01/0.05) and the deepest reversal outlives the naive scalar model at its own best point (1.251× at gamma=0.01, 3.84× at 0.05; the dephasing-free diagonal feeds the current back) -- while the (0,1) page of the same crack reads the clock exactly gamma-free, visibility wall exp(−πN/(2·Q·delta)). Which page is read decides what the watching does to the clock; the fast walk-time front never faced this. Read the zero crossing, not the peak (the peak is the flat hand, wiggle-limited at ~2e-3). Run mode `warble N [delta]` |
