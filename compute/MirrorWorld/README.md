...
number and every dynamics step is pinned from-below by `MirrorWorld.Tests` (421 tests).
...
| `BlindSeat.cs` | **the blind seat** (adopted 2026-08-24 from `experiments/THE_SEAT_THAT_CUTS.md` + `THE_BLIND_SITE.md`; home on the genre gate the way GammaFold's per-site turns were, F157 since the same evening): put the watching on ONE seat and count what it cannot touch -- blind(seat) = N − rank of the seat's Krylov matrix, an exact GF(p) rank at two primes, no eigensolver (the world adopts the COUNT; the gcd phrasing, the projector corner and every path object stay outside). On the uniform chain the count closes to one integer form per book, (gcd(2·seat+1, N)−1)/2 with ZZ on and gcd(seat+1, N+1)−1 with ZZ off; the XY book carries a PARITY-forced kind (odd seat of an odd chain, blind at every zero-free profile); span = 1 + blind on the zero-free chain, an identity that can break only at a degenerate spectrum (simplicity sufficient, not necessary; [1,1,0,1,1] breaks it at seats 1 and 4 while the zero-bond [1,0,1] holds). The count walks past the wall (an N × N integer rank, laws confirmed to N = 200); the span is an N²-sized elimination read at small N. Parent the frame: the sector is the Cone's memory cut REBUILT in integer arithmetic, not borrowed, and the count PREDICTS what a per-site-lit Cone does on the XY book, the guard pinning that prediction dynamically. Run mode `blind N` |
...
dotnet test compute/MirrorWorld.Tests                     # the 421-test from-below guard
```
