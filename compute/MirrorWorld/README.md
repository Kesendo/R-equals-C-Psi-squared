...
number and every dynamics step is pinned from-below by `MirrorWorld.Tests` (420 tests).
...
dotnet test compute/MirrorWorld.Tests                     # the 420-test from-below guard
```
...
any F-number exists; if the main repo mints one for the seat-cut laws, its scalar face lands in
`Formulas.cs` afterwards.
